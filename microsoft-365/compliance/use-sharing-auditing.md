...
Общий доступ — это ключевое действие в SharePoint Online и OneDrive для бизнеса, которое широко используется в организациях. Администраторы могут использовать аудит общего доступа в журнале аудита, чтобы определить, как общий доступ используется в Организации. 
...
Эти два поля в дополнение к другим свойствам из схемы журнала аудита, таким как User, Operation и Date, могут сообщить полное описание *того* *, какой* пользователь предоставил ресурс, с *которым* и *когда*. 
...
Если учетная запись пользователя конечного пользователя не находится в каталоге, SharePoint выполняет следующие действия: 
   ...
   - Записывает в журнал одно из следующих событий в зависимости от того, как общий доступ к ресурсу:
      ...
      - **Шарингинвитатионкреатед** (это событие записывается в журнал, только когда общий ресурс является сайтом)
...
Первый шаг — Поиск событий общего доступа в журнале аудита. Дополнительные сведения (в том числе необходимые разрешения), связанные с поиском в журнале аудита, можно найти в статье [Поиск в журнале аудита в центре безопасности & соответствия требованиям](search-the-audit-log-in-security-and-compliance.md).
