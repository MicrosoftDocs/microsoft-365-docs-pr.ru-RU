...
> [!NOTE]
> На портале доверия службы выберите **Диспетчер соответствия требованиям**, который является ознакомительной версией с самыми актуальными функциями. Не выбирайте элемент **"Диспетчер соответствия требованиям" (классический)**, который содержит устаревшие функции, не описанные в этой документации.

## <a name="administration"></a>Администрирование

Существуют специальные административные функции, доступные только глобальному администратору и отображаемые только при входе в учетную запись глобального администратора. Глобальный администратор может назначить разрешения пользователям и включить автоматическое обновление показателей безопасности.
...
1. Используйте один из предварительно настроенных шаблонов, предоставляемых корпорацией Майкрософт.
2. Настройте предварительно настроенный шаблон с собственными действиями и элементами управления с помощью процесса расширения.
3. Создайте собственный шаблон и импортируйте его в Диспетчер соответствия требованиям.

#### <a name="use-a-microsoft-pre-configured-template"></a>Использование предварительно настроенного шаблона Майкрософт

Предварительно настроенные шаблоны доступны на панели мониторинга **шаблонов** . Просмотр текущего [списка шаблонов](compliance-manager-overview.md#templates), который обновляется каждый раз, когда доступен новый шаблон.
...
6. Нажмите кнопку **Добавить в панель мониторинга**. Вы увидите новый шаблон, добавленный в панель мониторинга **шаблонов** .
...
4. Нажмите кнопку **Добавить в панель мониторинга**. Вы увидите новый шаблон, добавленный в панель мониторинга **шаблонов** .
...
> [!IMPORTANT]
> Если вы создали или настроили шаблоны в диспетчере соответствия требованиям, **этот процесс был обновлен** в рамках выпусков исправлений (Предварительная версия) 2020 апреля (Предварительная версия). **Внимательно изучите этот раздел.**
...
- **Dimension — назначение действия**: это необязательное поле. Если включить его, заголовок должен содержать префикс "Dimension –". Все измерения, которые вы включаете здесь, будут использоваться в качестве [фильтров в рейтинге соответствия](compliance-score-setup.md#filtering-your-dashboard-view) и будут отображаться на [странице "сведения о действиях по улучшению" в показателе соответствия требованиям](working-with-compliance-score.md#view-your-improvement-actions).
