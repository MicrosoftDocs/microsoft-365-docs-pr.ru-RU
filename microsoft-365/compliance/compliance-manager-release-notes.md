...
- **Доступ к диспетчеру соответствия на основе ролей:** Роль **доступа к** предназначению по умолчанию удалена. Чтобы пользователь мог получить доступ к диспетчеру соответствия требованиям, глобальный администратор должен [назначить каждому пользователю разрешение](compliance-manager-overview#permissions.md).
...
- **Обновленный показатель соответствия требованиям**: методология изменилась для поддержки синхронизации с показателем безопасности Майкрософт. Оценка рассчитывается на основе оценок действий, управляемых корпорацией Майкрософт, и показателей действий, управляемых клиентами.
