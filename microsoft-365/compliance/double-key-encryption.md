...
3. В Visual Studio выберите **палитру команд представления** и \>  выберите **Git: Clone**. Чтобы перейти к параметру в списке, начните вводить для фильтрации записей, а затем выберите его из `git: clone` выпадаемого. Например:
...
5. В **диалоговом окте Select Folder,** который отображается, просмотрите и выберите расположение для хранения репозитория. В запросе выберите **Открыть**.
    ...
    Репозиторий открывается в Visual Studio коде и отображает текущую ветвь Git в левом нижнем конце. Например, ветвь должна быть **основной**. Например:
