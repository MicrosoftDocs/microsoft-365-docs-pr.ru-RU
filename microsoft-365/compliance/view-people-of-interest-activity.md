...
## <a name="step-1-create-an-data-investigations-preview-audit-log-search"></a>Шаг 1: Создание запроса на расследования данных (Предварительная версия) Поиск в журнале аудита
   ...
   5. Чтобы выполнить поиск по указанным условиям, нажмите кнопку **Поиск**.  Результаты поиска загружаются, а через несколько секунд они отображаются в разделе результаты на странице поиска людей с действиями по процентам.
