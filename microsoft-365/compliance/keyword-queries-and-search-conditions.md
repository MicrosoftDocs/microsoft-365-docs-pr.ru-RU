...
ms.custom:
- seo-marvel-apr2020
description: Сведения о свойствах электронной почты и файлов, которые можно искать с помощью средств поиска и обнаружения электронных данных в Microsoft 365.
ms.openlocfilehash: 4ca444c7e1d7b90f76e8c3f1b23afc7edad8e44b
ms.sourcegitcommit: 153f413402f93b79be421741f3b9fed318d6d270
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 10/20/2020
ms.locfileid: "48600452"
---
# <a name="keyword-queries-and-search-conditions-for-content-search-and-ediscovery"></a>Запросы ключевых слов и условия поиска для поиска контента и обнаружения электронных данных

В этом разделе описываются свойства электронной почты и документов, которые можно искать в элементах электронной почты в Exchange Online и документах, хранящихся на сайтах SharePoint и OneDrive для бизнеса, с помощью функции "поиск контента" в центре соответствия требованиям Microsoft 365. Кроме того, для поиска этих свойств можно использовать командлеты ** \* – ComplianceSearch** в PowerShell центра безопасности & соответствия требованиям. В этом разделе также описывается:
...
Пошаговые инструкции по созданию поиска контента можно узнать в разделе [Поиск контента](content-search.md).

> [!NOTE]
> Поиск контента в центре соответствия требованиям Microsoft 365 и соответствующих командлетов ** \* ComplianceSearch** в центре безопасности & соответствия требованиям PowerShell используйте язык запросов по ключевым словам (KQL). Для получения более подробных сведений ознакомьтесь со статьей [Справочник по синтаксису языка запросов по ключевым словам](https://go.microsoft.com/fwlink/?LinkId=269603). 
  
## <a name="searchable-email-properties"></a>Доступные для поиска свойства электронного сообщения

В следующей таблице перечислены свойства сообщений электронной почты, которые можно искать с помощью функции "поиск контента" в центре соответствия требованиям Microsoft 365 или с помощью командлета **New – ComplianceSearch** или **Set – ComplianceSearch** . Таблица включает пример синтаксиса  _property:value_ и описание результатов поиска, возвращаемых этими примерами. Вы можете ввести эти  `property:value` пары в поле Ключевые слова для поиска контента. 
...
> [!NOTE]
> <sup>1</sup> в качестве значения свойства Recipient можно использовать адрес электронной почты (также называемый *именем участника-пользователя* или UPN), отображаемое имя или псевдоним для указания пользователя. Например, можно использовать значения annaye@contoso.com, annaye или "Анна Ермолаева".

### <a name="recipient-expansion"></a>Расширение получателя

При поиске в любом свойстве получателя (from, to, CC, BCC, участники и получатели) Microsoft 365 пытается расширить удостоверение каждого пользователя, выполнив поиск по ним в Azure Active Directory (Azure AD).  Если пользователь находится в Azure AD, запрос расширяется, включая адрес электронной почты пользователя (или имя участника-пользователя), псевдоним, отображаемое имя и LegacyExchangeDN. Например, запрос, например, `participants:ronnie@contoso.com` расширяется до `participants:ronnie@contoso.com OR participants:ronnie OR participants:"Ronald Nelson" OR participants:"<LegacyExchangeDN>"` .

Чтобы запретить расширение получателей, добавьте подстановочный знак (звездочка) в конце адреса электронной почты и используйте сокращенное доменное имя. Например, `participants:"ronnie@contoso*"` обязательно заключите адрес электронной почты в двойные кавычки.

Однако следует иметь в виду, что предотвращение расширения получателей в поисковом запросе может привести к тому, что релевантные элементы не будут возвращены в результатах поиска. Сообщения электронной почты в Exchange можно сохранять в разных текстовых форматах в полях получателей. Расширение получателей предназначено для устранения этого факта, возвращая сообщения, которые могут содержать разные текстовые форматы. Таким образом, предотвращение расширения получателей может привести к тому, что запрос поиска не будет возвращать все элементы, которые могут быть релевантны для вашего расследования.

> [!NOTE]
> Если необходимо просмотреть или уменьшить элементы, возвращенные поисковым запросом из-за расширения получателя, рекомендуется использовать расширенные функции обнаружения электронных данных. Можно искать сообщения (используя преимущества расширения получателя), добавлять их в набор рецензирования, а затем использовать обзорные запросы или фильтры для просмотра или сужения результатов. Дополнительную информацию можно узнать в статье [сбор данных для обращения](collecting-data-for-ediscovery.md) и [запрос данных в наборе рецензирования](review-set-search.md).
...
С помощью функции "поиск контента" в центре соответствия требованиям можно выполнять поиск конфиденциальных данных, таких как номера кредитных карт или номера социального страхования, которые хранятся в документах на сайтах SharePoint и OneDrive для бизнеса. Это можно сделать с помощью `SensitiveType` Свойства и имени типа конфиденциальной информации в запросе с ключевыми словами. Например, запрос `SensitiveType:"Credit Card Number"` возвращает документы, содержащие номер кредитной карты. Запрос  `SensitiveType:"U.S. Social Security Number (SSN)"` возвращает документы, содержащие номер социального страхования США. Чтобы просмотреть список типов конфиденциальных данных, которые можно найти, перейдите к разделу **классификация данных** \> **типы конфиденциальной информации** в центре соответствия требованиям Microsoft 365. Кроме того, вы можете использовать командлет **Get – DlpSensitiveInformationType** в PowerShell центра безопасности & соответствия требованиям, чтобы отобразить список типов конфиденциальной информации.
  
Для получения дополнительных сведений о создании запросов с помощью  `SensitiveType` свойства см. [запрос формы для поиска конфиденциальных данных, хранящихся на сайтах](form-a-query-to-find-sensitive-data-stored-on-sites.md).

### <a name="limitations-for-searching-sensitive-data-types"></a>Ограничения на поиск конфиденциальных типов данных

- Свойство можно использовать только `SensitiveType` для поиска встроенных типов данных конфиденциальной информации. Вы не можете искать настраиваемые типы конфиденциальных данных, созданные вами (или другим администратором) для вашей организации. Используйте столбец **Издатель** на вкладке **типы конфиденциальной информации** в центре соответствия (или в свойстве **издателя** в PowerShell) для различения встроенных и пользовательских типов конфиденциальной информации. Встроенные типы конфиденциальных данных определяются значением корпорации **Майкрософт** в столбце **Издатель** .
  
- Невозможно использовать типы данных конфиденциальной информации и `SensitiveType` свойство Search для поиска конфиденциальных данных на месте в почтовых ящиках Exchange Online. Тем не менее, вы можете использовать политики защиты от потери данных (DLP) для защиты конфиденциальных данных электронной почты при передаче. Дополнительные сведения см в разделе [Обзор политик защиты от потери данных](data-loss-prevention-policies.md) и [Поиск персональных данных](search-for-and-find-personal-data.md).
...
|Метка соответствия требованиям|Для электронной почты и документов метки хранения, назначенные сообщениям и документам, автоматически назначаются политиками автометки или метками хранения, назначенными пользователями вручную. Метки хранения используются для классификации электронной почты и документов для управления сведениями и применения правил хранения на основе параметров, определенных меткой. Вы можете ввести часть имени метки хранения и использовать подстановочный знак или ввести полное имя метки. Дополнительные сведения о метках хранения можно узнать в [статье сведения о политиках хранения и метках хранения](retention.md).|
|||
...
В приведенных ниже примерах показана версия поискового запроса с использованием графического интерфейса пользователя с условиями, синтаксис запроса поиска, отображаемый в области сведений выбранного поиска (который также возвращается командлетом **Get-ComplianceSearch** ), и логики соответствующего запроса KQL. 
...
Некоторые специальные символы не включены в индекс поиска и поэтому не могут быть доступны для поиска. Сюда также входят специальные символы, представляющие операторы поиска в поисковом запросе. Ниже приведен список специальных символов, которые либо заменяются пробелами в фактическом поисковом запросе, либо вызывают ошибку поиска.
...
- Изменения в членстве в группах, например добавление или удаление внешних пользователей в группы Microsoft 365 или группы безопасности Майкрософт 365. Свойство не будет автоматически обновляться для элементов, к которым у группы есть доступ.
...
Поисковый запрос, использующий свойство, будет возвращать только документы, для которых используется третий параметр (общий доступ для **определенных пользователей**)  `SharedWithUsersOWSUser` . 
...
- Логические операторы **and**, **or**, **Not**и **NEAR** должны быть прописными. 
...
- Вы можете экспортировать элементы в соответствии с типом сообщения. Например, чтобы экспортировать беседы и беседы Skype в Microsoft Teams, используйте синтаксис `kind:im` . Чтобы возвратить только сообщения электронной почты, используйте `kind:email` . Чтобы вернуть беседы, собрания и звонки в Microsoft Teams, используйте `kind:microsoftteams` .
