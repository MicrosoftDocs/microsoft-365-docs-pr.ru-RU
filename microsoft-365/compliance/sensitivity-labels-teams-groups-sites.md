...
- Эта функция по-прежнему находится на этапе развертывания у некоторых клиентов. Если политика условного доступа с выбранным контекстом проверки подлинности не действует, когда пользователь получает доступ к сайту, можно использовать PowerShell, чтобы проверить правильность конфигурации и соблюдение всех необходимых условий. Потребуется удалить метку конфиденциальности с сайта, затем настроить сайт для контекста проверки подлинности с помощью командлета [Set-SPOSite](/powershell/module/sharepoint-online/set-sposite) в текущей [командной консоли SharePoint Online](/powershell/sharepoint/sharepoint-online/connect-sharepoint-online). Если этот метод работает, подождите несколько дней перед следующей попыткой применения метки конфиденциальности.
    
    Для тестирования контекста проверки подлинности с помощью PowerShell выполните следующие действия:
    ...
    Для удаления контекста проверки подлинности (чтобы можно было снова попробовать применить метку конфиденциальности) выполните следующие действия:
