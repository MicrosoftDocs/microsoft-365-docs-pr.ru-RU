...
- В журнале аудита Office 365 с помощью операции поиска найдите последние действия над файлами и папками, выполненные пользователем, который является объектом запроса субъекта данных. Операция поиска в журнале аудита возвратит список записей аудита, содержащих имена и расположения ресурсов, с которыми пользователь недавно взаимодействовал. Вы, возможно, сможете использовать эту информацию для создания запроса на поиск контента. См. статью [Поиск в журнале аудита в Центре безопасности и соответствия требованиям Office 365](https://support.office.com/article/Search-the-audit-log-in-the-Office-365-Security-Compliance-Center-0d4d0f35-390b-4518-800e-0c7ec95e946c).
...
Задачи (называемые *делами*, сохраненными в *списках задач*) в Microsoft To-Do хранятся в виде задач в почтовом ящике Exchange Online пользователя. Это означает, что вы можете использовать средство "Поиск контента" для поиска, открытия, удаления и экспорта задач. Дополнительные сведения см. в статье [Настройка Microsoft To-Do](https://support.office.com/article/Set-up-Microsoft-To-Do-490c1a8c-2333-4952-8125-841afadb9620).
...
1. Откройте средство "Поиск контента" в Центре безопасности и соответствия требованиям.
2. На всплывающей странице поиска щелкните ![](media/o365-dsr_image21.png) **Дополнительно**, а затем — **Экспорт результатов**. Вы также можете экспортировать отчет.
...
Вам не удастся удалить сайт, который поставлен на удержание службой обнаружения электронных данных или к которому применена политика хранения. Чтобы можно было удалить такой сайт, сначала требуется отменить удержание службы обнаружения электронных данных или политику хранения.
...
|![Значок Project](media/o365-project-64x64.png) <br> [Project](#project-online) |![Значок Publisher](media/o365-publisher-64x64.png) <br> [Publisher](#publisher) |![Значок StaffHub](media/o365-staffhub-64x64.png) <br> [StaffHub](#staffhub) |
...
Вы можете экспортировать данные о заданиях определенного учащегося для всех классов, в которые он зачислен, с помощью скрипта PowerShell. См. следующие статьи:
...
Вы можете удалить данные о заданиях определенного учащегося для всех классов, в которые он зачислен, с помощью скрипта PowerShell. Это следует делать перед удалением учащегося из класса. См. следующие статьи:
...
3. На странице **Окружения** щелкните среду для потоков, к которым вы хотите получить доступ. У организаций Office 365 имеется среда, используемая по умолчанию.
...
- Данные профиля пользователя и его контакты
...
- [Глобальный администратор Office 365](https://support.office.com/article/assign-admin-roles-in-office-365-for-business-eac4d046-1afd-4f1a-85fc-8219c79e1504) или
- [Глобальный администратор Azure Active Directory](https://docs.microsoft.com/azure/active-directory/active-directory-assign-admin-roles-azure-portal)
...
В следующих разделах описано, как использовать встроенные функции Microsoft StaffHub для поиска, просмотра, экспорта и удаления персональных данных.
...
- **Сведения о планировании.** На вкладке **Планирование** в StaffHub доступны представления **Люди** и **Смены**, что позволяет просматривать расписания на день, неделю и месяц для участников команды или по рабочим сменам. Эти представления можно распечатать или скачать в Excel, нажав многоточие в верхнем левом углу веб-приложения StaffHub и выбрав пункт **Экспорт расписания**.
- **Общие файлы.** Все файлы в StaffHub хранятся в SharePoint Online, поэтому вы можете обнаруживать и экспортировать их, а также получать доступ к ним с помощью средства "Поиск контента", как упоминалось ранее. Кроме того, эти файлы можно просматривать в StaffHub на вкладке **Файлы**.
- **Обмен сообщениями.** В настоящее время доступ к приложениям можно получить, выполнив действия для доступа к пользовательским данным, описанные в следующем пункте. В будущем все сообщения в StaffHub будут храниться в Microsoft Teams, то есть вы сможете использовать средство "Поиск контента" для доступа к ним.
- **Пользовательские данные.** Пользовательские данные включают пользовательские настройки, журнал действий пользователя и журнал запросов смен, которые не могут просматривать участники команды и администраторы. Чтобы можно было получить доступ к пользовательским данным или экспортировать их, субъект данных должен войти в свою учетную запись StaffHub. Кроме того, вы можете получить эти данные, даже если субъект данных покинул организацию. Для этого администратор должен сбросить пароль пользователя для субъекта данных, чтобы вы или администратор могли войти в его учетную запись.
- **Kronos.** StaffHub поддерживает подключения к [Kronos](http://www.kronos.com/) — стороннему средству управления трудовыми ресурсами. StaffHub и Kronos являются независимыми обработчиками и обрабатывают данные организации в соответствии с особыми условиями, оговоренными с каждой стороной. Если ваш запрос субъекта данных относится к данным, хранящимся в Kronos, вам потребуется связаться с Kronos, чтобы получить помощь по DSR, и наоборот.
...
Глобальный администратор Office 365 может получить доступ к учетной записи пользователя в OneDrive для бизнеса.
...
- Профиль пользователя
    ...
    - Если у пользователя есть удостоверение Yammer, то он может полностью управлять своим профилем. Сведения о том, как просмотреть и изменить профиль, см. в статье [Изменение профиля и параметров в Yammer](https://support.office.com/article/change-my-yammer-profile-and-settings-a3aeca0e-de34-4897-9b59-de6516542851).
    - Если у пользователя есть удостоверение Office 365, система автоматически получает профиль пользователя в Yammer из Office 365, который, в свою очередь, получает сведения профиля из Azure Active Directory (AAD). Пользователи Yammer могут временно изменять свои профили в Yammer, но эти изменения будут перезаписаны при внесении изменений в AAD, поэтому лучше просматривать и изменять данные каталога в AAD. Пользователь должен запросить выполнение обновлений AAD. См. статьи [Управление пользователями Yammer в течение их жизненного цикла из Office 365](https://support.office.com/article/Manage-Yammer-users-across-their-life-cycle-from-Office-365-6c4c8fff-6444-404a-bffc-f9da0bcc3039) и [Добавление или изменение данных профиля пользователя в Azure Active Directory](https://docs.microsoft.com/azure/active-directory/active-directory-users-profile-azure-portal).
...
- **Данные популярных документов.** Это документы и вложения в электронные письма, которые могут быть интересны пользователю. Delve динамически упорядочивает эти документы и электронные письма на основе действий пользователя и сведений о том, с какими людьми пользователь работает в Office 365. Когда пользователь открывает Delve или щелкает **Главная**, Delve отображает документы или вложения, самые релевантные для пользователя на данный момент. Чтобы получить доступ к фактическим документам и вложениям либо экспортировать их, пользователь может перейти в службу Office 365, в которой доступен документ или вложение (например, Office.com, SharePoint Online, OneDrive для бизнеса или Exchange Online).
...
- **Данные профиля.** Пользователь может удалить данные профиля, выбрав пункты **Моя страница** \> **Обновить профиль**. В зависимости от параметров вашей организации в глобальном списке адресов у пользователей может не быть возможности удалять сведения в их профилях, например имена или должности.
...
Как указано выше, в Workplace Analytics используются данные Office 365 в сочетании с данными организации, предоставляемыми для создания интересующих вас отчетов. Вам не удастся уточнить данные в Office 365, так как они основаны на сведениях о действиях пользователя в почте и календаре. Но вы можете уточнить данные организации, отправленные в Workplace Analytics для создания отчета. Для этого вам потребуется уточнить исходные данные, отправить их и повторно создать отчет Workplace Analytics.
...
3. На странице **Запрос субъекта данных**, в разделе **Системные журналы**, щелкните **Экспорт журнала данных**.
    ...
    Отобразится страница **Экспорт журнала данных**. На этой странице отображается список запросов на экспорт данных, отправленных вашей организацией.

4. Чтобы создать запрос для пользователя, щелкните **Создать запрос на экспорт данных**.

После создания запроса он появится в списке на странице **Экспорт журнала данных**, на которой вы сможете отслеживать состояние запроса. После выполнения запроса вы можете перейти по ссылке, чтобы получить доступ к системным журналам, которые будут экспортированы в место хранения вашей организации в Azure в течение 30 дней с даты создания запроса. Данные сохраняются в стандартном машиночитаемом формате, например JSON или XML. Если у вас нет учетной записи Azure или места хранения в Azure, вам потребуется создать учетную запись Azure или место хранения в Azure для своей организации, чтобы средство экспорта журнала данных могло экспортировать системные журналы. Дополнительные сведения см. в статье [Общие сведения о службе хранилища Azure](https://docs.microsoft.com/azure/storage/common/storage-introduction).
...
- **Сколько времени нужно средству экспорта журнала данных для выполнения запроса?** Это может зависеть от ряда факторов. Обычно это занимает день или два, но может потребоваться до 30 дней.
...
- **У кого есть доступ к средству экспорта журналов данных для отправки запросов на доступ к системным журналам?** Доступ к служебной программе GDPR Log Manager есть у глобальных администраторов Office 365.

- **Какие данные возвращает средство экспорта журналов данных?** Средство экспорта журналов данных возвращает системные журналы, хранящиеся в службах Майкрософт. Экспортируемые данные извлекаются из различных служб Майкрософт, включая Office 365, Azure и Dynamics.

- **Каким образом данные возвращаются пользователю?** Данные экспортируются в ваше место хранения Azure организации. Администраторы организации определяют порядок отображения или возврата данных пользователям.

- **Как выглядят данные в системных журналах?** Вот пример записи системного журнала в формате JSON:
...
Для удаления системных журналов в следующих национальных облачных развертываниях глобальный ИТ-администратор должен выполнить указанные ниже действия.
...
В журнале аудита вы можете искать определенные действия, типы действий, действия, выполненные определенным пользователем, и применять другие критерии поиска. Помимо действий в SharePoint Online и OneDrive для бизнеса вы можете выполнять поиск действий в Flow, Power BI и Microsoft Teams. Записи аудита хранятся 90 дней. Поэтому вам не удастся найти действия пользователей, выполненные более 90 дней назад. Полный список действий, подвергаемых аудиту, и сведения о том, как выполнять поиск в журнале аудита, см. в статье [Поиск в журнале аудита в Центре безопасности и соответствия требованиям Office 365](search-the-audit-log-in-security-and-compliance.md).
...
|11.07.2019| [MyAnalytics](#myanalytics)|Возможность использования администратором средства для работы с делами DSR в Центре безопасности и соответствия требованиям Office 365 с целью экспорта данных MyAnalytics была удалена, так как теперь все пользователи могут просматривать свои данные в приложении MyAnalytics. |
||||
