...
2. Из выходных данных PowerShell можно просмотреть расположения с ошибками в поле "Ошибки" или сведения о состоянии ошибки из выходных данных поиска.
...
4. В поле результатов найдите данные, которые были экспортируются, и просматриваем все ошибки.
...
1. Раз break the search into smaller searches and run the search again.  Попробуйте использовать меньший диапазон дат или ограничить число мест поиска.
...
Ошибка распространения синхронизации политики удержания дела eDiscovery. Ошибка считывет:
...
2. Проверьте значение параметра DistributionDetail на ошибки, такие как следующие:
...
Чтобы устранить эту проблему, повторите [скачивание](export-search-results.md#step-2-download-the-search-results)результатов поиска, после чего перезапустите средство экспорта eDiscovery.
