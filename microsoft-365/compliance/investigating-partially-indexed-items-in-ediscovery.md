...
- Если указать диапазон дат для запроса поиска (включив его в запрос ключевого слова или с помощью условия), любой частично индексированный элемент, не совпадающий с диапазоном дат, не входит в число частично индексных элементов. Частично индексные элементы, которые входят в диапазон дат, включаются в число индексных элементов.
...
| `parserencrypted` <br/> |Содержимое вложения или сообщения электронной почты шифруется, и Microsoft 365 не может расшифровать содержимое.  <br/> |
...
| `parserunknowntype` <br/> |Вложение было типом файла, который Не удалось обнаружить Microsoft 365.  <br/> |
| `parserunsupportedtype` <br/> |Вложение имеет тип файла, который Office 365 может обнаруживать, но его размыв не поддерживается.  <br/> |
| `propertytoobig` <br/> |Значение свойства электронной почты в Exchange Store было слишком большим, чтобы получить и сообщение не может быть обработано. Обычно это происходит только с свойством тела сообщения электронной почты.  <br/> |
...
В следующих действиях покажите, как запустить скрипт PowerShell, который выполняет поиск всех элементов во всех почтовых ящиках Exchange, а затем создает отчет о соотношении частично индексных элементов электронной почты организации (по количеству и размеру) и отображает количество элементов (и их тип файла) для каждой ошибки индексации. Чтобы определить ошибку индексирования, используйте описания тегов ошибок в предыдущем разделе.
...
2. [Подключитесь к Центру безопасности и соответствия требованиям PowerShell](/powershell/exchange/exchange-online-powershell).
