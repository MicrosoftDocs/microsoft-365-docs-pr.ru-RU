...
- **Применение меток на стороне службы, когда содержимое уже сохранено (в SharePoint или OneDrive) или отправлено по электронной почте (обрабатывается в Exchange Online)**: используйте политику автоматического применения меток. 
    ...
    - Поддерживаются файлы Office для Word, PowerPoint и Excel. Поддерживается формат Open XML (например, DOCX или XLSX), но не формат Microsoft Office 97–2003 (например, DOC или XLS).
        - Эти файлы можно автоматически пометить во время хранения до или после создания политик автоматического применения меток. Обратите внимание, что файлы нельзя пометить автоматически, если они являются частью открытого сеанса (файл открыт).
