...
Метаданные меток включают сведения, идентифицирующие клиента и примененную метку конфиденциальности. Изменение, которое вносит этот параметр, — это формат метаданных и расположение метаданных для файлов Word, Excel и PowerPoint. Вам не нужно предпринимать никаких действий с зашифрованными файлами или сообщениями электронной почты. Изменение метаданных для зашифрованных файлов обратно совместимо и не изменяет сообщения электронной почты. Однако необходимо знать об изменениях метаданных для зашифрованных файлов, которые могут быть автоматически обновлены, но не поддерживают обратную совместимость.
...
В силу данных изменений не включайте этот параметр при наличии в вашей организации приложений, служб, сценариев или инструментов, которые считывают или записывают метаданные меток в старое расположение. В противном случае возможен ряд последствий:
...
- Приложения Microsoft 365 для предприятий:
    - **Windows**: минимальная версия 2105: 18 июня
    - **macOS**: минимальная версия 16.50
...
- Некоторые документы не удастся использовать с метками конфиденциальности из-за таких функций, как [защита паролем](https://support.microsoft.com/office/require-a-password-to-open-or-modify-a-workbook-10579f0e-b2d9-4c05-b9f8-4109a6bce643), [общие книги](https://support.microsoft.com/office/about-the-shared-workbook-feature-49b833c0-873b-48d8-8bf2-c1c59a628534), или из-за содержимого, включающего элементы ActiveX. Другие причины рассматриваются в статье [Устранение неполадок совместного редактирования в Office](https://support.microsoft.com/office/troubleshoot-co-authoring-in-office-bd481512-3f3a-4b6d-b7eb-ebf9d3626ae7). Для этих документов отображается сообщение **СБОЙ ОТПРАВКИ** и требуется выбрать параметр **Отменить изменения**. Пока эта проблема не будет исправлена, не применяйте метки к документам, для которых выводится это сообщение о сбое.
...
> [!CAUTION]
> Включение этого параметра является односторонним действием. Пока функция представлена в предварительной версии, включайте ее только после ознакомления с изменениями метаданных, предварительными требованиями, ограничениями и известными проблемами, описанными на этой странице.

1. Войдите в [Центр соответствия требованиям Microsoft 365](https://compliance.microsoft.com) в качестве глобального администратора для клиента.

2. В области навигации выберите **Параметры** > **Совместное редактирование файлов с файлами конфиденциальности**.

2. На странице **Совместное редактирование файлов с метками конфиденциальности (предварительная версия)** ознакомьтесь со сводным описанием, предварительными требованиями, сведениями о том, чего ожидать, и предупреждением о том, что вы не сможете отключить этот параметр после его включения.
    
    Затем выберите **Включить совместное редактирование для файлов с метками конфиденциальности** и нажмите **Применить**:
...
3. Подождите 24 часа, пока этот параметр будет реплицирован в вашей среде, прежде чем использовать эту новую функцию для совместного редактирования.
...
После включения функции совместного редактирования файлов с метками конфиденциальности для клиента, вы не сможете отключить этот параметр самостоятельно. Именно поэтому так важно проверить и понять предварительные требования, последствия и ограничения перед включением этого параметра.
...
Как видно на снимке экрана, когда этот параметр включен, вы можете обратиться в [службу поддержки Майкрософт](../business-video/get-help-support.md) и запросить отключение этого параметра. Этот запрос может занять несколько дней, и вам потребуется подтвердить, что вы являетесь глобальным администратором клиента. Ожидайте, что может быть применена обычная плата за поддержку. 
...
- Совместное редактирование и автосохранение больше не используются в вашем клиенте для помеченных и зашифрованных документов.
