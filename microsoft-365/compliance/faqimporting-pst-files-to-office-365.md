...
description: В этой статье содержатся ответы на часто задаваемые вопросы об импорте файлов PST в Microsoft 365 с помощью службы импорта Office 365.
...
**Эта статья предназначена для администраторов. Вам нужно импортировать PST-файлы в свой почтовый ящик? См. [Раздел "Импорт писем, контактов и календаря" из PST-файла Outlook](https://go.microsoft.com/fwlink/p/?LinkID=785075)**
   
Ниже приведены вопросы и ответы об использовании службы импорта Office 365 для массового импорта файлов PST в почтовые ящики Microsoft 365. Дополнительные сведения об импорте PST-файлов см. в статье ["Общие сведения об импорте PST-файлов в Office 365".](https://docs.microsoft.com/microsoft-365/compliance/importing-pst-files-to-office-365)
  
## <a name="using-network-upload-to-import-pst-files"></a>Импорт PST-файлов с помощью отправки по сети

Пошаговые инструкции см. в статье ["Импорт PST-файлов в Office 365 с помощью отправки по сети".](use-network-upload-to-import-pst-files.md)
...
В настоящее время отправка по сети доступна в следующих регионах: США, Канада, Бразилия, Соединенное Комиссия, Франция, Германия, Швейцария, Северная, Европа, Индия, Восточная Азия, Юго-Азия, Япония, Республика Корея, Австралия и Объединенные Арабские Емираты. Network upload will be available in more regions soon.
...
Существуют две версии формата PST-файлов: ANSI и Юникод. Мы рекомендуем импортировать PST-файлы в формате Юникода. Однако PST-файлы в формате ANSI, например используемые для языков с двухбайтовой кодировкой (DBCS), также можно импортировать в Office 365. For more information about importing ANSI PST files, see Step 4 in [Use network upload to import your organization's PST files to Office 365.](use-network-upload-to-import-pst-files.md#step-4-create-the-pst-import-mapping-file)
...
Пошаговые инструкции см. в статье ["Импорт PST-файлов в Office 365 с помощью отправки дисков.](use-drive-shipping-to-import-pst-files-to-office-365.md)
...
Для службы импорта Office 365 можно использовать только твердотельные накопители (SSD), 2,5-дюймовый или внутренние жесткие диски SATA II или III с форм-образцом 2,5" или внутренние жесткие диски SATA II или III с форм-образцами. емкостью до 10 ТБ. При выполнении заданий импорта на каждом жестком диске будет обрабатываться только первый том данных. Тома должны быть отформатированы в файловой системе NTFS. When copying data to a hard drive, you can attach it directly using a 2.5-inch SSD or 2.5-inch or 3.5-inch SATA II/III connector or you can attach it externally using an external 2.5-inch SSD or 2.5-inch sATA II/III USB adaptor.
...
После получения жесткого диска требуется от 7 до 10 рабочих дней для загрузки PST-файлов в хранилище Azure для вашей организации. PST-файлы будут загружены в контейнер BLOB-объектов Azure с именем `ingestiondata`. 
