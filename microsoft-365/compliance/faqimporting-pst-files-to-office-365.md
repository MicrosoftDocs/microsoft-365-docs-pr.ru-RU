...
В настоящее время в этих регионах доступна сетевая Отправка: США, Канада, Бразилия, Великобритания, Франция, Германия, Европа, Индия, Азиатско-Восточной Азии, Юго-Восток, Австралия, Республика Корея, Австралия и Объединенные Арабские Эмираты (ОАЭ). Network upload will be available in more regions soon.
...
Процесс импорта PST проверяет наличие дублирующихся элементов и не копирует элементы из файла PST в почтовый ящик или архив, если соответствующий элемент существует в целевой папке в целевом почтовом ящике или целевом архиве. При повторном импорте того же PST-файла и указания другой целевой папки (с помощью свойства TargetRootFolder в файле сопоставления PST-импорта), которое было указано в предыдущем задании импорта, все элементы в PST-файле будут импортироваться повторно.
...
После получения жесткого диска в центре обработки данных Майкрософт для отправки PST-файлов в область хранилища Azure для Организации потребуется от 7 до 10 рабочих дней. PST-файлы будут загружены в контейнер BLOB-объектов Azure с именем `ingestiondata`. 
