 ...
 **Как процесс импорта PST-файлов обрабатывает повторяющиеся элементы электронной почты?**

Процесс импорта PST-файлов проверяет наличие дублирующихся элементов и не копирует элементы из PST-файла в почтовый ящик или в архив, если в целевой папке в целевом почтовом ящике или целевом архиве существует совпадающий элемент. Если вы повторно импортируете один и тот же PST-файл и хотите указать другую целевую папку (с помощью свойства TargetRootFolder в файле сопоставления PST-импорта), которое было указано в предыдущем задании импорта, все элементы в PST-файле будут повторно импортированы.
...
Существуют две версии формата PST-файлов: ANSI и Юникод. Мы рекомендуем импортировать PST-файлы в формате Юникода. Однако PST-файлы в формате ANSI, например используемые для языков с двухбайтовой кодировкой (DBCS), также можно импортировать в Office 365. Дополнительные сведения об импорте PST-файлов в формате ANSI приведены в разделе Шаг 3 в разделе [Использование доставки дисков для импорта PST-файлов в Office 365](use-drive-shipping-to-import-pst-files-to-office-365.md#step-3-create-the-pst-import-mapping-file).
