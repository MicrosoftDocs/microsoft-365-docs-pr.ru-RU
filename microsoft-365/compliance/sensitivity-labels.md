...
> [!NOTE]
> В настоящее время для клиентов облака сообщества для государственных организаций США (GCC и GCC-HC) метки конфиденциальности поддерживаются только в клиенте и сканере унифицированных меток Azure Information Protection. 
> 
> Дополнительные сведения см. в записи блога [Объявление об общей доступности унифицированных меток в средах GCC и GCC-H](https://techcommunity.microsoft.com/t5/azure-information-protection/announcing-general-availability-of-unified-labeling-in-gcc-and/ba-p/1563215).
...
К каждому элементу контента может применяться одна метка конфиденциальности. К элементу может быть применена одна метка конфиденциальности и одна [метка хранения](retention.md#retention-labels) одновременно.
...
- **Защищайте содержимое в контейнерах, таких как сайты и группы**, включив [использование меток конфиденциальности с Microsoft Teams, группами Microsoft 365 и сайтами SharePoint](sensitivity-labels-teams-groups-sites.md).
    ...
    Параметры конфигурации для **настроек сайтов и групп** не отображаются, пока вы не включите эту возможность. Имейте в виду, что такая конфигурация меток не приводит к автоматической маркировке документов, а настройки меток защищают содержимое, контролируя доступ к контейнеру, в котором хранятся документы. Эти параметры включают уровень конфиденциальности, возможность добавления гостей в группу Microsoft 365 ее владельцем и уровень доступа к неуправляемому устройству.
...
- **Требовать от пользователей применения метки к их почте и документам.** Также называется обязательным применением меток. Вы можете требовать обязательное применение меток, перед тем как пользователи смогут сохранять документы и отправлять письма. Метка может присваиваться вручную пользователем, автоматически в результате применения настроенного условия или назначаться по умолчанию (вариант применения метки по умолчанию описан выше). Пример запроса, отображаемого в Outlook, когда пользователь должен назначить метку:
    ...
    > [!NOTE]
    > В настоящее время для обязательного присвоения меток требуется [клиент унифицированных меток Azure Information Protection](https://docs.microsoft.com/azure/information-protection/rms-client/install-unifiedlabelingclient-app). Этот клиент работает только в Windows, поэтому эта функция еще не поддерживается на устройствах Mac, на базе iOS и Android.
    
    Рекомендуется использовать этот параметр, чтобы расширить применение меток. Но без обучения пользователей этот параметр может приводить к неправильному применению метки. Кроме того, если вы не настроили метку по умолчанию, обязательное применение меток может раздражать пользователей частыми запросами. 
