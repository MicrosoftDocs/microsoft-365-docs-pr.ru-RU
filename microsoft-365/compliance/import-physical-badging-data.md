...
Можно настроить соединители данных в центре соответствия требованиям Microsoft 365 для импорта физических данных, например необработанных событий физического доступа сотрудника или любых физических сигналов тревоги доступа, созданных системой злонастроя вашей организации. Примерами физических точек доступа являются вход в здание или запись в серверную комнату или центр обработки данных. Физические плохие данные могут быть использованы решением управления рисками изнутри Microsoft 365, чтобы защитить организацию от вредоносных действий или кражи данных внутри организации. [](insider-risk-management.md)

Настройка физического соединитетеля плохой настройки состоит из следующих задач:

- Создание приложения в Azure Active Directory (Azure AD) для доступа к конечной точке API, которая принимает полезной нагрузки JSON, которая содержит физические плохие данные.

- Создание полезной нагрузки JSON с помощью схемы, определяемой физическим соединителем данных для ненадежных данных.

- Создание физического соединитетеля данных с плохими данными в центре соответствия требованиям Microsoft 365.
...
|AccessStatus|Значение `Success` или `Failed`| Строка|
|||
...
Следующий шаг — создание физического соединитетеля плохой обработки в центре соответствия требованиям Microsoft 365. После запуска сценария в шаге 4 файл JSON, созданный в шаге 3, будет обработан и отодвигается к конечной точке API, настроенной в шаге 1. На этом шаге обязательно скопируйте JobId, созданный при создании соединитетеля. При запуске сценария вы будете использовать JobId.
...
После запуска скрипта файл JSON, содержащий физические данные о плохом управлении, перенаправлен в организацию Microsoft 365, к которой можно получить доступ с помощью решения по управлению рисками. Мы рекомендуем ежедневно размещать физические данные о плохой обработке. Это можно сделать, автоматизируя процесс создания файла JSON каждый день из вашей физической системы плохой работы, а затем запланировать сценарий, чтобы нажать данные.
...
7. Запустите следующую команду, чтобы нажать физические плохие данные в файле JSON в облако Microsoft; Например:
   ...
   | Параметр | Описание |
   |:-------------|:--------------|
   |tenantId | Это Id для вашей организации Microsoft 365, полученный в шаге 1. Вы также можете получить tenantId для организации на лезвии **Обзор** в центре администрирования Azure AD. Это используется для идентификации организации. |
   |appId | Это Id приложения Azure AD для приложения, созданного в Azure AD на шаге 1. Это используется Azure AD для проверки подлинности при попытке скрипта получить доступ к организации Microsoft 365.                    |
...
После создания физического соединиттеля плохой защиты и нажатия данных физического ненадежного состояния можно просмотреть соединители и загрузить состояние в центре соответствия требованиям Microsoft 365. Если вы запланировать автоматический запуск скрипта на регулярной основе, вы также можете просмотреть текущее состояние после последнего запуска сценария.
