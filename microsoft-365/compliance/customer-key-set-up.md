...
3. Выберите **"Развернуть"** на  панели навигации  **ИЛИ** выберите "Просмотреть все ресурсы развертывания" на карточке "Развертывание информации" и просмотрите список текущих предложений.
...
3. Для завершения этого процесса обратитесь в корпорацию Майкрософт. Для команды SharePoint и OneDrive для бизнеса свяжитесь [с](mailto:spock@microsoft.com)spock@microsoft.com. Для Exchange Online и Skype для бизнеса свяжитесь [с](mailto:exock@microsoft.com)exock@microsoft.com. Включите в сообщение электронной почты следующие сведения:
...
Используйте соглашение об именовке для хранилищ ключей, отражающие предполагаемый использование DEP, с которым будут связываться хранилища. Рекомендации по именовам см. в разделе "Рекомендации" ниже.
...
Для создания хранилищ ключей также требуется создание групп ресурсов Azure, так как хранилищам ключей необходима емкость хранилища (хотя и небольшая), а ведение журнала Key Vault, если включено, также создает сохраненные данные. Корпорация Майкрософт рекомендует использовать отдельных администраторов для управления каждой группой ресурсов, при этом администрирование согласуется с набором администраторов, которые будут управлять всеми связанными ресурсами ключа клиента.
...
Если вы сможете быстро восстановить ключи, вы с меньшей вероятностью будете испытывать расширенные отключения службы из-за случайного или злонамеренного удаления ключей. Прежде чем использовать ключи с ключом клиента, необходимо включить эту конфигурацию, которая называется "Soft Delete". Включение функции soft Delete позволяет восстановить ключи или хранилища в течение 90 дней после удаления, не восстанавливая их из резервной копии.
...
3. Подтвердите, что для хранилища ключей настроено мягкое удаление, задав для этого **cmdlet Get-AzKeyVault.** Если мягкое удаление настроено правильно для хранилища ключей, свойство _Soft Delete Enabled_ возвращает значение **True:**
...
- *имя ключа* — это имя, которое вы хотите предоставить новому ключу.
  ...
  > [!TIP]
  > Клавиши имен, использующие аналогичные соглашения об именованиях, как описано выше, для хранилищ ключей. Таким образом, в средствах, которые показывают только имя ключа, строка описывается самостоятельно.
...
Обратитесь в группу безопасности, чтобы определить, требуются ли вышеуказанные проверки. Дополнительные сведения о создании ключа в локальной службе и его импорте в хранилище ключей см. в описании создания и передачи ключей, защищенных с использованием [HSM,](https://azure.microsoft.com/documentation/articles/key-vault-hsm-protected-keys/)для Azure Key Vault. Используйте инструкции Azure, чтобы создать ключ в каждом хранилище ключей.
...
Где *MailUserIdParameter* указывает пользователя почты (также известного как пользователь с включенной поддержкой почты). Дополнительные сведения о Set-MailUser см. в [сообщении Set-MailUser.](https://docs.microsoft.com/powershell/module/exchange/set-mailuser)
