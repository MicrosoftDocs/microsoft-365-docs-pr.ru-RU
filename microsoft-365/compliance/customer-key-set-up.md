...
Прежде чем связываться с командой Microsoft 365, необходимо сделать следующее для каждой подписки Azure, используемой с ключом клиента. Перед началом установки убедитесь, что у вас установлен модуль [Azure PowerShell Az.](https://docs.microsoft.com/powershell/azure/new-azureps-module-az)
...
В некоторых организациях такой подход используется для установления их ключей, а затем этот метод также предоставляет следующие проверки:
...
Если свойство _уровня_ восстановления возвращает что-либо, кроме значения **Recoverable+ProtectedSubscription,** убедитесь, что подписка включена в список "Не отменять" и что в каждом хранилище ключей включено мягкое удаление.
