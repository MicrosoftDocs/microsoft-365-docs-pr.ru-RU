...
7. In the left navigation pane under **Facebook Login,** click **Settings**, and add the OAuth redirect URI in the **Valid OAuth Redirect URI** box. Используйте формат **\<connectorserviceuri> /Views/FacebookOAuth,** где значением connectorserviceuri является URL-адрес службы приложений Azure для вашей организации, например `https://fbconnector.azurewebsites.net` .

   ![Добавление URI перенаправления OAuth в поле "Допустимые URI перенаправления OAuth"](../media/FBCimage31.png)

8. В левой области навигации  щелкните "Добавить продукты", а затем щелкните **"Веб-hooks".** В  оттягиваемом меню "Страница" щелкните **"Страница".**
...
13. Добавление пользователя в роль администратора или тестера.
...
3. На странице **"Бизнес-страницы Facebook"** нажмите кнопку **"Добавить соединителю".**
...
5. На странице **"Добавление учетных** данных для приложения соединителю" введите следующие сведения и нажмите кнопку **"Проверить подключение".**
   ...
   - В поле **"ИД** приложения Azure" введите или введите или введите значение ИД приложения (клиента), также называемого ИД приложения AAD, созданного на шаге 1.
