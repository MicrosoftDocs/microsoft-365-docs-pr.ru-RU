...
| Примечания | Комментарии по умолчанию для оповещения. Значения : *New Alert* (регистрируется при создания оповещений) и Alert *Updated* (в журнале при обновлении оповещений). Используйте AlertID для сопоставления обновлений для оповещения. |
...
| Version | Версия схемы оповещения о безопасности и соблюдении требований. |
...
Вместо того, чтобы быть открытыми для проверки всеми аналитиками и следователями, группам пользователей priority также может потребоваться ограничить действия по обзору определенным пользователям или группам ролей риска. Вы можете назначить отдельным пользователям и группам ролей для просмотра пользователей, оповещений, случаев и отчетов для каждой приоритетной группы пользователей. Приоритетные группы пользователей могут иметь разрешения на просмотр встроенных групп риска *insider,* *insider Risk Management Analysts* и *Insider Risk Management Investigators* role groups, one or more of these role groups, or to a custom selection of users.

Например, необходимо защититься от утечек данных для очень конфиденциального проекта, в котором пользователи имеют доступ к конфиденциальной информации. Вы решите создать *группу пользователей конфиденциальной Project* *пользователей* для пользователей в организации, которые работают над этим проектом. Кроме того, эта приоритетная группа пользователей не должна иметь пользователей, оповещений, случаев и отчетов, связанных с группой, видимыми всем администраторам, аналитикам и следователям по умолчанию. В **Параметры** вы создаете  группу Project пользователей конфиденциальности и назначаете двух пользователей в качестве рецензента, который может просматривать данные, связанные с группами. С помощью мастера  политики и утечек данных по шаблону политики приоритетных пользователей создается новая политика и назначается группа приоритетов Project пользователей политики.  Действия, рассмотренные политикой для членов приоритетной группы *пользователей Project Пользователей,* более чувствительны к рискам, и действия этих пользователей с большей вероятностью будут генерировать оповещение и иметь оповещения с более высоким уровнем серьезности.

### <a name="create-a-priority-user-group"></a>Создание приоритетной группы пользователей

Чтобы создать новую группу пользователей с приоритетом, вы будете использовать элементы управления настройками в решении по управлению рисками **insider** в Центр соответствия требованиям Microsoft 365. Чтобы создать группу пользователей с приоритетом, необходимо быть членом группы по управлению рисками *insider* или *Insider Risk Management Admin* role group.

Выполните следующие действия для создания приоритетной группы пользователей:

1. В [Центр соответствия требованиям Microsoft 365](https://compliance.microsoft.com)перейдите к **управлению** рисками insider и выберите параметры риска **insider.**
2. Выберите **вкладку Priority user groups (preview).**
3. На **вкладке Priority user groups (preview)** выберите **Создать** приоритетную группу пользователей, чтобы запустить мастер создания группы.
4. На странице **Имя и описание** выполните следующие поля:
...
8. На странице **Выбор, кто может** просматривать эту группу, необходимо определить, кто может просматривать пользователей, оповещения, случаи и отчеты для приоритетной группы пользователей. Должна быть назначена по крайней мере одна группа ролей управления рисками для пользователей или инсайдеров. Выберите **выбор пользователей и групп ролей** и выберите группы ролей для управления рисками, которые необходимо назначить приоритетной группе пользователей. Выберите **Добавить,** чтобы назначить выбранным пользователям или группам ролей группу.
9. Нажмите кнопку Далее, чтобы продолжить.
10. На странице **Обзор** просмотрите параметры, выбранные для приоритетной группы пользователей. Выберите **ссылки Редактирование,** чтобы изменить любое из групповых значений или выберите **Отправка** для создания и активации приоритетной группы пользователей.
11. На странице подтверждения выберите **Сделано,** чтобы выйти из мастера.

### <a name="update-a-priority-user-group"></a>Обновление приоритетной группы пользователей

Чтобы обновить существующую группу пользователей с приоритетом, в решении по управлению рисками **insider** в Центр соответствия требованиям Microsoft 365. Чтобы обновить приоритетную группу пользователей, необходимо быть членом группы управления рисками *insider* или *insider Risk Management Admin* role group.
...
4. На странице **Имя и описание** обнови поле Описание при необходимости. Вы не можете обновить имя приоритетной группы пользователей. Нажмите кнопку **Далее**, чтобы продолжить.
5. На странице **Выбор участников** добавьте новых членов в группу с помощью управления **"Выберите** членов". Чтобы удалить пользователя из группы, выберите "X" рядом с пользователем, который вы хотите удалить. Нажмите кнопку **Далее**, чтобы продолжить.
6. На странице **Выберите,** кто может просматривать эту группу, добавьте или удалите пользователей или группы ролей, которые могут просматривать пользователей, оповещения, случаи и отчеты для приоритетной группы пользователей.
7. Нажмите кнопку **Далее**, чтобы продолжить.
8. На странице **Обзор** просмотрите параметры обновления, выбранные для приоритетной группы пользователей. Выберите **ссылки Редактирование,** чтобы изменить любое из групповых значений или выберите **Отправка** для обновления приоритетной группы пользователей.
...
Чтобы удалить существующую группу пользователей с приоритетом, в решении по управлению рисками **insider** в Центр соответствия требованиям Microsoft 365. Чтобы удалить приоритетную группу пользователей, необходимо  быть членом группы "Управление рисками инсайдеров" или "Администратор роли управления *рисками".*
...
Чтобы настроить приоритетные физические активы, вы настроите соединители физической плохой настройки и используйте элементы управления настройками в решении управления рисками **insider** в Центр соответствия требованиям Microsoft 365. Чтобы настроить приоритетные физические активы, необходимо быть членом группы *insider Risk Management* или *Insider Risk Management Admin role group.*
...
2. В [Центр соответствия требованиям Microsoft 365](https://compliance.microsoft.com)перейдите к **управлению** рисками insider и выберите **параметры insider** risk  >  **priority physical assets**.
...
Чтобы удалить существующий физический актив приоритета, в решении по управлению рисками insider в Центр соответствия требованиям Microsoft 365. Чтобы удалить приоритетный физический актив, необходимо быть членом группы по управлению рисками insider или Insider Risk Management Admin role group.
...
1. В [Центр соответствия требованиям Microsoft 365](https://compliance.microsoft.com)перейдите к **управлению** рисками insider и выберите **параметры insider** risk  >  **priority physical assets**.
...
Чтобы создать поток Power Automate из рекомендуемого шаблона управления рисками из инсайдерской системы управления рисками, вы будете использовать элементы управления настройками в  решении управления рисками **insider** в Центр соответствия требованиям Microsoft 365 или  **параметре Управление** потоками Power Automate из управления автоматом при работе непосредственно в панели мониторинга "Случаи" или "Пользователи". 
...
1. В [Центр соответствия требованиям Microsoft 365](https://compliance.microsoft.com/)перейдите к **управлению** рисками insider и выберите параметры риска   >  **insider Power Automate потоков.** Вы также можете  получить  доступ со страниц панели мониторинга "Случаи" или "Пользователи", выбрав **автоматизированное** управление потоками  >  **Power Automate.**
...
3. Поток перечисляет встроенные подключения, необходимые для потока, и отмечает, доступны ли состояния подключения. При необходимости обнови все подключения, которые не отображаются как доступные. Нажмите **Продолжить**.
...
Чтобы удалить поток, вы будете использовать элементы управления настройками в решении управления рисками **insider** в Центр соответствия требованиям Microsoft 365  или параметре Управление потоками Power Automate из управления автоматом при работе непосредственно в панелях мониторинга "Случаи" или  **"Пользователи".**  При удалении потока он удаляется в качестве параметра для всех пользователей.
...
1. В [Центр соответствия требованиям Microsoft 365](https://compliance.microsoft.com)перейдите к **инсайдерской параметров** риска  >  **управления рисками.**
...
1. В [Центр соответствия требованиям Microsoft 365](https://compliance.microsoft.com)перейдите в **insider risk management**  >  **Cases** и выберите существующий случай.
