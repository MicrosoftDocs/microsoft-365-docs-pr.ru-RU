...
- **Индикатор нарушения политики безопасности**: эти индикаторы включают индикаторы от защитника Майкрософт для конечной точки, связанные с установкой неутвержденных или вредоносных программ, а также обхода средств безопасности. Для получения оповещений в управлении рисками для предварительной оценки необходимо включить функцию "активный защитник для лицензии на конечную точку" и "Предварительная интеграция риска". Дополнительные сведения о настройке защитника для конечной точки для интеграции управления рисками для участников программы можно найти [в статье Настройка дополнительных функций в защитнике Майкрософт для конечной точки](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/advanced-features\#share-endpoint-alerts-with-microsoft-compliance-center).
...
3. Установите браузер Microsoft Чромиум EDGE на устройстве Endpoint, чтобы отслеживать действия для действия отправки в облако. См. статью [Скачивание нового браузера Microsoft Edge на основе Chromium](https://support.microsoft.com/help/4501095/download-the-new-microsoft-edge-based-on-chromium).
...
### <a name="microsoft-defender-for-endpoint-preview"></a>Защитник Майкрософт для конечной точки (Предварительная версия)

[Защитник Майкрософт для конечной точки](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/microsoft-defender-advanced-threat-protection) — это платформа безопасности для корпоративных конечных точек, предназначенная для предотвращения, обнаружения, проверки и реагирования на дополнительные угрозы в корпоративных сетях. Чтобы лучше понять нарушение безопасности в Организации, вы можете импортировать и отфильтровывать защитник для оповещений о конечных точках для действий, используемых в политиках, созданных из шаблонов политики нарушений безопасности управления рисками.

В зависимости от требуемых типов сигналов можно выбрать Импорт оповещений в систему управления рисками в зависимости от состояния рассмотрения оповещения о конечных точках. В глобальных параметрах для импорта можно определить одно или несколько следующих состояний рассмотрения оповещения:
...
Оповещения от защитника для конечной точки импортируются ежедневно. В зависимости от выбранного состояния рассмотрения вы можете увидеть несколько действий пользователя для одного и того же оповещения, когда состояние рассмотрения изменяется в защитнике для конечной точки.

Например, если выбрано значение *создать*, *в ходе выполнения* и *разрешено* для этого параметра, то при создании оповещения Майкрософт для оповещения о конечной точке и изменении состояния " *новое*" для пользователя будет импортировано исходное действие оповещения. Когда состояние рассмотрения состояния конечной точки изменяется на " *выполняется*", для пользователя будет импортировано второе действие для этого оповещения. Если установлен параметр "завершено" для конечного состояния рассмотрения для конечной точки " *разрешено* ", третье действие для этого оповещения импортируется для пользователя в составе риска для оценки. Эта функция позволяет изучению отслеживать ход выполнения защитника для оповещений конечной точки и выбирать уровень видимости, необходимый для их расследования.
...
| Состояние | Состояние оповещения. Значения *активны* (*необходимо проанализировать* в риске для эксперта), *исследование* (*подтверждено* в риске для эксперта), *устранено* (*разрешено* в результате оценки), *закрыто* (было *закрыто* в угрозе "Эксперт"). |
