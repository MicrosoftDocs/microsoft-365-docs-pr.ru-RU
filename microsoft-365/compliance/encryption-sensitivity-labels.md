...
Параметры шифрования доступны при [создании метки конфиденциальности](create-sensitivity-labels.md) в Центре соответствия требованиям Microsoft 365. Также можно использовать прежний портал: Центр безопасности и соответствия требованиям.

## <a name="understand-how-the-encryption-works"></a>Как осуществляется шифрование

При шифровании используется служба управления правами Azure (Azure RMS) из Azure Information Protection. Это решение защиты использует политики шифрования, удостоверений и авторизации. Дополнительные сведения см. в статье [Управление правами Azure](/azure/information-protection/what-is-azure-rms) из документации по Azure Information Protection. 

При использовании этого решения шифрования функция **суперпользователей** гарантирует, что авторизованные пользователи и службы всегда смогут считывать и проверять данные, зашифрованные для вашей организации. При необходимости шифрование затем можно удалить или изменить. Дополнительные сведения см. в статье [Настройка суперпользователей для Azure Information Protection и служб обнаружения или восстановления данных](/azure/information-protection/configure-super-users).

## <a name="how-to-configure-a-label-for-encryption"></a>Настройка метки для шифрования

1. Выполните общие инструкции по [созданию или редактированию метки конфиденциальности](create-sensitivity-labels.md#create-and-configure-sensitivity-labels) и убедитесь, что для области метки выбрано **Файлы и сообщения электронной почты**. 
...
2. Затем на странице **Выбор параметров защиты для файлов и сообщений электронной почты** выберите **Шифрование файлов и сообщений электронной почты**
...
4.  На странице **Шифрование** мастера выберите один из следующих вариантов.
    
    - Параметр **Удалить шифрование, если файл зашифрован** поддерживается только клиентом для работы с унифицированными метками Azure Information Protection. Если выбрать этот параметр и использовать встроенные метки, метка может не отображаться в приложениях или отображаться и не вносить изменения в шифрование.
        
        Дополнительные сведения об этом сценарии см. в разделе [Что происходит с существующим шифрованием при применении метки](#what-happens-to-existing-encryption-when-a-labels-applied). Важно понимать, что выбор этого параметра может привести к тому, что пользователи не смогут применить метку конфиденциальности, если у них не будет необходимых разрешений.
    ...
    - **Настройка параметров шифрования**. Включает шифрование и отображает его параметры.
        ...
        Инструкции по настройке этих параметров см. в разделе [Настройка параметров шифрования](#configure-encryption-settings) ниже.
...
| | Шифрование: не выбрано | Шифрование: настроено | Шифрование: удалено <sup>\*</sup> |
|:-----|:-----|:-----|:-----|
...
<sup>\*</sup> Поддерживается только клиентом для работы с унифицированными метками Azure Information Protection

В случаях применения нового шифрования с меткой или удаления исходного шифрования эти действия выполняются, только если у пользователя, применяющего метку, есть право на использование или роль, поддерживающие это действие:

- [Право на использование](/azure/information-protection/configure-usage-rights#usage-rights-and-descriptions) экспорта или полного доступа.
- Роль [издателя в службе управления правами или владельца в службе управления правами](/azure/information-protection/configure-usage-rights#rights-management-issuer-and-rights-management-owner) или [суперпользователя](/azure/information-protection/configure-super-users).
...
Выбрав **Настройка параметров шифрования** на странице **Шифрование** мастера, чтобы создать или изменить метку конфиденциальности, выберите один из следующих вариантов.
...
Используйте следующие параметры, чтобы управлять доступом к сообщениям электронной почты и документам, к которым применена эта метка. Доступны следующие варианты:
...
Когда пользователь открывает в режиме автономного доступа документ или сообщение электронной почты, защищенные шифрованием службы Azure Rights Management, пользователю предоставляется лицензия на использование службы Azure Rights Management для такого содержимого. Такая лицензия на использование — это сертификат, содержащий права пользователя на использование такого документа или сообщения электронной почты, а также ключ шифрования, который использовался для шифрования этого содержимого. Лицензия на использование также содержит дату окончания срока действия, если она была задана, и сведения о том, как долго будет действительна эта лицензия на использование.
...
Помимо повторной проверки подлинности заново проверяются параметры шифрования и принадлежность пользователей к группам. Это означает, что результаты доступа к одну и тому же документу или сообщению электронной почты для пользователей могут быть разными, если параметров шифрования или принадлежность к группам изменились после предыдущего доступа к этому содержимому.

Чтобы узнать, как изменить стандартный параметр 30-дневного срока действия, см. раздел [Лицензия на использования службы Microsoft Azure AD Rights Management](/azure/information-protection/configure-usage-rights#rights-management-use-license).
...
- Любой определенный пользователь, группа безопасности с поддержкой электронной почты, группа рассылки или группа Microsoft 365 ([прежнее название — группа Office 365](https://techcommunity.microsoft.com/t5/microsoft-365-blog/office-365-groups-will-become-microsoft-365-groups/ba-p/1303601)) в Azure AD. В группах Microsoft 365 может быть статическое или [динамическое членство](/azure/active-directory/users-groups-roles/groups-create-rule). Обратите внимание, что вы не можете использовать [динамическую группу рассылки из Exchange](/Exchange/recipients/dynamic-distribution-groups/dynamic-distribution-groups), поскольку этот тип группы не синхронизируется с Azure AD, и вы не можете использовать группу безопасности, для которой не включена поддержка электронной почты.
...
> [!NOTE]
> Рассмотрите возможность использования этого параметра с [интеграцией SharePoint и OneDrive с Azure AD B2B](/sharepoint/sharepoint-azureb2b-integration-preview) при включенных метках конфиденциальности [для файлов Office в SharePoint и OneDrive](sensitivity-labels-sharepoint-onedrive-files.md).
...
- [Заранее определенный уровень разрешений](/azure/information-protection/configure-usage-rights#rights-included-in-permissions-levels) с предварительно заданной группой прав, например "Соавтор" или "Рецензент".
- Настраиваемые разрешения, в которых можно выбрать одно или несколько прав использования.

Дополнительные сведения о выборе соответствующих разрешений см. в разделе [Права на использование и их описание](/azure/information-protection/configure-usage-rights#usage-rights-and-descriptions).  
...
Дополнительные сведения см. в статье [Издатель в службе управления правами и владелец в службе управления правами](/azure/information-protection/configure-usage-rights#rights-management-issuer-and-rights-management-owner).
...
> [!IMPORTANT]
> Не все клиенты применения меток поддерживают все параметры, разрешающие пользователям назначать собственные разрешения. Подробные сведения см. в этом разделе.
...
- В Outlook пользователь может выбрать ограничения, эквивалентные параметру [Не пересылать](/azure/information-protection/configure-usage-rights#do-not-forward-option-for-emails) или [Только шифрование](/azure/information-protection/configure-usage-rights#encrypt-only-option-for-emails) для определенных получателей.
    ...
    Чтобы проверить минимальный номер версии приложений Outlook, использующих встроенное применение меток для поддержки применения параметра "Только шифрование" с меткой конфиденциальности, см. [таблицу возможностей для Outlook](sensitivity-labels-office-apps.md#sensitivity-label-capabilities-in-outlook) и строку **Предоставление пользователям возможности назначать разрешения: только шифрование**.
...
Если любой из этих параметров применяется к сообщению электронной почты, оно зашифровывается, а его получатели должны пройти проверку подлинности. После этого получатели автоматически получают ограниченные права использования:

- **Не пересылать**: получатели не смогут пересылать или печатать сообщение электронной почты, а также копировать из него фрагменты. Например, в клиенте Outlook кнопка "Переслать", пункты меню "Сохранить как" и "Печать" будут недоступны, и вы не сможете добавить или изменить получателей в полях "Кому", "Копия" или "СК".
    
    Дополнительные сведения о том, как работает этот параметр, см. в разделе [Параметр "Не пересылать" для электронной почты](/azure/information-protection/configure-usage-rights#do-not-forward-option-for-emails).
...
В каждом из следующих примеров для выполнения настройки на странице **Шифрование** мастера выберите **Настройка параметров шифрования**.
...
- При совместном использовании зашифрованных документов с людьми за пределами вашей организации может потребоваться создать гостевые учетные записи и изменить политики условного доступа. Дополнительные сведения см. в разделе [Совместное использование зашифрованных документов с внешними пользователями](sensitivity-labels-office-apps.md#support-for-external-users-and-labeled-content).
...
- Следующие действия с зашифрованными файлами не поддерживаются в приложениях Office (в Windows, Mac, Android и iOS), а для пользователей отображается сообщение об ошибке. При этом в качестве альтернативы можно использовать функциональность SharePoint.
...
- Активация защиты из Azure Information Protection
    ...
    Чтобы метки конфиденциальности применяли шифрование, в вашем клиенте должна быть активирована служба защиты (Azure Rights Management) из Azure Information Protection. В новых клиентах этот параметр используется по умолчанию, но вам может потребоваться активировать службу вручную. Дополнительные сведения см. в статье [Активация службы защиты из Azure Information Protection](/azure/information-protection/activate-service).

- Проверка требований к сети
    
    Возможно, потребуется внести некоторые изменения на сетевых устройствах, таких как брандмауэры. Подробные сведения см. в разделе [Брандмауэры и сетевая инфраструктура](/azure/information-protection/requirements#firewalls-and-network-infrastructure) документации по Azure Information Protection.

- Настройка Exchange для Microsoft Azure Information Protection
    
    Нет необходимости настраивать Exchange для Azure Information Protection, прежде чем пользователи смогут применять метки в Outlook, чтобы шифровать свои сообщения электронной почты. Тем не менее, до настройки Exchange для Azure Information Protection вы не сможете задействовать все функции при использовании защиты Microsoft Azure AD Rights Management с Exchange.
    ...
    - Для Exchange Online см. инструкции для [Exchange Online: настройка IRM](/azure/information-protection/configure-office365#exchangeonline-irm-configuration).
    - Для использования Exchange в локальной среде необходимо развертывание [соединителя RMS и настройка серверов Exchange](/azure/information-protection/deploy-rms-connector). 
