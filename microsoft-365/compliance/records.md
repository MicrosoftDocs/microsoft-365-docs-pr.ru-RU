...
По поводу записей необходимо учитывать нижеуказанные нюансы.
  ...
  - **Записи невозможно удалить**. При попытке пользователя удалить запись в Exchange элемент перемещается в папку "Элементы с возможностью восстановления" в соответствии с разделом [Что делает политика хранения с содержимым на месте](retention-policies.md#content-in-mailboxes-and-public-folders).
    ...
    При попытке пользователя удалить запись в OneDrive элемент перемещается в архивную библиотеку в соответствии с разделом  [Что делает политика хранения с содержимым на месте](retention-policies.md#content-in-onedrive-accounts-and-sharepoint-sites).
...
Создаваемую метку хранения можно использовать для классификации содержимого в качестве записи. Чтобы обозначить содержимое как запись, необходимо совершить указанные ниже действия.
...
:::image type="content" source="media/recordversioning7.png" alt-text="Область сведений о документе, отмеченном как запись":::
...
:::image type="content" source="media/recordversioning8.png" alt-text="Свойство состояния записи в документе, обозначенном как запись":::

> [!NOTE]
> Для управления версиями записей каждому пользователю, у которого есть разрешения на изменение обозначенного как запись содержимого на сайте SharePoint или в учетной записи OneDrive, требуется лицензия на Office 365 корпоративный E5. Пользователям с доступом только для чтения лицензия не требуется.

### <a name="locking-and-unlocking-a-record"></a>Блокировка и разблокировка записи

После присвоения любому документу метки записи любой пользователь, который (на уровне разрешений "Участие") входит в группу "Участники", используемую по умолчанию, может разблокировать запись и заблокировать разблокированную запись.
<br/><br/>

:::image type="content" source="media/recordversioning9.png" alt-text="Состояние записи демонстрирует, что документ, имеющий статус записи, разблокирован":::
...
:::image type="content" source="media/recordversioning10.png" alt-text="Отображение записи в архивной библиотеке":::
...
:::image type="content" source="media/recordversioning11.png" alt-text="Архивная библиотека":::

<br/><br/>

:::image type="content" source="media/recordversioning12.png" alt-text="Папка "Записи" в архивной библиотеке":::
...
:::image type="content" source="media/recordversioning13.png" alt-text="Поиск событий управления версиями записей в журнале аудита":::
