...
4. Зарегистрируйте приложение. В **поле "URI перенаправления" (необязательно)** выберите "Интернет" в списке типов приложений, а затем введите поле для  `https://portal.azure.com` URI.
...
5. На **вкладке "Ключи** и маркеры" в ключах API потребителя скопируйте ключ **API** и секретный ключ API и сохраните их в текстовом файле или другом хранилище. Затем нажмите **кнопку "Создать",** чтобы создать маркер доступа и секрет маркера доступа и скопировать их в текстовый файл или другое место хранения.
...
5. На странице **"Добавление учетных** данных для приложения соединителю" введите следующие сведения и нажмите кнопку **"Проверить подключение".**
   ...
   ![Ввод учетных данных приложения соединитела](../media/TCimage38.png)
    ...
    - В поле **"ИД** приложения Azure" введите или введите или введите значение ИД приложения приложения Azure (также называемый ИД клиента), полученный на шаге 1.
