...
<sup>\*</sup> Для меток хранения, которые не помечают содержимое как запись или нормативную запись, события аудита ограничены временем применения, изменения или удаления метки для элемента в SharePoint. Сведения об аудите меток хранения см. в разделе [Аудит действий хранения](#auditing-retention-actions) на этой странице.
...
- Когда метка хранения применяется, изменяется или удаляется для элемента в SharePoint:
...
- Если элемент с меткой в SharePoint или Exchange помечен как запись или нормативная запись и он окончательно удаляется:
