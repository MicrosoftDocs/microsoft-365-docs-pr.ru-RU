...
Например, к элементу может применяться одна политика хранения, настроенная только на удаление, и другая политика хранения, настроенная на сохранение, а затем удаление. Результат определяет от факт, что для этого элемента настроены одно действие по сохранению и два действия по удалению. Возможны конфликты между сохранением и удалением, а также между датами двух действий по удалению.

В целом вы можете быть уверены в том, что у хранения всегда приоритет перед удалением, и наиболее предпочтителен вариант с самым продолжительным сроком хранения. Эти два простые правила всегда определяют, как долго будет храниться элемент.
...
Блок-схема ниже поможет вам понять, какими будут результаты действий сохранения и удаления для одного элемента. В этой последовательности каждый уровень сверху вниз выступает в роли решающего критерия.
...
1. К элементу применены следующие настройки хранения:
    ...
    **Результат.** Элемент хранится в течение семи лет из-за приоритета хранения перед удалением, а семь лет — самый длительный срока хранения. По истечении этого срока хранения элемент удаляется, поскольку активизируется действие по удалению из политик хранения, отложенное на срок хранения элемента.
    ...
    Несмотря на то, что у двух политик разные даты для действий по удалению, элемент не может был удален ранее, чем по окончании самого длительного срока хранения, так что здесь конфликт не возникает.
...
2.  К элементу применены следующие настройки хранения:
    ...
    **Результат.** Элемент хранится в течение пяти лет, поскольку это самый длительный срок хранения.  По истечении этого срока хранения элемент удаляется, поскольку активизируется действие по удалению из метки хранения — удаление через три года, — отложенное на срок хранения элемента. Удаление из меток хранения имеет приоритет перед удалением из политик хранения.
