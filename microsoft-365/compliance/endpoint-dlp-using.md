...
Если для режима списка задан параметр **Разрешить**, пользователи смогут отправлять конфиденциальные данные **_только_* _ в эти домены, но доступ к отправке в другие домены будет запрещен.
...
### <a name="always-audit-file-activity-from-onboarded-devices"></a>Всегда проводите аудит действий с файлами с подключенных устройств

Управляйте возможностью автоматического аудита действия DLP для файлов Office, PDF и CSV, а также его доступностью для проверки в телеметрии аудита и обозревателе действий с подключенных устройств. 

Если эта возможность включена (по умолчанию), действия с файлами всегда подвергаются аудиту для подключенных устройств независимо от того, включены ли они в активную политику защиты от потери данных.
Если эта возможность отключена, действия с файлами для подключенных устройств подвергаются аудиту, только если они включены в активную политику защиты от потери данных. 
...
3. Настройте политики защиты от потери данных, чтобы определить типы конфиденциальных элементов, которые нельзя отправлять, включив параметры _ *Отправка в облачные службы** и **Доступ из запрещенного браузера**.
