...
---
# <a name="set-up-a-connector-to-archive-att-smsmms-data"></a>Настройка соединителя для архивации на&T SMS/MMS-данные
...
4. Соединитель импортирует мобильные элементы в почтовый ящик определенных пользователей. В почтовом ящике пользователя в почтовом ящике пользователя создается новая папка с именем **&T SMS/MMS Network WinRAR** и в нее импортируются элементы. Соединитель выполняет это сопоставление, используя значение свойства *электронного адреса пользователя* . Каждое сообщение SMS и MMS содержит это свойство, которое заполняется адресом электронной почты каждого участника сообщения.
...
Некоторые действия по реализации, необходимые для архивации данных в сети&T, являются внешними по отношению к Microsoft 365 и должны быть завершены до того, как вы сможете создать соединитель в центре соответствия требованиям.
