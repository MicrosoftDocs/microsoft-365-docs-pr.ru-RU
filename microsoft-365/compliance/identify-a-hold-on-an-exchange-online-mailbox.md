...
- **[Удержание на месте:](/Exchange/security-and-compliance/create-or-remove-in-place-holds)** Удерживает, которые применяются к почтовым ящикам пользователей с помощью In-Place средства & удержания в центре администрирования Exchange в Exchange Online. 
...
Чтобы подключиться к центру & безопасности PowerShell, см. в & Connect [to Security & PowerShell.](/powershell/exchange/connect-to-scc-powershell)
...
- Запустите команду [Set-RetentionCompliancePolicy \<Policy Name> -Identity -AddExchangeLocationException \<user mailbox> ](/powershell/module/exchange/set-retentioncompliancepolicy) в Центре обеспечения безопасности & PowerShell, чтобы исключить почтовый ящик из политики хранения Microsoft 365 для всей организации. Эта команда может использоваться только для политик хранения, где значение свойства *ExchangeLocation* равно `All` .
