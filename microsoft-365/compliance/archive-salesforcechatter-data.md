---
title: Настройка соединитетеля для архивации данных chatter Salesforce в Microsoft 365
f1.keywords:
- NOCSH
ms.author: markjjo
author: markjjo
...
Используйте соединитель Veritas в центре Microsoft 365 для импорта и архивации данных с платформы Chatter Salesforce в почтовые ящики пользователей в Microsoft 365 организации. Veritas предоставляет [соединитель чата Salesforce,](http://globanet.com/chatter/) который захватывает элементы из стороннего источника данных и импортирует эти элементы в Microsoft 365. Соединитетель преобразует содержимое, например чаты, вложения и сообщения из чата Salesforce в формат электронной почты, а затем импортирует эти элементы в почтовый ящик пользователя в Microsoft 365.
...
В следующем обзоре объясняется процесс использования соединитетеля для архивации данных chatter Salesforce в Microsoft 365.
...
2. Раз в 24 часа элементы чата Salesforce копируется на сайте Слияния1 Veritas. Соединитетель также элементы Chatter Salesforce в формате сообщений электронной почты.

3. Соединитель чата Salesforce, который создается в центре Microsoft 365 соответствия требованиям, подключается к сайту Veritas Merge1 каждый день и передает содержимое Chatter в безопасное служба хранилища Azure в облаке Майкрософт.
...
- Пользователь, создававший соединитетель чата Salesforce в шаге 1 (и завершавший его в шаге 3), должен быть назначен роли экспорта импорта почтовых ящиков в Exchange Online. Эта роль необходима для добавления соединители на **странице** соединители данных в центре Microsoft 365 соответствия требованиям. По умолчанию эта роль не назначена какой-либо группе ролей в Exchange Online. Вы можете добавить роль экспорта импорта почтовых ящиков в группу ролей управления организацией в Exchange Online. Или вы можете создать группу ролей, назначить роль экспортировать импорт почтовых ящиков, а затем добавить соответствующих пользователей в качестве участников. Дополнительные сведения см. в разделах [Создание](/Exchange/permissions-exo/role-groups#create-role-groups) групп ролей или [изменение](/Exchange/permissions-exo/role-groups#modify-role-groups) групп ролей в статье "Управление группами ролей в Exchange Online".

## <a name="step-1-set-up-the-salesforce-chatter-connector"></a>Шаг 1. Настройка соединиттеля чата Salesforce

Первым шагом является доступ  к странице Соединители данных в центре Microsoft 365 соответствия требованиям и создание соединитетеля для данных Chatter.
...
## <a name="step-2-configure-the-salesforce-chatter-on-the-veritas-merge1-site"></a>Шаг 2. Настройка чата Salesforce на сайте Veritas Merge1

Второй шаг — настройка соединителя чата Salesforce на сайте Veritas Merge1. Сведения о настройке соединиттеля чата Salesforce см. в руководстве по пользователю Merge1 Сторонние [соединители.](https://docs.ms.merge1.globanetportal.com/Merge1%20Third-Party%20Connectors%20SalesForce%20Chatter%20User%20Guide%20.pdf)

После **нажатия кнопки Сохранить &**  Finish отображается страница сопоставления пользователя в мастере соединители в центре Microsoft 365 соответствия требованиям.
...
После создания соединиттеля chatter Salesforce можно просмотреть состояние соединитетеля в центре Microsoft 365 соответствия требованиям.
