...
- Вам потребуется определить способ извлечения и экспорта данных из системы отдела кадров Организации (регулярно) и добавить CSV-файл, описанный в шаге 2. Скрипт, который вы запускаете на шаге 4, будет отправлять данные HR в CSV-файле в Microsoft Cloud.
...
Первым шагом является создание и регистрация нового приложения в Azure Active Directory (AAD). Приложение будет соответствовать соединителю HR, созданному на шаге 3.  Создание этого приложения позволит AAD проверить подлинность соединителя HR при его запуске и попытаться получить доступ к Организации. Это приложение также будет использоваться для проверки подлинности скрипта, выполняемого на шаге 4, для отправки данных отдела кадров в Microsoft Cloud. Во время создания приложения AAD обязательно сохраните следующие сведения. Эти значения будут использоваться на последующих этапах этого процесса.
...
Пошаговые инструкции по созданию приложения в AAD приведены в разделе [Создание приложения AAD](https://docs.microsoft.com/azure/kusto/management/access-control/how-to-provision-aad-app)
...
После создания CSV-файла с необходимыми данными о HR сохраните его на локальном компьютере или в сетевой папке, которую можно указать при запуске скрипта на шаге 4. Кроме того, следует реализовать стратегию обновления, чтобы CSV-файл всегда содержал актуальные сведения, чтобы при выполнении сценария все текущие данные о прекращении работы сотрудников передавались в Microsoft Cloud.
...
7. Выполните следующую команду, чтобы отправить данные отдела кадров в CSV-файл в Microsoft Cloud; Например:
   ...
   |`csvFilePath`|Это путь к файлу на локальном компьютере (который используется для запуска сценария) для CSV-файла, созданного в шаге 2. Если CSV-файл расположен в общей сетевой папке, необходимо указать полный путь к файлу для этого расположения. Старайтесь не использовать пробелы в пути к файлу; в противном случае используйте одинарные кавычки.|
   |||
...
3. В разделе **ход выполнения**щелкните ссылку **Журнал скачивания** , чтобы открыть (или сохранить) журнал состояний для соединителя. Этот журнал содержит сведения о каждом запуске скрипта и передает данные из CSV-файла в Microsoft Cloud. 
   ...
   Поле **рекордссавед** указывает количество строк в CSV-файле, который был отправлен. Например, если CSV-файл содержит 4 строки, то значение полей **рекордссавед** равно 4, если сценарий успешно отправил все строки в CSV-файле.
...
Чтобы гарантировать, что последние данные о HR из вашей организации доступны для таких средств, как решение для управления рисками, рекомендуется автоматически запускать сценарий регулярно, например, один раз в день. При этом также необходимо обновить данные отдела кадров в CSV-файле в соответствии с расписанием (если это не то же), чтобы оно содержало последние сведения о сотрудниках, которые покинут вашу организацию. Задача заключается в том, чтобы отправить наиболее актуальные данные о HR, чтобы соединитель кадров мог сделать его доступным для решения по управлению рисками для оценки.
...
8. В окне **Создание задачи** нажмите кнопку **ОК** , чтобы сохранить запланированную задачу. Возможно, вам будет предложено ввести учетные данные учетной записи пользователя.
   ...
   В последний раз отображается время запуска скрипта. Можно дважды щелкнуть задачу, чтобы изменить ее.
