...
- Чтобы запустить сценарий в шаге 3, необходимо быть членом группы ролей диспетчера электронных данных в Центре & безопасности и администратором SharePoint Online. Дополнительные сведения см. в дополнительных сведениях в центре Office [365 & eDiscovery.](assign-ediscovery-permissions.md)

- В удержание, связанном с делом об обнаружении электронных данных в Центре соответствия требованиям безопасности, можно добавить не более 10 & 00 почтовых ящиков и 100 сайтов. Если предположить, что каждый пользователь, который необходимо разместить на удержании, имеет OneDrive для бизнеса сайт, можно добавить не более 100 пользователей в удержание с помощью сценария в этой статье.
...
- Скрипт в этой статье поддерживает современную проверку подлинности при подключении к центру & безопасности PowerShell и SharePoint Online Management Shell. Сценарий можно использовать как есть, если вы Microsoft 365 или Microsoft 365 GCC организации. Если вы — Office 365, Microsoft 365 GCC или организация Microsoft 365 DoD, вам придется изменить сценарий, чтобы успешно запустить его. В частности, необходимо изменить строку и использовать параметры `Connect-IPPSSession` *ConnectionUri* и *AzureADAuthorizationEndpointUri* (а также соответствующие значения для типа организации) для подключения к Центру & соответствия требованиям PowerShell. Дополнительные сведения см. в примерах [в Подключение в центре & PowerShell.](/powershell/exchange/connect-to-scc-powershell#connect-to-security--compliance-center-powershell-without-using-mfa)

- Скрипт автоматически отключается от центра & безопасности PowerShell и SharePoint Online Management Shell.

- Сценарий включает минимальную обработку ошибок. Его основная цель состоит в том, чтобы быстро и легко разместить почтовый ящик и OneDrive для бизнеса каждого пользователя на удержание.
...
Первым шагом является установка SharePoint online Management Shell, если она еще не установлена на локальном компьютере. Вы не должны использовать оболочку в этой процедуре, но ее необходимо установить, так как она содержит предварительные требования, необходимые сценарию, который вы запустите в шаге 3. Эти условия позволяют скрипту взаимодействовать с SharePoint Online для получения URL-адресов для OneDrive для бизнеса сайтов.
  
Перейдите [](/powershell/sharepoint/sharepoint-online/connect-sharepoint-online) к настройкам SharePoint сетевой Windows PowerShell и выполните шаг 1 и шаг 2 для установки SharePoint online Management Shell на локальном компьютере.

## <a name="step-2-generate-a-list-of-users"></a>Шаг 2. Создание списка пользователей

Сценарий в шаге 3 создаст удержание, связанное с делом об обнаружении электронных данных, и добавит почтовые ящики и OneDrive для бизнеса сайты списка пользователей в удержание. Вы можете просто ввести адреса электронной почты в текстовом файле или запустить команду в Windows PowerShell, чтобы получить список адресов электронной почты и сохранить их в файле (расположен в той же папке, в которую вы сохраните сценарий в шаге 3).
  
Вот команда PowerShell (которую вы используете с помощью удаленной PowerShell, подключенной к вашей Exchange Online организации), чтобы получить список адресов электронной почты для всех пользователей в организации и сохранить его в текстовом файле с именем HoldUsers.txt.
...
- **Учетные данные пользователей:** Скрипт будет использовать учетные данные для подключения к Центру & безопасности с PowerShell. Он также будет использовать эти учетные данные для доступа SharePoint Online, чтобы получить OneDrive для бизнеса URL-адреса для списка пользователей.

- **Имя домена SharePoint:** Скрипт подсказывет вам ввести это имя, чтобы подключиться к центру SharePoint администратора. Он также использует доменное имя для URL OneDrive url-адресов в организации. Например, если URL-адрес центра администрирования и URL OneDrive для OneDrive, то вы введите, когда сценарий подсказит вам имя `https://contoso-admin.sharepoint.com` `https://contoso-my.sharepoint.com` `contoso` домена.
...
4. Введите сведения, которые вам подсказывует скрипт.
   ...
   Сценарий подключается к центру & безопасности PowerShell, а затем создает новое удержание в деле eDiscovery и добавляет почтовые ящики и OneDrive для бизнеса для пользователей в списке. Вы можете перейти к делу на странице **eDiscovery** в Центре & безопасности, чтобы просмотреть новое удержание.

После завершения работы скрипта он создает следующие файлы журналов и сохраняет их в папке, где находится сценарий.
  
- **LocationsOnHold.txt:** Содержит список почтовых ящиков и OneDrive для бизнеса сайтов, которые сценарий успешно поместил на удержание.

- **LocationsNotOnHold.txt:** Содержит список почтовых ящиков и OneDrive для бизнеса сайтов, которые сценарий не удерживал. Если у пользователя есть почтовый ящик, но не сайт OneDrive для бизнеса, он будет включен в список сайтов OneDrive для бизнеса, которые не были помещены в удержание.

- **GetCaseHoldPolicy.txt:** Содержит вывод **комлета Get-CaseHoldPolicy** для нового удержания, который сценарий запустил после создания нового удержания. Сведения, возвращенные этим комлетом, включают список пользователей, чьи почтовые ящики и сайты OneDrive для бизнеса были помещены на удержание и включено ли удержание или отключено. 
