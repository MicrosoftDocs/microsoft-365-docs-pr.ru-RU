...
- Каждый раз при запуске скрипта создаются новые сеансы безопасности & соответствия PowerShell и SharePoint Online PowerShell. Таким образом, вы можете использовать все сеансы PowerShell, доступные вам. Чтобы избежать этого, можно выполнить следующие команды, чтобы отключить активные сеансы PowerShell.
