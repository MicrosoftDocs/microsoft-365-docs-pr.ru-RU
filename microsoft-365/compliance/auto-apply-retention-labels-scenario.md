...
<p>&lt;d:EventType&gt;Прекращение производства&lt;/d:EventType&gt;</p>
...
Следующая таблица описывает параметры в свойстве "Текст" действия, которое должно быть настроено для этого сценария. 
...
<tr class="odd">
<td>Имя</td>
<td>Этот параметр указывает имя события, которое будет создано в Центре безопасности и соответствия требованиям. В нашем случае используется имя "Cessation Production xxx", где xxx — значение созданного ранее управляемого свойства ProductName. </th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td>EventType</td>
<td>Значение этого параметра соответствует типу событий, к которому будет применяться созданное событие. Этот тип событий был определен при создании метки хранения. В этом сценарии тип события — "Прекращение производства".</td>
</tr>
<tr class="even">
...
<tr class="odd">
<td>EventDateTime</td>
<td>Этот параметр определяет дату, когда происходит событие. Используйте текущий формат даты: <strong>formatDateTime(utcNow(),'yyyy-MM-dd'<strong>)</strong></td>
</tr>
</tbody>
</table>
