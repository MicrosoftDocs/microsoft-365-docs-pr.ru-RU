...
*Тип контента* можно создать и опубликовать с помощью [центра типов контента](https://support.office.com/article/manage-content-type-publishing-06f39ac0-5576-4b68-abbc-82b68334889b). Вы также можете создать и опубликовать тип контента с помощью средств подготовки сайта, например [платформы подготовки PnP](/sharepoint/dev/solution-guidance/pnp-provisioning-framework) или [схемы JSON для макетов сайта](/sharepoint/dev/declarative-customization/site-design-json-schema#define-a-new-content-type).
...
- **Метка записи**: Настройте метку хранения, чтобы пометить элементы как [запись](records-management.md#records). Это означает, что помеченные документы невозможно будет изменить или удалить.

- **Дескрипторы плана хранения:** Для упрощения сценария необязательные файловые дескрипторы не предоставлены.
...
1. На странице **определение параметров хранения** в мастере создания меток хранения, после **Запуска срока хранения на основе**, выберите **Создать новый тип события**:
    
    ![Создание нового типа события в диалоговом окне метки "Спецификация продукта"](../media/SPRetention6.png)

3. На странице **Имя типа события** , введите **Прекращение работы продукта** и дополнительное описание. Нажмите кнопку **Далее**, **Отправить**, и **Готово**.

4. Вернитесь назад на страницу **Определение параметров хранения**, чтобы **Запустить срок хранения на основе**, используйте выпадающий список для выбора **Прекращения работы продукта** типа события, которое вы создали.
...
6. Выберите **Создать метку**, и на следующей странице, когда появятся параметры для публикации метки, автоматического применения метки, или просто сохранения метки, выберите **Сохранить метку**, и затем нажмите **Готово**. 
...
## <a name="auto-apply-retention-labels-to-documents"></a>Метки хранения, применяемые автоматически к документам

Мы будем использовать язык KQL, чтобы [автоматически применять](apply-retention-labels-automatically.md) созданные метки хранения. KQL — это язык, который используется для создания поисковых запросов. С помощью KQL можно выполнять поиск по ключевым словам или управляемым свойствам. Больше информации см. в [Справочнике по синтаксису языка запросов по ключевым словам (KQL)](/sharepoint/dev/general-development/keyword-query-language-kql-syntax-reference).
...
Если ввести ***status** _ в поле _ *Свойства для обхода** и щелкнуть зеленую стрелку, появится приблизительно такой результат:
...
Дополнительные сведения о свойствах для обхода и управляемых свойствах см. в статье [Автоматически создаваемые управляемые свойства в SharePoint Server](/sharepoint/technical-reference/automatically-created-managed-properties-in-sharepoint).
...
1. В поле фильтра **Управляемое свойство** введите **_RefinableString00_** и щелкните зеленую стрелку.
...
3. Щелкните **Добавить сопоставление** и введите **_ows\_Doc\_x0020\_Type_*_ в поле _* Поиск имени свойства для обхода** в окне **Выбор свойства для обхода**. Нажмите кнопку **Найти**.  
...
Давайте проверим правильность настройки, запустив корпоративный поиск. В браузере перейдите по ссылке *https://\<your_tenant>.sharepoint.com/search*. В поле поиска введите ***RefinableString00:"Спецификация продукта"** _ и нажмите клавишу ВВОД. Этот поисковый запрос должен возвратить все документы, у которых свойство **_Doc Type_** имеет значение _ *Спецификация продукта**.

Теперь введите в поле поиска **RefinableString00:"Спецификация продукта" AND RefinableString01:Final** и нажмите клавишу ВВОД. Этот поисковый запрос должен возвратить все документы, у которых свойство **_Doc Type_*_ имеет значение **Спецификация продукта**, а _* Status** имеет значение **_Final_**.

### <a name="create-auto-apply-label-policies"></a>Создание политики автоматического применения меток

Итак, мы убедились, что запрос KQL работает. Теперь нужно создать политику автоматического применения меток, использующую запрос KQL для автоматического применения метки хранения "Спецификация продукта" к соответствующим документам.
...
2. В мастере создания политики автоматического применения меток, на странице **Назовите вашу политику автоматического применения** введите название, например **Автоматическое применение метки спецификации продукта**, и необязательное описание. Нажмите кнопку **Далее**.

3. На странице **Выберите тип содержимого, к которому хотите применить метку** выберите **Применить метку к содержимому с определенными словами, фразами или свойствами**, и затем нажмите **Далее**.
   ...
   Этот параметр позволяет указать тот же запрос KQL, который мы протестировали в предыдущем разделе. Этот запрос возвратил все документы типа "Спецификация продукта" со статусом *Окончательный*. Если сделать этот же запрос в политике автоматического применения меток, то метка хранения "Спецификация продукта" будет автоматически применена ко всем документам, которые соответствуют этому поисковому запросу.

4. На странице **Применить метку к содержимому, которое соответствует этому запросу** введите **RefinableString00:"Спецификация продукта" AND RefinableString01:Final**, затем нажмите кнопку **Далее**.

   ![Выберите запрос в поле "Редактор запросов по ключевому слову".](../media/SPRetention19.png)

5. На странице **Выберите расположения для применения политики** нужно выбрать расположения содержимого, к которым следует применить эту политику. В этом сценарии мы применяем политику только к расположениям SharePoint, поскольку все производственные документы хранятся в библиотеках документов SharePoint. Переключите статус для **почты Exchange**, **учетных записей OneDrive** и **групп Microsoft 365** в положение **Выкл**. Убедитесь, что переключатель для сайтов SharePoint **включен** и нажмите **Далее**: 
    
    ![Выберите сайты для автоматического присвоения меток.](../media/SPRetentionSPlocations.png)
    
   > [!TIP]
   > Вместо применения политики ко всем сайтам SharePoint можно щелкнуть **Выбрать сайт** и добавить URL-адреса определенных сайтов SharePoint.

6. На странице **Выбрать метку автоматического применения**, нажмите кнопку **Добавить метку**.

7. В списке меток хранения выберите **Спецификация продукта**. Затем щелкните **Добавить** и **Далее**.

8. Проверка параметров:

    ![Параметры автоматического применения метки](../media/SPRetention18.png)

9. Щелкните **Отправить**, чтобы создать политику меток.
...
Через 7 дней используйте [Обозреватель действий](data-classification-activity-explorer.md) в Центре соответствия требованиям, чтобы убедиться, что созданная нами политика автоматического применения меток автоматически применила метки хранения к документам о продуктах.
...
Однако в этом сценарии мы автоматически создадим это событие из внешней производственной системы. Вся система — это простой список SharePoint, где указано, находится ли продукт в производстве. Поток [Power Automate](/flow/getting-started), связанный со списком, вызывает событие. В реальном сценарии вы можете сформировать данное событие в разных системах, например в системе отдела кадров или CRM-системе. Power Automate содержит множество готовых взаимодействий и компонентов для рабочих нагрузок Microsoft 365, таких как Microsoft Exchange, SharePoint, Teams и Dynamics 365, а также для сторонних приложений, таких как Twitter, Box, Salesforce и Workdays. Эта функция упрощает интеграцию Power Automate с различными системами. Дополнительные сведения см. в статье [Автоматизация хранения на основе событий](./event-driven-retention.md#automate-events-by-using-a-rest-apiautomate-events-by-using-a-rest-api).
...
Метка хранения создана и автоматически применена, а поток настроен и создан. Если значение в столбце **В производстве** для продукта Spinning Widget из списка изменится с **_Да_*на*_Нет_*, запустится поток для создания события. Чтобы увидеть это событие в Центре соответствия требованиям, выберите* Управление записями** > **События**.
