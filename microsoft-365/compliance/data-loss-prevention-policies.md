...
Чтобы соответствовать бизнес-стандартам и отраслевым нормам, организации должны защищать конфиденциальные данные и предотвращать их случайное раскрытие. Конфиденциальная информация может включать финансовые данные или личные сведения, например номера кредитных карт, номера социального страхования или данные медицинских карт. Политика защиты от потери данных в Центре безопасности &amp;и соответствия требованиям Office 365 позволяет выявлять, отслеживать и автоматически защищать конфиденциальную информацию в Office 365.
...
- **Отслеживание и защита конфиденциальной информации в классических версиях Excel, PowerPoint и Word.**
    
    Эти классические приложения Office включают такие же возможности для определения конфиденциальных данных и применения политик защиты от потери данных, что и Exchange Online, SharePoint Online и OneDrive для бизнеса. Когда пользователи обмениваются контентом в этих приложениях Office, функции защиты от потери данных выполняют непрерывный мониторинг.
    
- **Информирование пользователей о способах обеспечения соответствия требованиям без прерывания их рабочих процессов.**
    
    Вы можете рассказать пользователям о политиках защиты от потери данных и о том, что соответствие требованиям не будет мешать их работе. Например, если пользователь пытается предоставить доступ к документу, содержащему конфиденциальную информацию, политика защиты от потери данных может отправить ему уведомление по электронной почте или в контексте библиотеки документов отобразить подсказку политики, которая позволяет переопределить политику при наличии веской деловой причины. Такие же подсказки политики отображаются и в Outlook в Интернете, Outlook, Excel, PowerPoint и Word.
...
![Подсказка политики для документа в учетной записи OneDrive](../media/f9834d35-94f0-4511-8555-0fe69855ce6d.png)
 ...
 Дополнительные сведения об уведомлениях пользователей и подсказках политик защиты от потери данных см. в статье [Использование уведомлений и подсказок политики](use-notifications-and-policy-tips.md).
...
Кроме того, вы также можете отправить отчет об инциденте с подробным описанием события. Отчет должен содержать сведения об элементе, попавшем под действие правила, фактическом содержимом, а также имя пользователя, который последним изменил это содержимое. Для электронных писем в отчет включается исходное сообщение, для которого сработала политика защиты от потери данных.
  
![Страница для настройки отчетов об инцидентах](../media/Alerts-and-incident-report.png)

Защита от потери данных сканирует электронную почту иначе, чем элементы в SharePoint Online или OneDrive для бизнеса. В SharePoint Online и OneDrive для бизнеса защита от потери данных сканирует как существующие, так и новые элементы и формирует отчет об оповещениях и инцидентах при обнаружении совпадения. В Exchange Online защита от потери данных сканирует только новые сообщения электронной почты и формирует отчет при обнаружении совпадения политике. Защита от потери данных * *_не_* _ сканирует и не сопоставляет ранее существующие элементы электронной почты, которые хранятся в почтовом ящике или архиве.
...
Например, во встроенной политике _ *акта HIPAA* * есть правило, в котором между группами используется оператор **AND** (И). С его помощью отбирают контент, который: 
...
- Использование метки хранения в политике **поддерживается только для элементов в SharePoint и OneDrive** _.
  ...
  - Вы опубликовали метку хранения под названием _*налоговый 2018 год**, которая после применения к налоговым документам за 2018 г., находящимся в SharePoint, обеспечивает их сохранение в течение 10 лет с последующим удалением. Кроме того, вы не хотите, чтобы эти элементы рассылались за пределы вашей организации, что можно сделать с помощью политики защиты от потери данных.
...
1. [Подключитесь к Центру безопасности и соответствия требованиям Office 365&amp; с помощью удаленного сеанса PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-scc-powershell).
    
2. Воспользуйтесь любыми из этих командлетов [policy-and-compliance-dlp cmdlets](https://docs.microsoft.com/powershell/module/exchange/export-dlppolicycollection)
...
1. [Подключитесь к Exchange Online с помощью удаленного сеанса PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-powershell).
