...
Microsoft 365 — это строго безопасная среда, которая обеспечивает расширенную защиту нескольких уровней: физическая безопасность центра обработки данных, безопасность сети, безопасность доступа, безопасность приложений и безопасность данных.
...
- **Клиентская связь с сервером** — обмен данными между SharePoint Online и OneDrive для бизнеса через Интернет использует подключения TLS.
...
В дополнение к шифрованию с шифрованием Microsoft 365 поддерживает ключ клиента, который создан на основе шифрования с учетом служб. Ключ клиента — это ключ, управляемый корпорацией Майкрософт, для шифрования службы Exchange Online, также включенный в план Майкрософт. Этот способ шифрования обеспечивает повышенную защиту, не предоставляемую BitLocker, так как она предоставляет разделение администраторов сервера и криптографические ключи, необходимые для расшифровки данных, а также из-за того, что шифрование применяется непосредственно к данным (в отличие от BitLocker, который применяет шифрование на томе логического диска), все данные клиентов, скопированные с сервера Exchange, остаются зашифрованными.
...
Teams использует протоколы TLS и MTLS для шифрования мгновенных сообщений. Весь трафик между серверами требует MTLS, независимо от того, насколько трафик ограничен внутренней сетью или он пересекает периметр внутренней сети.

В этой таблице представлены протоколы, используемые Teams.
...
|Между серверами|MTLS|
|Клиент-сервер (например, Обмен мгновенными сообщениями и присутствие)|TLS;|
|Потоки мультимедиа (например, Обмен аудио-и видеоданных для мультимедиа|TLS;|
|Обмен аудио и видео с мультимедиа|SRTP/TLS|
|Передача сигналов|TLS;|
|||

#### <a name="media-encryption"></a>Шифрование мультимедиа

Трафик мультимедиа шифруется с помощью Secure RTP (SRTP), профиля транспортного протокола (RTP), который обеспечивает конфиденциальность, проверку подлинности и защиту от атак с повторением для трафика RTP. SRTP использует ключ сеанса, созданный с помощью безопасного генератора случайных чисел и передается с помощью сигнального канала TLS. Клиентский трафик мультимедиа согласовывается через сигнальное соединение между серверами, но шифруется с помощью SRTP при переходе от клиента к клиенту напрямую.

Teams использует маркер на основе учетных данных для безопасного доступа к ретрансляции носителей по очереди. Средства ретрансляции мультимедиа обмениваются маркером по каналу, защищенному с помощью протокола TLS.

#### <a name="fips"></a>FIPS

В Teams для обмена ключами шифрования используются алгоритмы FIPS (стандарты федеральной обработки информации). Для получения дополнительной информации об реализации FIPS, ознакомьтесь со статьей [Федеральный стандарт обработки информации (FIPS) 140-2](https://docs.microsoft.com/microsoft-365/compliance/offering-fips-140-2?view=o365-worldwide).
