...
2. Создайте правило с помощью **cmdlet New-TransportRule** и задайте для параметра _RemoveOME_ параметр `$true` .
   ...
   - Параметр _SentToScope_ указывает расположение получателей сообщения. В этом примере используется значение, которое указывает `InOrganization` одно из следующих значений:
...
Ограничение на количество получателей составляет 500 получателей на сообщение или, в сочетании после расширения списка  рассылки, 11 980 символов в поле "Кому" сообщения в зависимости от того, на что оно приходит первым.
