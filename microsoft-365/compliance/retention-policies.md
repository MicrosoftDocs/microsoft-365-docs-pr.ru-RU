...
Можно создавать политики хранения и управлять ими в [Центре соответствия требованиям Microsoft 365](https://compliance.microsoft.com/): **Политики** > **Данные** > **Хранение**

Кроме того, можно перейти на эту же страницу Центра соответствия требованиям Microsoft 365 с помощью вкладки **Решения** > **Управление информацией** > **Хранение**. 

**Если вам требуется возможность просматривать контент перед его окончательным удалением,** рекомендуется использовать [метки хранения](labels.md) вместо политики хранения. При создании метки хранения можно настроить [проверку перед ликвидацией](disposition.md#disposition-reviews), чтобы просматривать контент в конце срока хранения.
...
- Политика хранения, примененная к группе Office 365, включает как сайт, так и почтовый ящик группы.

- Архивная библиотека использует квоту хранилища для сайта.
    
### <a name="content-in-onedrive-accounts-and-sharepoint-sites"></a>Содержимое учетных записей OneDrive и сайтов SharePoint

Политика хранения применяется на уровне семейства веб-сайтов. При включении семейства веб-сайтов SharePoint или учетной записи OneDrive в политику хранения для хранения документов используется архивная библиотека. Если эта библиотека еще не существует, она создается автоматически. Эту библиотеку можно просмотреть на странице **Содержимое сайта** на сайте верхнего уровня в семействе веб-сайтов. Большинство пользователей не могут просматривать архивную библиотеку, так как она видна только администраторам семейства веб-сайтов.
  
Если кто-то пытается изменить или удалить содержимое сайта, к которому применяется политика хранения, предусматривающая хранение и удаление или только хранение, сначала эта политика проверяет, было ли это содержимое изменено с момента ее применения. Если это первое изменение с момента назначения этой политики, содержимое копируется в архивную библиотеку, после чего пользователь сможет изменить или удалить исходное содержимое. В архивную библиотеку можно скопировать любое содержимое семейства веб-сайтов, даже если оно не соответствует запросу, используемому политикой хранения.

В архивную библиотеку копируется только то содержимое, которое существовало на момент применения политики хранения. Кроме того, все новое содержимое, которое было создано или добавлено в коллекцию сайтов после ее включения в политику будет сохранено после удаления. Однако новый контент не копируется в архивную библиотеку при первом изменении, а только при удалении. Чтобы сохранить все версии файла, включите управление версиями, как описано в следующем [разделе](#how-a-retention-policy-works-with-document-versions-in-a-site-collection).
...
1. **Если содержимое изменяется или удаляется** в течение срока хранения: в архивной библиотеке сохраняется копия исходного содержимого по состоянию на момент назначения политики. Периодически выполняемое задание таймера определяет элементы, срок хранения которых истек. Эти элементы перемещаются во вторую корзину, откуда они безвозвратно удаляются через 93 дня. Вторая корзина не видна конечным пользователям (видна только первая корзина), но администраторы семейства веб-сайтов могут просматривать и восстанавливать содержимое из нее.

    > [!NOTE]
    > Чтобы предотвратить случайную потерю данных, содержимое не удаляется автоматически из архивной библиотеки, а перемещается во вторую корзину. В течение 93 дней администратор при необходимости может восстановить это содержимое.
    
2. **Если содержимое не изменяется и не удаляется** в течение срока хранения: по окончании срока хранения документ перемещается в первую корзину. Если пользователь удаляет документ оттуда или очищает корзину, то документ перемещается во вторую корзину. 93-дневный срок хранения включает суммарное время хранения в первой и во второй корзине. Через 93 дня документ окончательно удаляется из того места, где он находится, то есть из первой или из второй корзины. Корзина не индексируется, поэтому недоступна для поиска. В результате при поиске для обнаружения электронных данных не удается найти содержимое корзины для постановки на удержание. 
...
1. **Если содержимое удалено** в течение срока хранения: документ перемещается в первую корзину. Если пользователь удаляет документ оттуда или очищает корзину, то документ перемещается во вторую корзину. 93-дневный срок хранения включает суммарное время хранения в первой и во второй корзине. Через 93 дня документ окончательно удаляется из того места, где он находится, то есть из первой или из второй корзины. Если содержимое изменяется в течение срока хранения, по истечении его срока действия используется тот же путь удаления.

2. **Если содержимое не удаляется** в течение срока хранения: по окончании срока хранения документ перемещается в первую корзину. Если содержимое удаляется в течение срока хранения, документ немедленно перемещается в первую корзину. Если пользователь удаляет документ оттуда или очищает корзину, то документ перемещается во вторую корзину. 93-дневный срок хранения включает суммарное время хранения в первой и во второй корзине. Через 93 дня документ окончательно удаляется из того места, где он находится, то есть из первой или из второй корзины. Корзина не индексируется, поэтому недоступна для поиска. В результате при поиске для обнаружения электронных данных не удается найти содержимое корзины для постановки на удержание.
...
1. **Если элемент не удаляется** в течение срока хранения: по окончании срока хранения элемент перемещается в папку "Элементы с возможностью восстановления". 
...
Политика сохранения (политика, сохраняющая содержимое вместо его удаления) сохраняет все версии документа в семействе сайтов SharePoint или в учетной записи OneDrive. При первом изменении документа, для которого действует политика сохранения или удержания, версия исходного документа копируется в архивную библиотеку. При удалении документа, для которого действует политика сохранения или удержания, все версии этого документа копируется в архивную библиотеку, если включено управление версиями. Каждая версия документа в архивной библиотеке существует как отдельный элемент с собственным сроком хранения:
...
Одна из наиболее эффективных функций политики хранения заключается в том, что она может применяться к расположениям в Office 365, в том числе:
...
- Группы Office 365 (применимо к содержимому почтового ящика группы и связанного сайта SharePoint).
...
Можно также применить политику хранения к определенным пользователям, группам Office 365 или сайтам. Для этого установите переключатель **Состояние** такого расположения во включенное положение и затем с помощью ссылок включите или исключите отдельных пользователей, группы Office 365 или сайты. 
  
Однако обратите внимание, что при настройке политики хранения, которая включает или исключает более 1000 определенных расположений, существуют следующие ограничения:
  
- Максимальное количество для политики хранения:
    - 1000 почтовых ящиков
    - 1000 групп Office 365
...
Хотя эти ограничения существуют, вы можете избежать их, применяя либо политику всей организации, либо политику, которая применяется ко всем местоположениям.
...
Если вы указываете сайты, которые не поддерживаются, они игнорируются политикой хранения.
...
В Teams файлы, которыми пользователь поделился в чате, сохраняются в учетной записи OneDrive этого пользователя. Файлы, добавленные в каналы, сохраняются на сайте SharePoint для команды. Таким образом, чтобы сохранять или удалять файлы в Teams, необходимо создать политику хранения, которая будет применяться к расположениям OneDrive и групп Microsoft 365. Если нужно применить политику ко всем файлам, которыми определенный пользователь или команда делятся в Teams, вы можете выбрать расположения OneDrive или групп Microsoft 365 и включить определенного пользователя или команду.
...
### <a name="microsoft-365-groups-locations"></a>Расположения групп Microsoft 365

Чтобы сохранить содержимое для группы Microsoft 365, вам нужно использовать расположение групп Microsoft 365. Несмотря на то, что в группе Microsoft 365 есть почтовый ящик Exchange, политика хранения, включающая все расположение Exchange, не будет включать содержимое в почтовые ящики группы Microsoft 365. Политика хранения, применяемая к группе Microsoft 365, включает как почтовый ящик группы, так и сайт. Политика хранения, применяемая к группе Microsoft 365, защищает ресурсы, созданные группой Microsoft 365, включая Microsoft Teams.
...
Для этого используйте  `ExcludedItemClasses`параметр `New-RetentionComplianceRule` и командлеты `Set-RetentionComplianceRule`.
...
Во-первых, [подключитесь к PowerShell Центра безопасности и соответствия требованиям Office 365](https://docs.microsoft.com/powershell/exchange/office-365-scc/connect-to-scc-powershell/connect-to-scc-powershell?view=exchange-ps).
...
Конкретную политику хранения можно легко применить ко всей организации и всем расположениям в Microsoft 365, включая Exchange Online, SharePoint Online, OneDrive для бизнеса и группы Microsoft 365. Если вам требуется хранить или удалять содержимое в Microsoft 365, рекомендуется использовать для этого политику хранения. (Вы также можете использовать метки с параметрами хранения. Дополнительные сведения см. в статье [Обзор меток](labels.md).)
  
Есть ряд других функций, которые ранее использовались для хранения и удаления содержимого в Microsoft 365. Они перечислены ниже. Эти функции будут работать наряду с политиками хранения и метками хранения. Но в дальнейшем из соображений эффективного управления информацией мы рекомендуем взамен всех этих функций использовать политику хранения или метки. Политика хранения — это единственная функция, которая позволяет как хранить, так и удалять содержимое в Microsoft 365, где бы оно ни располагалось.
...
1. [Подключитесь к интерфейсу PowerShell Центра безопасности и соответствия требованиям](https://docs.microsoft.com/powershell/exchange/office-365-scc/connect-to-scc-powershell/connect-to-scc-powershell).
    
2. Используйте следующие командлеты Центра безопасности и соответствия требованиям:
...
Дополнительные сведения см. в статье [Предоставление пользователям доступа к Центру безопасности и соответствия требованиям](https://docs.microsoft.com/microsoft-365/security/office-365-security/grant-access-to-the-security-and-compliance-center). 

Эти разрешения необходимы только для создания и применения политики хранения. Для реализации политики доступ к содержимому не требуется.

## <a name="more-information"></a>Дополнительные сведения

- [Политики хранения в Microsoft Teams](/microsoftteams/retention-policies#using-powershell )
