...
Используя ключи, которые вы предоставляете, можно создать политику шифрования данных (DEP) и назначить ее клиенту. Создадимый deP для клиента шифрует следующие данные:
...
- Exchange Online почтовых ящиков, которые еще не зашифрованы на уровне приложений
- Точные данные MIP (EDM) — (схемы файлов данных, пакеты правил и соли, используемые для хранения конфиденциальных данных)

Для microsoft Information Protection и Microsoft Teams клиентский ключ на уровне клиента шифрует новые данные с того времени, как вы назначите клиенту deP. Общедоступный предварительный просмотр не поддерживает шифрование прошлых данных. Для Exchange Online клиентский ключ шифрует все существующие и новые данные.

Вы можете создать несколько DEP на одного клиента, но можно назначить только один DEP одновременно. При назначении deP шифрование начинается автоматически, но занимает некоторое время в зависимости от размера клиента.

## <a name="tenant-level-policies-add-broader-control-to-customer-key-for-microsoft-365"></a>Политики уровня клиента добавляют в ключ клиента более широкий контроль для Microsoft 365

Если у вас уже есть клиентский ключ, Exchange Online и Sharepoint Online, вот как вписывается новый общедоступный предварительный просмотр на уровне клиента.

Политика шифрования на уровне клиента, которая создается, шифрует все данные для Microsoft Teams и Exchange Online рабочих Microsoft 365. Однако для Exchange Online, если для отдельных почтовых ящиков уже назначены DEP-адреса ключа клиента, политика уровня клиента не переопределит эти dePs. Политика уровня клиента шифрует только почтовые ящики, которые уже не назначены deP клиентского ключа уровня почтовых ящиков. При шифровании почтового ящика пользователя с помощью deP уровня клиента все его содержимое шифруется. Сведения о том, что шифруется с помощью deP на уровне приложений, см. в сообщении [Service encryption with Customer Key.](customer-key-overview.md)

## <a name="data-that-isnt-encrypted-with-customer-key-at-the-tenant-level"></a>Данные, которые не шифруются ключом клиента на уровне клиента

Ключ клиента не шифрует следующие типы данных на уровне клиента. Вместо этого Microsoft 365 другие типы шифрования для защиты этих данных.

- Exchange почтовые ящики, которые вы уже зашифрованы с помощью DEP ключа клиента на уровне приложения. Почтовые ящики, у которых нет назначенного им DEP ключа клиента, будут шифроваться с помощью deP уровня клиента. Это расположение означает, что некоторые почтовые ящики зашифрованы с deP уровня клиента, а некоторые почтовые ящики зашифрованы с dePs уровня приложений.
- SharePoint и OneDrive для бизнеса клиентский ключ на уровне приложения. Один deP шифрует контент в SharePoint для одного гео.
- Microsoft Teams файлы и Teams записи вызовов и собраний, сохраненные в OneDrive для бизнеса и SharePoint, шифруются SharePoint deP Online.

Любые рабочие нагрузки или сценарии, которые в настоящее время не поддерживаются клиентской клавишей для Microsoft 365.

- Другие Microsoft 365, такие как Yammer, planner и так далее.
- Teams Live Events и Q&A в live events. Для Teams этот сценарий является единственным, который не шифруется клиентской клавишей на уровне клиента.
...
- Убедитесь, что у вас есть соответствующее лицензирование для вашей организации. Используйте платную подписку Azure с Соглашение Enterprise или поставщика облачных служб. Подписки Azure, приобретенные с помощью планов Pay As You Go или с помощью кредитной карты, не поддерживаются для ключа клиента. С 1 апреля 2020 г. ключ клиента в Office 365 предлагается в Office 365 E5, Microsoft 365 E5, Соответствие требованиям Microsoft 365 E5 и Microsoft 365 E5 информационной защиты & управления. Office 365 Advanced Compliance SKU больше не доступен для новых лицензий. Существующие Office 365 Advanced Compliance лицензии будут по-прежнему поддерживаться. Хотя службу можно включить как минимум с одним надлежащим лицензированным пользователем в клиенте, необходимо убедиться, что все пользователи, пользующиеся услугами службы, имеют соответствующие лицензии.

### <a name="create-two-new-azure-subscriptions"></a>Создание двух новых подписок Azure

Ключ клиента требует двух ключей для каждой политики шифрования данных (DEP). Чтобы создать два ключа, необходимо создать две подписки Azure. В качестве наилучшей практики Корпорация Майкрософт рекомендует, чтобы отдельные члены организации настраивали один ключ в каждой подписке. Используйте эти подписки Azure только для администрирования ключей шифрования для Microsoft 365. Следуя этим рекомендациям, вы можете защитить организацию, если один из операторов случайно, намеренно или злонамеренно удаляет или иным образом неправильно передает ключи, за которые они несут ответственность.
...
3. Свяжитесь с Корпорацией Майкрософт, чтобы завершить процесс в [m365ck@microsoft.com.](mailto:m365ck@microsoft.com) Включите следующий контент в электронную почту:
...
По возможности создайте хранилища в непарных регионах. В парных регионах Azure обеспечивается высокая доступность для доменов сбоя службы. Поэтому региональные пары можно использовать как область резервного копирования друг друга. Ресурс Azure, размещенный в одном регионе, автоматически получает устойчивость к неисправностям в парном регионе. Выбор регионов для двух сводов, используемых в политике шифрования данных, где регионы сопряжены, означает, что используется только два региона доступности. В большинстве географических регионов только два региона, поэтому выбрать непарные регионы пока невозможно. По возможности выберите два непарных региона для двух сводов, используемых с политикой шифрования данных. Этот сценарий выгоден в общей сложности из четырех регионов доступности. Дополнительные сведения см. в дополнительных сведениях о непрерывности бизнеса и аварийном восстановлении [(BCDR): Azure Paired Regions](/azure/best-practices-availability-paired-regions) для текущего списка региональных пар.

### <a name="assign-permissions-to-each-key-vault"></a>Назначение разрешений для каждого хранилища ключей

Для каждого хранилища ключей необходимо определить три отдельных набора разрешений для ключа клиента в зависимости от реализации. Например, необходимо определить один набор разрешений для каждого из них:
...
- **Microsoft 365 в службе шифрования** отдыха, которая работает с клиентской клавишей на уровне клиента. Чтобы разрешить Microsoft 365, запустите **кодлет Set-AzKeyVaultAccessPolicy** с помощью следующего синтаксиса:
  ...
  Пример: для Microsoft 365 службы шифрования rest замените *Microsoft 365 appID*`c066d759-24ae-40e7-a56f-027002b5d3e4`
...
Microsoft 365, что подписка На хранилище ключей Azure установлена подписка "Не отменяйте", а клавиши, используемые клиентской клавишей, имеют возможность мягкого удаления. Эти параметры можно подтвердить, посмотрев уровень восстановления на клавишах.
...
В выводах см. соответствующую политику доступа и Microsoft 365 ID приложения (GUID). Все три операции, get, wrapKey и unwrapKey, должны быть показаны в статье Permissions to Keys.
...
|-Enabled|Параметр Включен включает или отключает политику шифрования данных. Перед отключением политики необходимо отогнать ее от клиента. Допустимые значения:</br > $true: политика включена</br > $true. Политика включена. Это значение задается по умолчанию.
|Да|
...
## <a name="offboarding-from-customer-key-at-the-tenant-level"></a>Отключение от ключа клиента на уровне клиента
...
Если вы решили отключить клиента из клиентского ключа [](mailto:m365ck@microsoft.com) на уровне клиента, m365ck@microsoft.com электронной почты с просьбой "отключить" службу для клиента.

> [!IMPORTANT]
> Offboarding — это не то же самое, что очистка данных. Очистка данных навсегда удаляет данные вашей организации из Microsoft 365, offboarding этого не делает. Вы не можете выполнить очистку данных для политики уровня клиента. Сведения о пути очистки данных см. в ссылке [Revoke your keys and start the data purge path process.](customer-key-manage.md#revoke-your-keys-and-start-the-data-purge-path-process)
...
Сведения о вращающихся или прокатных клавишах, которые вы используете с помощью ключа клиента, см. в журнале [Roll or rotate a Customer Key or an availability key.](customer-key-availability-key-roll.md) При обновлении deP для использования новой версии ключей вы запустите Set-M365DataAtRestEncryptionPolicy, как описано выше в этой статье.
