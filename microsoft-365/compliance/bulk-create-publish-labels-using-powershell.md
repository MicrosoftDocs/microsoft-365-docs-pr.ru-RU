...
Используйте сведения, файлы и примеры шаблонов, а также сценарий из этой статьи при массовом создании меток хранения и их публикации в политиках меток хранения. После этого метки хранения могут [применяться администраторами и пользователями](create-apply-retention-labels.md#how-to-apply-published-retention-labels).

Представленные инструкции не поддерживают метки хранения с автоматическим применением

Обзор 

1. Создайте в Excel список меток хранения и список соответствующих политик меток хранения.

2. Создайте метки хранения и политики меток хранения, указанные в этих списках, с помощью PowerShell.
...
## <a name="step-1-create-a-csv-file-for-the-retention-labels"></a>Шаг 1. Создание CSV-файла для меток хранения

1. Скопируйте приведенный ниже пример CSV-файла с шаблоном и примерами четырех различных меток хранения и вставьте их в Excel. 
...
- Не изменяйте и не переименовывайте заголовки столбцов в CSV-файле примера, иначе сценарий не будет работать.
    
### <a name="sample-csv-file-for-retention-labels"></a>Пример CSV-файла для меток хранения
...
## <a name="step-2-create-a-csv-file-for-the-retention-label-policies"></a>Шаг 2. Создание CSV-файла для политик меток хранения

1. Скопируйте приведенный ниже пример CSV-файла с шаблоном и примерами трех различных политик меток хранения и вставьте их в Excel. 
...
### <a name="sample-csv-file-for-retention-policies"></a>Пример CSV-файла для политик хранения
...
1. Скопируйте и вставьте следующий сценарий PowerShell в Блокнот.

2. Сохраните файл с расширением **.ps1** в таком месте, где его будет легко найти. Пример: `<path>CreateRetentionSchedule.ps1`

Примечания.

- Сценарий предложит указать два исходных файла, которые вы создали на предыдущих двух шагах.
    - Если вы не укажете исходный файл для создания меток хранения, сценарий перейдет к созданию политик меток хранения. 
    - Если вы не укажете исходный файл для создания политик меток хранения, сценарий создаст только метки хранения.

- Сценарий создает файл журнала, в который записываются все выполненные сценарием действия и их результат. Инструкции по поиску файла журнала см. в описании последнего шага.
...
1. В сеансе PowerShell Центра безопасности и соответствия требованиям введите путь, затем символы `.\` и имя файла сценария, после чего нажмите клавишу ВВОД, чтобы запустить сценарий. Например:
    
    ```powershell
    <path>.\CreateRetentionSchedule.ps1
    ```

2. Скрипт предлагает указать расположения ранее созданных CSV-файлов. Введите путь, затем символы `.\` и имя CSV-файла, после чего нажмите клавишу ВВОД. Например, для первого запроса:
    
    ```powershell
    <path>.\Labels.csv
    ```

## <a name="step-5-view-the-log-file-with-the-results"></a>Шаг 5. Просмотр файла журнала с результатами

Используйте созданный сценарием файл журнала для проверки результатов и выявления сбоев, которые необходимо устранить.

Файл журнала можно найти в указанной ниже папке (обратите внимание, что цифры в имени файла могут быть другими).
