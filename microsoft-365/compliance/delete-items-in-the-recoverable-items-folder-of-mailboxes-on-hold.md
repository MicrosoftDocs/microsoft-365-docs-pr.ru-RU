...
- Эту процедуру невозможно выполнить для почтового ящика, которому назначены параметры хранения с политикой, заблокированной с помощью блокировки сохранения. Это связано с тем, что эта блокировка препятствует удалению или исключению почтового ящика из политики и отключению помощника для управляемых папок в почтовом ящике. Дополнительные сведения о политиках блокировки для хранения приведены [в статье Использование блокировки сохранения для ограничения изменений в политиках хранения и политиках меток хранения](retention-preservation-lock.md).
...
Выполните следующую команду в Exchange Online PowerShell, чтобы определить In-Place удержание, размещенное в почтовом ящике. Используйте GUID для In-Place хранения, который вы определили на шаге 1.

```powershell
Get-MailboxSearch -InPlaceHoldIdentity <hold GUID> | FL Name
```

Определив In-Place удержания, можно удалить почтовый ящик из удержания с помощью центра администрирования Exchange или Exchange Online PowerShell. Дополнительные сведения см. [в статье Создание или удаление In-Place удержания](https://go.microsoft.com/fwlink/?linkid=852668).
  
### <a name="retention-policies-applied-to-specific-mailboxes"></a>Политики хранения, применяемые к определенным почтовым ящикам
  
Выполните следующую команду в [PowerShell центра безопасности & соответствия требованиям](https://go.microsoft.com/fwlink/?linkid=627084) , чтобы определить политику хранения, применяемую к почтовому ящику. Эта команда также возвращает политики хранения для беседы в Teams, примененные к почтовому ящику. Используйте GUID (не включая `mbx` `skp` префикс) для политики хранения, определенной на шаге 1.
...
Политики хранения на уровне Организации, на уровне Exchange и на уровне всей группы применяются ко всем почтовым ящикам в Организации. Они применяются на уровне Организации (не на уровне почтового ящика) и возвращаются при выполнении командлета **Get-OrganizationConfig** на этапе 1. Выполните следующую команду в [PowerShell центра безопасности & соответствия требованиям](https://go.microsoft.com/fwlink/?linkid=627084) , чтобы определить политики хранения на уровне Организации. Используйте идентификатор GUID (не включая  `mbx` префикс) для политик хранения на уровне Организации, определенных на этапе 1.
...
Дополнительные сведения о метках можно узнать в [статье сведения о политиках хранения и метках хранения](retention.md).
...
Теперь вы готовы к фактическому удалению элементов в папке "элементы с возможностью восстановления" с помощью командлетов [New – ComplianceSearch](https://docs.microsoft.com/powershell/module/exchange/new-compliancesearch) и [New ComplianceSearchAction](https://docs.microsoft.com/powershell/module/exchange/new-compliancesearchaction) в средстве безопасности & центра соответствия требованиям PowerShell.

Для поиска элементов, расположенных в папке "элементы с возможностью восстановления", рекомендуется выполнить *целевую коллекцию*. Это означает сужение области поиска только для элементов, расположенных в папке "элементы с возможностью восстановления". Это можно сделать, выполнив сценарий в статье [Использование поиска содержимого для целевых семейств](use-content-search-for-targeted-collections.md) . Этот скрипт возвращает значение свойства идентификатора папки для всех вложенных папок в папке "элементы с возможностью восстановления". Затем идентификатор папки используется в запросе поиска для возврата элементов, расположенных в этой папке.

Ниже представлен обзор процесса поиска и удаления элементов в папке "элементы для восстановления" пользователя:

1. Запустите скрипт целевой коллекции, который возвращает идентификаторы папок для всех папок в почтовом ящике целевого пользователя. Сценарий подключается к оболочке PowerShell для Exchange Online и безопасности & соответствия требованиям в одном сеансе PowerShell. Дополнительные сведения см. [в разделе Запуск сценария для получения списка папок для почтового ящика](use-content-search-for-targeted-collections.md#step-1-run-the-script-to-get-a-list-of-folders-for-a-mailbox-or-site).

2. Скопируйте идентификаторы папок для всех вложенных папок в папке "элементы с возможностью восстановления". Кроме того, можно перенаправить выходные данные сценария в текстовый файл.

   Ниже приведен список и описание вложенных папок в папке "элементы с возможностью восстановления", в которой можно выполнять поиск и удалять элементы из:

   - **Удаления** : содержит обратимо удаленные элементы, срок хранения удаленных элементов которых не истек. Пользователи могут восстанавливать обратимо удаленные элементы из этой вложенной папки с помощью средства восстановления удаленных элементов в Outlook.

   - **Очистка** : содержит окончательные удаленные элементы, срок хранения удаленных элементов которых истек. Пользователи также могут удалять элементы, удаляя элементы из папки "элементы с возможностью восстановления". Если почтовый ящик находится на удержании, будут сохранены окончательно удаленные элементы. Эта вложенная папка невидима для конечных пользователей.

   - **DiscoveryHolds** : содержит обратимо удаленные элементы, которые были сохранены при удержании электронных данных или политике хранения. Эта вложенная папка невидима для конечных пользователей.

   - **Субстратехолдс** : содержит окончательные удаленные элементы из Teams и других облачных приложений, которые были сохранены политикой хранения или другим типом удержания. Эта вложенная папка невидима для конечных пользователей.

3. Используйте командлет **New-ComplianceSearch** (в центре безопасности & соответствия требованиям PowerShell) или используйте средство поиска контента в центре соответствия требованиям для создания поиска контента, возвращающего элементы из папки "элементы с возможностью восстановления" целевого пользователя. Это можно сделать, включив в запрос поиска FolderId для всех вложенных папок, которые требуется найти. Например, следующий запрос возвращает все сообщения из вложенных папок "очистки" и "Едисковерихолдс":

   ```text
   folderid:<folder ID of Purges subfolder> OR folderid:<folder ID of DiscoveryHolds subfolder>
   ```

   Дополнительные сведения и примеры выполнения поиска контента, в котором используется свойство ID папки, приведены в разделе [Использование идентификатора папки или для выполнения целевой коллекции](use-content-search-for-targeted-collections.md#step-2-use-a-folder-id-or-documentlink-to-perform-a-targeted-collection).

   > [!NOTE]
   > Если вы используете командлет **New – ComplianceSearch** для поиска в папке "элементы с возможностью восстановления", не забудьте использовать командлет **Start – ComplianceSearch** для запуска поиска.

4. После создания поиска контента и проверки того, что он возвращает элементы, которые вы удаляете через глобальную сеть, используйте `New-ComplianceSearchAction -Purge -PurgeType HardDelete` команду (в статье Security & The Windows Security Center PowerShell) для окончательного удаления элементов, возвращенных службой поиска контента, созданной на предыдущем шаге. Например, можно выполнить команду, аналогичную следующей:

   ```powershell
   New-ComplianceSearchAction -SearchName "RecoverableItems" -Purge -PurgeType HardDelete
   ```

5. При выполнении предыдущей команды удаляется не более 10 элементов для почтового ящика. Это означает, что для `New-ComplianceSearchAction -Purge` удаления всех элементов, которые необходимо удалить, в папке "элементы с возможностью восстановления", может потребоваться несколько раз выполнить эту команду. Чтобы удалить дополнительные элементы, сначала необходимо удалить предыдущее действие по очистке поиска соответствия. Для этого необходимо выполнить `Remove-ComplianceSearchAction` командлет. Например, чтобы удалить действие очистки, выполненное на предыдущем этапе, выполните следующую команду:

   ```powershell
   Remove-ComplianceSearchAction "RecoverableItems_Purge"
   ```

   После этого можно создать новое действие по очистке поиска на соответствие для удаления элементов. Перед созданием новой операции очистки необходимо удалить каждое из них.

   Чтобы получить список действий поиска соответствия, можно выполнить `Get-ComplianceSearchAction` командлет. Очистка действий определяется путем `_Purge` добавления имени для поиска.
...
4. Повторно примените удержания, которые вы удалили на шаге 3. В зависимости от типа удержания используйте одну из следующих процедур.
    ...
    **In-Place Hold**
    ...
    Используйте центр администрирования Exchange (или Exchange Online PowerShell), чтобы добавить почтовый ящик обратно в In-Place удержание.
...
|Хранение на месте  <br/> | `c0ba3ce811b6432a8751430937152491` <br/> |Свойство  *InPlaceHolds*  содержит GUID In-Place хранения, который размещается в почтовом ящике. Вы можете указать, что это In-Place хранения, так как GUID не начинается с префикса.  <br/> Вы можете использовать  `Get-MailboxSearch -InPlaceHoldIdentity <hold GUID> | FL` команду в Exchange Online PowerShell, чтобы получить сведения о In-Place хранения для почтового ящика.  <br/> |
