...
- Количество фильтров разрешений поиска, которые могут быть созданы в организации, не ограничивается. Но производительность поиска будет влиять, когда существует более 100 фильтров разрешений поиска. Чтобы количество фильтров разрешений поиска в организации было как можно меньше, создайте фильтры, которые по возможности объединяют правила exchange, SharePoint и OneDrive в одном фильтре.

## <a name="connect-to-exchange-online-and-security--compliance-center-powershell-in-a-single-session"></a>Подключение к Exchange Online и & Центра соответствия требованиям PowerShell за один сеанс

Прежде чем успешно запустить скрипт в этом разделе, необходимо скачать и установить модуль Exchange Online PowerShell V2. Дополнительные сведения [см. в модуле Exchange Online PowerShell V2.](/powershell/exchange/exchange-online-powershell-v2#install-and-maintain-the-exo-v2-module)

1. Сохраните следующий текст в файле Windows PowerShell с помощью суффикса имени файла **.ps1**. Например, его можно сохранить в файле **с именемConnectEXO-SCC.ps1.**
...
2. На локальном компьютере откройте Windows PowerShell, перейдите в папку, в которой расположен сценарий, созданный на предыдущем шаге, а затем запустите сценарий; Например:

    ```powershell
    .\ConnectEXO-SCC.ps1
    ```

Как узнать, сработало ли это? После запуска скрипта в локальное сеанс Windows PowerShell безопасности импортируются & и security & PowerShell. Если не появляются сообщения об ошибках, значит, подключение установлено. Быстрый тест — запуск комлета Exchange Online и & Центра соответствия требованиям. Например, можно запустить **и Get-Mailbox** и **Get-ComplianceSearch.**
...
В этом примере пользователи не могут удалять содержимое из почтовых ящиков членов группы рассылки Executive Team (Руководство). Командлет Get-DistributionGroup Exchange Online PowerShell используется для поиска членов группы Executive Team.
...
В этом примере члены группы пользовательских ролей OneDrive eDiscovery Managers могут искать контент только в расположениях OneDrive для бизнеса в организации.
...
- **Позволяет ли пользователю искать все расположения контента в определенной службе и не позволяет ему искать расположения контента в другой службе?** Нет. Как объяснялось ранее, необходимо создать фильтр разрешений поиска, чтобы явно запретить пользователям искать расположения контента в определенной службе (например, запретить пользователю искать любой почтовый ящик Exchange или любой сайт SharePoint). Другими словами, создание фильтра разрешений поиска, который позволяет пользователю искать все сайты SharePoint в организации, не мешает этому пользователю искать почтовые ящики. Например, чтобы разрешить администраторам SharePoint искать только сайты SharePoint, необходимо создать фильтр, который не позволяет им искать почтовые ящики. Кроме того, чтобы разрешить администраторам Exchange только поиск почтовых ящиков, необходимо создать фильтр, который не позволяет им искать сайты.