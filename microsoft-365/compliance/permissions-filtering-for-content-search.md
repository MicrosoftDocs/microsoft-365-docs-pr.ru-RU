...
Как узнать, сработало ли это? После запуска скрипта в локальный сеанс Exchange Online и & powerShell Windows PowerShell безопасности. Если при этом не появляются сообщения об ошибках, то подключение установлено. Быстрый тест — запуск Exchange Online и & центра соответствия требованиям. Например, можно запустить **и Get-Mailbox** и **Get-ComplianceSearch.**
