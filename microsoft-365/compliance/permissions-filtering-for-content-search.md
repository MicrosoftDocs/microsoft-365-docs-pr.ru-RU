...
- Для использования фильтров соответствия требованиям необходимо подключиться к Exchange Online и PowerShell в Центре безопасности и & безопасности. Это необходимо, так как этим cmdlets требуется доступ к свойствам почтового ящика, поэтому необходимо подключиться к Exchange Online PowerShell. См. действия в следующем разделе.
...
- Количество фильтров разрешений поиска, которые можно создать в организации, не ограничивается. Однако производительность поиска будет влиять на более чем 100 фильтров разрешений поиска. Чтобы максимальное количество фильтров разрешений поиска в организации было максимально небольшим, по возможности создавайте фильтры, которые объединяют правила для Exchange, SharePoint и OneDrive в одном фильтре.
...
Как узнать, все ли получилось? После выполнения сценария в локальный сеанс Windows PowerShell будут импортироваться & Exchange Online и PowerShell для обеспечения Windows PowerShell безопасности. Если при этом не возникают ошибки, подключение установлено. Чтобы выполнить быструю проверку, запустите & Центра соответствия требованиям и Exchange Online. Например, можно запустить **и Get-Mailbox** и **Get-ComplianceSearch.**
...
**New-ComplianceSecurityFilter** используется для создания фильтра разрешений поиска. В следующей таблице описываются параметры этого cmdlet. Все они необходимы для создания фильтра соответствия требованиям безопасности.
...
Когда фильтр, содержащий список фильтров, обрабатывается во время поиска контента, создаются два фильтра разрешений поиска из списка фильтров: по одному для каждого фильтра, разделенного запятой. Поэтому в предыдущем примере создается один фильтр разрешений поиска в почтовом ящике и один фильтр разрешений поиска на сайте. 
