...
- Если результаты поиска контента старше 7 дней и вы представляете задание экспорта, отображается сообщение об ошибке, в результате которое будет отображаться повторное повторение поиска для обновления результатов поиска. Если это произойдет, отмените экспорт, повторно запустите поиск и снова запустите экспорт.
...
5. Настройка следующих дополнительных параметров:
   ...
   ![Параметры Exchange](../media/OtherExportOptions.png)
