...
Доменные имена, такие как contoso.com, управляются с помощью всемирной системы регистраторов доменов и баз данных. Система доменных имен (DNS) обеспечивает соответствие между понятными человеку именами компьютеров и IP-адресами, используемыми сетевым оборудованием. Понимание основ DNS и регистратора доменов поможет администраторам управлять доменами.
...
Зачем использовать домен третьего уровня? Вы можете создавать разные доменные имена в маркетинговых целях или для блога, Например, blog.contoso.com. Как правило, для работы с Microsoft создается домен второго уровня, такой как contoso.com, однако можно использовать и домены третьего уровня.
  
Узнайте больше о том, что можно делать с доменами в рамках каждого из предложений в разделе [описание служб Microsoft 365 и Office 365](https://go.microsoft.com/fwlink/?LinkId=402693).
  
## <a name="understand-dns-record-types"></a>Типы DNS-записей

Записи DNS, которые хранятся на узле DNS домена, используются для маршрутизации связанного с доменом трафика. В таблице ниже описаны часто используемые записи DNS и принципы их использования.
...
|SRV-запись (запись службы)  <br/> |Используется в Skype для бизнеса Online и Exchange Online, чтобы координировать поток сведений между службами Microsoft. Например, для просмотра сведений о присутствии в Outlook Web App и использования Skype для бизнеса Online, Skype или других средств обмена мгновенными сообщениями с людьми из других компаний требуются записи SRV.  <br/> |
...
Звучит сложно? Возможно, но мы поможем вам выполнить каждый этап настройки домена.
