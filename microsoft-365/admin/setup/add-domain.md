...
5. Выберите способ проверки того, что вы владеете доменом.
    ...
    1. Если ваш регистратор домена использует [Подключение к домену](#domain-connect-registrars-integrating-with-microsoft-365), корпорация Майкрософт настроит [свои записи автоматически](../get-help-with-domains/domain-connect.md) , выполнив вход в регистратор и подтверждая подключение к Microsoft 365. Вы вернетесь в центр администрирования, и Майкрософт затем автоматически проверит ваш домен.
    2. Вы можете подтвердить владение доменом с помощью записи типа TXT. Установите этот флажок и нажмите кнопку **Далее** , чтобы просмотреть инструкции по добавлению этой записи DNS на веб-сайт регистратора. Проверка после добавления записи может занять до 30 минут. 
    3. Вы можете добавить текстовый файл на веб-сайт вашего домена. Выберите и скачайте txt-файл из мастера установки, а затем отправьте его в папку верхнего уровня своего веб-сайта. Путь к файлу должен выглядеть примерно следующим образом: `http://mydomain.com/ms39978200.txt` . Мы подтвердите, что вы являетесь владельцем домена, найдя файл на веб-сайте.
    
6. Выберите способ внесения изменений DNS, необходимых корпорации Майкрософт для использования вашего домена.
    
    1. Выберите **добавить записи DNS для меня** , если регистратор поддерживает [Подключение к домену](#domain-connect-registrars-integrating-with-microsoft-365), и Майкрософт настроит [свои записи автоматически](../get-help-with-domains/domain-connect.md) , выполнив вход в регистратор и подтверждаю подключение к Microsoft 365.
...
7. Если вы решили *добавить записи DNS самостоятельно*  , нажмите кнопку **Далее** , чтобы увидеть страницу со всеми записями, которые необходимо добавить на веб-сайт регистраторов, чтобы настроить свой домен. 
    ...
    Если вы хотите подождать позже, отмените выбор всех служб и нажмите **продолжить**, или на предыдущем шаге подключения к домену выберите **Дополнительные параметры** и нажмите кнопку **пропустить**.
...
- Секуресервер или Вилдвестдомаинс (GoDaddy торговые посредники, использующие службу хостинга на Секуресервер DNS)
    - Примеры:
        - [домаинсприцедригхт](https://www.domainspricedright.com/products/domain-registration)
        - [домаинригхтнов](https://www.domainrightnow.com/)
