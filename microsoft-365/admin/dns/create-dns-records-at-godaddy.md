...
---
# <a name="create-dns-records-at-godaddy-for-microsoft"></a>Создание записей DNS в GoDaddy для Microsoft
...
После добавления этих записей в GoDaddy домен будет настроен для работы с службами Майкрософт.
...
2. В **доменах** выберите DNS в домене, который необходимо изменить.
...
5. В поля для новой записи введите (или скопируйте и вставьте) значения из таблицы ниже.
    ...
    (Выберите **значение TTL** из выпадаемого списка.)

    |**Record Type (Тип записи)**|**Host** (Узел)|**Points to** (Указывает на)|**Priority** (Приоритет)|**TTL** (Срок жизни)|
    |:-----|:-----|:-----|:-----|:-----|
    |MX (Mail Exchanger) (Почтовый обменник)  <br/> |@  <br/> | *\<domain-key\>*  .mail.protection.outlook.com  <br/> **Примечание:** Получите вашу  *\<domain-key\>*  учетную запись Майкрософт.           [Где это находится?](../get-help-with-domains/information-for-dns-records.md)          |10   <br/> Дополнительные сведения о приоритете см. в статье [Приоритет записей MX](../setup/domains-faq.yml).    <br/> |1 час  <br/> |
...
5. Создайте первую запись SRV.
    ...
    (Выберите тип **записи** и **значения TTL** из выпадаемого списка.)
