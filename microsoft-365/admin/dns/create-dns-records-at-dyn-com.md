...
---
# <a name="create-dns-records-at-dyncom-for-microsoft"></a>Создание записей DNS в Dyn.com microsoft

 Если вы не нашли то, что вы ищете, см. раздел **[Вопросы и ответы по доменам](../setup/domains-faq.yml)**. 
  
Если Dyn.com является поставщиком хостинга DNS, выполните действия в этой статье, чтобы проверить домен и настроить записи DNS для электронной почты, Skype для бизнеса Online и так далее.
...
2. На странице **Службы уровня зоны** выберите **DNS-службу DNS DYN для** домена, который необходимо изменить. 
    
3. На странице **DNS** для домена выберите **Параметры**.
    
4. Выберите **Включить интерфейс эксперта**.
...
5. In the **Add DNS Record** section, in the boxes for the new record, type or copy and paste the values from the following table. 
    ...
    |**Host (Узел)**|**TTL (Срок жизни)**|**Type (Тип)**|**Data (Данные)**|
    |:-----|:-----|:-----|:-----|
    |(Leave this field empty.)  <br/> |600  <br/> |MX  <br/> |10  *\<domain-key\>*  .mail.protection.outlook.com.  <br/> **Это значение ДОЛЖНО оканчиваться точкой (.).** <br/> **10**  значение приоритета MX. Добавьте его в начало значения MX, отделив от остальной части пробелом.  <br/> **Примечание:** Получите вашу  *\<domain-key\>*  учетную запись Майкрософт.           [Как его найти?](../get-help-with-domains/information-for-dns-records.md)      <br>    Дополнительные сведения о приоритете см. в статье [Что такое приоритет записей MX?](../setup/domains-faq.yml) <br/> |
...
7. Если есть какие-либо другие записи MX, удалите их, выбрав для каждого из них в столбце **Delete.** 
    
    ![Dyn-BP-Configure-2-3](../../media/f24f02cc-c0b7-42cf-a2ff-4d0fc203e4de.png)
  
8. Выберите **Применение изменений**.
...
5. Добавьте первую из шести записей CNAME.
    ...
    В разделе **Добавить запись DNS** в полях для новой записи введите или скопируйте и введите значения из первого ряда следующей таблицы. 
...
7. Добавьте оставшиеся пять записей CNAME.
    ...
    В разделе **Добавить запись DNS** создайте запись с помощью значений из следующей строки в таблице, а затем снова выберите **Создать** запись для завершения этой записи. 
...
1. Чтобы приступить к работе, откройте страницу со своими доменами на сайте Dyn.com по [этой ссылке](https://account.dyn.com/dns/). Сначала вам будет предложено войти в систему 
...
7. Добавьте вторую запись SRV.
    ...
    В разделе **Добавить запись DNS** создайте запись с помощью значений из второй строки в таблице, а затем снова выберите **Создать** запись для завершения этой записи. 
