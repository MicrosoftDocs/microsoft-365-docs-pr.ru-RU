...
---
# <a name="create-dns-records-at-dyncom-for-microsoft"></a>Создание записей DNS для Dyn.com Майкрософт

 Если вы не нашли то, что вы ищете, см. раздел **[Вопросы и ответы по доменам](../setup/domains-faq.yml)**. 
  
Если Dyn.com ваш поставщик услуг размещения DNS, выполните действия, которые необходимо в этой статье, чтобы подтвердить владение доменом и настроить записи DNS для электронной почты, Skype для бизнеса Online и других служб.
...
2. На странице **"Службы уровня зоны"** выберите **Dyn Standard DNS Service** для домена, который нужно изменить. 
...
7. Если есть какие-либо другие записи MX, удалите их, выбрав для каждой записи в столбце **"Удалить".** 
...
5. Добавьте первую из шести записей CNAME.
    ...
    В поля для новой записи в разделе "Добавление записи **DNS"** введите (или скопируйте и введите) значения из первой строки таблицы ниже. 
...
7. Добавьте оставшиеся пять записей CNAME.
    ...
    В разделе **"Добавление записи DNS"** создайте запись, используя значения из следующей  строки таблицы, а затем снова выберите "Создать запись", чтобы завершить запись. 
...
7. Добавьте вторую запись SRV.
    ...
    В разделе **"Добавление записи DNS"** создайте запись, используя значения из второй  строки таблицы, а затем снова выберите "Создать запись", чтобы завершить запись. 
