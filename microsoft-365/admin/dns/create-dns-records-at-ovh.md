...
---
# <a name="create-dns-records-at-ovh-for-microsoft"></a>Создание записей DNS в OVH для Microsoft
...
- [Создание записей DNS в OVH для Microsoft](#create-dns-records-at-ovh-for-microsoft)
...
После добавления этих записей в OVH домен будет настроен на работу с службами Майкрософт.
...
2. В **доменах** выберите имя домена, которое необходимо изменить.
...
6. В поля для новой записи введите (или скопируйте и вставьте) значения из таблицы ниже. Чтобы назначить значение TTL, выберите **Персонализированный** из выпадаемого списка, а затем введите значение в текстовом окне. 
...
6. In the boxes for the new record, type or copy and paste the values from the following table. Чтобы назначить значение TTL, выберите **Персонализированный** из выпадаемого списка, а затем введите значение в текстовом окне. 
    
    > [!NOTE]
    > По умолчанию OVH использует относительную нотацию для цели, которая добавляет доменное имя в конец целевой записи. Чтобы назначить значение TTL, выберите Personalized (Другое) из раскрывающегося списка, а затем введите значение в текстовом поле. 
    ...
    |:-----|:-----|:-----|:-----|:-----|
    |MX  <br/> |(оставьте пустым)  <br/> |(Оставьте пустым)  <br/> |10   <br/> Дополнительные сведения о приоритете см. в статье [Приоритет записей MX](../setup/domains-faq.yml).    <br/> |\<domain-key\>.mail.protection.outlook.com.  <br/> **Примечание:** Получите вашу  *\<domain-key\>*  учетную запись Майкрософт.  [Как его найти?](../get-help-with-domains/information-for-dns-records.md)  |
...
9. If there are any other MX records, delete them all in the list on the **DNS zone** page. Выберите каждую запись, а затем в столбце **Действия** выберите значок удаление **корзины.** 
...
6. OVH add CNAME record type
    ...
    В поля для новой записи введите (или скопируйте и вставьте) значения из первой строки таблицы ниже. Чтобы назначить значение TTL, выберите **Персонализированный** из выпадаемого списка, а затем введите значение в текстовом окне. 
...
6. Создайте первую запись SRV.
    ...
    В поля для новой записи введите (или скопируйте и вставьте) значения из первой строки приведенной ниже таблицы. Чтобы назначить значение TTL, выберите **Персонализированный** из выпадаемого списка, а затем введите значение в текстовом окне. 
