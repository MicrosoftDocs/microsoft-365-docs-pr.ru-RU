...
Если Azure является поставщиком хостинга DNS, выполните действия в этой статье, чтобы проверить домен и настроить записи DNS для электронной почты, Skype для бизнеса Online и так далее.
...
После добавления этих записей в Azure домен будет настроен для работы с службами Майкрософт.
...
Когда вы подписались на Azure, вы создали группу ресурсов в зоне DNS, а затем назначены доменное имя этой группе ресурсов. Это доменное имя регистрирует внешний регистратор домена; Azure не предлагает службы регистрации домена.
  
Чтобы проверить и создать DNS-записи для вашего домена в Microsoft, сначала необходимо изменить именников в регистраторе домена, чтобы они использовали именные имена Azure, назначенные вашей группе ресурсов.
...
2. Создайте две записи серверов доменных имен, используя значения из приведенной ниже таблицы, либо измените существующие записи серверов доменных имен, чтобы они соответствовали следующим значениям: Ниже показан пример назначенных имен azure.
    


**Первый nameserver:** Используйте значение сервера имен, назначенное Azure.  
**Второй nameserver:** Используйте значение сервера имен, назначенное Azure.  

![Azure-BP-Redelegate-1-1](../../media/3e4805ea-608a-4df9-8f68-1fbf70d13d08.png)
  
> [!TIP]
> You should use at least two name server records. Если на веб-сайте регистратора домена указаны другие серверы имен, их следует удалить. 
...
1. Чтобы начать работу, перейдите на страницу доменов в Azure с помощью [этой ссылки.](https://portal.azure.com ) Сначала вам потребуется выполнить вход.
    
    ![Azure-BP-Configure-1-1](../../media/ed377cad-0c47-4f9f-b322-c3e06b309b1f.png)
  
2. Используя панель **поиска на** странице **Панель мониторинга,** введите в **зонах DNS.** На дисплее результатов выберите **зоны DNS** в части **Services.** После перенаправления выберите домен, который необходимо обновить.
    
    ![Azure-BP-Configure-1-2](../../media/eb4baad2-92d7-49c9-95e5-1dd8510d5ec9.png)
  
3. На странице **Параметры** для домена в зоне **DNS** выберите **+ Набор записей.**
    
    ![Azure-BP-Configure-1-3](../../media/b04db89a-3dbc-4cd2-aaca-af17fda53a60.png)
  
4. В области **Добавить набор записей** в полях для нового набора записей выберите значения из следующей таблицы. 
    
    (Выберите значения **единиц Типа** и **TTL** из отсевных списков.) 
    
    |**Имя**|**Type** (Тип)|**TTL** (Срок жизни)|**Блок TTL**|**Значение**|
    |:-----|:-----|:-----|:-----|:-----|
...
4. В области **Добавить набор записей** в полях для нового набора записей выберите значения из следующей таблицы. 
    ...
    |**Имя**|**Type** (Тип)|**TTL** (Срок жизни)|**Блок TTL**|**Preference (Предпочтение)**|**Почтовая биржа**|
    |:-----|:-----|:-----|:-----|:-----|:-----|
    |@  <br/> |MX  <br/> |1  <br/> |Часы  <br/> |10   <br/> Дополнительные сведения о приоритете см. в статье [Что такое приоритет записей MX?](../setup/domains-faq.yml). <br/> | *\<domain-key\>*  .mail.protection.outlook.com  <br/> **Примечание:** Получите вашу  *\<domain-key\>*  учетную запись Майкрософт.   [Как его найти?](../get-help-with-domains/information-for-dns-records.md)  
...
6. Если в разделе **MX Records** указаны другие записи MX, их необходимо удалить. 
    
    Сначала в зоне **DNS** выберите набор **записей MX.**
    
    ![Azure-BP-Configure-2-3](../../media/9890da61-6fcd-4c61-888e-ccbb84f80cd0.png)
  
    Далее выберите запись MX, которая необходимо удалить.
    
    ![Azure-BP-Configure-2-4](../../media/afe54f12-66d2-4ff3-80e9-427448e4c32c.png)
  
7. Выберите меню **Context (...),** а затем выберите **Удалить**.
...
4. Добавьте первую из четырех записей CNAME.
    ...
    В области **Добавить набор** записей в полях для нового набора записей введите или скопируйте и введите значения из первой строки в следующей таблице. 
    
    (Выберите значения **единиц Типа** и **TTL** из отсевных списков.) 
    
    |**Имя**|**Type** (Тип)|**TTL** (Срок жизни)|**Блок TTL**|**Alias (Псевдоним)**|
    |:-----|:-----|:-----|:-----|:-----|
...
6. Добавьте остальные три записи CNAME.
    ...
    В зоне **DNS** выберите **+ Набор записей.** Затем в пустом наборе записей создайте запись с помощью значений из следующей строки в таблице и снова выберите **ОК** для завершения этой записи. 
    
    Повторяйте эту процедуру, пока не будут созданы все четыре записи CNAME.
    
7.  (Необязательный) Добавьте 2 записи CNAME для MDM.
...
4. В области **свойств набора** записей в полях для нового набора записей выберите значения из следующей таблицы. 
...
4. Добавьте первую из двух записей SRV.
    ...
    В области **Добавить набор записей** в полях для нового набора записей выберите значения из первой строки в следующей таблице. 
    
    (Выберите значения **единиц Типа** и **TTL** из отсевных списков.) 
    
    |**Имя**|**Type** (Тип)|**TTL** (Срок жизни)|**Блок TTL**|**Priority** (Приоритет)|**Weight** (Вес)|**Port** (Порт)|**Target (Назначение)**|
    |:-----|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
...
6. Добавьте вторую запись SRV.
    ...
    В полях для новой записи введите или скопируйте и введите значения из второго ряда таблицы.
