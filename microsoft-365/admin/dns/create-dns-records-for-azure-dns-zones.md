...
При регистрации в Azure вы создали группу ресурсов в зоне DNS, а затем написав имя домена этой группе ресурсов. Это доменное имя зарегистрировано у внешнего регистратора доменных имен; Azure не предлагает службы регистрации доменов.
...
> [!TIP]
> You should use at least two name server records. Если на веб-сайте регистратора доменных имен указаны другие серверы доменных имен, их следует удалить. 
...
4. В области **"Добавить набор записей"** в поля для нового набора записей выберите значения из таблицы ниже. 
    ...
    (Выберите значения **единиц type** и **TTL** в выпадаемом списке.) 
