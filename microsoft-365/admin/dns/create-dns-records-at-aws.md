...
---
# <a name="create-dns-records-at-amazon-web-services-aws-for-microsoft"></a>Создание записей DNS в Amazon Web Services (AWS) для Microsoft

 Если вы не нашли то, что вы ищете, см. раздел **[Вопросы и ответы по доменам](../setup/domains-faq.yml)**. 
  
Если AWS является поставщиком хостинга DNS, выполните действия в этой статье, чтобы проверить домен и настроить записи DNS для электронной почты, Skype Online для бизнеса и так далее.
  
После добавления этих записей в AWS домен будет настроен для работы с службами Майкрософт.
...
2. На странице **Ресурсы** выберите **хост-зоны**.
    
3. На странице **Hosted Zones** в столбце **Имя** домена выберите имя домена, которое необходимо изменить. 
...
## <a name="add-an-mx-record-so-email-for-your-domain-will-come-to-microsoft-365"></a>Добавьте запись MX, чтобы сообщение электронной почты для домена пришло в Microsoft 365
...
7. Если есть другие записи MX, удалите их.
    ...
    > [!IMPORTANT]
    > AWS хранит записи MX в виде наборов, которые могут содержать сразу несколько записей. **Не выберите** **удалить набор записей,** так как это удалит все записи MX, включая только что добавленные. Вместо этого сделайте следующее: 
...
7. Добавьте остальные четыре записи CNAME.
    ...
    На странице **Hosted Zones** выберите **Создание** набора записей, создайте запись с помощью значений из следующей строки в таблице, а затем снова выберите **Создать** для завершения этой записи. 
...
5. Добавьте первую запись SRV.
    ...
    |**Имя**|**Тип**|**Alias (Псевдоним)**|**TTL (Seconds) (Срок жизни, в секундах)**|**Value (Значение)**|**Routing Policy (Политика маршрутизации)**|
    |:-----|:-----|:-----|:-----|:-----|:-----|
    |_sip._tls|SRV  указатель служб|No (Нет)|300|100 1 443 sipdir.online.lync.com. **Это значение должно закончиться периодом (.)**><br> **Примечание.** Рекомендуется скопировать и вставить эту запись, чтобы сохранить все пробелы.               |Simple (Простая)|
