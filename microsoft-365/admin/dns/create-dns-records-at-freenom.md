...
---
# <a name="create-dns-records-at-freenom-for-microsoft"></a>Создание записей DNS в Freenom для Microsoft

Если вы не нашли нужную информацию, см. статью [Вопросы и ответы о доменах](../setup/domains-faq.yml). 
  
> [!CAUTION]
> На веб-сайте Freenom не поддерживаются записи SRV, поэтому некоторые функции Skype для бизнеса Online и Outlook Web App не будут работать. Независимо от того, какой microsoft планирует использовать, существуют значительные ограничения службы, и вы можете перейти к другому поставщику DNS-хостинга. 
  
Если, несмотря на ограничения службы, вы решите управлять собственными записями Microsoft DNS в Freenom, следуйте шагам в этой статье, чтобы проверить домен и настроить записи DNS для электронной почты и других служб.
...
1. Чтобы начать работу, перейдите на страницу доменов в Freenom с помощью [этой ссылки.](https://my.freenom.com/) Вам потребуется выполнить вход.
...
3. Для домена, который необходимо изменить, выберите **Управление доменом**.
...
6. В поля для новой записи введите (или скопируйте и вставьте) значения из таблицы ниже. 
    ...
    |:-----|:-----|:-----|:-----|
    |(оставьте пустым)  <br/> |TXT  <br/> |3600 (секунд)  <br/> |MS=msXXXXXXXXXXX  <br/> **Примечание.** Это пример. Используйте здесь свое конкретное значение **Назначение или адрес "Указывает на"** из этой таблицы.           [Как его найти?](../get-help-with-domains/information-for-dns-records.md)          |
...
4. Установите имя, которое служит для домена, на серверы имен Freenom по умолчанию. Выберите **средства управления,** а затем **выберите Nameservers**.
    
    ![Freenom Nameservers setting](../../media/a6ae877a-c248-42b9-bae9-210a80cd01e7.png)
  
5. **Убедитесь, что выбраны** имена имен по умолчанию, а затем выберите **Имена изменений.**
...
8. В поля для новой записи введите (или скопируйте и вставьте) значения из первой строки таблицы ниже. 
    ...
    |:-----|:-----|:-----|:-----|:-----|
    |(оставьте пустым)  <br/> |MX (почтовый обменник)  <br/> |3600 (секунд)  <br/> |\<domain-key\>.mail.protection.outlook.com  <br/> **Примечание:** Получите вашу  *\<domain-key\>*  учетную запись Майкрософт.   [Где это находится?](../get-help-with-domains/information-for-dns-records.md)          |10   <br/> Дополнительные сведения о приоритете см. в статье [What is MX priority?](../setup/domains-faq.yml).    <br/> |
...
10. Если есть другие записи MX, удалите их все. Для каждой записи выберите **Delete**. Когда **появится сообщение Вы действительно хотите удалить эту запись?** выберите **ОК**.
