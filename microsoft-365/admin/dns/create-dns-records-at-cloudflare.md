...
Добавленный домен был приобретен у Cloudflare или у отдельного регистратора доменных имен. Чтобы проверить и создать записи DNS для домена в Microsoft 365, сначала необходимо изменить доменные имена у регистратора доменных имен, чтобы они использовали их.
...
4. На странице **управления DNS** нажмите кнопку **"Добавить запись"** и выберите значения из таблицы ниже.  
    ...
    |**Тип**|**Name (Имя)**|**TTL (Срок жизни)**|**Content (Содержимое)**|
    |:-----|:-----|:-----|:-----|
...
> [!IMPORTANT]
> Помните, что Cloudflare отвечает за обеспечение этой функциональности. Если вы видите несоответствия между действиями ниже и текущим графическим пользовательским интерфейсом Cloudflare ( Графический пользовательский интерфейс), используйте [Сообщество Cloudflare.](https://community.cloudflare.com/) 
...
4. Добавьте первую из двух записей SRV.
    ...
    На странице **управления DNS** нажмите кнопку **"Добавить** запись" и выберите значения из первой строки следующей таблицы.
