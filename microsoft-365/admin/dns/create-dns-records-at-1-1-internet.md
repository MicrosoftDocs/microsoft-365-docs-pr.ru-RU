...
> [!CAUTION]
> Обратите внимание, что 1&IONOS 1 не позволяет домену иметь запись MX и запись CNAME автоотнаружения верхнего уровня. Это ограничивает способы настройки Exchange Online для Майкрософт. Существует обходное решение, но мы рекомендуем  использовать его, только если у вас уже есть опыт создания поддоменов в 1&1 IONOS. > Если несмотря [](https://docs.microsoft.com/microsoft-365/admin/setup/domains-faq) на это ограничение службы вы решили управлять своими записями Microsoft DNS на сайте 1&1 IONOS, выполните действия, определенные в этой статье, чтобы подтвердить владение доменом и настроить записи DNS для электронной почты, Skype для бизнеса Online и других служб. 
...
5. В разделе **"Обзор** **поддомена"** найдите только что созданный поддомен **lyncdiscover,** а затем выберите для этого поддомена панель (v). <br/>В области **"Параметры поддомена"** выберите "Изменить **параметры DNS".**
...
8. В поле "Я  в курсе" выберите "Сохранить". 
...
17. Выберите для оговорки "Я в **курсе"** и выберите **"Сохранить".**
