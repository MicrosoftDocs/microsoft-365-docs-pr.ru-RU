...
Вы можете использовать Basic Mobility and Security для создания политик устройств, которые помогут защитить сведения организации в Microsoft 365 от несанкционированного доступа. Политики можно применить к любому мобильному устройству в организации, где пользователь устройства имеет применимую лицензию Microsoft 365 и зарегистрировал устройство в Basic Mobility and Security.
...
- Создайте группы безопасности, которые включают пользователей Microsoft 365, которые необходимо развернуть политикам для пользователей и для пользователей, которые могут быть заблокированы для доступа к Microsoft 365. Перед развертыванием новой политики организации рекомендуется проверить политику, развернув ее для небольшого количества пользователей. Вы можете создать и использовать группу безопасности, которая включает только себя или небольшое число пользователей Microsoft 365, которые могут протестировать политику для вас. Дополнительные данные о группах безопасности см. в [публикации Create, edit или delete a security group.](../email/create-edit-or-delete-a-security-group.md)
- Чтобы создать и развернуть политики базовой мобильности и безопасности в Microsoft 365, необходимо быть глобальным администратором Microsoft 365. Дополнительные сведения см. [в сайте Permissions in the Security & Compliance Center.](https://support.microsoft.com/office/d10608af-7934-490a-818e-e68f17d0e9c1)
- Перед развертыванием политик дайте организации знать о потенциальных последствиях регистрации устройства в Basic Mobility and Security. В зависимости от того, как настроить политики, незавершенным устройствам может быть заблокирован доступ к Microsoft 365, а данные, включая установленные приложения, фотографии и персональные данные на зарегистрированных устройствах, могут быть удалены.

>[!NOTE]
>Политики и правила доступа, созданные в базовой мобильности и безопасности для Microsoft 365 Business Standard, переопределяют Exchange ActiveSync почтовых ящиков мобильных устройств и правила доступа к устройствам, созданные в центре администрирования Exchange. После регистрации устройства в стандарте базовой мобильности и безопасности для Microsoft 365 бизнес-стандарт любое Exchange ActiveSync почтовых ящиков мобильных устройств или правило доступа к устройству игнорируется. Дополнительные дополнительные Exchange ActiveSync см. [в Exchange ActiveSync Exchange Online.](/exchange/clients-and-mobile-in-exchange-online/exchange-activesync/exchange-activesync)
...
Политика передается на устройство каждого пользователя, которое политика применяется к следующему входу в Microsoft 365 с помощью мобильного устройства. Если раньше пользователи не применяли политику к мобильному устройству после развертывания политики, они получают уведомление на своем устройстве, которое включает действия по регистрации и активации Basic Mobility and Security. Дополнительные сведения см. в [записи мобильного устройства с помощью Basic Mobility and Security.](enroll-your-mobile-device.md) Пока они не завершат регистрацию в службе "Базовая мобильность и безопасность", доступ к электронной почте, OneDrive и другим службам ограничен. После завершения регистрации с помощью приложения Портал компании Intune они могут использовать службы, и политика применяется к устройству.
...
3. Чтобы заблокировать неподдергваемую поддержку устройств, выберите **Блок,** если устройство не поддерживается базовой мобильностью и безопасностью **для Microsoft 365,** а затем выберите **Сохранить**.
...
3. Выберите **Добавить,** чтобы добавить группу безопасности с пользователями, которые необходимо исключить из заблокированного доступа к Microsoft 365. Когда пользователь был добавлен в этот список, он может получить доступ к электронной почте Microsoft 365 при использовании неподтверченного устройства.
...
|Блокировать видеоконференции |Нет|Нет|Да|FaceTime заблокирован на iOS, а не в Skype или других.|
...
|Блокировать подключение Bluetooth |См. заметки|См. заметки|Да|Мы не можем отключить BlueTooth как параметр на Android. Вместо этого мы отключаем все транзакции, для которые требуется BlueTooth: advanced Audio Distribution, Audio/Video Remote Control, устройства без рук, гарнитура, доступ к книге телефонов и серийный порт. При использовании этих функций в нижней части страницы появляется небольшое всплывающее сообщение.|
...
При удалении политики или удалении пользователя из группы, в которую была развернута политика, параметры политики, профиль электронной почты Microsoft 365 и кэшировали сообщения электронной почты могут быть удалены с устройства пользователя. В следующей таблице см., что удаляется для различных типов устройств.
