...
Дополнительный метод проверки не используется до тех пор, пока не будет проверен пароль пользователя. При использовании MFA, даже если надежный пользовательский пароль скомпрометирован, у злоумышленника нет вашего смартфона или отпечатка пальца, которые нужны для завершения входа в систему.
...
В обоих случаях вход в MFA использует для дополнительной проверки "что-то с нелегко дублированным" методом. Включить MFA для Microsoft 365 и Office 365 можно несколькими способами.
...
|---|---|---|
|Все планы Microsoft 365|Используйте параметры безопасности по умолчанию, что требует применения MFA для всех учетных записей пользователей. <p> Вы также можете настроить каждую пользователь MFA для отдельных учетных записей пользователей, но это не рекомендуется.|Версия для малого бизнеса|
...
### <a name="legacy-per-user-mfa-not-recommended"></a>Устаревший для пользователя MFA (не рекомендуется)
