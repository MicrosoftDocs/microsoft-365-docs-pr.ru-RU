...
Дополнительный метод проверки не используется до проверки пароля пользователя. При использовании MFA, даже если надежный пользовательский пароль скомпрометирован, у злоумышленника нет вашего смартфона или отпечатка пальца, которые нужны для завершения входа в систему.
...
### <a name="legacy-per-user-mfa-not-recommended"></a>Устаревшая многофаксная многофайловая многофайловая версия (не рекомендуется)
...
|**Устаревшая многофаксная многофайловая многофайловая версия (не рекомендуется)**|Переопределяет параметры безопасности по умолчанию и политики условного доступа, требующие MFA для каждого входа в систему.|Переопределено параметрами безопасности по умолчанию и политиками условного доступа|Указываются пользователем во время регистрации MFA|
||||
