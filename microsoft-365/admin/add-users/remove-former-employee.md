...
> [!NOTE]
> Для инициировать выход необходимо быть глобальным администратором.

В течение часа или после того, как они покидают текущую страницу Microsoft 365, им будет предложено снова войти. Маркер доступа хорошо работает в течение часа, поэтому временная шкала зависит от того, сколько времени осталось на маркере, и от того, будут ли они переходить с текущей веб-страницы.
...
- Если вы настроите переадресацию электронной почты, только  *новые*  сообщения, отправленные бывшему сотруднику, будут отправляться действующему сотруднику.
 ...
 > [!IMPORTANT]
 > Если вы настраивали пересылку электронной почты или общий почтовый ящик, не удаляйте учетную запись бывшего сотрудника. Учетная запись должна быть там, чтобы прикрепить пересылку электронной почты или общий почтовый ящик.
...
## <a name="block-a-former-employees-access-to-microsoft-365-data"></a>Блокировка доступа бывшего сотрудника к данным Microsoft 365
 ...
 > [!IMPORTANT]
 > Блокировка учетной записи может занять до 24 часов. Если необходимо немедленно запретить пользователю вход в учетную запись, необходимо сбросить его пароль, а затем инициировать разовую выход из сеансов Microsoft 365 на всех устройствах. [](reset-passwords.md) См. ["Выйти"!](#sign-out-now)

::: moniker range="o365-worldwide"

1. В Центре администрирования откройте страницу **Пользователи** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=834822" target="_blank">Активные пользователи</a>.
2. Выберите имя сотрудника, которого необходимо заблокировать, и под именем пользователя выберите символ **блокировки этого пользователя.**
...
2. Выберите подписку, из которой нужно удалить лицензии.
3. На странице сведений выберите **"Удалить лицензии".**
4. В области **"Удалить лицензии"** в поле  "Новое количество" введите общее количество лицензий для этой подписки. Например, если у вас 25 лицензий и вы хотите удалить одну из них, введите 24.
5. Нажмите кнопку **Сохранить**.
::: moniker-end
...
Когда вы [добавите](add-users.md) другого человека в свою деятельность, вам будет предложено приобрести лицензию одновременно, выполив всего один шаг!
...
Если необходимо настроить переадстройку звонков для номера телефона сотрудника, то параметр переадстройки звонков в политиках звонков может настраивать переадстройку, в которой входящие вызовы можно перенаадностить другим пользователям или звонить другому пользователю одновременно. Дополнительные сведения см. [в политиках звонков в Microsoft Teams.](https://docs.microsoft.com/microsoftteams/teams-calling-policy)
