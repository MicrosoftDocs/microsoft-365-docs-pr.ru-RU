...
4. Выберите имя пользователя, чтобы перейти в область свойств, и на вкладке **учетная запись** выберите команду **начать выход**.
...
> [!NOTE]
> Для запуска выхода необходимо быть глобальным администратором.

В течение часа или после того, как они покидают текущую страницу Microsoft 365, в которой они находятся, — они попросят войти повторно. Маркер доступа хорошо подходит для часа, поэтому временная шкала зависит от того, сколько времени осталось на этом маркере, и от того, выходят ли они из текущей веб-страницы.
  
> [!IMPORTANT]
> Если пользователь находится в Outlook в Интернете, просто щелкайте его в своем почтовом ящике, вы не сможете немедленно отойти. После того как вы выберете другой плитку, например OneDrive, или обновите браузер, начата выход.
...
|:-----|:-----|
|**Шаг** <br/> |**Причина** <br/> |
|1. [Сохранение содержимого почтового ящика бывшего сотрудника](#save-the-contents-of-a-former-employees-mailbox) <br/> |Это полезно для человека, который собирается принимать участие сотруднику, или в случае судебного разбирательства.  <br/> |
...
|5a. Что делать, если доступ к OneDrive и SharePoint осуществлялся с личного компьютера?  <br/> |Если для скачивания файлов из OneDrive и SharePoint использовался не корпоративный, а личный компьютер, удалить сохраненные файлы невозможно.  <br/><br/> Они продолжают получать доступ ко всем файлам, которые были синхронизированы с компьютером.  <br/> |
...
2. Перед удалением учетной записи пользователя поместить почтовый ящик на удержание для судебного разбирательства или удержание на месте. Этот вариант гораздо сложнее первого, но имеет смысл в следующих случаях: если ваш корпоративный план предусматривает архивацию и удержание по юридическим причинам, если есть вероятность судебного разбирательства и если вы располагаете технически сильным ИТ-отделом.
    ...
    После преобразования почтового ящика в "неактивный почтовый ящик", "Администраторы", "руководители соответствия" и "руководители соответствия" могут использовать In-Place средств обнаружения электронных данных в Exchange Online для доступа к содержимому и его поиска.
...
4. Включите параметр **Включить переадресацию всей почты, поступающей в этот почтовый ящик**. В поле **адрес пересылки** введите адрес электронной почты текущего сотрудника, который будет получать электронную почту.
5. Нажмите **Сохранить**.
...
Если ваш бывший сотрудник имеет телефонный номер Организации, вы можете использовать центр администрирования Exchange для очистки и блокирования этого устройства, чтобы все данные организации удалялись с устройства и не могли подключаться к Office 365.
...
3. Выберите пользователя и в разделе **мобильные устройства**выберите **Просмотреть сведения**.
4. На странице " **сведения о мобильном устройстве** " в разделе **мобильные устройства**Выберите мобильное устройство, нажмите кнопку **очистить** ![ устройство очистки данных ](../../media/1c113a36-53cb-4974-884f-3ecd9535506e.png) , а затем выберите пункт **блокировать**.
5. Нажмите **Сохранить**.
   > [!TIP]
   > Не забудьте удалить или отключить пользователя из локальной корпоративной службы BlackBerry. Кроме того, необходимо отключить все устройства BlackBerry для пользователя. Обратитесь к руководству по администрированию BlackBerry Business Cloud Services, если необходимо выполнить определенные действия по отключению пользователя.
...
**Чтобы уменьшить количество лицензий, по которым вы оплачиваете** , пока не нанимаете другого человека, выполните следующие действия:

::: moniker range="o365-worldwide"
1. В центре администрирования перейдите на страницу **выставление счетов** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=842054" target="_blank">продуктов</a> и выберите вкладку **продукты** .
2. Выберите подписку, из которой вы хотите удалить лицензии.
3. На странице сведения выберите **Удалить лицензии**.
4. В области **Удалить лицензии** в разделе новое количество в поле **всего лицензий** введите общее количество лицензий, которые вы хотите использовать для этой подписки. Например, если у вас есть 25 лицензий и вы хотите удалить одну из них, введите 24.
5. Нажмите **Сохранить**.
::: moniker-end
...
Дополнительные сведения об управлении пользовательскими лицензиями для Microsoft 365 для бизнеса приведены в статье [Назначение лицензий пользователям в microsoft 365 для бизнеса](../manage/assign-licenses-to-users.md)и [Отмена назначения лицензий пользователям в Microsoft 365 для бизнеса](../manage/remove-licenses-from-users.md).
...
## <a name="set-up-call-forwarding-to-people-in-your-organization"></a>Настройка переадресации звонков для людей в Организации

Если вам нужно настроить переадресацию звонков для номера телефона увольнения сотрудника, параметр переадресация звонков в разделе политики вызовов может настроить переадресацию, в которой входящие звонки могут пересылаться другим пользователям или звонить другому человеку одновременно. Дополнительные сведения см в разделе [политики вызовов в Microsoft Teams](https://docs.microsoft.com/microsoftteams/teams-calling-policy).
...
Чтобы узнать, как удалить и восстановить учетную запись пользователя в Active Directory, ознакомьтесь со статьей [Удаление учетной записи пользователя](https://go.microsoft.com/fwlink/?linkid=841808).
  
Если вы используете Azure Active Directory, ознакомьтесь с разкомандлетом [Remove – MsolUser](https://go.microsoft.com/fwlink/?linkid=842230) PowerShell.
...
Вышеуказанные операции можно выполнить в трех местах:
