...
- Какие сайты SharePoint нужно обходить по темам
...
Параметры обнаружения тем указывают, какие сайты SharePoint используются в качестве источников для тем. Вы можете включить все сайты SharePoint, определенный список сайтов или нет сайтов. Мы рекомендуем выбрать все сайты, чтобы в разделе было обнаружено большое количество хороших тем для пользователей.

При настройках Разделы можно выбрать из следующих вариантов:

- **Все сайты.** Все сайты SharePoint в вашей организации. Это включает текущие и будущие сайты.
- **Все, кроме выбранных сайтов:** все сайты, за исключением тех, которые вы указываете. Сайты, созданные в будущем, будут включены в качестве источников для обнаружения тем. 
...
Если вы выбираете **все,** кроме выбранных сайтов или только выбранных **сайтов,** вы можете загрузить файл csv со списком сайтов. Эти параметры полезны, если вы делаете пилотный проект и хотите включить ограниченное число сайтов для запуска.
...
Если ваша организация развернула [Microsoft 365 Multi-Geo,](/microsoft-365/enterprise/microsoft-365-multi-geo)центр темы размещен в центре расположения, и в качестве источников для тем доступны только сайты SharePoint в центральном расположении. (Если выбрать **все сайты,** Viva Topics будет использовать все сайты в центре расположения.)
...
Зрители темы могут видеть сведения о страницах тем, результатах поиска и о выделении тем в контенте, например на страницах SharePoint. Пользователи могут видеть обнаруженные темы только при доступе к файлам и страницам, в которые была обнаружена тема.
...
Если вы хотите исключить темы на уровне администратора, необходимо добавить их в файл csv и загрузить файл. Это можно сделать во время установки или позже.
...
> [!div class="checklist"]
> * Список сайтов, которые необходимо включить или исключить, если не включать все сайты для обнаружения темы
> * Группа безопасности для зрителей тем, если не позволяет всем пользователям просматривать темы
> * Группа безопасности для участников темы, если не позволяет всем пользователям создавать и редактировать темы
> * Группа безопасности для руководителей тематических знаний, если не позволяет всем пользователям управлять темами
...
[Изменение имени центра темы в Microsoft 365](topic-experiences-administration.md)
