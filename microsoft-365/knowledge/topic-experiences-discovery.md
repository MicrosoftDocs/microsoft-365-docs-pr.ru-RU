...
## <a name="select-sharepoint-topic-sources"></a>Выберите источники темы SharePoint
...
Если вы добавляете сайты с помощью выборщика сайтов, они добавляются в существующий список сайтов, чтобы включить или исключить. Если вы загрузите файл CSV, он переопишет любой существующий список. Если вы ранее включили или исключили определенные сайты, скачайте список в качестве файла CSV, внося изменения и загрузите новый список.

Выбор сайтов для обнаружения тем

1. На **вкладке Открытие Темы** в **разделе Выбор источников темы SharePoint** выберите **Изменить**.
2. На странице **Выбор источников темы SharePoint** выберите, какие сайты SharePoint будут обходиться в качестве источников для ваших тем во время обнаружения. К ним относятся:
    - **Все сайты.** Все сайты SharePoint в вашем клиенте. Это захватывает текущие и будущие сайты.
    ...
    - **Нет сайтов.** Темы не будут автоматически генерируться или обновляться с помощью контента SharePoint. Существующие темы остаются в центре тем.

    ![Снимок экрана пользовательского интерфейса источников темы SharePoint](../media/k-manage-select-topic-source.png)
   
3. Щелкните **Сохранить**.

## <a name="exclude-topics-by-name"></a>Исключение тем по имени

Вы можете исключить темы из обнаружения, загрузив список с помощью файла .csv. Если ранее были исключены темы, вы можете скачать .csv, внести изменения и загрузить его снова.

1. На **вкладке Открытие Темы,** в разделе **Исключить темы,** выберите **Изменить**.
2. Нажмите **Кнопку Исключить темы по имени**.
3. Если вам нужно создать список, скачайте шаблон csv и добавьте разделы, которые необходимо исключить (см. раздел Работа с *шаблоном CSV* ниже). Когда файл будет готов, нажмите **кнопку Обзор** и загрузите файл. Если есть существующий список, вы можете скачать .csv, содержащий список.
