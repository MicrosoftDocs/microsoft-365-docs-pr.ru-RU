...
| ------- | ------- | ------- |
| Когда использовать каждую из моделей | Используется для полуструктурированных форматов файлов, например документов Office с различными макетами, но сходными данными для извлечения. | Используется для неструктурированных форматов файлов, например PDF, для содержимого форм, например счетов или заказов на покупку, у которых макет и форматирование одинаковы. |
| Создание модели | Модель, созданная в конструкторе ИИ с беспрепятственным доступом из библиотеки документов SharePoint.| Модель, созданная во встроенном интерфейсе Центра контента SharePoint.|
| Тип классификации| Настраиваемый классификатор с использованием машинного обучения, благодаря которому система узнаёт, какие данные нужно извлечь.| Обучаемый классификатор с дополнительными модулями для извлечения данных. Использует машинное обучение, чтобы узнать, какие данные нужно извлечь.|
| Locations | Ограничено одной библиотекой документов, кроме случаев, когда используется Power Platform для извлечения из CDS.| Может применяться к нескольким библиотекам.|
...
| Интеграция с управляемыми метаданными | Нет | Да, за счет настройки столбцов библиотеки документов до обучения модели.|
| Интеграция функций соответствия требованиям, если включена защита информации (Майкрософт) | Установите метки хранения.<br>Поддержка меток конфиденциальности появится в ближайшее время. | Установите метки хранения.<br>Поддержка меток конфиденциальности появится в ближайшее время. |
...
| Стоимость транзакции | Использует кредиты AI Builder.<br>Кредиты можно приобрести пакетами в 1 млн.<br>При приобретении 300 и более лицензий на SharePoint Syntex в покупку включается 1 млн кредитов.<br>Кредит в 1 млн позволяет обрабатывать 2000 страниц файлов.| Н/Д |
| Пропускная способность | Подготовлено для общей среды службы данных по умолчанию.| Нет ограничений на пропускную способность|
| Поддерживаемые языки| Английский <br>Будут позже в 2021 г.: испанский, немецкий, французский, итальянский| Модели работают на всех языках латинского алфавита. Помимо английского: немецкий, шведский, французский, испанский, итальянский и португальский.|
