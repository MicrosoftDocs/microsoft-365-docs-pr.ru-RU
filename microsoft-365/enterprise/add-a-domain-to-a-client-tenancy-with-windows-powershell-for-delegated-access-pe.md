...
Вы можете создавать и связывать новые домены с арендой клиента с PowerShell для Microsoft 365 быстрее, чем с помощью центра администрирования Microsoft 365.
  
Партнеры по делегированным правам доступа (DAP)  партнеры по синдикации и поставщики облачных решений (CSP). Они часто бывают поставщиками сети или телекоммуникационных услуг в других компаниях. Они пакет microsoft 365 подписки в свои предложения службы для своих клиентов. Когда они продают подписку Microsoft 365, они автоматически получают разрешения администрирования от имени (AOBO) для клиентов tenancies, чтобы они могли администрировать и сообщать о клиентских tenancies.
## <a name="what-do-you-need-to-know-before-you-begin"></a>Что нужно знать перед началом работы

Процедуры в этом разделе требуют подключения к [Microsoft 365 с PowerShell.](connect-to-microsoft-365-powershell.md)
...
### <a name="get-the-data-for-the-dns-txt-verification-record"></a>Получение данных для записи TXT проверки DNS
 ...
 Microsoft 365 будет создавать конкретные данные, которые необходимо разместить в записи проверки DNS TXT. Чтобы получить эти данные, выполните указанную ниже команду.
...
Прежде чем Microsoft 365 начнет принимать трафик, который направляется на зарегистрированное доменное имя, необходимо доказать, что у вас есть разрешения администратора на домен. Для этого нужно создать для домена запись типа TXT. Такая запись не выполняет никаких действий в домене, и ее можно удалить, когда ваше владение доменом будет подтверждено. Чтобы создать записи TXT, выполните процедуры в [Add DNS records для подключения домена.](../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md) Если эти действия вам не помогут, потребуется найти процедуры для своего регистратора DNS.
...
На последнем шаге вы проверяете в Microsoft 365, что вы владеете общедоступным доменом. После этого шага Microsoft 365 начнет принимать трафик, на который будет наносяся новое доменное имя. Чтобы завершить процесс создания и регистрации домена, выполните следующую команду. 
