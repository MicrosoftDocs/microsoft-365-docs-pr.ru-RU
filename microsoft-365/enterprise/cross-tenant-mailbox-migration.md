...
В этом разделе не содержатся конкретные действия, необходимые для подготовки объектов пользователей MailUser в целевом каталоге, а также пример команды для отправки пакета миграции. Эти сведения см. в подготовьте целевые [объекты пользователей к](#prepare-target-user-objects-for-migration) миграции.
...
5. Запустите сценарий со следующими параметрами и значениями.
    ...
    |---------------------------------------------|-----------------|--------------|
    | -ResourceTenantDomain                       | Исходный домен клиента, например fabrikam.onmicrosoft.com. | Обязательный |
    ...
    | -TargetTenantDomain                         | Целевой домен клиента, например contoso.onmicrosoft.com. | Обязательный |
    | -ResourceTenantId                           | Код организации-источника клиента (GUID). | Обязательный |
    | -SubscriptionId                             | Подписка Azure, используемая для создания ресурсов. | Обязательный |
    | -ResourceGroup                              | Имя группы ресурсов Azure, которая содержит хранилище Key Vault или будет содержать его. | Обязательный |
    | -KeyVaultName                               | Экземпляр Azure Key Vault, в который будет храниться сертификат/секрет приложения миграции почтовых ящиков. | Обязательный |
    ...
    | -ExistingApplicationId                      | Приложение миграции почты, которое будет использовать, если приложение уже создано. | Необязательный |
    | -AzureAppPermissions                        | Разрешения, необходимые для приложения миграции почтовых ящиков, такие как Exchange или MSGraph (Exchange для перемещения почтовых ящиков, MSGraph для использования этого приложения для отправки приглашения ссылки на согласие в клиент ресурсов). | Обязательный |
    | -UseAppAndCertGeneratedForSendingInvitation | Параметр для использования приложения, созданного для миграции, который будет использоваться для отправки приглашения ссылки на согласие администратору клиента-источника. Если этот запрос не затмеен, учетные данные целевого администратора будут предложены подключиться к диспетчеру приглашений Azure и отправить приглашение от имени целевого администратора. | Необязательный |
    | -KeyVaultAuditStorageAccountName            | Учетная запись хранения, в которой будут храниться журналы аудита Хранилища Key Vault. | Необязательный |
    | -KeyVaultAuditStorageResourceGroup          | Группа ресурсов, которая содержит учетную запись хранения для хранения журналов аудита Key Vault. | Необязательный |
    ||||
...
4. Скачайте скрипт SetupCrossTenantRelationshipForTargetResource.ps1 для настройки клиента источника из репозитория GitHub [https://github.com/microsoft/cross-tenant/releases/tag/Preview](https://github.com/microsoft/cross-tenant/releases/tag/Preview) здесь: 
...
7. Запустите сценарий с помощью следующих необходимых параметров и значений.
    ...
    | -TargetTenantId | ИД клиента целевого клиента. Например, ИД клиента Azure AD contoso.onmicrosoft.com клиента. |
    |||
...
Да, необходимо обновить targetAddress (RemoteRoutingAddress/ExternalEmailAddress) для пользователей локального источника, когда почтовый ящик клиента источника перемещается в целевой клиент.  Маршруты почты могут следовать рекомендации для нескольких пользователей почты с разными адресами targetAddresses, но запросы о занятости для почтовых пользователей должны быть нацелены на расположение пользователя почтового ящика. При подыском о занятости не будет гоняться несколько перенаправлений. 
...
**Какие роли RBAC Exchange необходимы для выполнения перекрестного перемещения клиентов?**
...
**Как меняются разрешения почтового ящика?**
...
Почтовый ящик будет расшифровываться перед перемещением. Убедитесь, что ключ клиента настроен в целевом клиенте, если он по-прежнему требуется. Дополнительные [сведения](https://docs.microsoft.com/microsoft-365/compliance/customer-key-overview) см. здесь.  
