...
В этой статье описывается настройка тестовой среды Microsoft 365 для azure AD Seamless SSO.
...
Следуйте инструкциям в [синхронизации хаширования паролей для Microsoft 365](password-hash-sync-m365-ent-test-environment.md). 
...
- Azure AD Connect выполняется в APP1 для синхронизации домена TESTLAB Active Directory Domain Services (AD DS) с клиентом Azure AD подписки Microsoft 365.
...
На этом этапе настройте Azure AD Connect в APP1 для бесшовной SSO Azure AD и убедитесь, что она работает.
...
8. В **диалоговом** окне Windows Security введите **user1** и пароль учетной записи user1, выберите **ОК** и выберите **Далее**.

9. На странице **Готовый к настройке** выберите **Настройка**.

10. На странице **Полная конфигурация** выберите **Exit**.

11. На портале Azure в левой области выберите **Azure Active Directory**  >  **Azure AD Connect.** Убедитесь, что **функция бесшовной единой регистрации** отображается как **Включенная.**
...
- Пробная версия microsoft 365 E5 или платная подписка с тестовой плитой домена DNS.\<*your domain name*> -
- Упрощенная интрасеть организации, подключенная к Интернету, состоящая из виртуальных машин DC1, APP1 и CLIENT1 в подсети виртуальной сети Azure.
...
- Azure AD Seamless SSO включен таким образом, чтобы компьютеры на смоделированной интрасети могли войти в облачные ресурсы Microsoft 365 без указания пароля учетной записи пользователя.
