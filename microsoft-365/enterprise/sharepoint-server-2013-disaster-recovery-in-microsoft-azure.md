...
Следуйте указаниям из статьи [Microsoft Azure Architectures for SharePoint 2013](microsoft-azure-architectures-for-sharepoint-2013.md), чтобы разработать среду аварийного восстановления, включая ферму восстановления SharePoint. Вы можете использовать графику в решении аварийного восстановления [SharePoint в файле Azure](https://go.microsoft.com/fwlink/p/?LinkId=392554) Visio, чтобы начать процесс разработки. Рекомендуем полностью разработать среду, прежде чем приступать к работе в среде Azure.
...
![Две виртуальные машины, развернутые в виртуальной сети Azure и подсети фермы SharePoint, являются контроллерами домена реплики и DNS-серверами](../media/AZarch-HyADdomainConfig.png)
...
Для внешнего доступа к ферме SharePoint можно создать запись хоста на внешнем DNS-сервере с тем же URL-адресом, который клиенты используют в интрасети (например), который указывает на внешний IP-адрес в брандмауэре. `https://sharepoint.contoso.com` (В этом примере лучше всего настроить разделенную DNS так, чтобы внутренний DNS-сервер был доверен и маршрутил запросы непосредственно в кластер фермы SharePoint, а не маршрутизация DNS-запросов на внешний `contoso.com` DNS-сервер.) Затем можно сопооставить внешний IP-адрес с внутренним IP-адресом локального кластера, чтобы клиенты могли найти необходимые ресурсы.
...
Если вы используете именовые коллекции веб-сайтов, как рекомендуется в архитектуре и развертывании именуемого узла [(SharePoint 2013),](https://docs.microsoft.com/SharePoint/administration/host-named-site-collection-architecture-and-deployment)в ферме SharePoint может быть несколько коллекций сайтов, которые размещены одним веб-приложением с уникальными DNS-именами (например, `https://sales.contoso.com` `https://marketing.contoso.com` и). В этом случае вы можете создать для каждого семейства веб-сайтов записи DNS, указывающие на IP-адрес кластера. Когда запросы достигают интерфейсных веб-серверов SharePoint, эти серверы обрабатывают маршрутизацию запросов в соответствующие семейства веб-сайтов.
...
Мы использовали параметр _skipRegisterAsDistributedCachehost_  приpsconfig.exeв командной строке. Дополнительные сведения см. в сведениях о планировании веб-каналов и службы распределенного кэша [в SharePoint Server 2013.](https://docs.microsoft.com/sharepoint/administration/plan-for-feeds-and-the-distributed-cache-service) 
