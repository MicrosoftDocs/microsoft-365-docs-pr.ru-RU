...
description: В этой статье описывается использование Azure для создания среды аварийного восстановления для локальной SharePoint фермы.
...
Дополнительные сведения о решениях аварийного восстановления см. в статьях [High availability and disaster recovery concepts in SharePoint 2013](/SharePoint/administration/high-availability-and-disaster-recovery-concepts) и [Choose a disaster recovery strategy for SharePoint 2013](/SharePoint/administration/plan-for-disaster-recovery).
...
- [Службы репликации распределенной файловой системы (DFS)](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj127250(v=ws.11))
    
- [Отказоустойчивая кластеризация Windows Server (WSFC) с SQL Server](/sql/sql-server/failover-clusters/windows/windows-server-failover-clustering-wsfc-with-sql-server)
    
- [Группы доступности AlwaysOn (SQL Server)](/sql/database-engine/availability-groups/windows/always-on-availability-groups-sql-server)
    
- [Резервное копирование и восстановление баз данных SQL Server](/sql/relational-databases/backup-restore/back-up-and-restore-of-sql-server-databases)
    
- [Установка SharePoint Server 2013 и развертывание в ферме](/SharePoint/install/installation-and-configuration-overview)
...
Следуйте указаниям из статьи [Microsoft Azure Architectures for SharePoint 2013](microsoft-azure-architectures-for-sharepoint-2013.md), чтобы разработать среду аварийного восстановления, включая ферму восстановления SharePoint. Вы можете использовать графику в SharePoint аварийного восстановления в [файле Azure](https://go.microsoft.com/fwlink/p/?LinkId=392554) Visio для запуска процесса разработки. Рекомендуем полностью разработать среду, прежде чем приступать к работе в среде Azure.
...
> [!NOTE]
> Важно изучить необходимые условия для участия базы данных в группе доступности SQL Server AlwaysOn. Дополнительные сведения см. в статье [Предварительные требования, ограничения и рекомендации по использованию групп доступности AlwaysOn](/sql/database-engine/availability-groups/windows/prereqs-restrictions-recommendations-always-on-availability). 
...
![Две виртуальные машины, развернутые в виртуальной сети Azure, и подсеть SharePoint Farm — это контроллеры реплики домена и DNS-серверы](../media/AZarch-HyADdomainConfig.png)
...
Перед развертыванием Active Directory в Azure прочитайте [руководства по развертыванию Windows Server Active Directory на виртуальных машинах Azure](/windows-server/identity/ad-ds/introduction-to-active-directory-domain-services-ad-ds-virtualization-level-100). Эти указания помогут вам определить требуются ли для вашего решения другие параметры конфигурации.
  
Подробные указания по настройке контроллера домена в Azure см. в статье [Установка реплики контроллера домена Active Directory в виртуальных сетях Azure](/windows-server/identity/ad-ds/introduction-to-active-directory-domain-services-ad-ds-virtualization-level-100).
...
Разверните ферму SharePoint в своей виртуальной сети в соответствии с проектными планами. Рекомендуем ознакомиться со статьей [Планирование для SharePoint 2013 в службах инфраструктуры Azure](/previous-versions/azure/dn275958(v=azure.100)) перед развертыванием ролей SharePoint в Azure.
...
|**Название**|**Описание**|
|:-----|:-----|
|[Репликация](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc770278(v=ws.11)) <br/> |Статья по управлению DFS на сайте TechNet, включающая ссылки для репликации  <br/> |
...
|[Репликация DFS. Вопросы и ответы](/previous-versions/windows/it-pro/windows-server-2003/cc773238(v=ws.10)) <br/> |Статья о репликации DFS на сайте TechNet  <br/> |
|[Блог Хосе Баррето (Jose Barreto)](/archive/blogs/josebda/) <br/> |Блог главного руководителя проекта из группы файловых серверов Майкрософт  <br/> |
...
> [!IMPORTANT]
> Поддержка доставки журналов в SharePoint Server ограничена определенными базами данных. Дополнительные сведения см. в статье [Поддерживаемые варианты обеспечения высокого уровня доступности и аварийного восстановления для баз данных SharePoint (SharePoint 2013)](/SharePoint/administration/supported-high-availability-and-disaster-recovery-options-for-sharepoint-databas). 
...
Для внешнего доступа к SharePoint ферме можно создать на внешнем DNS-сервере запись с тем же URL-адресом, который клиенты используют в интрасети (например), который указывает на внешний IP-адрес в брандмауэре. `https://sharepoint.contoso.com` (В этом примере лучше всего настроить разделенный DNS-сервер таким образом, чтобы внутренний DNS-сервер был авторитетным для и маршрутизация запросов непосредственно в кластер фермы SharePoint, а не маршрутизация запросов DNS на внешний `contoso.com` DNS-сервер.) Затем можно сопостерть внешний IP-адрес с внутренним IP-адресом локального кластера, чтобы клиенты могли найти необходимые ресурсы.
...
Если вы используете коллекции сайтов с именем узла, как это рекомендуется в архитектуре и развертывании коллекции сайтов с именем host-named [(SharePoint 2013),](/SharePoint/administration/host-named-site-collection-architecture-and-deployment)у вас может быть несколько коллекций сайтов, размещенных одним и тем же веб-приложением в ферме SharePoint, с уникальными именами DNS (например, `https://sales.contoso.com` и `https://marketing.contoso.com` ). В этом случае вы можете создать для каждого семейства веб-сайтов записи DNS, указывающие на IP-адрес кластера. Когда запросы достигают интерфейсных веб-серверов SharePoint, эти серверы обрабатывают маршрутизацию запросов в соответствующие семейства веб-сайтов.
...
Мы использовали _параметр skipRegisterAsDistributedCachehost,_ когдаpsconfig.exeна командной строке.  Дополнительные сведения см. в [веб-сведениях Plan for feeds and the Distributed Cache service in SharePoint Server 2013.](/sharepoint/administration/plan-for-feeds-and-the-distributed-cache-service) 
...
[Поддерживаемые варианты обеспечения высокой доступности и аварийного восстановления для баз данных SharePoint](/sharepoint/administration/supported-high-availability-and-disaster-recovery-options-for-sharepoint-databas)
  
[Настройка групп обеспечения доступности SQL Server 2012 AlwaysOn для SharePoint 2013](/SharePoint/administration/configure-an-alwayson-availability-group)
