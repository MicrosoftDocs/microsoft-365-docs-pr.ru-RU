...
- Ent_O365
description: В этой статье можно устранить проблемы с производительностью Office 365 и даже устранить некоторые из наиболее распространенных проблем.
...
Необходимо ли знать, какие действия необходимо предпринять для выявления и устранения задержек, зависания и медленной производительности между SharePoint Online, OneDrive для бизнеса, Exchange Online или Skype для бизнеса Online и клиентского компьютера? Перед вызовом службы поддержки эта статья поможет устранить проблемы с производительностью Office 365 и даже устранить некоторые из наиболее распространенных проблем.
...
Если вы не знаете о сетевой производительности и хотите сделать долгосрочный план мониторинга производительности между клиентской машиной и Office 365, взгляните на настройку производительности и устранение неполадок [Office 365 - Admin и IT Pro.](performance-tuning-using-baselines-and-history.md)
...
- Установите средства для сбора и записи сведений:
  ...
  - Используйте регистратор экрана или запустите регистратор шагов (PSR.exe), который поставляется с Windows Vista и более поздней версии, для записи действий, которые вы принимаете во время тестирования.
...
- Необязательный. Если вы тестируете Exchange Online, запустите средство анализатора производительности клиентов Exchange с консоли администрирования Office 365.
- Воспроизведение точных действий, которые вызывают проблему производительности.
- Остановите след Netmon или другого средства.
- В командной строке запустите маршрут трассировки подписки Office 365, введя следующую команду и нажав кнопку ENTER:
...
1. Откройте командную подсказку **(см. командную** команду Start \> **Run** или \>  **клавишу Windows).** \> 
...
Средство мониторинга сети Майкрософт[(Netmon)](https://www.microsoft.com/download/details.aspx?id=4865)анализирует пакеты, то есть трафик, который проходит между компьютерами в сетях. С помощью Netmon для отслеживания трафика с Office 365 можно захватить, просмотреть и прочитать заголовки пакетов, идентифицировать взаимозаменяющие устройства, проверить важные параметры сетевого оборудования, найти выброшенные пакеты и следить за потоком трафика между компьютерами в корпоративной сети и Office 365. Так как фактический текст трафика зашифрован, то есть он (отправляется в порт 443 через SSL/TLS), вы не можете прочитать отправленные файлы. Вместо этого вы получите неотбитый след пути, который принимает пакет, который может помочь вам отслеживать поведение проблемы.
...
Этот снимок экрана из профессиональной версии HTTPWatch. Можно открыть следы, взятые в базовой версии, на компьютере с профессиональной версией и прочитать ее там. Дополнительные сведения могут быть доступны из трассировки с помощью этого метода.
...
1. Либо используйте **тип запуска** PSR.exeОК, либо щелкните тип \>  \>  \>  **ключа Windows** \> **PSR.exe** \> нажмите кнопку ENTER.
...
Если вы хотите получить навыки чтения сетевых трассировок для сайтов Office 365, нет лучшего учителя, чем регулярное создание следов загрузки страниц и получение опыта чтения. Например, если у вас есть шанс, загрузить службу Office 365 и проследить процесс. Фильтр трассировки для трафика DNS или поиск в FrameData для имени службы, которая просматривается. Сканируйте след, чтобы получить представление о действиях, которые происходят при загрузке службы. Это поможет вам узнать, как должна выглядеть нормальная нагрузка страницы, и в случае устранения неполадок, особенно в области производительности, сравнение хороших и плохих следов может научить вас многому.
...
Трафик между клиентом и Office 365 передается через TLS, что означает, что тело трафика будет зашифровано и не читается в общем следе Netmon. Анализ производительности не должен знать специфику сведений в пакете. Однако он очень заинтересован в загонах пакетов и информации, которая в них содержится.

### <a name="tips-to-get-a-good-trace"></a>Советы, чтобы получить хороший след
...
В примере с Office 365 Outlook Online трафик начинается так:

- Стандартный запрос DNS и ответ DNS для outlook.office365.com с соответствием queryIDs. Важно отметить смещение времени для этого поворота, а также то, где в мире глобальная DNS Office 365 отправляет запрос на разрешение имен. В идеале, как можно локально, а не на полпути по всему миру.

- Запрос HTTP GET, отчет о состоянии которого перенесен навсегда (301)

- Трафик RWS, включая запросы RWS Connect и ответы Connect. (Это удаленный Уинсок, который делает подключение для вас.)
...
Все части трафика важны и подключены, но небольшие части трассировки содержат сведения, особенно важные с точки зрения устранения неполадок производительности, поэтому мы сосредоточимся на этих областях. Кроме того, поскольку мы сделали достаточное количество устранения неполадок производительности Office 365 в Корпорации Майкрософт, чтобы составить список наиболее распространенных проблем, мы сосредоточимся на этих проблемах и о том, как использовать средства, которые мы должны искоренить далее.
...
Чтобы увидеть значение масштабирования Windows, используемую сетевым подключением, развяжи сначала СИН, а затем связанный СИН/ACK.
...
### <a name="tcp-idle-time-settings"></a>Параметры времени ожидания TCP
...
В Netmon посмотрите на поле Смещение времени для круговой поездки. Круговая поездка — это время между отправкой запроса клиента на сервер и получением ответа обратно. Проверьте между клиентом и точкой отступа (например. Клиент -- \> Прокси) или клиент в Office 365 (Клиент -- \> Office 365). Это можно увидеть во многих типах пакетов.
...
> [!TIP]
> Не знаете, принадлежит ли IP-адрес вашего следа вашему DNS-серверу? Попробуйте найти его в командной строке. Нажмите  \> **кнопку Начните запуск** \> и введите **cmd** или нажмите **клавишу Windows и** \> введите **cmd**. В запросе введите  `nslookup <the IP address from the network trace>` . Чтобы проверить, используйте nslookup для IP-адреса собственного компьютера. > список ip-диапазонов Корпорации Майкрософт см. в списке URL-адресов [Office 365 и IP-адресов.](./urls-and-ip-address-ranges.md)

Если возникла проблема, ожидайте появления длинных смещений времени в данном случае (Outlook Online), особенно в пакетах TLS:TLS, которые показывают прохождение данных приложений (например, в Netmon вы можете найти пакеты данных приложений с помощью  `.Protocol.TLS AND Description == "TLS:TLS Rec Layer-1 SSL Application Data"` ). Вы должны видеть плавную прогрессию во время сеанса. Если при обновление Outlook Online вы видите длительные задержки, это может быть вызвано высокой степенью отгружания.
...
Калькуляторы пропускной способности Office 365 доступны на этой странице Планирование сети и настройка производительности [для Office 365.](network-planning-and-performance.md)
...
Чтобы отслеживать задержку в следе, вы сможете зафиксировать IP-адрес клиентского компьютера и IP-адрес DNS-сервера в Office 365. Это делается для более легкой фильтрации трассировки. При подключении через прокси-сервер вам потребуется IP-адрес клиента, IP-адрес прокси-сервера и IP-адрес egress Office 365 DNS, чтобы облегчить работу.

В запросе ping, отправленном в outlook.office365.com, будет названо имя центра обработки данных, получающие запрос, даже если  *ping*  не сможет подключиться для отправки последовательных пакетов ICMP товарного знака. Если вы используете PsPing (бесплатный инструмент для скачивания) и укатерите порт (443) и, возможно, для использования IPv4 (-4), вы получите среднее время в пути для отправленных пакетов. Это будет работать для других URL-адресов в службах Office 365, например `psping -4 yourSite.sharepoint.com:443` . В самом деле, вы можете указать ряд 1000 000, чтобы получить больший пример для среднего, попробуйте что-то вроде `psping -4 -n 20 yourSite-my.sharepoint.com:443` .

> [!NOTE]
> PsPing не отправляет пакеты ICMP. Он pings с пакетами TCP над определенным портом, поэтому вы можете использовать любой, который вы знаете, чтобы быть открытым. В Office 365, которая использует SSL/TLS, попробуйте прикрепить порт :443 к psPing.
...
5. Установка PsPing.
6. Откройте командную подсказку (командный код типа Начните запуск или см. тип ключа Windows) и измените каталог в каталог, в котором установлен PsPing для запуска \> \> команды \> PsPing. В моих примерах вы можете увидеть, как я сделал папку "Perf" на корневой части C. Вы можете сделать то же самое для быстрого доступа.
7. Введите команду, чтобы сделать psPing на IP-адресЕ DNS-сервера Office 365 из предыдущего следа Netmon, включая номер порта, например `psping -n 20 132.245.24.82:445` . Это позволит вам сделать выборку из 20 пингов и усреднить задержку при остановке psPing.

Если вы собираетесь в Office 365 через прокси-сервер, действия немного отличаются. Сначала psPing на прокси-сервере, чтобы получить среднее значение задержки в миллисекунд до прокси/egress и обратно, а затем либо запустить PsPing на прокси-сервере, либо на компьютере с прямым подключением к Интернету, чтобы получить недостающее значение (например, в Office 365 и обратно).

Если вы решите выполнить psPing из прокси-сервера, у вас будет два миллисекундных значения: клиентский компьютер на прокси-сервер или точку отступа, а прокси-сервер в Office 365. И вы закончили! Ну, записи значений, в любом случае.

Если вы запустите psPing на другом клиентском компьютере, который имеет прямое подключение к Интернету, то есть без прокси-сервера, у вас будет два миллисекундных значения: клиентский компьютер на прокси-сервер или точку отступа и клиентский компьютер в Office 365. В этом случае вычитайте значение клиентского компьютера на прокси-сервер или точку регресса из значения клиентского компьютера в Office 365, и вы будете иметь номера RTT с клиентского компьютера на прокси-сервер или точку отступа, а с прокси-сервера или отступа на Office 365.
...
> [!NOTE]
> Ваш IP-адрес может быть другим, чем ip-адреса, показанные здесь, например, ваш ing может возвращать что-то большее, как 157.56.0.0/16 или аналогичный диапазон. Список диапазонов, используемых Office 365, ознакомьтесь с URL-адресами [Office 365 и диапазонами IP-адресов.](./urls-and-ip-address-ranges.md)
...
Если проверка подлинности прокси-серверов происходит каждый раз, когда вы делаете новое подключение TCP к Office 365 для получения информации, необходимо пройти процесс проверки подлинности за кулисами. Например, при переходе с календаря на почту в Outlook Online вы пройдете проверку подлинности. И в SharePoint Online, если страница отображает носители или данные с нескольких сайтов или местоположений, вы будете проверки подлинности для каждого подключения TCP, необходимого для отрисовки данных.
...
Проверка подлинности прокси — это параметр на сервере прокси-сервера egress. Если это вызывает проблемы с производительностью в Office 365, необходимо проконсультироваться с сетевой командой.
...
![След Netmon от нагрузки Outlook Online, фильтруемой DNS, и с помощью поиска бесед затем DNS, чтобы сузить результаты.](../media/763cf20e-7b48-4a37-9449-c9978cfe118b.PNG)
...
Если вы хотите больше исследовать время разрешения DNS, попробуйте psPing в отношении порта DNS, используемого TCP (например,  `psping <IP address of DNS server>:53` ) . Вы все еще видите проблему производительности? Если это так, то проблема, скорее всего, будет более широкой сетевой проблемой, чем проблема определенного приложения DNS, которое вы нажимаем для разрешения. Кроме того, еще раз необходимо упомянуть, что в outlook.office365.com 1000 000 000 000 000 000 000 000 000 000 000 000 000 000 000 000 000 outlook-namnorthwest.office365.com).
...
Такие службы, как Outlook Online в Office 365, предоставляет клиентам несколько долгосрочных подключений. Поэтому каждый пользователь может использовать больше подключений, для которых требуется более длительный срок службы.
...
Цель состоит в том, чтобы увидеть MSS в 1460 bytes для передачи данных. Если вы находитесь за прокси-сервером или используете NAT, не забудьте выполнить этот тест от клиента до прокси/egress/NAT и от прокси/egress/NAT до Office 365 для наилучших результатов! Это разные сеансы TCP.
...
> [!NOTE]
> Если вы ищете след и хотите найти трафик, соответствующий вашему разговору, фильтруйте IP-адрес клиента или IP-адрес прокси-сервера или точки отступа или оба. Если вы идете напрямую, вам потребуется перейти к URL-адресу, который вы тестируете для IP-адреса Office 365 в следе, и отфильтровать его.
...
Если вы находитесь за прокси-сервером или используете NAT, не забудьте выполнить этот тест от клиента до прокси/egress/NAT и от прокси/egress/NAT до Office 365 для наилучших результатов! Это разные сеансы TCP.
...
Где в мире Office 365 пытается разрешить эффекты вызова DNS скорость подключения.

В Outlook Online после завершения первого DNS-анализа расположение этого DNS будет использоваться для подключения к ближайшему центру обработки данных. Вы будете подключены к серверу CAS Outlook Online, который будет использовать магистрали сети для подключения к центру обработки данных (DC), где хранятся ваши данные. Это быстрее.

При доступе к SharePoint Online пользователь, перемещающийся за границу, будет направлен в активный центр обработки данных — это центр DC, расположение которого основано на домашней базе клиента SPO (так, dC в США, если пользователь, если он основан на США).
...
Запросы на разрешение имен с DNS-серверов клиента на DNS-серверы Майкрософт в большинстве случаев должны привести к возвращению Microsoft DNS IP-адреса регионального центра обработки данных (dC). Что это значит для вас? Если ваша штаб-квартира находится в Бангалоре, Индия, но вы путешествуете по США, когда браузер запрашивает Outlook Online, DNS-серверы Майкрософт должны передать IP-адреса центрам обработки данных в США — региональным центрам обработки данных. Если почта требуется из Outlook, эти данные будут перенахождение по быстрой магистрали сети Майкрософт между центрами обработки данных.
...
### <a name="office-365-application-troubleshooting"></a>Устранение неполадок приложения Office 365
