...
Если у пользователя уже есть сайт OneDrive, созданный в клиенте, при настройке его PDL не будет автоматически выполняться перемещение его сайта OneDrive. Чтобы переместить пользовательский OneDrive, [см. OneDrive для бизнеса Geo Move](move-onedrive-between-geo-locations.md).

> [!NOTE]
> Exchange Online автоматически пересылает почтовый ящик пользователя, если код PLD изменяется, а почтовый ящик MailboxRegion больше не совпадает с кодом геолокации базы данных почтовых ящиков. Дополнительные сведения см. в [Exchange Online администрирование почтовых ящиков в среде с несколькими географическими условиями.](./administering-exchange-online-multi-geo.md)
