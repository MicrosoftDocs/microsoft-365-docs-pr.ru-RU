...
Корпоративные клиенты с гибридным развертыванием Exchange Online и локальной службой Exchange Server мастер гибридной конфигурации (HCW) и AAD Connect для поддержания и создания гибридной установки. При переходе из Microsoft Cloud Deutschland в область Office 365 в Германии администратор должен повторно запустить последнюю сборку HCW в режиме "Office 365 Germany" до начала миграции Exchange (фаза 5). Затем снова запустите HCW в режиме "Office 365 Worldwide" после завершения этапа 5, чтобы завершить локальное развертывание с помощью параметров региона Office 365 в Германии. Атрибуты Directory синхронизируются между Office 365 и Azure AD с локальной развертыванием через AAD Connect. 

| Step(s) | Описание | Влияние |
|:-------|:-------|:-------|
| (Предварительная фаза 5) — повторное запуск HCW с помощью параметров Office 365 Germany <br><br> <i>Вы можете начать это действие сразу после получения уведомления центра сообщений о начале миграции клиента Office 365 (этап 1).</i>| При отсечении и повторном запуске HCW (17.0.5378.0 или выше) до этапа 5 будет обеспечиваться готовность локальной конфигурации к отправке и приему почты как с пользователями Microsoft Cloud Deutschland, так и с пользователями, которые мигрируют в [https://aka.ms/hybridwizard](https://aka.ms/hybridwizard) регион Office 365 Германии. <p><li> В HCW для списка под **организацией My Office 365** находится выбор Office **365 Germany.** | Невыполнение этой задачи до начала фазы 5 [Миграция exchange] может привести к NDRs для почты, на маршруте между локальной развертыванием Exchange и Office 365.  
| (Post-Phase 5) — повторное запуск HCW с использованием параметров Office 365 по всему миру <br><br> <i>Вы можете начать это действие после получения уведомления центра сообщений о том, что миграция Exchange завершена (этап 5).</i>| При отсечении и повторном запуске HCW после фазы 5 будет сброшена конфигурация локальной конфигурации для гибридной конфигурации только [https://aka.ms/hybridwizard](https://aka.ms/hybridwizard) с Office 365 глобальной. <p><li> В поле списка ниже **Организация My Office 365** находится на сайте , выберите **Office 365 По всему миру**. | Невыполнение этой задачи до этапа 9 [Миграция завершена] может привести к NDRs для почты, маршрутной между локальной развертыванием Exchange и Office 365.  
...
| (Предварительная фаза 5) — сохранение параметров общих почтовых ящиков | Некоторые гибридные клиенты превратили облачные почтовые ящики пользователей в "общие" почтовые ящики с помощью команд Exchange Online. Эта конфигурация облачного почтового ящика записана в почтовый ящик и локальный каталог Exchange Online, однако она не синхронизируется с активным каталогом клиента с помощью AAD Connect. Результатом является несоответствие между представлением Active Directory значений УдаленногоRecipientType почтового ящика и RemoteDisplayType и тем, что в Exchange Online определяет почтовый ящик как общий. <br><br> Клиент несет ответственность за то, чтобы все общие почтовые ящики были надлежащим образом обуярены с помощью `New-RemoteMailbox -Shared` `Enable-RemoteMailbox -Shared` , или `Set-RemoteMailbox -Shared` .  См. эту ссылку для преобразования почтового ящика пользователя [в гибридной среде.](https://docs.microsoft.com/microsoft-365/admin/email/convert-user-mailbox-to-shared-mailbox?view=o365-worldwide)| Невыполнения этой задачи до этапа 5 [Миграция exchange Online] может привести к NDRs для общих почтовых ящиков, которые преобразуются в нелицензируемые почтовые ящики и потеря общего доступа для затронутых почтовых ящиков. [Общие почтовые ящики](https://docs.microsoft.com/exchange/troubleshoot/user-and-shared-mailboxes/shared-mailboxes-unexpectedly-converted-to-user-mailboxes) неожиданно преобразуются в почтовые ящики пользователей после выполнения синхронизации каталогов в гибридном развертывании Exchange.  
||||
