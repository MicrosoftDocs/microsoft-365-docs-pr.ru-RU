...
Чтобы решить проблему ".local", зарегистрируйте новый суффикс или суффикс upN в доменных службах AD DS в соответствие с доменом (или доменами), проверенными в Microsoft 365. После регистрации нового суффикса имена пользователей обновляются, заменив имя ".local" новым именем домена, например, чтобы учетная запись пользователя выглядела billa@contoso.com.
