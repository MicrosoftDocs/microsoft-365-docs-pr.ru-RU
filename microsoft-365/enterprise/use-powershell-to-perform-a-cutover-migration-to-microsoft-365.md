...
Вы можете одновременно перенести содержимое почтовых ящиков пользователей из системы электронной почты из источника в Microsoft 365, используя десъемку. В этой статье описаны задачи, которые выполняются при прямой миграции электронной почты с помощью Exchange Online PowerShell.

Просмотрев раздел "Что нужно знать о миграции электронной почты в [Microsoft 365",](https://go.microsoft.com/fwlink/p/?LinkId=536688)вы можете получить обзор процесса миграции. Ознакомившись с содержимым этой статьи, начните переносить почтовые ящики из одной почтовой системы в другую, руководствуясь приведенными в этой статье сведениями.

> [!NOTE]
> Для выполнения миграции можно также использовать Центр администрирования Exchange. См. ["Выполнение миграции электронной почты в Microsoft 365".](https://go.microsoft.com/fwlink/p/?LinkId=536689)
...
- **Добавьте свою организацию Exchange в качестве принятого домена организации Microsoft 365.** Служба миграции использует SMTP-адрес ваших почтовых ящиков для создания Microsoft Online Services и адреса электронной почты для новых почтовых ящиков Microsoft 365. Миграция не удастся, если ваш домен Exchange не является принятым доменом или основным доменом вашей организации Microsoft 365. Дополнительные сведения см. в под [проверке домена.](https://go.microsoft.com/fwlink/p/?LinkId=534110)
...
- **Назначьте для локальной учетной записи необходимые разрешения на доступ к почтовым ящикам в организации Exchange.** Учетная запись локального пользователя, используемая для подключения к локальной организации Exchange (также называемая администратором миграции), должна иметь необходимые разрешения для доступа к почтовым ящикам локальной организации, которые вы хотите перенести в Microsoft 365. Эта учетная запись пользователя необходима для создания конечной точки миграции в вашей локальной организации.
...
- **Группы безопасности и делегаты** Служба миграции электронной почты не может определить, являются ли группы безопасности локальной службой Active Directory, поэтому она не может подготовка перенесенных групп в качестве групп безопасности в Microsoft 365. Если вы хотите, чтобы в клиенте Microsoft 365 были группы безопасности, необходимо сначала подчищать пустую группу безопасности с включенной поддержкой почты в клиенте Microsoft 365, прежде чем начать предварительную миграцию. Кроме того, при этом способе перемещаются только почтовые ящики, почтовые пользователи, контакты и группы с поддержкой электронной почты. Если какой-либо другой объект Active Directory, например пользователь, не перенесенный в Microsoft 365, назначен менеджером или делегатом переносимого объекта, перед миграцией его необходимо удалить из объекта.
...
После изменения записи MX и проверки того, что вся электронная почта маршрутизна в почтовые ящики Microsoft 365, уведомите пользователей о том, что их почта будет отправлена в Microsoft 365. Затем можно удалить пакет прямой миграции. Убедитесь в следующем, прежде чем удалить пакет миграции.
 ...
 **Активируйте учетные записи пользователей Microsoft 365 для перенесенных учетных записей, назначив лицензии.** Если вы не назначите лицензию, почтовый ящик отключается после окончания льготного периода (30 дней). Чтобы назначить лицензию в Центре администрирования Microsoft 365, см. "Назначение или [назначение лицензий".](https://docs.microsoft.com/microsoft-365/admin/manage/assign-licenses-to-users)
...
- **Создайте DNS-запись автообнаружения, чтобы пользователи смогли с легкостью получить доступ к своим почтовым ящикам.** После переноса всех почтовых ящиков в Microsoft 365 вы можете настроить запись DNS автообнаружия для организации Microsoft 365, чтобы пользователи могли легко подключаться к своим новым почтовым ящикам Microsoft 365 с помощью Outlook и мобильных клиентов. Эта новая DNS-запись автооружия должна использовать то же пространство имен, что и для вашей организации Microsoft 365. Например, если пространство имен облачной службы имеет значение cloud.contoso.com, необходимо создать запись DNS автообнаружения autodiscover.cloud.contoso.com.
    ...
    Если вы храните Exchange Server, убедитесь, что после миграции запись CNAME DNS автообнаружия должна указать на Microsoft 365 как во внутренней, так и во внешней DNS, чтобы клиент Outlook мог подключиться к правильному почтовому ящику.
...
- **Спишите локальные сервера Exchange.** После проверки того, что вся электронная почта передается непосредственно в почтовые ящики Microsoft 365, и вам больше не нужно поддерживать свою организацию электронной почты или не планируете внедрять решение единого входов, вы можете удалить Exchange с серверов и удалить свою организацию Exchange.
