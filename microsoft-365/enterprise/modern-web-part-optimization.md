...
search.appverid:
- MET150
description: Узнайте, как использовать диагностику страниц для оптимизации производительности веб-частей на современных страницах сайтов SharePoint Online.
ms.openlocfilehash: ca1b9328ad71fdd4a3f3c6c6be47eaa3993d4fc7
ms.sourcegitcommit: 786f90a163d34c02b8451d09aa1efb1e1d5f543c
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 02/18/2021
ms.locfileid: "50287153"
---
...
- **Показано,** является ли веб-часть пользовательской или Microsoft OOTB.
- **Имя и ИД** показывают идентификацию сведений, которые помогут найти веб-часть на странице.
- **Общее** время загрузки модуля, инициализации и отрисовки веб-части. Это общее относительное время отрисовки веб-части на странице от начала до конца.
- **Загрузка** модуля показывает время загрузки, оценки и загрузки файлов JavaScript и CSS расширений. Затем будет запускаться процесс Init.
- **Отложенная** загрузка показывает время отложенной загрузки веб-частей, не отображаемой в основном разделе страницы. Существуют определенные условия, в которых слишком много веб-частей для отрисовки, и они находятся в очереди для отрисовки, чтобы свести к минимуму время загрузки страницы.
- **Инициализация** показывает время, необходимое веб-части для инициализации данных.
    Это асинхронный вызов, а время инситива — это вычисление времени для функции onInit, когда возвращенное обещание разрешено.
- **Отрисовка** показывает время, необходимое для отрисовки пользовательского интерфейса (пользовательского интерфейса) после завершения загрузки модуля и init.
    Время выполнения JavaScript для реализации DOM в документе (странице).
    Отрисовка асинхронных ресурсов, например изображений, может занять дополнительное время.
...
- Извлечение или кэширование данных
  - Если веб-часть использует дополнительные вызовы сервера для получения данных для отображения, убедитесь, что эти серверные API быстры и/или реализуют кэшацию на стороне клиента (например, использование _localStorage_ или _IndexedDB_ для больших наборов).
