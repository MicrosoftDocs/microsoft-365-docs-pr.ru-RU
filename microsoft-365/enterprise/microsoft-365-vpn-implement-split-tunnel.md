...
| **Модель** | **Описание** |
| --- | --- |
| [1. VPN Принудительный туннель](#1-vpn-forced-tunnel) | 100 % трафика попадает в VPN-туннель, включая локальное подключение, Интернет и весь O365/M365 |
...
| [4. VPN избирательный туннель](#4-vpn-selective-tunnel) | VPN-туннель используется только для служб на основе corpnet. Маршрут по умолчанию (Интернет и все интернет-службы) проходит напрямую. |
| [5. Без VPN](#5-no-vpn) | Вариант #2, где вместо устаревших VPN все службы corpnet публикуются с помощью современных подходов к обеспечению безопасности (таких как Zscaler ZPA, прокси-сервер Azure Active Directory (Azure AD), MCAS и т. д.) |
...
Эта модель значительно эффективнее для работы предприятия, так как она позволяет небольшому количеству контролируемых и определенных конечных точек с очень высокой нагрузкой и задержкой обойти VPN-туннель и перейти непосредственно в службу Office 365 в этом примере. Это значительно повышает производительность для разгруженных служб, а также снижает нагрузку на инфраструктуру VPN, что позволяет элементам, которые по-прежнему требуют от нее работы, с меньшей нагрузкой на ресурсы. Именно эта модель в этой статье посвящена помощи при переходе, так как она позволяет очень быстро принимать простые, определенные действия с множеством положительных результатов.
...
Третья модель расширяет область модели 2, так как вместо отправки небольшой группы определенных конечных точек напрямую весь трафик отправляется доверенным службам, таким как Office 365 и SalesForce. Это еще больше снижает нагрузку на корпоративную инфраструктуру VPN и повышает производительность определенных услуг. Поскольку эта модель, скорее всего, займет больше времени для оценки целесообразности и реализации, скорее всего, это шаг, который можно выполнить итеративно позже после успешного внедрения модели 2.
...
Более современная версия модели номер 2, при которой все внутренние службы публикуются с помощью современного подхода к обеспечению безопасности или решения SDWAN, такого как прокси Azure AD, MCAS, Zscaler ZPA и т. д.
...
В разделе [URL-адресов и диапазонов IP-адресов Office 365](urls-and-ip-address-ranges.md) корпорация Майкрософт четко определяет ключевые конечные точки, которые необходимо оптимизировать, и классифицирует их как **Оптимизировать**. В настоящее время требуется оптимизировать только четыре URL-адреса и 20 IP-подсетей. На эту небольшую группу конечных точек приходится около 70% - 80% объема трафика в службу Office 365, включая чувствительные к задержкам конечные точки, например, для Teams СМИ. По сути, это трафик, который нам нужно облагоделять, а также тот трафик, который будет оказывать сильное нагрузку на традиционные сетевые пути и инфраструктуру VPN.
...
| Команды Media IPs (без URL) | UDP 3478, 3479, 3480, и 3481 | Распределение обнаружения ретрансляции и трафик в режиме реального времени (3478), аудио (3479), видео (3480) и общий доступ к видеоэкранам (3481). Это конечные точки, используемые для трафика мультимедиа Skype для бизнеса и Microsoft Teams (звонки, собрания и т. д.). Большинство конечных точек предоставляются, когда клиент Microsoft Teams устанавливает вызов (и содержатся в IP-адресах, указанных для службы). Использование протокола UDP требуется для оптимального качества медиа.   |
...
На момент написания записи диапазоны IP-адресов, которые соответствуют этим конечным точкам, являются следующими. **Настоятельно** рекомендуется использовать [сценарий, такой как этот](https://github.com/microsoft/Office365NetworkTools/tree/master/Scripts/Display%20URL-IPs-Ports%20per%20Category) пример, [веб-службу IP-адресов и URL-адресов Office 365](microsoft-365-ip-web-service.md) или [страницу URL-адресов / IP-адресов](urls-and-ip-address-ranges.md), чтобы проверить наличие обновлений при применении конфигурации, и установить политику для этого на регулярной основе. основа.
...
Клиент VPN должен быть настроен таким образом, чтобы трафик на IP-адреса **Оптимизация** направлялся таким образом. Это позволяет трафику использовать локальные ресурсы Майкрософт, такие как Front Door службы Office 365, такие как входные точки [Azure,](https://azure.microsoft.com/blog/azure-front-door-service-is-now-generally-available/) которые доставляют службы Office 365 и конечные точки подключения как можно ближе к вашим пользователям. Это позволяет нам предоставлять пользователям чрезвычайно высокий уровень производительности, где бы они ни находились, и в полной мере использовать преимущества [глобальной сети Microsoft мирового класса](https://azure.microsoft.com/blog/how-microsoft-builds-its-fast-and-reliable-global-network/), которая, скорее всего, займет всего несколько миллисекунд от прямого выхода ваших пользователей.
...
Для вызовов и собраний, если в таблице маршрутов правильно указана необходимая оптимизация IP-подсетей для мультимедиа Teams, когда Teams вызывает функцию [GetBestRoute,](https://docs.microsoft.com/windows/win32/api/iphlpapi/nf-iphlpapi-getbestroute) чтобы определить, какой локальный интерфейс соответствует маршруту, который он должен использовать для определенного назначения, локальный интерфейс будет возвращен для назначений Майкрософт в блоках IP Майкрософт, перечисленных выше.
...
>[!IMPORTANT]
>Чтобы трафик мультимедиа Teams маршрутился с помощью нужного метода во всех сценариях VPN, убедитесь, что пользователи используют клиент Microsoft Teams **версии 1.3.00.13565** или более новой. Эта версия включает улучшения в том, как клиент определяет доступные сетевые пути.

Сигнальный трафик выполняется по протоколу HTTPS и не так чувствителен к задержке, как трафик мультимедиа, и помечен как "Разрешить" в данных URL/IP, поэтому при желании может безопасно маршрутизироваться через VPN-клиент. 
...
Основным контраргументом этого является то, что медиа-трафик уже зашифрован с помощью _безопасного транспортного протокола реального времени (SRTP)_, профиля транспортного протокола реального времени (RTP), который обеспечивает конфиденциальность, аутентификацию и защиту от атак повторного воспроизведения для трафика RTP. Сам SRTP использует случайно сгенерированный сеансовый ключ, которым обмениваются через защищенный канал сигнализации TLS. Это подробно описано в [этом руководстве по безопасности](https://docs.microsoft.com/skypeforbusiness/optimizing-your-network/security-guide-for-skype-for-business-online), но основной раздел, представляющий интерес, - это шифрование мультимедиа.
...
Информацию о том, как Команды смягчают общие проблемы безопасности, такие как голосовые утилиты или _утилиты обхода сеанса для усиленных атак NAT (STUN)_, можно [найти в этой статье](https://docs.microsoft.com/openspecs/office_protocols/ms-ice2/69525351-8c68-4864-b8a6-04bfbc87785c).
...
- Запустите [тест подключения Microsoft 365,](https://aka.ms/netonboard) который будет запускать тесты подключения, включая маршруты трассировки, как было ранее. Мы также добавляем в это средство тесты VPN, которые также должны предоставлять дополнительные сведения.
...
- Простой tracert к конечной точке в пределах области разделенного туннеля должен показать выбранный путь, например:
  ...
  После этого вы увидите путь через локального isP к этой конечной точке, который должен разрешаться в IP-адрес в диапазонах Teams, настроенных для раздельного туннелирований.
...
Если вам нужны дополнительные данные для устранения неполадок или вы запрашиваете помощь от службы поддержки Microsoft, получение следующей информации должно помочь вам ускорить поиск решения. Универсальный инструментарий скриптов устранения неполадок на основе CSS Windows, поддерживаемый службой **TSS Windows,** поможет вам собрать соответствующие журналы простым способом. Инструмент и инструкции по применению можно найти по адресу <https://aka.ms/TssTools.>
...
- **Проверка точки VPN**: [настройка раздельного туннеля для Office 365 и других приложений SaaS](https://supportcenter.checkpoint.com/supportcenter/portal?eventSubmit_doGoviewsolutiondetails=&solutionid=sk167000)

## <a name="faq"></a>Вопросы и ответы

Группа безопасности (Майкрософт) [](https://www.microsoft.com/security/blog/2020/03/26/alternative-security-professionals-it-achieve-modern-security-controls-todays-unique-remote-work-scenarios/) опубликовала статью, которая описывает основные способы для специалистов по безопасности и ИТ-специалистов по обеспечению современных средств контроля безопасности в современных уникальных сценариях удаленной работы. Кроме того, ниже приведены некоторые распространенные вопросы и ответы клиентов по этому вопросу.
...
Ответ - [функция, называемая ограничениями арендатора](https://docs.microsoft.com/azure/active-directory/manage-apps/tenant-restrictions). Трафик аутентификации не имеет большого объема и не особенно чувствителен к задержке, поэтому его можно отправить через решение VPN на локальный прокси-сервер, где применяется данная функция. Здесь сохраняется список надежных клиентов, и если клиент пытается получить маркер для клиента, который не является доверенным, прокси-сервер просто отказано в запросе. Если арендатору доверяют, токен доступен, если у пользователя есть необходимые учетные данные и права.
...
Чтобы предотвратить случайное раскрытие конфиденциальной информации, Office 365 имеет богатый набор [встроенных инструментов](https://docs.microsoft.com/microsoft-365/compliance/data-loss-prevention-policies). Вы можете использовать встроенные [возможности DLP](https://docs.microsoft.com/microsoft-365/compliance/data-loss-prevention-policies) Teams и SharePoint для обнаружения ненадлежащим образом хранимых или передаваемых конфиденциальных данных. Если в стратегии удаленной работы используется политика BYOD, вы можете использовать [](https://docs.microsoft.com/azure/active-directory/conditional-access/app-based-conditional-access) условный доступ на основе приложений, чтобы предотвратить загрузку конфиденциальных данных на личные устройства пользователей.
...
Кроме того, Office 365 обеспечивает защиту для отмеченных конечных точек Оптимизация на различных уровнях в самой службе, [описанной в этом документе](https://docs.microsoft.com/office365/Enterprise/office-365-malware-and-ransomware-protection). Как было отмечено, гораздо эффективнее предоставлять эти элементы безопасности в самой службе, чем пытаться сделать это в соответствии с устройствами, которые могут не полностью понимать протоколы и трафик. По умолчанию SharePoint Online автоматически [сканирует](https://docs.microsoft.com/microsoft-365/security/office-365-security/virus-detection-in-spo) отправку файлов на известные вредоносные программы

Для конечных точек Exchange, перечисленных выше, [Exchange Online Protection](https://docs.microsoft.com/office365/servicedescriptions/exchange-online-protection-service-description/exchange-online-protection-service-description) и Microsoft Defender для Office [365](https://docs.microsoft.com/office365/servicedescriptions/office-365-advanced-threat-protection-service-description) отлично обеспечивают безопасность трафика в службу.
...
Приоритет должен быть отдан отмеченным конечным точкам **Оптимизировать**, поскольку они дадут максимальную выгоду для низкого уровня работы. Однако при желании для работы службы требуются конечные точки с пометкой "Разрешить" и для конечных точек, которые можно использовать при необходимости.

Существуют также различные поставщики, предлагающие облачные решения для прокси-сервера и безопасности, называемые защищенными веб-шлюзами, которые обеспечивают центральную безопасность, контроль и применение корпоративной политики для общего просмотра веб-страниц. Эти решения могут хорошо работать в облаке в первую очередь, если это высокая доступность, высокая доступность, высокая доступность и подготовка в непосредственной близости от пользователей, позволяя безопасному доступу к Интернету доставляться из облачного расположения, близкого к пользователю. Это устраняет необходимость использования шпильки через VPN / корпоративную сеть для общего просмотра трафика, в то же время позволяя осуществлять централизованный контроль безопасности.
...
### <a name="does-split-tunnel-configuration-work-for-teams-running-in-a-browser"></a>Работает ли конфигурация с разделением туннеля для Teams, работая в браузере?

**Нет**. Он работает только в клиенте Microsoft Teams версии 1.3.00.13565 или более новой. Эта версия включает улучшения в том, как клиент определяет доступные сетевые пути.
