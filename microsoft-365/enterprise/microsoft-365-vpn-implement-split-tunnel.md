...
![Конфигурация Split Tunnel VPN](../media/vpn-split-tunneling/vpn-ent-challenge.png)
...
>[!NOTE]
>Microsoft взяла на себя обязательство приостановить изменения в **Оптимизации** конечных точек для Office 365 как минимум до **30 июня 2020 года**, что позволяет клиентам сосредоточиться на других задачах, а не поддерживать белый список конечных точек после первоначальной реализации. Эта статья будет обновлена, чтобы отразить любые будущие изменения.
...
Как для вызовов, так и для встреч, если необходимые таблицы Оптимизации IP для медиа Teams правильно размещены в таблице маршрутов, когда Teams вызывает метод _GetBestRoute_, чтобы определить, какой интерфейс следует использовать для определенного назначения, будет возвращен локальный интерфейс. для пунктов назначения Microsoft в блоках Microsoft IP, перечисленных выше.
