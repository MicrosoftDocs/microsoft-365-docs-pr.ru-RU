...
---
# <a name="step-3-deploy-windows-10-enterprise-for-new-devices-with-windows-autopilot"></a><span data-ttu-id="d2ccb-104">Шаг 3: Развертывание Windows 10 Enterprise для новых устройств с автопилот Windows</span><span class="sxs-lookup"><span data-stu-id="d2ccb-104">Step 3: Deploy Windows 10 Enterprise for new devices with Windows Autopilot</span></span>

<span data-ttu-id="d2ccb-105">*В этой статье относятся к E3 и E5 версиям Microsoft 365 для предприятия*</span><span class="sxs-lookup"><span data-stu-id="d2ccb-105">*This article applies to both the E3 and E5 versions of Microsoft 365 Enterprise*</span></span>

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

<span data-ttu-id="d2ccb-p101">Если у вас есть новый 10 ПК под управлением Windows, можно использовать автопилот Windows для настройки out--поле взаимодействия (OOBE) для вашей организации и развертывания новой системы с помощью приложения и уже настроенные параметры. Существует не изображения для развертывания, не драйверы для вставки и не инфраструктуры для управления. Пользователи могут перейти в процессе развертывания независимо, без необходимости обратитесь к ИТ-администратор.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p101">If you have new Windows 10 PCs, you can use Windows Autopilot to customize the out-of-box-experience (OOBE) for your organization and deploy a new system with apps and settings already configured. There are no images to deploy, no drivers to inject, and no infrastructure to manage. Users can go through the deployment process independently, without the need consult their IT administrator.</span></span>

<span data-ttu-id="d2ccb-p102">Можно настроить и предварительно настроить новые устройства Windows 10 и получают их Готово к использованию производительность пользователей, с помощью Windows автопилот. Чтобы узнать больше о автопилот Windows, включая преимущества и сценарии Windows автопилот видеть [Обзор автопилот Windows](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot). Когда все будет готово, выполните следующие элементы, чтобы начать установку новых устройств.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p102">You can set up and pre-configure new Windows 10 devices and get them ready for productive use using Windows Autopilot. To learn more about Windows Autopilot, including benefits and Windows Autopilot scenarios, see [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot). When ready, follow these parts to start setting up new devices.</span></span>

## <a name="part-1-start-windows-autopilot-deployment"></a><span data-ttu-id="d2ccb-112">Часть 1: Развертывание автопилот Windows Пуск</span><span class="sxs-lookup"><span data-stu-id="d2ccb-112">Part 1: Start Windows Autopilot deployment</span></span>
<span data-ttu-id="d2ccb-113">[Общие сведения о автопилот Windows](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) , чтобы увидеть:</span><span class="sxs-lookup"><span data-stu-id="d2ccb-113">See [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) to:</span></span>

1. <span data-ttu-id="d2ccb-p103">Узнайте о и выполнить необходимые компоненты для развертывания Windows автопилот. Следующие необходимые компоненты:</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p103">Learn about and complete the prerequisites for Windows Autopilot deployment. The prerequisites include:</span></span>
    - <span data-ttu-id="d2ccb-116">**регистрацию устройств и настройку запуска при первом включении компьютера;**</span><span class="sxs-lookup"><span data-stu-id="d2ccb-116">**Device registration and OOBE customization**</span></span>

        <span data-ttu-id="d2ccb-p104">Чтобы зарегистрировать устройств, необходимо получить их код оборудования и зарегистрировать его. Мы активно работающих с различными поставщиками оборудования, чтобы они могли укажите необходимую информацию для вас или отправить от вашего имени. Вы также можете для захвата эту информацию с помощью вручную с помощью скрипта PowerShell, которое создает CSV-файл с идентификатором устройства оборудования.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p104">To register devices, you need to acquire their hardware ID and register it. We are actively working with various hardware vendors to enable them to provide the required information to you, or upload it on your behalf. You also have the option to capture this information by yourself using a PowerShell script that generates a .csv file with the device's hardware ID.</span></span>

        <span data-ttu-id="d2ccb-120">После регистрации устройства существует параметров настройки OOBE, можно настроить, включая пропускать параметры конфиденциальности и лицензионного соглашения.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-120">Once devices are registered, there are OOBE customization options that you can configure including skipping privacy settings and EULA.</span></span>

    - <span data-ttu-id="d2ccb-121">**фирменную символику для запуска при первом включении компьютера;**</span><span class="sxs-lookup"><span data-stu-id="d2ccb-121">**Company branding for OOBE**</span></span>

        <span data-ttu-id="d2ccb-122">Это позволяет Добавление фирменной символики в при выполнении устройства OOBE.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-122">This allows you to add branding to appear during device OOBE.</span></span>

    - <span data-ttu-id="d2ccb-123">**автоматическую регистрацию MDM в Microsoft Intune;**</span><span class="sxs-lookup"><span data-stu-id="d2ccb-123">**MDM auto-enrollment in Microsoft Intune**</span></span>
        
        <span data-ttu-id="d2ccb-p105">Автоматической подачи заявок позволяет пользователям запрашивать свои устройства Windows 10 Intune для управления устройствами при подключении устройства к Azure AD. Для регистрации, пользователи добавьте учетную запись их работы в свои устройства владельцем раскрывающие личность или подключить корпоративных устройств Azure AD. В фоновом режиме также регистрации устройство для управления с Intune.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p105">Automatic enrollment lets users enroll their Windows 10 devices in Intune for device management when they connect their devices to Azure AD. To enroll, users add their work account to their personally-owned devices or join corporate-owned devices to Azure AD. In the background, the device is also enrolled for management with Intune.</span></span>

    - <span data-ttu-id="d2ccb-127">**сетевое подключение к облачным службам, которые использует Windows Autopilot;**</span><span class="sxs-lookup"><span data-stu-id="d2ccb-127">**Network connectivity to cloud services used by Windows Autopilot**</span></span>

        <span data-ttu-id="d2ccb-128">Программа развертывания автопилот Windows использует ряд облачных служб для получения устройство в состояние производительность пользователей и эти службы должны быть доступны из устройств, зарегистрированных в качестве устройства Windows автопилот.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-128">The Windows Autopilot Deployment Program uses a number of cloud services to get your devices to a productive state and these services must be accessible from devices registered as Windows Autopilot devices.</span></span> 

    - <span data-ttu-id="d2ccb-129">**предварительную установку Windows 10 версии 1703 или выше на устройствах;**</span><span class="sxs-lookup"><span data-stu-id="d2ccb-129">**Devices must be pre-installed with Windows 10, version 1703 or later**</span></span>

2. <span data-ttu-id="d2ccb-p106">Узнайте о и установите программу Windows автопилот развертывания для вашей организации. Можно выбрать один из этих программ развертывания:</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p106">Learn about and select the Windows Autopilot Deployment Program for your organization. You can select from these deployment programs:</span></span>
    - <span data-ttu-id="d2ccb-132">**Microsoft Store для бизнеса**</span><span class="sxs-lookup"><span data-stu-id="d2ccb-132">**Microsoft Store for Business**</span></span>
    - <span data-ttu-id="d2ccb-133">**Microsoft Intune**</span><span class="sxs-lookup"><span data-stu-id="d2ccb-133">**Microsoft Intune**</span></span>
    - <span data-ttu-id="d2ccb-134">**Партнерский центр**</span><span class="sxs-lookup"><span data-stu-id="d2ccb-134">**Partner Center**</span></span>

## <a name="part-2-set-up-a-windows-10-device-for-microsoft-365"></a><span data-ttu-id="d2ccb-135">Часть 2: Настройка устройства Windows 10 365 Microsoft</span><span class="sxs-lookup"><span data-stu-id="d2ccb-135">Part 2: Set up a Windows 10 device for Microsoft 365</span></span>
<span data-ttu-id="d2ccb-136">Перед настройкой устройств Windows для пользователей Microsoft 365 убедитесь, что все устройства Windows запущены Windows 10, версия 1703 (создателей обновление) или более поздней версии.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-136">Before you can set up Windows devices for Microsoft 365 users, make sure all the Windows devices are running Windows 10, version 1703 (Creators Update) or later.</span></span>

<span data-ttu-id="d2ccb-137">После всех устройств Windows в вашей организации либо была обновлена до Windows Update создателей 10 или уже под управлением Windows Update создателей 10, можно объединить эти устройства с Azure Active Directory вашей организации.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-137">After all Windows devices in your organization have either been upgraded to Windows 10 Creators Update or are already running Windows 10 Creators Update, you can join these devices to your organization’s Azure Active Directory.</span></span>

### <a name="set-up-a-brand-new-or-newly-upgraded-windows-10-device"></a><span data-ttu-id="d2ccb-138">Настройка устройства Windows 10 совершенно новых или недавно обновлена</span><span class="sxs-lookup"><span data-stu-id="d2ccb-138">Set up a brand new or newly-upgraded Windows 10 device</span></span>
<span data-ttu-id="d2ccb-139">Выполните следующие действия для настройки устройства с помощью Windows 10 OOBE на совершенно новый устройств под управлением Windows Update создателей 10 (или более поздней версии) или на устройстве, который был обновления Windows Update создателей 10 (или более поздней версии), но не было сделано из стандартной программы установки.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-139">Follow these steps to set up a device using the Windows 10 OOBE on a brand new device running Windows 10 Creators Update (or later) or on a device that was upgrade to Windows 10 Creators Update (or later) but has not gone through out-of-box setup.</span></span>

1. <span data-ttu-id="d2ccb-140">Если у вас нет настройки беспроводной сети, убедитесь, что подключение устройства к Интернету через проводной или Ethernet-подключение.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-140">If you don't have a wireless network configured, make sure you connect the device to the internet through a wired or Ethernet connection.</span></span>
2. <span data-ttu-id="d2ccb-p107">Пройти через процесс установки устройств Windows. На устройстве новые или интегральные установки начинается с **давайте Пуск с областью. В этом ли?** экран.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p107">Go through the Windows device setup experience. On a new or reset device, the setup experience starts with the **Let's start with region. Is this right?** screen.</span></span>
3. <span data-ttu-id="d2ccb-p108">Перейдите через настройку устройства Windows 10 до **как настройка?** страницы. Здесь также выберите **для организации**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p108">Go through Windows 10 device setup until you get to the **How would you like to set up?** page. Here, select **Set up for an organization**.</span></span>
4. <span data-ttu-id="d2ccb-p109">Войдите в учетную запись и пароль пользователя Microsoft 365. В зависимости от параметра пароль пользователя, может потребоваться обновить пароль.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p109">Sign in using the Microsoft 365 user's account and password. Depending on the user password setting, you may be prompted to update the password.</span></span> 
5. <span data-ttu-id="d2ccb-147">Завершите настройку Windows 10 на устройстве.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-147">Finish Windows 10 device setup.</span></span>

<span data-ttu-id="d2ccb-148">После завершения устройство будет подключено к вашей организации Azure AD.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-148">After you’re done, the device will be connected to your organization’s Azure AD.</span></span>

### <a name="set-up-a-device-that-has-already-completed-out-of-box-setup"></a><span data-ttu-id="d2ccb-149">Настройка устройства, который уже выполнил стандартной программы установки</span><span class="sxs-lookup"><span data-stu-id="d2ccb-149">Set up a device that has already completed out-of-box setup</span></span>
<span data-ttu-id="d2ccb-150">Если устройство имеет создателей 10 Windows Update (или более поздней версии) и имеет уже сделано стандартной установки, выполните следующие действия.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-150">If your device has Windows 10 Creators Update (or later) and has already gone through the out-of-box setup, follow these steps.</span></span>

1. <span data-ttu-id="d2ccb-151">На Компьютере пользователя Windows, на котором выполняется Windows 10, версия 1703 (создателей обновления), выберите логотип **Windows** и затем выберите значок « **Параметры** ».</span><span class="sxs-lookup"><span data-stu-id="d2ccb-151">On your user's Windows PC that is running Windows 10, version 1703 (Creators Update), select the **Windows** logo, and then select the **Settings** icon.</span></span>
2. <span data-ttu-id="d2ccb-152">В окне **Параметры** выберите элемент **Учетные записи**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-152">In **Settings**, go to **Accounts**.</span></span>
3. <span data-ttu-id="d2ccb-153">На странице **Укажите сведения** выберите **доступа к работе и в школе** > **подключение**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-153">On the **Your info** page, select **Access work or school** > **Connect**.</span></span>
4. <span data-ttu-id="d2ccb-154">В диалоговом окне **Настройка рабочего или школы учетной записи** , в области **действий**выберите **присоединиться к устройству Azure Active Directory**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-154">On the **Set up a work or school account** dialog, under **Alternate actions**, select **Join this device to Azure Active Directory**.</span></span>
5. <span data-ttu-id="d2ccb-155">На странице **давайте вы вход** введите работу или школе учетной записи и выберите **Далее**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-155">On the **Let's get you signed in** page, enter your work or school account, and select **Next**.</span></span>
6. <span data-ttu-id="d2ccb-156">На странице **Введите пароль** введите пароль и выбрать **Вход**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-156">On the **Enter password** page, enter your password, and select **Sign in**.</span></span>
7. <span data-ttu-id="d2ccb-157">На странице **Убедитесь в том, что это вашей организации** убедитесь, что сведения правильные и выберите **присоединиться к**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-157">On the **Make sure this is your organization** page, verify that the information is correct, and select **Join**.</span></span>
8. <span data-ttu-id="d2ccb-p110">На **вы все set!** выберите **выполнить**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p110">On the **You're all set!** page, select **Done**.</span></span>

<span data-ttu-id="d2ccb-160">После этого пользователя будет подключен к вашей организации Azure AD.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-160">After you're done, the user will be connected to your organization's Azure AD.</span></span>

### <a name="verify-the-device-is-connected-to-azure-ad"></a><span data-ttu-id="d2ccb-161">Проверка подключения устройства к Azure AD</span><span class="sxs-lookup"><span data-stu-id="d2ccb-161">Verify the device is connected to Azure AD</span></span>
<span data-ttu-id="d2ccb-162">Выполните следующие действия, чтобы проверить состояние синхронизации устройства с Azure AD и нажмите начать использовать учетную запись Майкрософт 365 на устройстве.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-162">Follow these steps to verify the device’s sync status with Azure AD, and then start using your Microsoft 365 account on the device.</span></span> 

1. <span data-ttu-id="d2ccb-163">Откройте **Параметры**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-163">Open **Settings**.</span></span>
2. <span data-ttu-id="d2ccb-164">На странице **доступа к работе и в школе** выберите \*\*, подключенных к <organization name> \*\* область для предоставления кнопок **Info** "и" **Отключить**.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-164">On the **Access work or school** page, select the **Connected to <organization name>** area to expose the buttons **Info** and **Disconnect**.</span></span>
3. <span data-ttu-id="d2ccb-165">Выберите **сведения** для получения состояния синхронизации.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-165">Select **Info** to get your synchronization status.</span></span>
4. <span data-ttu-id="d2ccb-166">На странице **состояния синхронизации** выберите **синхронизации** для получения последних мобильного устройства политик управления на персональном Компьютере.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-166">On the **Sync status** page, select **Sync** to get the latest mobile device management policies onto the PC.</span></span>
5. <span data-ttu-id="d2ccb-167">Начать работу с использованием учетной записи Майкрософт 365, перейдите на кнопку Windows **Пуск** , щелкните правой кнопкой мыши изображение текущей учетной записи, а затем выберите **переключатель** запись.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-167">To start using the Microsoft 365 account, go to the Windows **Start** button, right-click your current account picture and then select **Switch** account.</span></span>
6. <span data-ttu-id="d2ccb-168">Войдите, используя адрес электронной почты и пароль организации.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-168">Sign in by using your organization email and password.</span></span>

<span data-ttu-id="d2ccb-p111">Если возникают проблемы с помощью Windows 10 в корпоративной среде, можно обратиться к [лучшие решения службы поддержки Майкрософт для наиболее распространенных проблем](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). Эти ресурсы включают статьи статья БАЗЫ знаний, обновления и статьи библиотеки.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-p111">If you experience issues when using Windows 10 in an enterprise environment, you can consult [top Microsoft Support solutions for the most common issues](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). These resources include KB articles, updates, and library articles.</span></span>

<span data-ttu-id="d2ccb-171">Прежде чем перейти к следующему шагу, проверьте [условия](windows10-exit-criteria.md#crit-windows10-step3), при выполнении которых можно считать данный шаг завершенным.</span><span class="sxs-lookup"><span data-stu-id="d2ccb-171">As an interim checkpoint, you can see the [exit criteria](windows10-exit-criteria.md#crit-windows10-step3) corresponding to this step.</span></span>

## <a name="next-step"></a><span data-ttu-id="d2ccb-172">Следующее действие</span><span class="sxs-lookup"><span data-stu-id="d2ccb-172">Next step</span></span>

|||
|:-------|:-----|
|![](./media/stepnumbers/Step4.png)| [<span data-ttu-id="d2ccb-173">Для проверки работоспособности устройств используйте Windows Analytics</span><span class="sxs-lookup"><span data-stu-id="d2ccb-173">Use Windows Analytics to monitor device health</span></span>](windows10-enable-windows-analytics.md) |
