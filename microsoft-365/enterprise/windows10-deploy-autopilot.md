...
ms.locfileid: "32289259"
---
# <a name="step-3-deploy-windows-10-enterprise-for-new-devices-with-windows-autopilot"></a><span data-ttu-id="2f111-104">Шаг 3: развертывание Windows 10 Корпоративная для новых устройств с помощью Windows автопилота</span><span class="sxs-lookup"><span data-stu-id="2f111-104">Step 3: Deploy Windows 10 Enterprise for new devices with Windows Autopilot</span></span>

<span data-ttu-id="2f111-105">*Эта статья относится как к корпоративным, так и к версиям Microsoft 365 для Microsoft*</span><span class="sxs-lookup"><span data-stu-id="2f111-105">*This article applies to both the E3 and E5 versions of Microsoft 365 Enterprise*</span></span>

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

<span data-ttu-id="2f111-106">Если у вас есть новые компьютеры с Windows 10, вы можете использовать автопилот Windows для настройки готового к работе (OOBE) для вашей организации и развертывания новой системы с приложениями и настройками, которые уже настроены.</span><span class="sxs-lookup"><span data-stu-id="2f111-106">If you have new Windows 10 PCs, you can use Windows Autopilot to customize the out-of-box-experience (OOBE) for your organization and deploy a new system with apps and settings already configured.</span></span> <span data-ttu-id="2f111-107">Нет изображений для развертывания, нет драйверов для вставки и нет инфраструктуры для управления.</span><span class="sxs-lookup"><span data-stu-id="2f111-107">There are no images to deploy, no drivers to inject, and no infrastructure to manage.</span></span> <span data-ttu-id="2f111-108">Пользователи могут проходить через процесс развертывания независимо, без необходимости обращаться к ИТ ИТ ИТ.</span><span class="sxs-lookup"><span data-stu-id="2f111-108">Users can go through the deployment process independently, without the need consult their IT administrator.</span></span>

<span data-ttu-id="2f111-109">Вы можете настроить и предварительно настроить новые устройства с Windows 10 и подготовить их к эффективному использованию с помощью Windows "Автопилот".</span><span class="sxs-lookup"><span data-stu-id="2f111-109">You can set up and pre-configure new Windows 10 devices and get them ready for productive use using Windows Autopilot.</span></span> <span data-ttu-id="2f111-110">Для получения дополнительных сведений о Windows автопилоте, в том числе о преимуществах и сценариях Windows для автопилота, ознакомьтесь со статьей [Обзор Windows автопилот](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot).</span><span class="sxs-lookup"><span data-stu-id="2f111-110">To learn more about Windows Autopilot, including benefits and Windows Autopilot scenarios, see [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot).</span></span> <span data-ttu-id="2f111-111">Когда вы будете готовы, выполните следующие части, чтобы начать настройку новых устройств.</span><span class="sxs-lookup"><span data-stu-id="2f111-111">When ready, follow these parts to start setting up new devices.</span></span>

## <a name="part-1-start-windows-autopilot-deployment"></a><span data-ttu-id="2f111-112">Часть 1: запуск развертывания Windows для автопилота</span><span class="sxs-lookup"><span data-stu-id="2f111-112">Part 1: Start Windows Autopilot deployment</span></span>
<span data-ttu-id="2f111-113">Просмотрите [Обзор Windows Автопилот для выполнения](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) следующих действий:</span><span class="sxs-lookup"><span data-stu-id="2f111-113">See [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) to:</span></span>

1. <span data-ttu-id="2f111-114">Узнайте о предварительных требованиях для развертывания Windows с помощью средства "автопилотная версия".</span><span class="sxs-lookup"><span data-stu-id="2f111-114">Learn about and complete the prerequisites for Windows Autopilot deployment.</span></span> <span data-ttu-id="2f111-115">Ниже приведены необходимые условия.</span><span class="sxs-lookup"><span data-stu-id="2f111-115">The prerequisites include:</span></span>
    - <span data-ttu-id="2f111-116">**регистрацию устройств и настройку запуска при первом включении компьютера;**</span><span class="sxs-lookup"><span data-stu-id="2f111-116">**Device registration and OOBE customization**</span></span>

        <span data-ttu-id="2f111-117">Для регистрации устройств необходимо получить идентификатор оборудования и зарегистрировать его.</span><span class="sxs-lookup"><span data-stu-id="2f111-117">To register devices, you need to acquire their hardware ID and register it.</span></span> <span data-ttu-id="2f111-118">Мы активно работаем с различными поставщиками оборудования, чтобы позволить им предоставить вам необходимые сведения или отправить их от вашего имени.</span><span class="sxs-lookup"><span data-stu-id="2f111-118">We are actively working with various hardware vendors to enable them to provide the required information to you, or upload it on your behalf.</span></span> <span data-ttu-id="2f111-119">Вы также можете записать эти сведения самостоятельно, используя скрипт PowerShell, который создает CSV-файл с аппаратным ИДЕНТИФИКАТОРом устройства.</span><span class="sxs-lookup"><span data-stu-id="2f111-119">You also have the option to capture this information by yourself using a PowerShell script that generates a .csv file with the device's hardware ID.</span></span>

        <span data-ttu-id="2f111-120">После регистрации устройств можно настроить параметры настройки OOBE, включая пропуск параметров конфиденциальности и ЛИЦЕНЗИОНного соглашения.</span><span class="sxs-lookup"><span data-stu-id="2f111-120">Once devices are registered, there are OOBE customization options that you can configure including skipping privacy settings and EULA.</span></span>

    - <span data-ttu-id="2f111-121">**фирменную символику для запуска при первом включении компьютера;**</span><span class="sxs-lookup"><span data-stu-id="2f111-121">**Company branding for OOBE**</span></span>

        <span data-ttu-id="2f111-122">Это позволяет добавить фирменную символику, которая будет отображаться во время OOBE устройства.</span><span class="sxs-lookup"><span data-stu-id="2f111-122">This allows you to add branding to appear during device OOBE.</span></span>

    - <span data-ttu-id="2f111-123">**автоматическую регистрацию MDM в Microsoft Intune;**</span><span class="sxs-lookup"><span data-stu-id="2f111-123">**MDM auto-enrollment in Microsoft Intune**</span></span>
        
        <span data-ttu-id="2f111-124">Автоматическая регистрация позволяет пользователям регистрировать свои устройства с Windows 10 в Intune для управления устройствами при подключении своих устройств к Azure AD.</span><span class="sxs-lookup"><span data-stu-id="2f111-124">Automatic enrollment lets users enroll their Windows 10 devices in Intune for device management when they connect their devices to Azure AD.</span></span> <span data-ttu-id="2f111-125">Чтобы зарегистрировать, пользователи добавляют свои рабочие учетные записи в собственные устройства или присоединяются к устройствам, принадлежащим корпоративным корпоративным устройствам, в Azure AD.</span><span class="sxs-lookup"><span data-stu-id="2f111-125">To enroll, users add their work account to their personally-owned devices or join corporate-owned devices to Azure AD.</span></span> <span data-ttu-id="2f111-126">В фоновом режиме устройство также регистрируется для управления с помощью Intune.</span><span class="sxs-lookup"><span data-stu-id="2f111-126">In the background, the device is also enrolled for management with Intune.</span></span>

    - <span data-ttu-id="2f111-127">**сетевое подключение к облачным службам, которые использует Windows Autopilot;**</span><span class="sxs-lookup"><span data-stu-id="2f111-127">**Network connectivity to cloud services used by Windows Autopilot**</span></span>

        <span data-ttu-id="2f111-128">Программа развертывания Windows для автопилота использует несколько облачных служб для достижения производительного состояния устройств, и эти службы должны быть доступны на устройствах, зарегистрированных как устройства с Windows автопилотной версии.</span><span class="sxs-lookup"><span data-stu-id="2f111-128">The Windows Autopilot Deployment Program uses a number of cloud services to get your devices to a productive state and these services must be accessible from devices registered as Windows Autopilot devices.</span></span> 

    - <span data-ttu-id="2f111-129">**предварительную установку Windows 10 версии 1703 или выше на устройствах;**</span><span class="sxs-lookup"><span data-stu-id="2f111-129">**Devices must be pre-installed with Windows 10, version 1703 or later**</span></span>

2. <span data-ttu-id="2f111-130">Узнайте о программе развертывания Windows и выберите ее для своей организации.</span><span class="sxs-lookup"><span data-stu-id="2f111-130">Learn about and select the Windows Autopilot Deployment Program for your organization.</span></span> <span data-ttu-id="2f111-131">Вы можете выбрать из следующих программ развертывания:</span><span class="sxs-lookup"><span data-stu-id="2f111-131">You can select from these deployment programs:</span></span>
    - <span data-ttu-id="2f111-132">**Microsoft Store для бизнеса**</span><span class="sxs-lookup"><span data-stu-id="2f111-132">**Microsoft Store for Business**</span></span>
    - <span data-ttu-id="2f111-133">**Microsoft Intune**</span><span class="sxs-lookup"><span data-stu-id="2f111-133">**Microsoft Intune**</span></span>
    - <span data-ttu-id="2f111-134">**Партнерский центр**</span><span class="sxs-lookup"><span data-stu-id="2f111-134">**Partner Center**</span></span>

## <a name="part-2-set-up-a-windows-10-device-for-microsoft-365"></a><span data-ttu-id="2f111-135">Часть 2: Настройка устройства Windows 10 для Microsoft 365</span><span class="sxs-lookup"><span data-stu-id="2f111-135">Part 2: Set up a Windows 10 device for Microsoft 365</span></span>
<span data-ttu-id="2f111-136">Прежде чем настраивать устройства Windows для пользователей Microsoft 365, убедитесь, что все устройства Windows работают под управлением Windows 10, версия 1703 (создатели) или более поздней версии.</span><span class="sxs-lookup"><span data-stu-id="2f111-136">Before you can set up Windows devices for Microsoft 365 users, make sure all the Windows devices are running Windows 10, version 1703 (Creators Update) or later.</span></span>

<span data-ttu-id="2f111-137">После обновления всех устройств Windows в Организации до обновления Windows 10 Creators или обновления Windows 10 Creators вы можете присоединить эти устройства к Azure Active Directory вашей организации.</span><span class="sxs-lookup"><span data-stu-id="2f111-137">After all Windows devices in your organization have either been upgraded to Windows 10 Creators Update or are already running Windows 10 Creators Update, you can join these devices to your organization’s Azure Active Directory.</span></span>

### <a name="set-up-a-brand-new-or-newly-upgraded-windows-10-device"></a><span data-ttu-id="2f111-138">Настройка нового или только что обновленного устройства с Windows 10</span><span class="sxs-lookup"><span data-stu-id="2f111-138">Set up a brand new or newly-upgraded Windows 10 device</span></span>
<span data-ttu-id="2f111-139">Выполните указанные ниже действия, чтобы настроить устройство с помощью ПРИВЕТСТВИя Windows 10 с новым устройством, на котором выполняется обновление Windows 10 Creators (или более поздней версии), или на устройстве, которое было обновлено до Windows 10 Creators (или более поздней версии), но не проходилось с помощью готовой программы установки.</span><span class="sxs-lookup"><span data-stu-id="2f111-139">Follow these steps to set up a device using the Windows 10 OOBE on a brand new device running Windows 10 Creators Update (or later) or on a device that was upgrade to Windows 10 Creators Update (or later) but has not gone through out-of-box setup.</span></span>

1. <span data-ttu-id="2f111-140">Если у вас нет беспроводной сети, убедитесь, что устройство подключено к Интернету через проводное или Ethernet подключение.</span><span class="sxs-lookup"><span data-stu-id="2f111-140">If you don't have a wireless network configured, make sure you connect the device to the internet through a wired or Ethernet connection.</span></span>
2. <span data-ttu-id="2f111-141">ПроЙдите процесс установки устройства Windows.</span><span class="sxs-lookup"><span data-stu-id="2f111-141">Go through the Windows device setup experience.</span></span> <span data-ttu-id="2f111-142">На новом устройстве или перезагрузке с помощью программы установки начинается с региона "Приступая к работе" **. Это правильно?** странице.</span><span class="sxs-lookup"><span data-stu-id="2f111-142">On a new or reset device, the setup experience starts with the **Let's start with region. Is this right?** screen.</span></span>
3. <span data-ttu-id="2f111-143">Начните настройку Windows 10 на устройстве и дойдите до страницы **Выбор способа настройки**.</span><span class="sxs-lookup"><span data-stu-id="2f111-143">Go through Windows 10 device setup until you get to the **How would you like to set up?** page.</span></span> <span data-ttu-id="2f111-144">Выберите **Настройка для Организации**.</span><span class="sxs-lookup"><span data-stu-id="2f111-144">Here, select **Set up for an organization**.</span></span>
4. <span data-ttu-id="2f111-145">Войдите в систему, используя учетную запись и пароль пользователя Microsoft 365.</span><span class="sxs-lookup"><span data-stu-id="2f111-145">Sign in using the Microsoft 365 user's account and password.</span></span> <span data-ttu-id="2f111-146">В зависимости от настройки пароля пользователя может быть предложено обновить пароль.</span><span class="sxs-lookup"><span data-stu-id="2f111-146">Depending on the user password setting, you may be prompted to update the password.</span></span> 
5. <span data-ttu-id="2f111-147">Завершите настройку Windows 10 на устройстве.</span><span class="sxs-lookup"><span data-stu-id="2f111-147">Finish Windows 10 device setup.</span></span>

<span data-ttu-id="2f111-148">После этого устройство будет подключено к Azure AD вашей организации.</span><span class="sxs-lookup"><span data-stu-id="2f111-148">After you’re done, the device will be connected to your organization’s Azure AD.</span></span>

### <a name="set-up-a-device-that-has-already-completed-out-of-box-setup"></a><span data-ttu-id="2f111-149">Настройка устройства, которое уже выполнило установку после завершения работы</span><span class="sxs-lookup"><span data-stu-id="2f111-149">Set up a device that has already completed out-of-box setup</span></span>
<span data-ttu-id="2f111-150">Если на устройстве имеется обновление Windows 10 Creators (или более поздней версии) и уже прошло установку, выполните указанные ниже действия.</span><span class="sxs-lookup"><span data-stu-id="2f111-150">If your device has Windows 10 Creators Update (or later) and has already gone through the out-of-box setup, follow these steps.</span></span>

1. <span data-ttu-id="2f111-151">На компьютере пользователя с Windows, работающем под управлением Windows 10, версии 1703 (Creators Update), выберите логотип **Windows** , а затем щелкните значок " **Параметры** ".</span><span class="sxs-lookup"><span data-stu-id="2f111-151">On your user's Windows PC that is running Windows 10, version 1703 (Creators Update), select the **Windows** logo, and then select the **Settings** icon.</span></span>
2. <span data-ttu-id="2f111-152">В окне **Параметры** выберите элемент **Учетные записи**.</span><span class="sxs-lookup"><span data-stu-id="2f111-152">In **Settings**, go to **Accounts**.</span></span>
3. <span data-ttu-id="2f111-153">На странице " **сведения** " выберите **доступ к рабочему или учебному** > **подключению**.</span><span class="sxs-lookup"><span data-stu-id="2f111-153">On the **Your info** page, select **Access work or school** > **Connect**.</span></span>
4. <span data-ttu-id="2f111-154">В диалоговом окне **Настройка рабочей или учебной учетной записи** в разделе **альтернативные действия**выберите Присоединить **это устройство к Azure Active Directory**.</span><span class="sxs-lookup"><span data-stu-id="2f111-154">On the **Set up a work or school account** dialog, under **Alternate actions**, select **Join this device to Azure Active Directory**.</span></span>
5. <span data-ttu-id="2f111-155">На странице **получить вход в** систему введите рабочую или учебную учетную запись и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="2f111-155">On the **Let's get you signed in** page, enter your work or school account, and select **Next**.</span></span>
6. <span data-ttu-id="2f111-156">На странице **Введите пароль** введите пароль и нажмите кнопку **войти**.</span><span class="sxs-lookup"><span data-stu-id="2f111-156">On the **Enter password** page, enter your password, and select **Sign in**.</span></span>
7. <span data-ttu-id="2f111-157">Убедитесь, что на странице **ваша организация** указана правильная информация, и выберите команду присоединиться \*\*\*\*.</span><span class="sxs-lookup"><span data-stu-id="2f111-157">On the **Make sure this is your organization** page, verify that the information is correct, and select **Join**.</span></span>
8. <span data-ttu-id="2f111-158">На странице **Готово!**</span><span class="sxs-lookup"><span data-stu-id="2f111-158">On the **You're all set!**</span></span> <span data-ttu-id="2f111-159">Нажмите кнопку **done (Готово**).</span><span class="sxs-lookup"><span data-stu-id="2f111-159">page, select **Done**.</span></span>

<span data-ttu-id="2f111-160">После этого пользователь будет подключен к Azure AD вашей организации.</span><span class="sxs-lookup"><span data-stu-id="2f111-160">After you're done, the user will be connected to your organization's Azure AD.</span></span>

### <a name="verify-the-device-is-connected-to-azure-ad"></a><span data-ttu-id="2f111-161">Проверка подключения устройства к Azure AD</span><span class="sxs-lookup"><span data-stu-id="2f111-161">Verify the device is connected to Azure AD</span></span>
<span data-ttu-id="2f111-162">Выполните следующие действия, чтобы проверить состояние синхронизации устройства с помощью Azure AD, а затем начните использовать свою учетную запись Microsoft 365 на устройстве.</span><span class="sxs-lookup"><span data-stu-id="2f111-162">Follow these steps to verify the device’s sync status with Azure AD, and then start using your Microsoft 365 account on the device.</span></span> 

1. <span data-ttu-id="2f111-163">Откройте **Параметры**.</span><span class="sxs-lookup"><span data-stu-id="2f111-163">Open **Settings**.</span></span>
2. <span data-ttu-id="2f111-164">На странице " **доступ к работе или учебному доступу** " выберите область \*\*подключено <organization name> \*\* , чтобы отобразить **сведения о** кнопках и **Отключить**ее.</span><span class="sxs-lookup"><span data-stu-id="2f111-164">On the **Access work or school** page, select the **Connected to <organization name>** area to expose the buttons **Info** and **Disconnect**.</span></span>
3. <span data-ttu-id="2f111-165">Выберите **сведения** , чтобы получить состояние синхронизации.</span><span class="sxs-lookup"><span data-stu-id="2f111-165">Select **Info** to get your synchronization status.</span></span>
4. <span data-ttu-id="2f111-166">На странице " **состояние синхронизации** " выберите вариант **Синхронизация** , чтобы получить последние политики управления мобильными устройствами на компьютер.</span><span class="sxs-lookup"><span data-stu-id="2f111-166">On the **Sync status** page, select **Sync** to get the latest mobile device management policies onto the PC.</span></span>
5. <span data-ttu-id="2f111-167">Чтобы начать работу с учетной записью Microsoft 365, перейдите к кнопке **Пуск** Windows, щелкните правой кнопкой мыши текущий рисунок учетной записи, а затем выберите пункт Переключить учетную запись. \*\*\*\*</span><span class="sxs-lookup"><span data-stu-id="2f111-167">To start using the Microsoft 365 account, go to the Windows **Start** button, right-click your current account picture and then select **Switch** account.</span></span>
6. <span data-ttu-id="2f111-168">Войдите, используя адрес электронной почты и пароль организации.</span><span class="sxs-lookup"><span data-stu-id="2f111-168">Sign in by using your organization email and password.</span></span>

<span data-ttu-id="2f111-169">Если у вас возникли проблемы при использовании Windows 10 в корпоративной среде, вы можете обратиться к наиболее распространенным решениям по [поддержке корпорации Майкрософт](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions).</span><span class="sxs-lookup"><span data-stu-id="2f111-169">If you experience issues when using Windows 10 in an enterprise environment, you can consult [top Microsoft Support solutions for the most common issues](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions).</span></span> <span data-ttu-id="2f111-170">Эти ресурсы включают статьи базы знаний, обновления и статьи библиотеки.</span><span class="sxs-lookup"><span data-stu-id="2f111-170">These resources include KB articles, updates, and library articles.</span></span>

<span data-ttu-id="2f111-171">Прежде чем перейти к следующему шагу, проверьте [условия](windows10-exit-criteria.md#crit-windows10-step3), при выполнении которых можно считать данный шаг завершенным.</span><span class="sxs-lookup"><span data-stu-id="2f111-171">As an interim checkpoint, you can see the [exit criteria](windows10-exit-criteria.md#crit-windows10-step3) corresponding to this step.</span></span>

## <a name="next-step"></a><span data-ttu-id="2f111-172">Следующее действие</span><span class="sxs-lookup"><span data-stu-id="2f111-172">Next step</span></span>

|||
|:-------|:-----|
|![](./media/stepnumbers/Step4.png)| [<span data-ttu-id="2f111-173">Отслеживание работоспособности и соответствия устройств требованиям</span><span class="sxs-lookup"><span data-stu-id="2f111-173">Monitor device health and compliance</span></span>](windows10-enable-windows-analytics.md) |
