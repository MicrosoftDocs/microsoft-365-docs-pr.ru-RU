...
---
# <a name="step-3-deploy-windows-10-enterprise-for-new-devices-with-windows-autopilot"></a>Шаг 3: Развертывание Windows 10 Enterprise для новых устройств с автопилот Windows

*В этой статье относятся к E3 и E5 версиям Microsoft 365 для предприятия*

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

Если у вас есть новый 10 ПК под управлением Windows, можно использовать автопилот Windows для настройки out--поле взаимодействия (OOBE) для вашей организации и развертывания новой системы с помощью приложения и уже настроенные параметры. Существует не изображения для развертывания, не драйверы для вставки и не инфраструктуры для управления. Пользователи могут перейти в процессе развертывания независимо, без необходимости обратитесь к ИТ-администратор.

Можно настроить и предварительно настроить новые устройства Windows 10 и получают их Готово к использованию производительность пользователей, с помощью Windows автопилот. Чтобы узнать больше о автопилот Windows, включая преимущества и сценарии Windows автопилот видеть [Обзор автопилот Windows](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot). Когда все будет готово, выполните следующие элементы, чтобы начать установку новых устройств.

## <a name="part-1-start-windows-autopilot-deployment"></a>Часть 1: Развертывание автопилот Windows Пуск
[Общие сведения о автопилот Windows](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) , чтобы увидеть:

1. Узнайте о и выполнить необходимые компоненты для развертывания Windows автопилот. Следующие необходимые компоненты:
    - **регистрацию устройств и настройку запуска при первом включении компьютера;**

        Чтобы зарегистрировать устройств, необходимо получить их код оборудования и зарегистрировать его. Мы активно работающих с различными поставщиками оборудования, чтобы они могли укажите необходимую информацию для вас или отправить от вашего имени. Вы также можете для захвата эту информацию с помощью вручную с помощью скрипта PowerShell, которое создает CSV-файл с идентификатором устройства оборудования.

        После регистрации устройства существует параметров настройки OOBE, можно настроить, включая пропускать параметры конфиденциальности и лицензионного соглашения.

    - **фирменную символику для запуска при первом включении компьютера;**

        Это позволяет Добавление фирменной символики в при выполнении устройства OOBE.

    - **автоматическую регистрацию MDM в Microsoft Intune;**
        
        Автоматической подачи заявок позволяет пользователям запрашивать свои устройства Windows 10 Intune для управления устройствами при подключении устройства к Azure AD. Для регистрации, пользователи добавьте учетную запись их работы в свои устройства владельцем раскрывающие личность или подключить корпоративных устройств Azure AD. В фоновом режиме также регистрации устройство для управления с Intune.

    - **сетевое подключение к облачным службам, которые использует Windows Autopilot;**

        Программа развертывания автопилот Windows использует ряд облачных служб для получения устройство в состояние производительность пользователей и эти службы должны быть доступны из устройств, зарегистрированных в качестве устройства Windows автопилот. 

    - **предварительную установку Windows 10 версии 1703 или выше на устройствах;**

2. Узнайте о и установите программу Windows автопилот развертывания для вашей организации. Можно выбрать один из этих программ развертывания:
    - **Microsoft Store для бизнеса**
    - **Microsoft Intune**
    - **Партнерский центр**

## <a name="part-2-set-up-a-windows-10-device-for-microsoft-365"></a>Часть 2: Настройка устройства Windows 10 365 Microsoft
Перед настройкой устройств Windows для пользователей Microsoft 365 убедитесь, что все устройства Windows запущены Windows 10, версия 1703 (создателей обновление) или более поздней версии.

После всех устройств Windows в вашей организации либо была обновлена до Windows Update создателей 10 или уже под управлением Windows Update создателей 10, можно объединить эти устройства с Azure Active Directory вашей организации.

### <a name="set-up-a-brand-new-or-newly-upgraded-windows-10-device"></a>Настройка устройства Windows 10 совершенно новых или недавно обновлена
Выполните следующие действия для настройки устройства с помощью Windows 10 OOBE на совершенно новый устройств под управлением Windows Update создателей 10 (или более поздней версии) или на устройстве, который был обновления Windows Update создателей 10 (или более поздней версии), но не было сделано из стандартной программы установки.

1. Если у вас нет настройки беспроводной сети, убедитесь, что подключение устройства к Интернету через проводной или Ethernet-подключение.
2. Пройти через процесс установки устройств Windows. На устройстве новые или интегральные установки начинается с **давайте Пуск с областью. В этом ли?** экран.
3. Перейдите через настройку устройства Windows 10 до **как настройка?** страницы. Здесь также выберите **для организации**.
4. Войдите в учетную запись и пароль пользователя Microsoft 365. В зависимости от параметра пароль пользователя, может потребоваться обновить пароль. 
5. Завершите настройку Windows 10 на устройстве.

После завершения устройство будет подключено к вашей организации Azure AD.

### <a name="set-up-a-device-that-has-already-completed-out-of-box-setup"></a>Настройка устройства, который уже выполнил стандартной программы установки
Если устройство имеет создателей 10 Windows Update (или более поздней версии) и имеет уже сделано стандартной установки, выполните следующие действия.

1. На Компьютере пользователя Windows, на котором выполняется Windows 10, версия 1703 (создателей обновления), выберите логотип **Windows** и затем выберите значок « **Параметры** ».
2. В окне **Параметры** выберите элемент **Учетные записи**.
3. На странице **Укажите сведения** выберите **доступа к работе и в школе** > **подключение**.
4. В диалоговом окне **Настройка рабочего или школы учетной записи** , в области **действий**выберите **присоединиться к устройству Azure Active Directory**.
5. На странице **давайте вы вход** введите работу или школе учетной записи и выберите **Далее**.
6. На странице **Введите пароль** введите пароль и выбрать **Вход**.
7. На странице **Убедитесь в том, что это вашей организации** убедитесь, что сведения правильные и выберите **присоединиться к**.
8. На **вы все set!** выберите **выполнить**.

После этого пользователя будет подключен к вашей организации Azure AD.

### <a name="verify-the-device-is-connected-to-azure-ad"></a>Проверка подключения устройства к Azure AD
Выполните следующие действия, чтобы проверить состояние синхронизации устройства с Azure AD и нажмите начать использовать учетную запись Майкрософт 365 на устройстве. 

1. Откройте **Параметры**.
2. На странице **доступа к работе и в школе** выберите **, подключенных к <organization name> ** область для предоставления кнопок **Info** "и" **Отключить**.
3. Выберите **сведения** для получения состояния синхронизации.
4. На странице **состояния синхронизации** выберите **синхронизации** для получения последних мобильного устройства политик управления на персональном Компьютере.
5. Начать работу с использованием учетной записи Майкрософт 365, перейдите на кнопку Windows **Пуск** , щелкните правой кнопкой мыши изображение текущей учетной записи, а затем выберите **переключатель** запись.
...
|||
|:-------|:-----|
|![](./media/stepnumbers/Step4.png)| [Наблюдение за работоспособностью системы устройств и обеспечения соответствия требованиям](windows10-enable-windows-analytics.md) |
