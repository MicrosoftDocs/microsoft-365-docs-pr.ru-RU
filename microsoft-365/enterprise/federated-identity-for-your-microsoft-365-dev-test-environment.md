...
В этой статье описывается настройка федерационной проверки подлинности для тестовой среды Microsoft 365, в результате чего:
...
> [!NOTE]
> Эту тестовую среду нельзя настроить с помощью пробной подписки Azure.
...
Следуйте инструкциям по синхронизации паролей для [Microsoft 365.](password-hash-sync-m365-ent-test-environment.md) Итоговая конфигурация выглядит так:
...
- Упрощенная интрасеть организации, подключенная к Интернету, состоящая из виртуальных машин DC1, APP1 и CLIENT1 в подсети виртуальной сети Azure. Azure AD Connect работает на APP1 для периодической синхронизации домена TESTLAB доменных служб Active Directory (AD DS) с клиентом Azure AD подписки Microsoft 365.
...
Затем добавьте правило в группу безопасности сети для подсети CorpNet, чтобы разрешить нежелательный входящий трафик из Интернета на частный IP-адрес PROXY1 и TCP-порт 443. Выполните эти команды в командной строке Azure PowerShell на локальном компьютере.
...
```
Эти команды создают внутреннюю запись A DNS, чтобы виртуальные машины в виртуальной сети Azure могли разрешать внутреннее FQDN службы федерации в частный IP-адрес ADFS1.
...
Затем создайте учетную запись службы AD FS с помощью этой команды в командной Windows PowerShell DC1:
  
```powershell
New-ADUser -SamAccountName ADFS-Service -AccountPassword (read-host "Set user password" -assecurestring) -name "ADFS-Service" -enabled $true -PasswordNeverExpires $true -ChangePasswordAtLogon $false
```
Обратите внимание, что при выполнении этой команды вам будет предложено ввести пароль учетной записи. Выберите надежный пароль и запишите его в безопасном месте. Оно потребуется для этого этапа и для этапа 5.
...
1. Выберите **"Начните",** **mmc.exe** и нажмите кнопку **ВВОД.**
    
2. Select **File**  >  **Add/Remove Snap-in**.
    
3. В **области "Добавление и** удаление оснастки" дважды щелкните "Сертификаты" в списке доступных оснастк, выберите "Учетная запись компьютера" и нажмите кнопку **"Далее".**  
    
4. В **области "Выбор компьютера"** выберите **"Готово"** и "ОК". 
    
5. В области дерева выберите **Сертификаты (локальный компьютер) > Личное > Сертификаты**.
    
6. Выберите и удерживайте (или щелкните правой кнопкой мыши) сертификат с FQDN службы федерации, выберите "Все задачи" и выберите **"Экспорт"**.
    
7. На странице **приветствия** выберите **"Далее".**
    
8. На странице **"Экспорт закрытого ключа" выберите** **"Да"** и **"Далее".**
    
9. На странице **"Формат экспортируемой** файла" выберите **"Экспортировать все** расширенные свойства", а затем выберите **"Далее".**
    
10. На странице **"Безопасность"** выберите **"Пароль"** и введите пароль в **"Пароль"** и **"Подтверждение пароля".**
    
11. На странице **"Файл для экспорта"** выберите **"Обзор".**
    
12. Перейдите в **папку C: \\ Certs,** введите **SSL** в имени файла **и** выберите **"Сохранить".**
    
13. На странице **"Файл для экспорта"** выберите **"Далее".**
    
14. На странице **завершения работы мастера экспорта сертификатов** выберите **"Готово".** При запросе выберите **"ОК"**.
...
1. Выберите **"Начните"** и выберите **значок диспетчера** серверов.
    
2. В области дерева диспетчера серверов выберите **AD FS.**
    
3. На панели инструментов в верхней части выберите оранжевый символ предупреждения, а затем выберите "Настройка службы федерации **на этом сервере".**
    
4. На странице **приветствия** мастера настройки служб федерации Active Directory выберите **"Далее".**
    
5. На странице **"Подключение к AD DS"** выберите **"Далее".**
    
6. На странице **Настройка свойств службы**:
    
  - Для **SSL-сертификата** выберите стрелку вниз, а затем выберите сертификат с именем FQDN службы федерации.
    
  - В **отображаемом имени** службы федерации введите имя вымышленной организации.
    
  - Нажмите кнопку **Далее**.
    
7. На странице **"Указание учетной записи** службы" выберите **"Выбрать** имя **учетной записи".**
    
8. В **списке "Выбор пользователя или учетной записи службы"** введите **ADFS-Service,** выберите "Проверить **имена"** и выберите "ОК". 
    
9. Введите **пароль для** учетной записи ADFS-Service и выберите **"Далее".**
    
10. На странице **"Указание базы данных конфигурации"** выберите **"Далее".**
    
11. На странице **"Параметры проверки"** выберите **"Далее".**
    
12. На странице **"Предварительные проверки"** выберите **"Настроить".**

13. На странице **"Результаты"** выберите **"Закрыть".**
    
14. Выберите **"Пуск",** выберите значок питания, перезапустите и выберите  **"Продолжить".**
...
5. В области дерева откройте личные сертификаты сертификатов **(локальный**  >    >  **компьютер).**
    
6. Выберите и удерживайте (или щелкните правой кнопкой мыши) **личные,** выберите **все задачи,** а затем выберите **"Импорт"**.
    
7. На странице **приветствия** выберите **"Далее".**
    
8. На странице **"Импортируемый** файл" введите **\\ \\ adfs1 \\ certs \\ ssl.pfx** и выберите **"Далее".**
    
9. На странице **защиты закрытых ключей** введите пароль сертификата в **пароле** и выберите **"Далее".**
    
10. На странице **"Хранилище сертификатов"** выберите **"Далее".**
    
11. На странице **завершения выберите** **"Готово"**.
...
15. Выберите и удерживайте (или щелкните правой кнопкой мыши) сертификат, а затем выберите **"Копировать".**
    
16. В области дерева откройте сертификаты доверенных корневых  >  **сертификационных органов.**
    
17. Переместите указатель мыши под списком установленных сертификатов, выберите и удерживайте (или щелкните правой кнопкой мыши), а затем **выберите "Ветвь".**
...
1. Выберите **"Начните"** и выберите **"Диспетчер серверов".**
    
2. В области дерева выберите **удаленный доступ.**
    
3. На панели инструментов в верхней части выберите оранжевый символ предупреждения, а затем выберите "Открыть мастер прокси-сервера **веб-приложения".**
    
4. На странице **приветствия** мастера настройки прокси-сервера веб-приложения выберите **"Далее".**
    
5. На странице **Сервер федерации**:
    
  - В поле **"Имя службы федерации"** введите имя FQDN службы федерации.
...
6. На странице "Сертификат прокси-сервера **AD FS"** выберите стрелку вниз, выберите сертификат с FQDN службы федерации, а затем выберите **"Далее".**
    
7. На странице **подтверждения** выберите **"Настроить".**
...
2. На странице **"Добро пожаловать в Azure AD Connect"** выберите **"Настроить".**
    
3. На странице **"Дополнительные задачи"** выберите "Изменить **вход пользователя"** и выберите **"Далее".**
    
4. На странице **"Подключение к Azure AD"** введите имя и пароль учетной записи глобального администратора, а затем выберите **"Далее".**
    
5. На странице **"Вход пользователя" выберите** "Федерация с **AD FS"** и выберите **"Далее".**
    
6. На странице **фермы AD FS** выберите "Использовать существующую ферму **AD FS",** введите **ADFS1** в поле "Имя сервера" и выберите **"Далее".** 
    
7. При запросе учетных данных сервера введите учетные данные учетной записи CORP \\ User1 и выберите "ОК". 
    
8. На странице **учетных** данных администратора домена введите **CORP \\ User1** в  поле **"Имя** пользователя", введите пароль учетной записи в поле "Пароль" и выберите **"Далее".**
...
10. На странице **"Домен Azure AD"** в домене выберите имя домена, который вы ранее создали и добавили в подписку на этапе 1, а затем выберите **"Далее".**
    
11. На странице **"Готово к настройке"** выберите **"Настроить".**
    
12. На странице **завершения установки выберите** **"Проверить".**
    
    Должны отобраться сообщения о том, что конфигурация интрасети и Интернета проверена.
    
13. На странице **завершения установки выберите** **"Выход".**
...
2. Для учетных данных для входа **введите** \<*the domain created in Phase 1*> user1@.
    
    Например, если тестовый домен **testlab.contoso.com,** введите "user1@testlab.contoso.com". Нажмите **клавишу TAB** или разрешайте Microsoft 365 автоматически перенаправлять вас.
    
    Теперь вы должны увидеть, что **ваше подключение не является частной страницей.** Это происходит потому, что вы установили на ADFS1 самозаверя сертификат, который настольный компьютер не может проверить. В производственном развертывании федераированной проверки подлинности используется сертификат от доверенного сертификационного органа, и пользователи не увидят эту страницу.
