...
После идентификации пользователей, чьи почтовые ящики локально необходимо перенести в Microsoft 365, для создания пакета миграции используется разделенный запятой файл (CSV). Каждая строка в CSV-файле, используемой Microsoft 365 для запуска миграции, содержит сведения о локальном почтовом ящике. 
