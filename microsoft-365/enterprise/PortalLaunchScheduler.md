...
> - Если ваша организация использует [Viva Connections,](/SharePoint/viva-connections)пользователи могут видеть значок вашей организации в панели приложений Microsoft Teams, однако при выборе значка пользователи не смогут получить доступ к порталу до запуска волны.
...
- Учетные данные администратора необходимы для [SharePoint PowerShell](/powershell/sharepoint/sharepoint-online/introduction-sharepoint-online-management-shell)
...
7. Определите, кому нужно сразу просмотреть сайт и ввести их сведения в **поле Пользователи, освобожденные от полей волн.** Эти пользователи исключены из волн и не будут перенаправлены до, во время или после запуска.
    ...
    > [!NOTE]
    > Для всего запуска можно использовать до 50 различных пользователей или групп безопасности. Каждый запуск не зависит друг от друга, поэтому если вы запланировать запуск на другом портале, для этого запуска можно использовать до 50 групп пользователей и безопасности. Кроме того, на одной волне можно использовать до 20 различных пользователей или групп безопасности. 
    >
    > План-график запуска портала поддерживает группы безопасности и группы безопасности с включенной почтой. 
...
> [!NOTE]
>
> - Этот процесс следует выполнять только в том случае, если вы отвечаете следующим требованиям:
> - Страница запуска завершена.
> - [Руководство по охране здоровья портала](https://aka.ms/portalhealth) было последовано за.
> - Дата запуска не более 14 дней.
