...
---
# <a name="launch-your-portal-using-the-sharepoint-portal-launch-scheduler"></a>Запуск портала с помощью планера запуска портала SharePoint

Портал — это сайт коммуникации SharePoint в интрасети с высоким трафиком — сайт с количеством от 10 000 до 100 000 зрителей в течение нескольких недель. Используйте план-график запуска портала для запуска портала, чтобы обеспечить пользователям возможность плавного просмотра при доступе к новому порталу SharePoint.
<br>
<br>
План-график запуска Портала предназначен для того, чтобы помочь вам следовать поэтапному подходу к выкатыву, пакетив просмотры в волнах и управляя URL-адресами для нового портала. Во время запуска каждой волны можно собирать отзывы пользователей, отслеживать производительность портала и приостанавлить запуск, чтобы устранить проблемы перед началом следующей волны. Узнайте больше о планировании [запуска портала в SharePoint.](https://docs.microsoft.com/microsoft-365/Enterprise/Planportallaunchroll-out?view=o365-worldwide) 

**Существует два типа перенаправления:**

- **Bidirectional:** запустите новый современный портал SharePoint, чтобы заменить существующий классический или современный портал SharePoint
- **Перенаправление** на временную страницу: запустите новый современный портал SharePoint без существующего портала SharePoint
...
> [!NOTE]
> - Эта функция будет доступна с панели **Параметры** на домашней странице сайтов связи SharePoint для целевых клиентов выпуска начиная с мая 2021 г. и станет доступна для всех клиентов к июлю 2021 г.
> - Версия PowerShell этого средства доступна сегодня
> - Эту функцию можно использовать только на современных сайтах связи SharePoint
...
> - Перед планированием запуска портала необходимо запустить средство [Page Diagnostics for SharePoint,](https://aka.ms/perftool) чтобы убедиться, что домашняя страница сайта здорова
...
> - Эта функция недоступна для Office 365 в Германии, Office 365, выполняемой 21Vianet (Китай) или microsoft 365 планы правительства США
...
**Версия SharePoint PowerShell:**

- Учетные данные администратора необходимы для использования [SharePoint PowerShell](https://docs.microsoft.com/powershell/sharepoint/sharepoint-online/introduction-sharepoint-online-management-shell?view=sharepoint-ps) 
...
2.  Затем приступить к планированию запуска портала с помощью доступа к планировщику запуска портала одним из двух способов:
    ...
    **Вариант 2.** В любое время можно перейти на домашную страницу  сайта связи SharePoint, выбрать параметры и затем запланировать запуск сайта, чтобы запланировать запуск портала. 
    
    ![Изображение области Параметры с расписанием запуска сайта выделено](../media/portal-launch-settings-2.png)

3.  Далее подтвердите оценку состояния здоровья портала и в случае необходимости с помощью средства [Page Diagnostics for SharePoint](https://aka.ms/perftool) убедитесь, что ваш портал не получит **полезный** результат. Затем выберите **Далее**.
...
5.  Затем определите **необходимый тип перенаправления:**
    ...
    Вариант 1. Отправка пользователей на существующую страницу **SharePoint (bidirectional)** — используйте этот параметр при запуске нового современного портала SharePoint для замены существующего портала SharePoint. Пользователи в активных волнах будут перенаправлены на новый сайт независимо от того, переходят ли они на старый или новый сайт. Пользователи не запущенной волны, которые пытаются получить доступ к новому сайту, будут перенаправлены обратно на старый сайт до запуска волны.
    
    > [!NOTE] 
    > При использовании параметра bidirectional у лица, запланив запуск, также должны быть разрешения владельца сайта на другой портал SharePoint.
       
    **Вариант 2.** Отправка пользователей на автогенерированную временную страницу (временное перенаправление страницы) — Использование временной перенаправления страниц следует использовать, если не существует существующего портала SharePoint. Пользователи направляются на новый современный портал SharePoint, и если пользователь находится в волне, которая еще не запущена, они будут перенаправлены на временную страницу.
...
6.  Разгон аудитории на волны. Добавьте до 20 групп безопасности на одну волну. Сведения о волне можно изменить до запуска каждой волны. Каждая волна может длиться как минимум один день (24 часа) и не более семи дней. Это позволяет SharePoint и вашей технической среде присоединяться к большому объему пользователей сайтов и масштабировать их. При планировании запуска через пользовательский интерфейс часовой пояс основан на региональных параметрах сайта. 
    ...
    >  - Группы Microsoft 365 не поддерживаются этой версией план-графика запуска Портала.
...
8.  Подтверждение сведений о запуске портала и выберите **Расписание.** После запланированного запуска все изменения на домашней странице портала SharePoint должны получить здоровый диагностический результат до возобновления запуска портала.
...
1.  Чтобы изменить сведения о запуске портала, перейдите в **параметры** и выберите **Расписание запуска сайта.**
...
1.  Чтобы отменить запуск портала, перейдите на **параметры** и **расписание запуска сайта.**
...
Изначально средство расписания запуска портала SharePoint было доступно только через [SharePoint PowerShell](https://docs.microsoft.com/powershell/sharepoint/sharepoint-online/introduction-sharepoint-online-management-shell?view=sharepoint-ps) и будет по-прежнему поддерживаться через PowerShell для клиентов, которые предпочитают этот метод. Эти же заметки в начале этой статьи применяются к обеим версиям планера запуска Портала. 

>[!NOTE]
> Для использования SharePoint PowerShell необходимы разрешения администратора.
> Сведения о запуске портала для запусков, созданных в PowerShell, будут отображаться и управляться ими можно в новом средстве расписания запуска портала в SharePoint.
