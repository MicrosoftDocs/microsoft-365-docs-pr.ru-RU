...
> [!NOTE]
> Microsoft Defender для конечной точки не поддерживается в Azure Express Route.

> [!NOTE]
> Мы не рекомендуем ExpressRoute для Microsoft 365, так как в большинстве обстоятельств она не обеспечивает лучшую модель подключения для службы. Поэтому для использования этой модели подключения для Microsoft 365 требуется авторизация Майкрософт. Мы проверяем каждый запрос клиента и уполномочаем ExpressRoute для Microsoft 365 только в тех редких случаях, когда это необходимо. Ознакомьтесь с руководством [ExpressRoute для Microsoft 365,](https://aka.ms/erguide) чтобы получить дополнительные сведения, и после всестороннего анализа документа с группами производительности, сети и безопасности поработать с командой учетных записей Майкрософт, чтобы при необходимости отправить исключение. Несанкционированные подписки, которые пытаются создать фильтры маршрутов для Office 365, получат сообщение [об ошибке.](https://support.microsoft.com/kb/3181709)
