...
>[!NOTE]
>Корпорация Майкрософт не рекомендует ExpressRoute для Microsoft 365 и не предоставляет лучшую модель подключения для службы практически при любых обстоятельствах. Поэтому для использования этой модели подключения для службы требуется авторизация Майкрософт. Корпорация Майкрософт проверяет каждый запрос клиента и разрешает ExpressRoute для Microsoft 365 только в тех редких случаях, когда это необходимо. Ознакомьтесь с [руководством ExpressRoute для Microsoft 365,](https://aka.ms/erguide) чтобы получить дополнительные сведения и поработать с командой учетных записей Майкрософт, чтобы отправить исключение, если потребуется, после всестороннего обзора руководства.
Несанкционированные подписки, которые пытаются создать фильтры маршрутов для Microsoft 365, получат сообщение [об ошибке.](https://support.microsoft.com/kb/3181709)
