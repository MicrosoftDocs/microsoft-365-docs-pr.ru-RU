...
Виртуальная сеть (VNet) в службах инфраструктуры Azure, подключенных к другим сетям, должна иметь подсеть шлюза. Наиболее оптимальные методы определения подсети шлюза:

- Длина префикса подсети шлюза может иметь максимальную длину префикса 29 (например, 10.119.255.248/29), но текущая рекомендация в том, что используется длина префикса 27 (например, 10.119.255.224/27).
- При определении адресного пространства подсети шлюза используйте последнюю часть адресного пространства VNet.

Во второй рекомендации можно определить адресное пространство подсети шлюза, установив биты, используемые для подсети шлюза, до 0, а оставшиеся биты в адресной области VNet - до 1. Чтобы быстро рассчитать адресное пространство подсети шлюза без преобразования в двоичное и обратно в десятичное, можно использовать консольное приложение, записанное в C# или Python или с командным блоком PowerShell.

В этой статье C# кодовые блоки Python и PowerShell, которые вычисляют адресное пространство подсети шлюза на основе значений w.x.y.z/n для префикса адресов VNet и длины префикса подсети шлюза.

## <a name="c-code-block"></a>C# блок кода

Используйте этот блок кода для создания приложения консоли в C#.
...
Этот блок кода используется для создания консоли приложения в Python.
...
## <a name="powershell-command-block"></a>Командный блок PowerShell

Заполните значения и запустите командный блок в окне PowerShell или в среде интегрированных скриптов PowerShell (ISE).
