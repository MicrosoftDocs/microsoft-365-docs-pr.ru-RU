...
Вот ДЕМИЛИТАРИЗОВАНная группа Contoso, содержащая общедоступный веб-сайт, экстрасеть партнера и набор серверов служб федерации Active Directory (AD FS). DMZ подключается к Интернету, который содержит клиентов, партнеров и Интернет-служб.
...
Узнайте, как в компании Contoso для [развертывания и сохранения текущей операционной системы Windows 10 Корпоративная](contoso-win10.md) в организации используется инфраструктура Configuration Manager конечных точек Майкрософт.
