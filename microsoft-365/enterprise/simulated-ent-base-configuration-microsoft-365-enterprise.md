...
В этой статье описано, как создать упрощенную среду для Microsoft 365 для предприятий, включающую:
...
На этом этапе создайте имитируемую интрасеть в службах инфраструктуры Azure, которая включает контроллер домена доменных служб Active Directory (AD DS), сервер приложений и клиентский компьютер.

Вы будете использовать эти компьютеры в дополнительных [руководствах по лаборатории тестирования Microsoft 365 для предприятий](m365-enterprise-test-lab-guides.md) , чтобы настроить и продемонстрировать гибридные удостоверения и другие возможности.
...
В этом методе шаблон Azure Resource Manager используется для создания имитации интрасети. Шаблоны Azure Resource Manager содержат все инструкции по созданию инфраструктуры сети Azure, виртуальным машинам и их настройке.

Прежде чем развертывать шаблон, ознакомьтесь со [страницей readme шаблона](https://github.com/maxskunkworks/TLG/tree/master/tlg-base-config_3-vm.m365-ems) и получите следующие сведения.

- Общедоступное DNS-имя домена тестовой среды (testlab. \<*your public domain*> ). Это имя будет введено в поле **доменное имя** страницы **настраиваемого развертывания** .
...
Прочтите инструкции, чтобы приступить к работе, выберите **развернуть в Azure** на [странице сведений о шаблоне](https://github.com/maxskunkworks/TLG/tree/master/tlg-base-config_3-vm.m365-ems) .

>[!Note]
>Для имитации интрасети, созданной с помощью шаблона Azure Resource Manager, требуется платная подписка Azure.

После завершения шаблона ваша конфигурация будет выглядеть следующим образом:
...
На этом этапе создается виртуальная сеть Azure и добавляется виртуальная машина DC1, которая является контроллером домена для домена доменных служб Active Directory.
...
Укажите свою подписку Azure. Замените все в кавычках, в том числе угловые скобки ("<" и ">"), с правильным именем.
...
Создайте группу ресурсов с помощью следующих команд. Замените все в кавычках, в том числе угловые скобки, на правильные имена.
...
Затем создайте виртуальную сеть TestLab, в которой будет размещаться подсеть корпоративной сети имитируемой среды предприятия, и защитите ее с помощью группы безопасности сети. Введите имя группы ресурсов и выполните приведенные ниже команды в командной строке PowerShell на локальном компьютере.
...
1. На [портале Azure](https://portal.azure.com)выберите **группы ресурсов** > <***имя новой группы ресурсов***> > **DC1**  >  **Подключение**DC1.
    
2. В области Открыть выберите **скачать RDP-файл**. Откройте скачанный файл DC1. RDP и нажмите кнопку **подключить**.
    
3. Укажите имя учетной записи локального администратора DC1:
    
   - Для Windows 7:
    
     В диалоговом окне " **Безопасность Windows** " выберите **использовать другую учетную запись**. В поле **имя пользователя**введите ** \\ ** < *имя учетной записи локального администратора* DC1>.
    
   - Для Windows 8 и Windows 10:
    
     В диалоговом окне **Безопасность Windows** выберите **Дополнительные варианты**, а затем выберите **использовать другую учетную запись**. В поле **имя пользователя**введите ** \\ ** < *имя учетной записи локального администратора* DC1>.
    
4. В поле **пароль**введите пароль учетной записи локального администратора, а затем нажмите кнопку **ОК**.
...
Затем настройте DC1 как контроллер домена и DNS-сервер для домена **testlab.**\<*your public domain*> . Укажите имя общедоступного домена, удалите угловые скобки, а затем выполните приведенные ниже команды в командной строке Windows PowerShell с правами администратора на компьютере DC1.
...
1. На [портале Azure](https://portal.azure.com)выберите **группы ресурсов** > <*имя группы ресурсов*> > **DC1**  >  **Подключение**DC1.
    
2. Запустите скачанный файл DC1. RDP, а затем выберите **подключить**.
    
3. В окне **Безопасность Windows**выберите **использовать другую учетную запись**. В поле **имя пользователя**введите ** \\ ** < *имя учетной записи локального администратора* TESTLAB>.
    
4. В поле **пароль** введите пароль учетной записи локального администратора, а затем нажмите кнопку **ОК**.
    
5. При появлении соответствующего запроса нажмите **кнопку Да**.
    
Затем создайте учетную запись пользователя в Active Directory, которая будет использоваться при входе в систему на компьютерах, TESTLAB членом домена. От имени администратора выполните указанную ниже команду в командной строке Windows PowerShell.
  
```powershell
New-ADUser -SamAccountName User1 -AccountPassword (read-host "Set user password" -assecurestring) -name "User1" -enabled $true -PasswordNeverExpires $true -ChangePasswordAtLogon $false
```

Обратите внимание на то, что при выполнении этой команды вам будет предложено ввести пароль учетной записи User1. Эта учетная запись будет использоваться для подключений к удаленному рабочему столу для всех рядовых компьютеров TESTLAB домена, поэтому выберите надежный пароль. Запишите пароль к учетной записи User1 и храните его в безопасном месте.
...
Обратите внимание, что после выполнения команды **Add – Computer** необходимо указать \\ учетные данные учетной записи домена TESTLAB User1.
...
Чтобы создать виртуальную машину Azure для CLIENT1, введите имя группы ресурсов и выполните приведенные ниже команды в командной строке на локальном компьютере.
...
2. В окне **свойства для компьютера CLIENT1** **нажмите кнопку Далее в окне** **Конфигурация усиленной безопасности Internet Explorer**.
    
3. В окне **Конфигурация усиленной безопасности Internet Explorer**выберите пункт **выкл** для **администраторов** и **пользователей**, а затем нажмите кнопку **ОК**.
    
4. На начальном экране выберите **Internet Explorer**и нажмите кнопку **ОК**.
    
5. В адресной строке введите **HTTP<span>://</span>App1. тестаб.** \<*your public domain name*> **/** , а затем нажмите клавишу **Ввод**. Вы увидите страницу служб IIS по умолчанию для APP1.
    
6. На панели задач рабочего стола выберите значок проводник.
    
7. В адресной строке введите ** \\ \\ APP1 \\ Files**и нажмите клавишу **Ввод**. Вы увидите окно папки с содержимым общей папки Files.
...
- Использовать отдельную производственную подписку на Microsoft 365 E5 с небольшим количеством лицензий.
  ...
  Это дополнительная стоимость, но она обеспечивает работу тестовой среды, срок действия которой не ограничен; в нем можно испытать функции, конфигурации и сценарии. Вы можете использовать ту же тестовую среду в долгосрочной перспективе для экспериментов, демонстрации по одноранговым компонентам и управлению, а также разработке и тестированию приложений. Это рекомендуемый способ.

### <a name="sign-up-for-an-office-365-e5-trial-subscription"></a>Оформление пробной подписки на Office 365 E5

На портале Azure подключитесь к компьютеру CLIENT1 с помощью учетной записи Corp\user1..
