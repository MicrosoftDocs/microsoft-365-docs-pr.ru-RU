...
Современная проверка подлинности — это метод управления удостоверениями, который обеспечивает более безопасную проверку подлинности и авторизацию пользователей, доступен для локального сервера Skype для бизнеса и локального сервера Exchange, а также гибридов Skype для бизнеса с раздельным доменом Skype для бизнеса.
  
 **Важно** Хотите узнать больше о современной проверке подлинности и о том, почему вы можете использовать ее в своей компании или организации? Ознакомьтесь [с этим документом.](hybrid-modern-auth-overview.md) Если вам нужно знать, какие топологии Skype для бизнеса поддерживаются с помощью ma, это задокументировано здесь!
...
- Локальное skype для бизнеса (SFB)
...
1. Поскольку **для** Skype для бизнеса и Exchange используется множество необходимых условий, см. статью обзор для контрольного списка [предварительного переопречня.](hybrid-modern-auth-overview.md) Сделайте  *это,*  прежде чем приступить к любым шагам в этой статье.

1. Сбор информации, определенной для HMA, необходимой в файле или OneNote.
...
1. Включи гибридную современную проверку подлинности для локальной exchange.

1. Включи гибридную современную проверку подлинности для локального Skype для бизнеса.
...
См. [раздел Поддержка для Skype для бизнеса с ma](/skypeforbusiness/plan-your-deployment/modern-authentication/topologies-supported) для поддерживаемых topologies.
...
- **Идентификатор клиента**
  ...
  - GUID, представляю для клиента Office 365 (при входе contoso.onmicrosoft.com).

- **URL-адреса веб-службы CU5 2015**

Для всех развернутых пулов SfB 2015 вам потребуется URL-адреса внутренних и внешних веб-служб. Чтобы получить эти данные, запустите следующие данные из skype for Business Management Shell:
...
Если вы используете сервер Standard Edition, внутренний URL-адрес будет пустым. В этом случае используйте fqdn пула для внутреннего URL-адреса.
...
## <a name="turn-on-hybrid-modern-authentication-for-exchange-on-premises"></a>Включи гибридную современную проверку подлинности для локальной exchange
...
## <a name="turn-on-hybrid-modern-authentication-for-skype-for-business-on-premises"></a>Включи гибридную современную проверку подлинности для локального Skype для бизнеса

### <a name="add-on-premises-web-service-urls-as-spns-in-azure-active-directory"></a>Добавление URL-адресов веб-службы на локальной основе в качестве SPNs в Azure Active Directory
...
1. Сначала подключите к Azure Active Directory (Azure AD) с [этими инструкциями.](/powershell/azure/active-directory/overview?view=azureadps-1.0)
...
2. Запустите эту команду локально, чтобы получить список URL-адресов веб-службы SFB.
   ...
   Обратите внимание (и снимок экрана для более позднего сравнения) вывода этой команды, которая будет включать URL-адрес SE и WS, но в основном состоит из spNs, которые начинаются `00000004-0000-0ff1-ce00-000000000000/` с .
...
Это шаг, который фактически включает ma. Все предыдущие действия можно выполнить заранее, не изменяя поток проверки подлинности клиента. Когда вы будете готовы изменить поток проверки подлинности, запустите эту команду в командной командной команде Skype для бизнеса.
...
Чтобы проверить, работает ли HMA после включения, выйдите из тестового клиента SFB Windows и нажмите кнопку "Удалите мои учетные данные". Войми снова. Теперь клиент должен использовать поток modern Auth, и теперь в вашем входе будет включена подсказка **Office 365** для учетной записи "Работа или школа", которая будет видна перед тем, как клиент свянется с сервером и войдите в него.
  
Вы также должны проверить "Сведения о конфигурации" для клиентов Skype для бизнеса для "OAuth Authority". Чтобы сделать это на клиентский компьютер, удерживайте клавишу CTRL в то же время, щелкнув правой кнопкой мыши Значок Skype для бизнеса в подносе Уведомления Windows. Щелкните **Сведения о конфигурации** в меню, которое отображается. В окне "Информация о конфигурации бизнеса Skype для бизнеса", которое появится на рабочем столе, посмотрите следующее:
...
Нужно ли знать, как использовать современную проверку подлинности (ADAL) для клиентов Skype для бизнеса? У нас есть шаги [здесь](./hybrid-modern-auth-overview.md).