...
В зависимости от конфигурации удостоверения можно управлять паролями учетных записей пользователей Microsoft 365 несколькими различными способами. Вы можете управлять учетными записями пользователей в [центре администрирования Microsoft 365](https://docs.microsoft.com/microsoft-365/admin/add-users/), в доменных службах Active Directory (AD DS) или в центре администрирования Azure Active Directory (Azure AD).

## <a name="plan-for-where-and-how-you-will-manage-your-user-account-passwords"></a>Планирование того, где и как будут управляться пароли учетных записей пользователей

Здесь и как можно управлять учетными записями пользователей зависит от модели удостоверений, которую вы хотите использовать для вашей Microsoft 365. Две модели являются только облачными и гибридными.
  
### <a name="cloud-only"></a>Только облако

Управление паролями учетных записей пользователей в:
...
При использовании гибридного удостоверения пароли хранятся в доменных СЛУЖБах Active Directory, поэтому для управления паролями учетных записей пользователей необходимо использовать локальные средства AD DS. Даже при использовании синхронизации со хешированием паролей (ФС), в которой Azure AD сохраняет хешированную версию уже хешированной версии в AD DS, пользователи должны управлять своими паролями в доменных СЛУЖБах Active Directory.

С помощью [обратной записи паролей](#pw_writeback)пользователи могут изменять свои пароли AD DS с помощью Azure AD.
...
Вы можете запретить неправильные пароли [в облаке](https://docs.microsoft.com/azure/active-directory/authentication/concept-password-ban-bad) и для [локальных доменных служб Active Directory](https://docs.microsoft.com/azure/active-directory/authentication/concept-password-ban-bad-on-premises).

## <a name="simplify-user-sign-in"></a>Упрощение входа пользователей

Единый вход в Azure AD (единый вход Azure AD) работает с ФС и сквозной проверкой подлинности (ПТА), чтобы позволить пользователям входить в службы, использующие учетные записи пользователей Azure AD, не вводя их пароли, и во многих случаях их имена. Это упрощает доступ пользователей к облачным приложениям, например Office 365, без необходимости в дополнительных локальных компонентах, таких как серверы федерации удостоверений.
...
## <a name="simplify-password-updates-to-ad-ds"></a>Упрощение обновления паролей для доменных служб Active Directory

С помощью обратной записи паролей можно разрешить пользователям сбрасывать свои пароли через Azure AD, которая затем реплицируется в AD DS. Пользователям не требуется доступ к локальным доменным СЛУЖБам Active Directory для обновления своих паролей. Это важно для роуминга и удаленных пользователей, у которых нет подключения к локальной сети для удаленного доступа.
...
Самостоятельный сброс паролей (SSPR) позволяет пользователям сбрасывать или разблокировать пароли и учетные записи. Чтобы получать оповещения о неправильном или несанкционированном использовании, можно использовать подробные отчеты, которые позволяют отслеживать доступ пользователей к системе, а также уведомления. Для развертывания сброса паролей необходимо включить [обратную запись пароля](#pw_writeback) .
