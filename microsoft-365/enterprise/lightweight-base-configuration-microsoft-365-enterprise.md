...
В этой статье описывается создание упрощенной среды с подпиской Microsoft 365 E5 и компьютером под управлением Windows 10 Корпоративная.
...
Создание облегченной тестовой среды состоит из пяти этапов:
...
Используйте итоговую среду для тестирования функций и функций [Microsoft 365 для предприятий.](https://www.microsoft.com/microsoft-365/enterprise)

![Руководства по лаборатории тестирования для облака Майкрософт](../media/m365-enterprise-test-lab-guides/cloud-tlg-icon.png)
  
> [!TIP]
> Visual map to all the articles in the Microsoft 365 for enterprise Test Lab Guide stack, see [Microsoft 365 for enterprise Test Lab Guide Stack](../downloads/Microsoft365EnterpriseTLGStack.pdf).
...
Начнем с пробной подписки Microsoft 365 E5, а затем добавим в нее подписку Microsoft 365 E5.
...
2. На шаге 1 страницы "Благодарим вас за выбор **Office 365 E5"** введите новый адрес учетной записи электронной почты.
3. На шаге 2 процесса подписки введите запрашиваемую информацию и выполните проверку.
4. В шаге 3 введите название организации, а затем имя учетной записи, которая будет глобальным администратором подписки.
...
6. Запишите здесь идентификатор пользователя: ![Линия](../media/Common-Images/TableLine.png).onmicrosoft.com  
   Зафиксировать пароль, который вы ввели в надежном месте.
   Это значение будет называться **именем глобального администратора**.
7. Выберите **"Перейти к установке"**.
...
Чтобы подключиться к подписке с помощью модуля Azure Active Directory PowerShell для Graph с компьютера, воспользуйтесь инструкциями в этой теме, чтобы подключиться к [Microsoft 365 с помощью PowerShell.](connect-to-microsoft-365-powershell.md#connect-with-the-azure-active-directory-powershell-for-graph-module)
    
В **диалоговом Windows PowerShell запроса** учетных данных введите имя глобального администратора (например, *jdoe@contosotoycompany.onmicrosoft.com)* и пароль.
  
Заполните название организации (например, *contosotoycompany),* двух символьный код страны для расположения, общий пароль учетной записи, а затем запустите следующие команды в командной записи PowerShell:
...
Если эти значения еще не записаны, зафиксировать их теперь:
...
Дополнительные руководства по лаборатории тестирования, применимые к Office 365 и Microsoft 365, см. в руководстве по лаборатории тестирования [Microsoft 365 для предприятий.](m365-enterprise-test-lab-guides.md)
...
1. В частном окне браузера используйте учетные данные глобального администратора, чтобы войти в Центр администрирования Microsoft 365 по вашему [https://admin.microsoft.com](https://admin.microsoft.com) сайту.
    
2. На странице **Центра администрирования Microsoft 365** в области навигации слева выберите "Выставление **> Приобретение служб".**
    
3. На странице **"Приобретение служб"** выберите **Microsoft 365 E5** и выберите **"Получить бесплатную пробную версия".**

4. На странице "Пробная версия **Microsoft 365 E5"** решите получить текстовое сообщение или телефонный звонок, введите номер телефона, а затем выберите "Sms **me"** или **"Позвонить мне".** Выполните проверку.

5. На странице **"Подтверждение заказа"** выберите **"Попробовать сейчас".**

6. На странице **квитанции** "Заказ" выберите **"Продолжить".**

7. В Центре администрирования Microsoft 365 выберите пользователей > **активных пользователей.**

8. В **активных пользователях** выберите учетную запись администратора.
...
11. Выберите **"Сохранить изменения",** а затем закроем области сведений об учетной записи пользователя.
...
> [!NOTE]
> Срок действия пробной подписки microsoft 365 E5 составляет 30 дней. Чтобы создать постоянную среду тестирования, переведите пробную подписку в платную подписку с небольшим количеством лицензий.
...
На персональном компьютере установите Windows 10 Корпоративная. Пробную версия Windows 10 Корпоративная можно скачать [здесь.](https://www.microsoft.com/evalcenter/evaluate-windows-10-enterprise)
  
### <a name="virtual-machine"></a>Виртуальная машина

Создайте виртуальную машину с помощью гипервизора, а затем установите на нее Windows 10 Корпоративная. Пробную версия Windows 10 Корпоративная можно скачать [здесь.](https://www.microsoft.com/evalcenter/evaluate-windows-10-enterprise)
...
Создайте группу ресурсов с помощью следующих команд. Замените все символы в кавычках \< and > правильными именами.
...
> [!NOTE]
> Для виртуальной машины в Azure воспользуйтесь этими  [инструкциями,](https://docs.microsoft.com/azure/virtual-machines/windows/connect-logon) чтобы подключиться к ней.
  
Затем присоедините компьютер WIN10 к клиенту Azure AD, отвечающему за вашу подписку на Microsoft 365 E5.
  
1. На рабочем столе компьютера WIN10 выберите параметры start **> > Accounts > Access work or school > Connect.**
    
2. В **диалоговом окне "Настройка** учетной записи для работы или учебного заведения" выберите **"Присоединить это устройство к Azure Active Directory".**
...
4. Введите **пароль для** учетной записи глобального администратора и выберите **"Войти".**
    
5. Когда вам будет предложено убедиться, что это ваша организация, выберите **"Присоединиться"** и выберите **"Готово".**
    
6. Закройте окно параметров.
    
Затем установите приложения Microsoft 365 для предприятий на компьютере WIN10:
  
1. Откройте браузер Microsoft Edge и войте в Центр администрирования [Microsoft 365](https://admin.microsoft.com) учетные данные глобального администратора.
    
2. На **вкладке Microsoft Office "Главная"** выберите **"Установить Office".**
    
3. Когда вам будет предложено что-то сделать, выберите "Выполнить", а затем выберите **"Да"** для контроля **учетных записей пользователей.**
    
4. Подождите, пока Office завершит установку. When you see **You're all set!**, select **Close** twice.
...
- Установленные приложения Microsoft 365 для предприятий.
