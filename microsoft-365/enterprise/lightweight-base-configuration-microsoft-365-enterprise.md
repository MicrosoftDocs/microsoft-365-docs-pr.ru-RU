...
В этой статье описывается, как создать упрощенную среду с Microsoft 365 E5 подпиской и компьютером с Windows 10 Корпоративная.
...
- [Этап 1. Создание Microsoft 365 E5 подписки](#phase-1-create-your-microsoft-365-e5-subscription)
...
> [!TIP]
> Визуальную карту для всех статей в стеке руководства по Microsoft 365 тестовой лаборатории см. в Microsoft 365 руководства по корпоративной лаборатории [тестирования.](../downloads/Microsoft365EnterpriseTLGStack.pdf)
...
## <a name="phase-1-create-your-microsoft-365-e5-subscription"></a>Этап 1. Создание Microsoft 365 E5 подписки

Мы начинаем с пробной Microsoft 365 E5, а затем добавляем Microsoft 365 E5 подписку на нее.
...
2. В шаге 1 из "Спасибо за выбор Office 365 **E5"** введите новый адрес учетной записи электронной почты.
...
8. В Office 365 E5 Настройка выберите Продолжить использование **организации .onmicrosoft.com** для электронной почты и регистрации, а затем выберите **Exit и продолжить позже**.
...
Чтобы подключиться к подписке с Azure Active Directory PowerShell для Graph с компьютера, используйте инструкции в Подключение для [Microsoft 365 с PowerShell](connect-to-microsoft-365-powershell.md#connect-with-the-azure-active-directory-powershell-for-graph-module).
...
Дополнительные руководства по лаборатории тестирования, применимые как к Office 365, так и к Microsoft 365 см. Microsoft 365 руководства по корпоративной лаборатории [тестирования.](m365-enterprise-test-lab-guides.md)
...
2. На странице **Microsoft 365 центра администрирования** в левой навигации выберите службы > **покупки.**
    
3. На странице **Службы покупки** выберите **Microsoft 365 E5,** а затем выберите **бесплатную пробную пробную версия.**

4. На странице **Microsoft 365 E5,** решите получить текстовое сообщение или телефонный звонок, введите номер телефона, а затем выберите **Текст** мне или **Позвоните мне**. Выполните проверку.
...
> [!NOTE]
> Продолжительность пробной Microsoft 365 E5 составляет 30 дней. Чтобы создать постоянную среду тестирования, переведите пробную подписку в платную подписку с небольшим количеством лицензий.
...
В результате конфигурация, которая добавляет Microsoft 365 E5, выглядит так:
...
На персональном компьютере установите Windows 10 Корпоративная. Вы можете скачать Windows 10 Корпоративная [здесь](https://www.microsoft.com/evalcenter/evaluate-windows-10-enterprise).
  
### <a name="virtual-machine"></a>Виртуальная машина

Используйте гипервизор по вашему выбору для создания виртуальной машины, а затем Windows 10 Корпоративная на ней. Вы можете скачать Windows 10 Корпоративная [здесь](https://www.microsoft.com/evalcenter/evaluate-windows-10-enterprise).
...
> [!NOTE]
> Для указанных ниже последовательностей команд используется последняя версия Azure PowerShell. См. [начало работы с Azure PowerShell.](/powershell/azureps-cmdlets-docs/) Эти наборы команд создают виртуальную машину под управлением Windows 10 Корпоративная с именем WIN10, а также всю необходимую инфраструктуру, включая группу ресурсов, учетную запись хранения и виртуальную сеть. Если вы уже знакомы со службами инфраструктуры Azure, адаптируем эти инструкции в соответствии с развернутой инфраструктурой.
...
1. На рабочем столе компьютера WIN10 выберите Пуск > Параметры > учетные записи > доступ или **школьные > Подключение**.
    
2. В **диалоговом окне** Настройка работы или учебной учетной записи выберите Присоединитесь к этому **устройству, чтобы Azure Active Directory**.
    
3. В **work или учебной учетной** записи введите имя глобальной учетной записи администратора подписки Microsoft 365 E5, а затем выберите **Далее**.
...
Далее установите Приложения Microsoft 365 для предприятий на компьютер WIN10:
  
1. Откройте браузер Microsoft Edge и вопишитесь [](https://admin.microsoft.com) в центр администрирования Microsoft 365 с учетными данными глобальной учетной записи администратора.
...
- Приложения Microsoft 365 для предприятий установлен.
