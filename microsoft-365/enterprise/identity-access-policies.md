...
|:---------------|:-------|:----------------|
|**Трудозатрат**|[Требовать, чтобы риск входа в систему был *средним* или *высоким*](#require-mfa-based-on-sign-in-risk)| |
|        |[Блокировка клиентов, не поддерживающих современные проверки подлинности](#block-clients-that-dont-support-modern-authentication)|Клиенты, для которых не используется современная проверка подлинности, могут обходить правила условного доступа, поэтому важно заблокировать эти|
...
|        |[Требовать соответствующие компьютеры](#require-compliant-pcs-but-not-compliant-phones-and-tablets)|Обеспечивает управление компьютерами в Intune|
...
|**Строго регулируемая**|[*Всегда* требовать MFA](#require-mfa-based-on-sign-in-risk)|
| | |
...
|:---|:---------|:-----|:----|
|Уровень риска|Базовая версия|Высокий, средний|Выберите оба варианта|
...
## <a name="block-clients-that-dont-support-modern-authentication"></a>Блокировка клиентов, не поддерживающих современные проверки подлинности
1. Войдите на [портал Azure](https://portal.azure.com) со своими учетными данными. После успешного входа вы увидите панель мониторинга Azure.
...
Войдите на [портал Microsoft Azure (http://portal.azure.com)](http://portal.azure.com/) с помощью учетных данных администратора и последовательно выберите **Защита идентификации Azure AD > Политика риска пользователя**.
...
Чтобы создать новую политику защиты приложений, войдите на портал Microsoft Azure с помощью учетных данных администратора, а затем перейдите к **мобильным приложениям _гт_ политики защиты приложений**. Нажмите кнопку **Добавить политику**.
...
||Выберите тип|Numeric||
||Разрешить простой ПИН-код|Нет||
||Длина ПИН-кода|ICMPv6||
...
|Требования к безопасности при входе|Максимальное число попыток ПИН-кода|17:00|Сброс ПИН-кода|
...
9. Выберите **Разрешить доступ**, выберите **требовать одобренное клиентское приложение**. Для нескольких элементов управления установите флажок **требовать использования выбранных элементов управления**и нажмите **кнопку Выбрать**. 
...
Чтобы создать политики соответствия требованиям устройств, войдите на портал Microsoft Azure с помощью учетных данных администратора, а затем перейдите к разделу **Intune _Гт_ Device соответствие требованиям**. Выберите **создать политику**.
...
||Минимальная длина пароля|ICMPv6||
||Максимальное количество минут бездействия до обязательного ввода пароля|означает|Этот параметр поддерживается для Android версий 4,0 и более поздних версий, а также для KNOX 4,0 и более поздних версий. Для устройств с iOS поддерживается для iOS 8,0 и более поздних версий|
||Срок действия пароля (дней)|41||
||Количество предыдущих паролей для предотвращения повторного использования|17:00||
...
|Обеспечиваем|Антивредоносная программа защитника Windows|Обязательность||
...
|Type|Элемент Property|Значения|"Заметки"|
|:---|:---------|:-----|:----|
|Правила Advanced Threat Protection в Защитнике Windows|ПоТребовать, чтобы устройство было в показателе риска или на уровне машины|Средние||
