...
---
# <a name="step-2-deploy-windows-10-enterprise-for-existing-devices-as-an-in-place-upgrade"></a><span data-ttu-id="69de9-104">Шаг 2: Развертывания предприятия 10 Windows для существующего устройства как обновление на месте</span><span class="sxs-lookup"><span data-stu-id="69de9-104">Step 2: Deploy Windows 10 Enterprise for existing devices as an in-place upgrade</span></span>

<span data-ttu-id="69de9-105">*В этой статье относятся к E3 и E5 версиям Microsoft 365 для предприятия*</span><span class="sxs-lookup"><span data-stu-id="69de9-105">*This article applies to both the E3 and E5 versions of Microsoft 365 Enterprise*</span></span>

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

<span data-ttu-id="69de9-p101">Простой путь для обновления ПК, в настоящее время под управлением Windows 7 или Windows 8.1 до Windows 10 — это обновление на месте. Последовательность задач System Center Configuration Manager (диспетчер конфигурации) можно использовать для автоматизации процесса полностью.</span><span class="sxs-lookup"><span data-stu-id="69de9-p101">The simplest path to upgrade PCs currently running Windows 7 or Windows 8.1 to Windows 10 is through an in-place upgrade. You can use a System Center Configuration Manager (Configuration Manager) task sequence to completely automate the process.</span></span> 

<span data-ttu-id="69de9-p102">При наличии существующих компьютеров под управлением Windows 7 или Windows 8.1, мы рекомендуем этому пути, если ваша организация развертывание Windows 10. Это использует программа установки Windows (Setup.exe), чтобы выполнить обновление на месте, который автоматически сохраняет все данные, параметры, приложения, и драйверы из существующей версии операционной системы. Для этого необходимо как минимум усилий ИТ, так как не требуется для любого сложного развертывания инфраструктуры.</span><span class="sxs-lookup"><span data-stu-id="69de9-p102">If you have existing computers running Windows 7 or Windows 8.1, we recommend this path if your organization is deploying Windows 10. This leverages the Windows installation program (Setup.exe) to perform an in-place upgrade, which automatically preserves all data, settings, applications, and drivers from the existing operating system version. This requires the least IT effort, because there is no need for any complex deployment infrastructure.</span></span>

<span data-ttu-id="69de9-111">Выполните следующие действия для настройки и развертывания образа Windows 10 Enterprise, с помощью диспетчера конфигураций как обновление на месте.</span><span class="sxs-lookup"><span data-stu-id="69de9-111">Follow these steps to configure and deploy a Windows 10 Enterprise image using Configuration Manager as an in-place upgrade.</span></span>

## <a name="part-1-verify-readiness-to-upgrade-windows"></a><span data-ttu-id="69de9-112">Часть 1: Проверьте готовность к обновлению Windows</span><span class="sxs-lookup"><span data-stu-id="69de9-112">Part 1: Verify readiness to upgrade Windows</span></span>

<span data-ttu-id="69de9-p103">Во-первых, использовать возможность проверки готовности к обновлению Windows Analytics для предоставления мощные исследования и рекомендации относительно компьютеры, приложения и драйверы в вашей организации, не стоимость и без дополнительной инфраструктуры требования. Эта новая служба описывается обновление и компонент обновления проектов, с помощью рабочих процессов, основанных на Microsoft, рекомендации по работе. Обновлять список данных позволяет распределения затрат и риска в проектах обновления.</span><span class="sxs-lookup"><span data-stu-id="69de9-p103">First, use the Upgrade Readiness capability of Windows Analytics to provide powerful insights and recommendations about the computers, applications, and drivers in your organization, at no extra cost and without additional infrastructure requirements. This new service guides you through upgrade and feature update projects using a workflow based on Microsoft recommended practices. Up-to-date inventory data allows you to balance cost and risk in your upgrade projects.</span></span>

<span data-ttu-id="69de9-116">В разделе [Управление Windows обновления с помощью проверки готовности к обновлению](https://docs.microsoft.com/windows/deployment/upgrade/manage-windows-upgrades-with-upgrade-readiness) Дополнительные сведения, начало работы, использование и устранение неполадок проверки готовности к обновлению.</span><span class="sxs-lookup"><span data-stu-id="69de9-116">See [Manage Windows upgrades with Upgrade Readiness](https://docs.microsoft.com/windows/deployment/upgrade/manage-windows-upgrades-with-upgrade-readiness) to learn more, get started, use, and troubleshoot Upgrade Readiness.</span></span>

<span data-ttu-id="69de9-p104">Затем выполните руководство обновление с помощью System Center Configuration Manager (текущий филиала) Windows 7 или более поздней версии операционной системы Windows 10. Как и в любом развертывании высокого риска, мы рекомендуем резервное копирование данных пользователя перед тем как перейти. OneDrive Облачное хранилище готова к использованию лицензированным пользователям Microsoft 365 и может использоваться для безопасного хранения файлов. Дополнительные сведения см в [OneDrive краткое руководство](https://aka.ms/ODfBquickstartguide). Для доступа к этой странице, необходимо выполнить вход как администратор клиента или глобального администратора в Office 365 или Microsoft 365 клиентов.</span><span class="sxs-lookup"><span data-stu-id="69de9-p104">Next, follow the guide to use System Center Configuration Manager (Current Branch) to upgrade Windows 7 or later operating system to Windows 10. As with any high-risk deployment, we recommend backing up user data before proceeding. OneDrive cloud storage is ready to use for licensed Microsoft 365 users and can be used to securely store their files. For more info, see [OneDrive quick start guide](https://aka.ms/ODfBquickstartguide). To access this page, you must sign in as a tenant admin or global admin in an Office 365 or Microsoft 365 tenant.</span></span>

<span data-ttu-id="69de9-122">Список версий Диспетчер конфигураций и соответствующие Windows 10 версии клиента, которые поддерживаются в разделе [Поддержка для Windows 10 для System Center Configuration Manager](https://aka.ms/supportforwin10sccm).</span><span class="sxs-lookup"><span data-stu-id="69de9-122">For a list of Configuration Manager versions and the corresponding Windows 10 client versions that are supported, see [Support for Windows 10 for System Center Configuration Manager](https://aka.ms/supportforwin10sccm).</span></span>

<span data-ttu-id="69de9-123">**Чтобы проверить готовность к обновлению Windows**</span><span class="sxs-lookup"><span data-stu-id="69de9-123">**To verify readiness to upgrade Windows**</span></span>

<span data-ttu-id="69de9-124">Просмотрите эти требования перед началом развертывания Windows 10:</span><span class="sxs-lookup"><span data-stu-id="69de9-124">Review these requirements before starting your Windows 10 deployment:</span></span>

- <span data-ttu-id="69de9-p105">**Обновление версии Windows, подходящие для** - устройство должен работать под управлением выпуски Windows 7 или Windows 8.1, которые могут быть использованы для обновления до 10 Windows Enterprise. Список поддерживаемых версий в разделе [Windows 10 пути обновления](https://aka.ms/win10upgradepaths).</span><span class="sxs-lookup"><span data-stu-id="69de9-p105">**Windows editions eligible for upgrade** - Your devices must be running editions of Windows 7 or Windows 8.1 that are eligible for upgrade to Windows 10 Enterprise. For a list of supported editions, see [Windows 10 upgrade paths](https://aka.ms/win10upgradepaths).</span></span> 
- <span data-ttu-id="69de9-p106">**Поддерживаемые устройства** - большинство компьютеров, совместимые с Windows 8.1 будут совместимы с Windows 10. Может потребоваться установить обновленные драйверы Windows 10 для устройств функции должным образом. Дополнительные сведения в разделе [спецификаций Windows 10](https://aka.ms/windows10specifications) .</span><span class="sxs-lookup"><span data-stu-id="69de9-p106">**Supported devices** - Most computers that are compatible with Windows 8.1 will be compatible with Windows 10. You may need to install updated drivers in Windows 10 for your devices to properly function. See [Windows 10 specifications](https://aka.ms/windows10specifications) for more info.</span></span>
- <span data-ttu-id="69de9-130">**Подготовка к развертыванию** - убедитесь, что у вас есть следующие перед началом настройки развертывания:</span><span class="sxs-lookup"><span data-stu-id="69de9-130">**Deployment preparation** - Make sure you have the following before you start configuring the deployment:</span></span>
    - <span data-ttu-id="69de9-p107">Установочный носитель Windows 10 - установочного носителя должна размещаться на отдельном диске с ISO уже подключен. ISO можно получить из [Центра загрузки для подписчиков MSDN](https://aka.ms/msdn-subscriber-downloads) или [Volume Licensing Service Center](https://aka.ms/mvlsc).</span><span class="sxs-lookup"><span data-stu-id="69de9-p107">Windows 10 installation media - The installation media must be located on a separate drive, with the ISO already mounted. You can obtain the ISO from [MSDN Subscriber Downloads](https://aka.ms/msdn-subscriber-downloads) or from the [Volume Licensing Service Center](https://aka.ms/mvlsc).</span></span>
    - <span data-ttu-id="69de9-p108">Резервные копии данных о пользователях - несмотря на то, что данные пользователя будут перенесены в обновления, рекомендуется настроить резервного копирования сценария. К примеру Экспортируйте все данные пользователя для учетной записи OneDrive, флэш-накопитель USB BitLocker To Go зашифрованных или сетевую папку на сервере. Для получения дополнительных сведений см [резервное копирование или перенос данных в Windows](https://aka.ms/backuptransferdatawindows).</span><span class="sxs-lookup"><span data-stu-id="69de9-p108">Backups of user data - Although user data will be migrated in the upgrade, best practice is to configure a backup scenario. For example, export all user data to a OneDrive account, BitLocker To Go-encrypted USB flash drive, or network file server. For more information, see [Back up or transfer data in Windows](https://aka.ms/backuptransferdatawindows).</span></span>
- <span data-ttu-id="69de9-p109">**Подготовка среды** - существующей структуры диспетчер конфигурации сервера будет использоваться для подготовки к развертыванию операционной системы. В дополнение к базовой установки следующих конфигураций должна создаваться в среде Configuration Manager:</span><span class="sxs-lookup"><span data-stu-id="69de9-p109">**Environment preparation** - You will use an existing Configuration Manager server structure to prepare for operating system deployment. In addition to the base setup, the following configurations should be made in the Configuration Manager environment:</span></span>
    1. <span data-ttu-id="69de9-138">[Расширение схемы Active Directory](https://aka.ms/extendadschema) и [создать контейнер системы управления](https://aka.ms/createsysmancontainer).</span><span class="sxs-lookup"><span data-stu-id="69de9-138">[Extend the Active Directory Schema](https://aka.ms/extendadschema) and [create a System Management container](https://aka.ms/createsysmancontainer).</span></span>
    2. <span data-ttu-id="69de9-p110">Лес Active Directory включить обнаружение и обнаружение системы Active Directory. Дополнительные сведения содержатся в разделе [Настройка методы обнаружения для System Center Configuration Manager](https://aka.ms/configurediscoverymethods).</span><span class="sxs-lookup"><span data-stu-id="69de9-p110">Enable Active Directory Forest Discovery and Active Directory System Discovery. For more info, see [Configure discovery methods for System Center Configuration Manager](https://aka.ms/configurediscoverymethods).</span></span>
    3. <span data-ttu-id="69de9-p111">Создание границы диапазона IP-адресов и границы группы для назначения содержимого и сайтов. Дополнительные сведения в разделе [Определение сайта ограничения и границы групп для System Center Configuration Manager](https://aka.ms/definesiteboundaries).</span><span class="sxs-lookup"><span data-stu-id="69de9-p111">Create IP range boundaries and boundary group for content and site assignment. For more info, see [Define site boundaries and boundary groups for System Center Configuration Manager](https://aka.ms/definesiteboundaries).</span></span>
    4. <span data-ttu-id="69de9-p112">Добавление и настройка диспетчера конфигураций отчетов роль точка служб. Дополнительные сведения содержатся в разделе [Настройка отчетов в диспетчере конфигурации](https://aka.ms/configurereporting).</span><span class="sxs-lookup"><span data-stu-id="69de9-p112">Add and configure the Configuration Manager reporting services point role. For more info, see [Configuring Reporting in Configuration Manager](https://aka.ms/configurereporting).</span></span>
    5. <span data-ttu-id="69de9-145">Создайте файл системы папки для пакетов.</span><span class="sxs-lookup"><span data-stu-id="69de9-145">Create a file system folder structure for packages.</span></span>
    6. <span data-ttu-id="69de9-146">Создайте структуру папок консоли диспетчера конфигураций для пакетов.</span><span class="sxs-lookup"><span data-stu-id="69de9-146">Create a Configuration Manager console folder structure for packages.</span></span>
    7. <span data-ttu-id="69de9-147">Установка обновлений System Center Configuration Manager (текущий филиала) и любые дополнительные компоненты Windows 10.</span><span class="sxs-lookup"><span data-stu-id="69de9-147">Install System Center Configuration Manager (Current Branch) updates and any additional Windows 10 prerequisites.</span></span>

## <a name="part-2-add-a-windows-10-os-image-using-configuration-manager"></a><span data-ttu-id="69de9-148">Часть 2: Добавление образа операционной системы Windows 10, с помощью диспетчера конфигураций</span><span class="sxs-lookup"><span data-stu-id="69de9-148">Part 2: Add a Windows 10 OS image using Configuration Manager</span></span>
<span data-ttu-id="69de9-p113">Теперь необходимо создать пакет обновления операционной системы, который содержит полный установочного носителя Windows 10. В следующих действиях используется для создания пакета обновления для Windows 10 Enterprise x64 Configuration Manager.</span><span class="sxs-lookup"><span data-stu-id="69de9-p113">Now you'll need to create an operating system upgrade package that contains the full Windows 10 installation media. In the following steps, you’ll use Configuration Manager to create an upgrade package for Windows 10 Enterprise x64.</span></span>

<span data-ttu-id="69de9-151">**Чтобы добавить образа операционной системы Windows 10, с помощью диспетчера конфигураций**</span><span class="sxs-lookup"><span data-stu-id="69de9-151">**To add a Windows 10 OS image using Configuration Manager**</span></span>

1. <span data-ttu-id="69de9-152">В рабочей области для **Библиотеки программного обеспечения** с помощью консоли диспетчера конфигураций, щелкните правой кнопкой мыши узел **Пакетов обновления операционной системы** и выберите **Добавить пакет обновления операционной системы**.</span><span class="sxs-lookup"><span data-stu-id="69de9-152">Using the Configuration Manager console, in the **Software Library** workspace, right-click the **Operating System Upgrade Packages** node, and then select **Add Operating System Upgrade Package**.</span></span>
2. <span data-ttu-id="69de9-153">На странице " **Источник данных** " Укажите UNC-путь к мультимедиа Windows 10 Enterprise x64 и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-153">On the **Data Source** page, specify the UNC path to the Windows 10 Enterprise x64 media, and then select **Next**.</span></span>
3. <span data-ttu-id="69de9-154">На странице " **Общие** " укажите **обновления Windows 10 Enterprise x64**и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-154">On the **General** page, specify **Windows 10 Enterprise x64 Upgrade**, and then select **Next**.</span></span> 
4. <span data-ttu-id="69de9-155">На странице " **Сводка** " нажмите кнопку **Далее**и затем выберите команду **Закрыть**.</span><span class="sxs-lookup"><span data-stu-id="69de9-155">On the **Summary** page, select **Next**, and then select **Close**.</span></span> 
5. <span data-ttu-id="69de9-156">Щелкните правой кнопкой мыши созданный пакет **обновления Windows 10 Enterprise x64** и выберите **Распределения контента**.</span><span class="sxs-lookup"><span data-stu-id="69de9-156">Right-click the created **Windows 10 Enterprise x64 Update** package, and then select **Distribute Content**.</span></span> 
6. <span data-ttu-id="69de9-157">Выберите точку распространения.</span><span class="sxs-lookup"><span data-stu-id="69de9-157">Choose your distribution point.</span></span>

## <a name="part-3-configure-deployment-settings"></a><span data-ttu-id="69de9-158">Часть 3: Настройка параметров развертывания</span><span class="sxs-lookup"><span data-stu-id="69de9-158">Part 3: Configure deployment settings</span></span>
<span data-ttu-id="69de9-p114">На этом этапе вы настроите последовательность обновления задач, содержащего параметры для обновления Windows 10. Будет затем определите устройств для обновления, а затем разверните последовательность задач для этих устройств.</span><span class="sxs-lookup"><span data-stu-id="69de9-p114">In this step, you'll configure an upgrade task sequence that contains the settings for the Windows 10 upgrade. You'll then identify the devices to upgrade, and then deploy the task sequence to those devices.</span></span>

### <a name="create-a-task-sequence"></a><span data-ttu-id="69de9-161">Создание последовательности задач</span><span class="sxs-lookup"><span data-stu-id="69de9-161">Create a task sequence</span></span>
<span data-ttu-id="69de9-162">Чтобы создать последовательность обновления задач, выполните следующие действия:</span><span class="sxs-lookup"><span data-stu-id="69de9-162">To create an upgrade task sequence, perform the following steps:</span></span>
  
1. <span data-ttu-id="69de9-163">В консоли диспетчера конфигураций в рабочей области **Библиотеки программного обеспечения** разверните **операционные системы**.</span><span class="sxs-lookup"><span data-stu-id="69de9-163">In the Configuration Manager console, in the **Software Library** workspace, expand **Operating Systems**.</span></span> 
2. <span data-ttu-id="69de9-164">Щелкните правой кнопкой мыши узел **Последовательности задач** и выберите **Создать последовательность задач**.</span><span class="sxs-lookup"><span data-stu-id="69de9-164">Right-click the **Task Sequences** node, and then select **Create Task Sequence**.</span></span>
3. <span data-ttu-id="69de9-165">На странице **Создание нового последовательности задач** выберите **обновление операционной системы из пакета обновления**и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-165">On the **Create a new task sequence** page, select **Upgrade an operating system from upgrade package**, and then select **Next**.</span></span>
4. <span data-ttu-id="69de9-166">На странице **Сведений о последовательности задач** укажите **обновления Windows 10 Enterprise x64**и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-166">On the **Task Sequence Information** page, specify **Windows 10 Enterprise x64 Upgrade**, and then select **Next**.</span></span>
5. <span data-ttu-id="69de9-167">На странице **обновление операционной системы Windows** выберите **Обзор** и выберите **Обновление пакета обновления операционной системы Windows 10 Enterprise x64**кнопку « **ОК»** и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-167">On the **Upgrade the Windows operating system** page, select **Browse** and choose the **Windows 10 Enterprise x64 Upgrade operating system upgrade package**, select **OK**, and then select **Next**.</span></span>
6. <span data-ttu-id="69de9-168">Продолжите работу на оставшихся страницах мастера, а затем выберите **Закрыть**.</span><span class="sxs-lookup"><span data-stu-id="69de9-168">Continue through the remaining wizard pages, and then select **Close**.</span></span>

### <a name="create-a-device-collection"></a><span data-ttu-id="69de9-169">Создание семейства устройств</span><span class="sxs-lookup"><span data-stu-id="69de9-169">Create a device collection</span></span>
<span data-ttu-id="69de9-170">После создания последовательность обновления задач, вам потребуется создать коллекцию, содержащую устройства, которые будут обновления.</span><span class="sxs-lookup"><span data-stu-id="69de9-170">After you create the upgrade task sequence, you'll need to create a collection that contains the devices you will upgrade.</span></span>

> [!NOTE]
> <span data-ttu-id="69de9-p115">Используйте следующие параметры для проверки развертывания на одном устройстве. Для включения групп устройств, когда вы будете готовы, можно использовать различные правила. Дополнительные сведения см в [Создание коллекций в System Center Configuration Manager](https://aka.ms/sccm-create-collections).</span><span class="sxs-lookup"><span data-stu-id="69de9-p115">Use the following settings to test the deployment on a single device. You can use different membership rules to include groups of devices when you are ready. For more info, see [How to create collections in System Center Configuration Manager](https://aka.ms/sccm-create-collections).</span></span>

1. <span data-ttu-id="69de9-174">В консоли диспетчера конфигураций в рабочей области **активы и обеспечения соответствия требованиям** щелкните правой кнопкой мыши **Семейств сайтов устройства**и выберите **Создание семейства сайтов устройства**.</span><span class="sxs-lookup"><span data-stu-id="69de9-174">In the Configuration Manager console, in the **Assets and Compliance** workspace, right-click **Device Collections**, and then select **Create Device Collection**.</span></span> 
2. <span data-ttu-id="69de9-175">В мастере создания семейства устройств на странице " **Общие** " введите следующие параметры и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-175">In the Create Device Collection wizard, on the **General** page, enter the following settings and then select **Next**:</span></span>
    - <span data-ttu-id="69de9-176">Имя: Обновление Windows 10 предприятия x64</span><span class="sxs-lookup"><span data-stu-id="69de9-176">Name: Windows 10 Enterprise x64 Upgrade</span></span>
    - <span data-ttu-id="69de9-177">Ограничивающим семейство сайтов: Все системы</span><span class="sxs-lookup"><span data-stu-id="69de9-177">Limiting Collection: All Systems</span></span>
3. <span data-ttu-id="69de9-178">На странице " **Правила** " выберите команду **Добавить правило** > **прямого правило** Чтобы запустить мастер создания правила непосредственного членства.</span><span class="sxs-lookup"><span data-stu-id="69de9-178">On the **Membership Rules** page, select **Add Rule** > **Direct rule** to launch the Create Direct Membership Rule Wizard.</span></span>
4. <span data-ttu-id="69de9-179">На странице **приветствия** мастера создания правила непосредственного членства нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-179">On the **Welcome** page of the Create Direct Membership Rule Wizard, select **Next**.</span></span>
5. <span data-ttu-id="69de9-180">На странице **поиска для ресурсов** введите следующие параметры, заменив имя устройства, которое выполняется обновление замещающий текст **значение** :</span><span class="sxs-lookup"><span data-stu-id="69de9-180">On the **Search for Resources** page, enter the following settings, replacing the placeholder **Value** text with the name of the device you are upgrading:</span></span> 
    - <span data-ttu-id="69de9-181">Класс ресурсов: Системных ресурсов</span><span class="sxs-lookup"><span data-stu-id="69de9-181">Resource Class: System Resource</span></span>
    - <span data-ttu-id="69de9-182">Атрибут имени: имя</span><span class="sxs-lookup"><span data-stu-id="69de9-182">Attribute Name: Name</span></span>
    - <span data-ttu-id="69de9-183">Значение: *PC0003*</span><span class="sxs-lookup"><span data-stu-id="69de9-183">Value: *PC0003*</span></span>
6. <span data-ttu-id="69de9-184">На странице **Выбор ресурсов** выберите нужное устройство и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-184">On the **Select Resources** page, select your device, and select **Next**.</span></span>
7. <span data-ttu-id="69de9-185">Выполните мастер создания семейства устройств и мастера создания правил прямого членства.</span><span class="sxs-lookup"><span data-stu-id="69de9-185">Complete the Create Direct Membership Rule wizard and the Create Device Collection Wizard.</span></span>  
8. <span data-ttu-id="69de9-p116">Просмотрите обновления семейства Windows 10 Enterprise x64. Не выполняйте, пока появятся компьютеров вы добавили в коллекции.</span><span class="sxs-lookup"><span data-stu-id="69de9-p116">Review the Windows 10 Enterprise x64 Upgrade collection. Do not continue until you see the machines you added in the collection.</span></span>

### <a name="create-an-operating-system-deployment"></a><span data-ttu-id="69de9-188">Создание развертывания операционной системы</span><span class="sxs-lookup"><span data-stu-id="69de9-188">Create an operating system deployment</span></span>
<span data-ttu-id="69de9-189">Выполните следующие действия для создания развертывания для последовательности задач.</span><span class="sxs-lookup"><span data-stu-id="69de9-189">Follow these steps to create a deployment for the task sequence.</span></span>

1. <span data-ttu-id="69de9-190">В консоли диспетчера конфигураций в рабочей области **Библиотеки программного обеспечения** щелкните правой кнопкой мыши последовательности задач, созданной на предыдущем шаге и затем выберите команду **Развернуть**.</span><span class="sxs-lookup"><span data-stu-id="69de9-190">In the Configuration Manager console, in the **Software Library** workspace, right-click the task sequence you created in a previous step, and then select **Deploy**.</span></span>
2. <span data-ttu-id="69de9-191">На странице " **Общие** " выберите семейство **Windows 10 Enterprise x64 обновления** и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-191">On the **General** page, select the **Windows 10 Enterprise x64 Upgrade** collection, and then select **Next**.</span></span>
3. <span data-ttu-id="69de9-192">На странице **содержимого** нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-192">On the **Content** page, select **Next**.</span></span>
4. <span data-ttu-id="69de9-193">На странице " **Параметры развертывания** " выберите следующие параметры и нажмите кнопку **Далее**:</span><span class="sxs-lookup"><span data-stu-id="69de9-193">On the **Deployment Settings** page, select the following settings, and then select **Next**:</span></span>

    > [!NOTE]
    > <span data-ttu-id="69de9-p117">Для этого тестового развертывания будет значение назначения **доступен**, что требует вмешательства пользователя для запуска развертывания. В рабочей среде вы можете для автоматизации развертывания с помощью необходимых цели — Настройка дополнительных параметров, например планирование при выполнении развертывания.</span><span class="sxs-lookup"><span data-stu-id="69de9-p117">For this test deployment, you'll set the purpose to **Available**, which requires user intervention to start the deployment. In a production environment, you may wish to automate the deployment using the Required purpose, which involves configuring additional options such as scheduling when the deployment is run.</span></span> 

    - <span data-ttu-id="69de9-196">Действие: Установка</span><span class="sxs-lookup"><span data-stu-id="69de9-196">Action: Install</span></span>
    - <span data-ttu-id="69de9-197">Цель: доступные</span><span class="sxs-lookup"><span data-stu-id="69de9-197">Purpose: Available</span></span>

5. <span data-ttu-id="69de9-198">На странице " **Планирование** " примите параметры по умолчанию и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-198">On the **Scheduling** page, accept the default settings, and then select **Next**.</span></span>
6. <span data-ttu-id="69de9-199">На странице " **Взаимодействие с пользователем** " примите параметры по умолчанию и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-199">On the **User Experience** page, accept the default settings, and then select **Next**.</span></span>
7. <span data-ttu-id="69de9-200">На странице " **оповещения** " примите параметры по умолчанию и нажмите кнопку **Далее**.</span><span class="sxs-lookup"><span data-stu-id="69de9-200">On the **Alerts** page, accept the default settings, and then select **Next**.</span></span>
8. <span data-ttu-id="69de9-201">На странице " **Сводка** " нажмите кнопку **Далее**и затем выберите команду **Закрыть**.</span><span class="sxs-lookup"><span data-stu-id="69de9-201">On the **Summary** page, select **Next**, and then select **Close**.</span></span>

## <a name="part-5-start-the-windows-10-upgrade-task-sequence"></a><span data-ttu-id="69de9-202">Часть 5: Последовательность обновления задач Пуск Windows 10</span><span class="sxs-lookup"><span data-stu-id="69de9-202">Part 5: Start the Windows 10 upgrade task sequence</span></span>
<span data-ttu-id="69de9-203">Выполните следующие действия для запуска последовательности задач Windows обновления 10 на устройстве, который обновляется.</span><span class="sxs-lookup"><span data-stu-id="69de9-203">Follow these steps to start the Windows 10 Upgrade task sequence on the device that you are upgrading.</span></span>
 
1. <span data-ttu-id="69de9-204">Выполните вход на компьютер Windows и запустите **Центр программного обеспечения**.</span><span class="sxs-lookup"><span data-stu-id="69de9-204">Log on to the Windows computer and start **Software Center**.</span></span>
2. <span data-ttu-id="69de9-205">Выберите последовательность задач, созданной на предыдущем шаге, а затем выберите **установить**.</span><span class="sxs-lookup"><span data-stu-id="69de9-205">Select the task sequence that you created in a previous step, and then select **Install**.</span></span>
3. <span data-ttu-id="69de9-206">После начала последовательности задач, он автоматически запускает процесс обновления на месте, запуск программы установки Windows (Setup.exe) необходимых параметров командной строки, чтобы выполнить автоматическое обновление, сохранив при этом все данные, параметры, приложения, и драйверы.</span><span class="sxs-lookup"><span data-stu-id="69de9-206">When the task sequence begins, it automatically initiates the in-place upgrade process by invoking the Windows setup program (Setup.exe) with the necessary command-line parameters to perform an automated upgrade, which preserves all data, settings, apps, and drivers.</span></span>
4. <span data-ttu-id="69de9-207">После успешного завершения последовательности задач, компьютер будет полностью обновлена до Windows 10.</span><span class="sxs-lookup"><span data-stu-id="69de9-207">After the task sequence completes successfully, the computer will be fully upgraded to Windows 10.</span></span>

<span data-ttu-id="69de9-p118">Если возникают проблемы с помощью Windows 10 в корпоративной среде, можно обратиться к [лучшие решения службы поддержки Майкрософт для наиболее распространенных проблем](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). Эти ресурсы включают статьи статья БАЗЫ знаний, обновления и статьи библиотеки.</span><span class="sxs-lookup"><span data-stu-id="69de9-p118">If you experience issues when using Windows 10 in an enterprise environment, you can consult [top Microsoft Support solutions for the most common issues](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). These resources include KB articles, updates, and library articles.</span></span>

<span data-ttu-id="69de9-p119">Во время развертывания обновлений внутри организации используйте возможность обновить соответствия Windows Analytics для создания единого представления соответствия требованиям обновления операционной системы, выполнение развертывания обновления и сбоя, устранение неполадок для устройств Windows 10. Эта новая служба использует диагностических данных, включая ход выполнения установки, настройки центра обновления Windows и другие сведения для предоставления таких исследования, не стоимость и без дополнительной инфраструктуры требования. Будет ли используется Центр обновления Windows для бизнеса или другие средства управления, можно быть уверенным, устройство надлежащим образом обновляются.</span><span class="sxs-lookup"><span data-stu-id="69de9-p119">During the rollout of updates across your organization, use the Update Compliance capability of Windows Analytics to provide a holistic view of OS update compliance, update deployment progress, and failure troubleshooting for Windows 10 devices. This new service uses diagnostic data including installation progress, Windows Update configuration and other information to provide such insights, at no extra cost and without additional infrastructure requirements. Whether it's used with Windows Update for Business or other management tools, you can be assured that your devices are properly updated.</span></span>

<span data-ttu-id="69de9-213">В разделе [монитор обновления Windows и антивирусная программа Защитник Windows с соответствием обновление](https://docs.microsoft.com/windows/deployment/update/update-compliance-monitor) Дополнительные сведения, начало работы и использованию соответствия требованиям обновления.</span><span class="sxs-lookup"><span data-stu-id="69de9-213">See [Monitor Windows Updates and Windows Defender Antivirus with Update Compliance](https://docs.microsoft.com/windows/deployment/update/update-compliance-monitor) to learn more, get started, and use Update Compliance.</span></span>

<span data-ttu-id="69de9-214">Прежде чем перейти к следующему шагу, проверьте [условия](windows10-exit-criteria.md#crit-windows10-step2), при выполнении которых можно считать данный шаг завершенным.</span><span class="sxs-lookup"><span data-stu-id="69de9-214">As an interim checkpoint, you can see the [exit criteria](windows10-exit-criteria.md#crit-windows10-step2) corresponding to this step.</span></span>

## <a name="next-step"></a><span data-ttu-id="69de9-215">Следующее действие</span><span class="sxs-lookup"><span data-stu-id="69de9-215">Next step</span></span>

|||
|:-------|:-----|
|![](./media/stepnumbers/Step3.png)| [<span data-ttu-id="69de9-216">Развертывание Windows 10 Enterprise для новых устройств с автопилот Windows</span><span class="sxs-lookup"><span data-stu-id="69de9-216">Deploy Windows 10 Enterprise for new devices with Windows Autopilot</span></span>](windows10-deploy-autopilot.md) |
