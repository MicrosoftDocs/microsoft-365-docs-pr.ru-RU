...
---
# <a name="high-availability-federated-authentication-phase-5-configure-federated-authentication-for-microsoft-365"></a>Этап 5 высокой доступности для федеративной проверки подлинности: Настройка федеративной проверки подлинности для Microsoft 365

На последнем этапе развертывания федеративной проверки подлинности с высоким уровнем доступности для Microsoft 365 в службах инфраструктуры Azure можно получить и установить сертификат, выданный общедоступным центром сертификации, проверить конфигурацию, а затем установить и запустить Azure AD Connect на сервере синхронизации службы каталогов. Azure AD Connect настраивает свою подписку на Microsoft 365 и прокси-серверы служб федерации Active Directory и прокси-серверов веб-приложений для федеративной проверки подлинности.
  
[В статье Развертывание федеративной проверки подлинности с высоким уровнем доступности для Microsoft 365 в Azure](deploy-high-availability-federated-authentication-for-microsoft-365-in-azure.md) для всех фаз.
  
## <a name="get-a-public-certificate-and-copy-it-to-the-directory-synchronization-server"></a>Получение общедоступного сертификата и его копирование на сервер синхронизации каталогов
...
Получив сертификат, скопируйте его в папку на диске C: на сервере синхронизации службы каталогов. Например, назовите файл SSL. pfx и сохраните его в папке C: \\ certs на сервере синхронизации каталогов.
  
## <a name="verify-your-configuration"></a>Проверка конфигурации

Теперь необходимо подготовиться к настройке Azure AD Connect и федеративной проверки подлинности для Microsoft 365. Проверьте, так ли это, с помощью этого контрольного списка:
  
- Общедоступный домен вашей организации добавляется в вашу подписку на Microsoft 365.
    
- Учетные записи пользователей Microsoft 365 настроены на общедоступное доменное имя вашей организации и могут войти в систему.
...
- Общедоступный центр сертификации — цифровой сертификат соединений, подходящие для SSL-соединений со службой SAN, настроенной на полное доменное имя службы федерации — это PFX-файл, хранящийся на сервере синхронизации службы каталогов.
...
Средство Azure AD Connect позволяет настроить серверы AD FS, прокси-серверы веб-приложений и Microsoft 365 для федеративной проверки подлинности, выполнив следующие действия:
  
1. Создайте подключение к удаленному рабочему столу сервера синхронизации каталогов с учетной записью домена с правами локального администратора.
    
2. На рабочем столе сервера синхронизации службы каталогов откройте Internet Explorer и перейдите по адресу [https://aka.ms/aadconnect](https://aka.ms/aadconnect) .
...
8. На странице **Подключение к Azure AD** введите имя и пароль учетной записи глобального администратора для своей подписки на Microsoft 365, а затем нажмите кнопку **Далее**.
    
9. На странице **Подключение каталогов** убедитесь, что локальный лес доменных служб Active Directory (AD DS) выбран в **лесу**, введите имя и пароль учетной записи администратора домена, нажмите кнопку **Добавить каталог**, а затем нажмите кнопку **Далее**.
...
Инфраструктура федеративной проверки подлинности с высоким уровнем доступности для Microsoft 365 в Azure завершена.
...
[Федеративная идентификация для Microsoft 365](https://support.office.com/article/Understanding-Office-365-identity-and-Azure-Active-Directory-06a189e7-5ec6-4af2-94bf-a22ea225a7a9#bk_federated)
