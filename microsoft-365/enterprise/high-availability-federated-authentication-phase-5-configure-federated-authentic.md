...
На этом заключительном этапе развертывания федерарной проверки подлинности высокой доступности для Microsoft 365 в службах инфраструктуры Azure вы получаете и устанавливаете сертификат, выданный государственным органом сертификации, проверяете конфигурацию, а затем установите и запустите Azure AD Подключение на сервере синхронизации каталогов. Azure AD Подключение настраивает подписку на Microsoft 365 и службы федерации Active Directory (AD FS) и прокси-серверы веб-приложений для федерарной проверки подлинности.
  
См. в рублях Развертывание [федерарной](deploy-high-availability-federated-authentication-for-microsoft-365-in-azure.md) проверки подлинности с высокой доступностью для Microsoft 365 в Azure для всех этапов.
...
Теперь вы должны быть готовы к настройке проверки подлинности Azure AD Подключение федерарной проверки подлинности для Microsoft 365. Проверьте, так ли это, с помощью этого контрольного списка:
  
- Общественное достояние организации добавляется в Microsoft 365 подписку.
    
- Учетные записи Microsoft 365 настраиваются на имя общественного домена организации и могут успешно войти.
...
Средство Azure AD Подключение настраивает серверы AD FS, прокси-серверы веб-приложений и Microsoft 365 проверку подлинности с помощью этих действий:
...
8. На странице **Подключение Azure AD** введите имя и пароль учетной записи глобального администратора для Microsoft 365 подписки, а затем нажмите **кнопку Далее**.
    
9. На странице **Подключение** каталогов убедитесь, что лес служб домена Active Directory (AD DS) выбран в лесу, введите имя и пароль учетной записи администратора домена, щелкните Add **Directory** и нажмите кнопку **Далее**.
...
Инфраструктура федерарной проверки подлинности высокой доступности для Microsoft 365 Azure завершена.
