...
Azure Active Directory (Azure AD) Connect (ранее известный как средство синхронизации каталогов, средство синхронизации каталогов или средство DirSync.exe) — это приложение, которое вы устанавливаете на сервере, подключенном к домену, для синхронизации локальной службы доменных служб Active Directory (AD DS) с клиентом Azure AD подписки Microsoft 365. Microsoft 365 использует Azure AD для службы каталогов. Подписка microsoft 365 включает клиента Azure AD. Этот клиент также может использоваться для управления удостоверениями организации с другими облачными рабочими нагрузками, включая другие приложения и приложения SaaS в Azure.
...
## <a name="overview-of-deploying-microsoft-365-directory-synchronization-in-azure"></a>Обзор развертывания синхронизации каталогов Microsoft 365 в Azure

На следующей схеме показано, как Azure AD Connect работает на виртуальной машине в Azure (сервер синхронизации каталогов), который синхронизирует локальное лесное управление AD dS с подпиской Microsoft 365.
...
-  Azure AD Connect отправляет изменения учетным записям и паролям в экземпляр Azure AD подписки Microsoft 365. Поскольку сервер синхронизации каталогов находится в расширенной части локальной сети, эти изменения отправляются через прокси-сервер локальной сети.
...
2. Установите [Azure AD Connect](https://www.microsoft.com/download/details.aspx?id=47594) на подключенную к домену виртуальную машину в Azure, а затем синхронизуйте локальное приложение AD DS с Microsoft 365. Этот процесс включает следующие этапы:
    ...
    Настройка Azure AD Connect требует учетных данных (имя пользователя и пароль) учетной записи администратора Azure AD и учетной записи корпоративного администратора AD DS. Azure AD Connect запускается немедленно и на постоянной основе для синхронизации локального леса AD DS с Microsoft 365.
...
- У вас есть подписка microsoft 365, которая включает функцию интеграции Active Directory. Сведения о подписках Microsoft 365 перейдите на страницу [подписки Microsoft 365.](https://products.office.com/compare-all-microsoft-office-products?tab=2)
    
- Предоставление одной виртуальной машины Azure, которая запускает Azure AD Connect для синхронизации локального леса AD DS с Microsoft 365.
...
2. На сервере синхронизации каталогов откройте синхронизацию настройка каталога для [статьи Microsoft 365](set-up-directory-synchronization.md) и следуйте указаниям по синхронизации каталогов с синхронизацией с помощью хаши паролей.
