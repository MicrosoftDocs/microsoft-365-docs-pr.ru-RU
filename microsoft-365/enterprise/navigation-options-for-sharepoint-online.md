...
## <a name="pros-and-cons-of-sharepoint-online-navigation-options"></a>Плюсы и минусы SharePoint навигации в Интернете
...
>[!NOTE]
>Применение того же принципа, что и современные SharePoint сайтов, упрощая общую структуру сайта в более лестную, не иерархическую структуру, повышает производительность и упрощает переход на современные SharePoint сайты. Это означает, что вместо одной коллекции сайтов с сотнями сайтов (subwebs) лучше использовать множество коллекций сайтов с очень немногими подвидами (subwebs).

## <a name="analyzing-navigation-performance-in-sharepoint-online"></a>Анализ производительности навигации в SharePoint Online

Средство [page Diagnostics for SharePoint](./page-diagnostics-for-spo.md) является расширением браузера для браузеров Microsoft Edge и Chrome, которое анализирует современный портал SharePoint Online и классические страницы сайтов публикации. Этот инструмент работает только для SharePoint Online и не может использоваться на SharePoint странице системы.

Средство создает отчет для каждой анализируемой страницы с указанием того, как страница выполняется в отношении заранее определенного набора правил, и отображает подробные сведения, когда результаты теста выпадают за пределы базового значения. SharePoint Администраторы и дизайнеры в Интернете могут использовать этот инструмент для устранения проблем с производительностью, чтобы обеспечить оптимизацию новых страниц перед публикацией.

**SPRequestDuration** , в частности, это время, SharePoint для обработки страницы. Тяжелая навигация (например, в том числе страницы в навигации), сложные иерархии сайтов и другие параметры конфигурации и топологии могут значительно способствовать более длительным срокам.
...
В **статье Site Параметры** Look and Feel Navigation можно проверить, выбрана ли структурная навигация для глобальной навигации или текущей  >    >  навигации. Выбор страниц **Show отрицательно** скажется на производительности.

![Структурная навигация с выбранными подвидами Show](../media/SPONavOptionsStructuredShowSubsites.png)

Кэшинг может быть включен или отключен на уровне коллекции сайтов и на уровне сайта, и включен для обоих по умолчанию. Чтобы включить на уровне коллекции сайтов, **в** Параметры веб-Параметры администрирование веб-сайтов, проверьте поле для  >    >   **включить кэшировать**.

![Включить кэшинг на уровне сайта](../media/structural-nav/structural-nav-caching-site-coll.png)

Чтобы включить на уровне сайта в Параметры сайте, проверьте поле  >  для **кэшинга включить**.
...
Для реализации управляемой навигации необходимо настроить термины с URL-адресами, соответствующими структуре навигации сайта. Управляемые навигации можно даже вручную курировать, чтобы заменить структурную навигацию во многих случаях. Пример.

![SharePoint Структура веб-сайтов в Интернете](../media/SPONavOptionsListOfSites.png))
...
Существуют и другие популярные варианты создания **настраиваемого поставщика навигации.** Дополнительные рекомендации по построению пользовательского поставщика навигации [SharePoint навигационных](/sharepoint/dev/solution-guidance/portal-navigation) решений для порталов SharePoint Online.
...
>[!NOTE]
>При использовании настраиваемого JavaScript убедитесь, что CDN включен и файл находится в CDN месте.
