...
---
# <a name="take-response-actions-on-a-file"></a>Действия отклика в файле
...
## <a name="stop-and-quarantine-files-in-your-network"></a>Остановка и карантин файлов в сети
...
>
> - Устройство, на которое вы принимаете действие, работает под управлением Windows 10 версии 1703 или более поздней версии
> - Файл не принадлежит доверенным сторонним издателям или не подписан Корпорацией Майкрософт
> - Антивирус Microsoft Defender должен по крайней мере работать в пассивном режиме. Дополнительные сведения см. в [веб-сайте Совместимость антивирусных программ Microsoft Defender.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/microsoft-defender-antivirus-compatibility)
...
Это действие действует на устройствах с Windows 10 версии 1703 или более поздней версии, где файл был замечен в течение последних 30 дней.
...
1. Выберите файл, который необходимо остановить, и карантин. Вы можете выбрать файл из любого из следующих представлений или использовать поле Поиска:
   ...
   - **Оповещений** — щелкните соответствующие ссылки из описания или подробных сведений в временной шкале Артефакт
...
Выбор файла **Загрузка** из ответных действий позволяет скачать локальный архив с защищенным паролем архивом .zip, содержащим файл. Появится вылет, в котором можно записать причину загрузки файла и установить пароль.
...
> [!IMPORTANT]
>
> - Эта функция доступна, если в организации используется антивирус Microsoft Defender и включена облачная защита. Дополнительные сведения см. в [сведениях Manage cloud-delivered protection.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/deploy-manage-report-microsoft-defender-antivirus)
>
...
> - Это действие отклика доступно для устройств в Windows 10, версии 1703 или более поздней версии.
...
Дополнительные сведения о потенциально скомпрометированном устройстве или уже скомпрометированном устройстве обратитесь к эксперту по угрозам Майкрософт. Эксперты microsoft Threat напрямую участвуют в центре безопасности Защитника Майкрософт для быстрого и точного ответа. Эксперты предоставляют сведения о потенциально скомпрометированном устройстве и помогают разобраться в сложных угрозах и целевых уведомлениях об атаке. Они также могут предоставлять сведения о оповещениях или контексте разведки угроз, которые вы видите на панели мониторинга портала.
...
## <a name="deep-analysis"></a>Глубокий анализ
...
Глубокий анализ в настоящее время поддерживает обширный анализ переносных исполняемых (PE) файлов (в том числе _файлов .exe_ и _DLL)._
...
> [!NOTE]
> Автоматически можно собирать только файлы из Windows 10.

Вы также можете отправить образец через портал Центра безопасности [Майкрософт,](https://www.microsoft.com/security/portal/submission/submit.aspx) если файл не был замечен на устройстве Windows 10, и дождаться, когда будет доступна кнопка **Отправка** глубокого анализа.
...
Когда образец собран, Defender for Endpoint запускает файл в безопасной среде. Затем создается подробный отчет о наблюдаемых поведениях и связанных с ними артефактах, таких как файлы, сброшенные на устройства, связь с ИП и изменения реестра.
...
1. Выберите файл, который необходимо отправить для глубокого анализа. Вы можете выбрать или поискать файл из любого из следующих представлений:
    ...
    - Оповещений — выберите ссылки на файл из **Описания** или **Сведения** в временной шкале Артефакт
...
2. В **вкладке Глубокий анализ** представления файла выберите **Отправить**.
   ...
   > [!NOTE]
   > Поддерживаются только файлы PE, включая _файлы .exe_ и _dll._
...
> [!NOTE]
> В зависимости от доступности устройства время сбора образцов может отличаться. Существует 3-часовой период времени для коллекции образцов. Коллекция не будет работать, и операция прервается, если в это время нет отчетов о устройствах Windows 10 в Интернете. Вы можете повторно отправить файлы для глубокого анализа, чтобы получить свежие данные в файле.
...
1. Убедитесь, что этот файл является файлом PE. Pe-файлы обычно имеют _расширения .exe_ или _dll_ (исполняемые программы или приложения).
...
- [Действия реагирования на устройстве](respond-machine-alerts.md)
