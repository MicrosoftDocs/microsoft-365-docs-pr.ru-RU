...
Извлекает коллекцию [машинных действий.](machineaction.md)
<br>Поддерживает [запросы OData V4.](https://www.odata.org/documentation/)
<br>Запрос OData поддерживается ```$filter``` на: ```status``` , , ```machineId``` и ```type``` ```requestor``` ```creationDateTimeUtc``` свойства.
...
В случае успешной работы этот метод возвращает 200 код ответа Ok с набором [сущностями machineAction.](machineaction.md)
...
Вот пример запроса в организации, которая имеет три MachineActions.
...
Вот пример запроса, который фильтрует MachineActions с помощью машинного ИД и отображает последние два machineActions.
