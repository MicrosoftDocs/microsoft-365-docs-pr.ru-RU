...
MAU включает средство командной строки, называемое *msupdate,* которое предназначено для ИТ-администраторов, чтобы они могли более точно контролировать при применении обновлений. Инструкции по использованию этого средства можно найти в [update Office для Mac с помощью msupdate.](/deployoffice/mac/update-office-for-mac-using-msupdate)
...
| **Ключ** | ChannelName |
| **Тип данных** | String |
| **Возможные значения** | Бета-версия <br/> Предварительный просмотр <br/> Current |
|||
...
| **Возможные значения** | True (по умолчанию) <br/> False (Ложь) |


### <a name="disable-insider-checkbox"></a>Отключение почтового ящика Insider

Установите для true, чтобы сделать "Присоединиться к программе Office инсайдерской ..." checkbox недоступный или серый для пользователей.
...
| **Тип данных** | Логический |
| **Возможные значения** | False (по умолчанию) <br/> True (Истина) |
...
- Поместите устройство в канал Production
...
>[!WARNING]
>Приведенная ниже конфигурация — это пример конфигурации, которая не должна использоваться в производстве без надлежащего анализа параметров и настройки конфигураций.
