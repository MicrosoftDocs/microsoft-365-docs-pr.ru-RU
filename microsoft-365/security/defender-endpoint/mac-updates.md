...
---
# <a name="deploy-updates-for-microsoft-defender-for-endpoint-for-mac"></a>Развертывание обновлений для Microsoft Defender для конечной точки для Mac
...
Для обновления Microsoft Defender для конечной точки для Mac используется программа с именем Microsoft AutoUpdate (MAU). По умолчанию MAU автоматически проверяет обновления ежедневно, но их можно изменить на еженедельные, ежемесячные или вручную.
...
В MAU идентификатор приложения для Microsoft Defender для конечной точки для Mac *— WDAV00.* Чтобы скачать и установить последние обновления для Microsoft Defender для конечной точки для Mac, выполните следующую команду из окна терминала:
...
>[!WARNING]
>Этот параметр изменяет канал для всех приложений, которые обновляются с помощью Microsoft AutoUpdate. Чтобы изменить канал только для Microsoft Defender для конечной точки для Mac, выполните следующую команду после замены `[channel-name]` на нужный канал:
> ```bash
...
|:--|:--|
| **Домен** | com.microsoft.autoupdate2 |
| **Раздел** | EnableCheckForUpdatesButton |
...
|:--|:--|
| **Домен** | com.microsoft.autoupdate2 |
| **Раздел** | SendAllTelemetryEnabled |
| **Тип данных** | Логический |
| **Возможные значения** | True (по умолчанию) <br/> False |
|||
