---
title: Устранение ложных срабатыва-минусов в Microsoft Defender для конечной точки
...
keywords: оповещение, исключение, защита atp, false positive, false negative
...
ms.locfileid: "51075310"
---
# <a name="address-false-positivesnegatives-in-microsoft-defender-for-endpoint"></a>Устранение ложных срабатыва-минусов в Microsoft Defender для конечной точки
...
![Определение ложнопозитивных и отрицательных в Защитник Windows для конечных точек](images/false-positives-overview.png)

К счастью, можно предпринять шаги для устранения и уменьшения подобных проблем. Если в центре безопасности [Microsoft Defender](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/use)вы видите ложные срабатыва-ности, операции по обеспечению безопасности могут предпринять действия для их устранения с помощью следующего процесса:
...
Кроме того, вы [можете](#still-need-help) получить помощь, если после выполнения задач, описанных в этой статье, у вас по-прежнему остаются проблемы с ложными срабатываниями и отрицательными срабатываниями.

![Действия по устранению ложных срабатыва-](images/false-positives-step-diagram.png)

> [!NOTE]
> Эта статья предназначена в качестве руководства для операторов безопасности и администраторов безопасности, использующих [Microsoft Defender для конечной точки.](microsoft-defender-advanced-threat-protection.md)
...
3. Выберите оповещение, чтобы получить дополнительные сведения о оповещении. [(См. оповещение о просмотре.)](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/review-alerts)
...
> [!TIP]
> Нужна помощь с правилами подавления? См. [статью Suppress an alert and create a new suppression rule.](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/manage-alerts#suppress-an-alert-and-create-a-new-suppression-rule)
...
- [Отменять одно действие одновременно;](#undo-an-action)
- [Отмена нескольких действий одновременно;](#undo-multiple-actions-at-one-time) и 
- [Удаление файла из карантина на нескольких устройствах.](#remove-a-file-from-quarantine-across-multiple-devices) 
...
### <a name="undo-an-action"></a>Отмена действия
...
В общем, не нужно определять исключения для антивируса Microsoft Defender. Убедитесь, что исключения определяются экономно и включаются только файлы, папки, процессы и открытые процессом файлы, в результате чего срабатывалось ложное срабатывание. Кроме того, не забудьте регулярно проверять определенные исключения. Рекомендуется использовать [Microsoft Endpoint Manager](https://docs.microsoft.com/mem/endpoint-manager-overview) для определения или редактирования исключений антивирусов; Однако вы можете использовать другие [](https://docs.microsoft.com/azure/active-directory-domain-services/manage-group-policy) методы, например групповую политику (см. [в статью Управление Защитником Майкрософт для конечной точки).](manage-atp-post-migration.md)

> [!TIP]
> Нужна помощь с антивирусными исключениями? См. [в рубке Настройка и проверка исключений для проверки антивирусных программ Microsoft Defender.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/configure-exclusions-microsoft-defender-antivirus)
...
7. На **вкладке Теги Области,** если вы используете теги области в организации, укажите теги области для создаемой политики. [(См. теги Область](https://docs.microsoft.com/mem/intune/fundamentals/scope-tags).)
8. На **вкладке Назначения** укажите пользователей и группы, к которым должна применяться политика, а затем выберите **Далее**. (Если вам нужна помощь с назначениями, см. в справке Назначение профилей пользователей и устройств [в Microsoft Intune.)](https://docs.microsoft.com/mem/intune/configuration/device-profile-assign)
...
При создании [индикатора "разрешить"](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/indicator-file)для файла, например исполняемого, это помогает предотвратить блокировку файлов, которые использует ваша организация. Файлы могут включать переносные исполняемые (PE) файлы, такие как `.exe` и `.dll` файлы. 

Перед созданием индикаторов для файлов убедитесь, что следующие требования выполнены:
- Антивирус Microsoft Defender настроен с включенной облачной защитой (см. [управление облачной защитой)](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/deploy-manage-report-microsoft-defender-antivirus)
...
- Включена [функция Block или allow](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/advanced-features) 

#### <a name="indicators-for-ip-addresses-urls-or-domains"></a>Индикаторы IP-адресов, URL-адресов или доменов

При создании [индикатора "разрешить" для IP-адреса, URL-адреса](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/indicator-ip-domain)или домена это помогает предотвратить блокировку сайтов или IP-адресов, которые использует организация.

Прежде чем создавать индикаторы для IP-адресов, URL-адресов или доменов, убедитесь, что следующие требования выполнены:
- Защита сети в Защитнике для конечной точки включена в режиме блокировки (см. [включить защиту сети)](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/enable-network-protection)
...
Настраиваемые сетевые индикаторы включены в Центре безопасности Защитника Майкрософт (см. [расширенные функции)](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/advanced-features)   

#### <a name="indicators-for-application-certificates"></a>Индикаторы для сертификатов приложений 

При создании [индикатора "разрешить"](https://docs.microsoft.com/microsoft-365/security/defender-endpoint/indicator-certificates)для сертификата приложений это помогает предотвратить блокировку приложений, таких как внутренне разработанные приложения, которые использует организация. `.CER` или `.PEM` поддерживаются расширения файлов.   
...
1. Просмотрите рекомендации здесь: [Отправка файлов для анализа](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide).
...
3.  Просмотрите рекомендации здесь: [Отправка файлов для анализа](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide).
...
> [!TIP]
> Дополнительные данные см. в [материалах Отправка файлов для анализа.](https://docs.microsoft.com/windows/security/threat-protection/intelligence/submission-guide#how-does-microsoft-prioritize-submissions)
...
### <a name="cloud-delivered-protection"></a>Защита с облачным доставкой
...
> [!TIP]
> Дополнительные подробности о настройке облачной защиты см. в этой [информации.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/specify-cloud-protection-level-microsoft-defender-antivirus)

Рекомендуется использовать [Microsoft Endpoint Manager](https://docs.microsoft.com/mem/endpoint-manager-overview) для редактирования или установки параметров облачной защиты; Однако вы можете использовать другие [](https://docs.microsoft.com/azure/active-directory-domain-services/manage-group-policy) методы, например групповую политику (см. [в статью Управление Защитником Майкрософт для конечной точки).](manage-atp-post-migration.md)
...
6. На **вкладке Теги области,** если вы используете теги области в организации, укажите теги области для политики. [(См. теги Область](https://docs.microsoft.com/mem/intune/fundamentals/scope-tags).)
8. На **вкладке Назначения** укажите пользователей и группы, к которым должна применяться политика, а затем выберите **Далее**. (Если вам нужна помощь с назначениями, см. в справке Назначение профилей пользователей и устройств [в Microsoft Intune.)](https://docs.microsoft.com/mem/intune/configuration/device-profile-assign)
9. На **вкладке Обзор + создайте** вкладку, просмотрите параметры и выберите **Создать**.  
...
> [!TIP]
> Дополнительные новости о PUA см. в статью Обнаружение и [блокировка потенциально нежелательных приложений.](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-antivirus/detect-block-potentially-unwanted-apps-microsoft-defender-antivirus)
...
Рекомендуется использовать [Microsoft Endpoint Manager](https://docs.microsoft.com/mem/endpoint-manager-overview) для редактирования или установки параметров защиты PUA; Однако вы можете использовать другие [](https://docs.microsoft.com/azure/active-directory-domain-services/manage-group-policy) методы, например групповую политику (см. [в статью Управление Защитником Майкрософт для конечной точки).](manage-atp-post-migration.md)
