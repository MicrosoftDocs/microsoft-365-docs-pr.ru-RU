...
Настройка развертывания [необходимого](mac-install-with-jamf.md)пакета приложений с пакетом установки (wdav.pkg), загруженным из [Центр безопасности в Microsoft Defender.](mac-install-with-jamf.md)
...
Решение MDM может называться как "Настраиваемый Параметры профиль", так как Microsoft Defender для конечной точки на macOS не является частью macOS.

Используйте список свойств jamf/WindowsDefenderATPOnboarding.plist, который можно извлечь из бортового пакета, загруженного из [Центр безопасности в Microsoft Defender](mac-install-with-jamf.md).
