...
Microsoft Defender для конечной  точки  поддерживает поток грантов авторизации и поток учетных данных клиента для получения доступа к обнаружениям Azure Active Directory (AAD) в качестве сервера авторизации.
...
>[!NOTE]
>Центр безопасности в Microsoft Defender объединяет аналогичные обнаружения оповещений в единое оповещение. Этот API извлекает обнаружение оповещений в сыром виде на основе заданных параметров запроса, что позволяет применять собственную группировку и фильтрацию. 
...
CloudCreatedMachineTags | Строка | Теги устройств, созданные в Центр безопасности в Microsoft Defender.
