...
3. Запустите следующую команду, чтобы выполнить back up Defender для журналов Endpoint. Файлы будут храниться внутри .zip архива.
...
- На странице компьютерной информации портала Центр безопасности в Microsoft Defender, несмотря на то, что продукт работает как ожидалось, можно увидеть "Нет данных датчиков, нарушена связь". Мы работаем над решением этой проблемы.
- Во время входа в систему пользователи не отображаются на Центр безопасности в Microsoft Defender портале.
