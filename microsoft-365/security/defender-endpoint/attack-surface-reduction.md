...
---
# <a name="use-attack-surface-reduction-rules-to-prevent-malware-infection"></a>Использование правил сокращения направлений атак для предотвращения заражения вредоносными программами
...
- [Windows 10, версия 1809](/windows/whats-new/whats-new-windows-10-version-1809) или более поздней
...
Уведомления и все созданные оповещения можно просмотреть на портале Microsoft 365 Defender (ранее назывался [https://security.microsoft.com](https://security.microsoft.com) [Центр безопасности в Microsoft Defender).](microsoft-defender-security-center.md)
...
Хотя правила уменьшения поверхности атаки не требуют лицензии Windows E5, если у вас есть Windows [E5,](/windows/deployment/deploy-enterprise-licenses)вы получите расширенные возможности управления. Расширенные возможности, доступные только в Windows E5, включают:

- Мониторинг, аналитика и рабочий процесс, доступные в [Defender для конечной точки](microsoft-defender-endpoint.md)
- Возможности отчетности и конфигурации в [Microsoft 365 Defender](/microsoft-365/security/defender/overview-security-center).

Эти расширенные возможности недоступны с лицензией Windows Professional или Windows E3. Однако, если у вас есть эти лицензии, вы можете использовать viewer событий и журналы антивирусная программа в Microsoft Defender для просмотра событий правила уменьшения поверхности атаки.

## <a name="review-attack-surface-reduction-events-in-the-microsoft-365-defender-portal"></a>Просмотр событий уменьшения поверхности атаки на Microsoft 365 Defender портале

Defender for Endpoint предоставляет подробные отчеты о событиях и блоках в рамках сценариев расследования оповещений.

Вы можете запрашивать данные Defender для [конечных](microsoft-defender-security-center.md) точек в Microsoft 365 Defender с помощью [продвинутой охоты.](advanced-hunting-query-language.md) Если вы работаете в режиме [аудита,](audit-windows-defender.md)вы можете использовать расширенный режим охоты, чтобы понять, как правила уменьшения поверхности атаки могут влиять на среду.
...
2. Введите слова *Viewer*, в меню , чтобы открыть Windows просмотра событий.
...
Правило **Block для эксплуатации** уязвимых подписанных драйверов не блокирует загрузку уже существующего в системе драйвера.

>[!NOTE]
>
> Это правило можно настроить с помощью [meM OMA-URI](enable-attack-surface-reduction.md#mem) для пользовательских правил MEM OMA-URI.
>
> Вы также можете настроить это правило с помощью [PowerShell.](enable-attack-surface-reduction.md#powershell)
>
> Чтобы обследовать драйвер, используйте этот веб-сайт для отправки драйвера [для анализа.](https://www.microsoft.com/en-us/wdsi/driversubmission)
...
- [Windows 10, версия 1809](/windows/whats-new/whats-new-windows-10-version-1809)
...
Это правило блокирует запуск исполняемых файлов, таких как .exe, .dll или .scr, при условии выполнения следующих условий:

- Распространенность: исполняемые файлы находятся на более чем 1000 конечных точках
- Возраст: исполняемые файлы были выпущены более 24 часов назад
- Расположение. Исполняемые файлы включены в надежный список или список исключений

Запуск ненастоячивых или неизвестных исполняемых файлов может быть рискованным, так как изначально может быть не ясно, являются ли файлы вредоносными.
...
Вредоносные программы, Office как вектор, могут попытаться выйти из Office и сохранить вредоносные компоненты на диске. Эти вредоносные компоненты выживут при перезагрузке компьютера и будут сохраняться в системе. Поэтому это правило защищает от обычной техники сохранения.
