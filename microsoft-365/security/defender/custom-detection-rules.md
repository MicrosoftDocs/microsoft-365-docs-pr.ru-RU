...
- **МЕТОДЫ ATT&<1> <4> CK**— один или несколько методов атаки, которые определены правилом, как описано в рамках [ATT MITRE&CK](https://attack.mitre.org/). Этот раздел скрыт для определенных категорий оповещений, в том числе вредоносных программ, программ-вымогателей, подозрительных действий и нежелательного программного обеспечения
...
>[!TIP]
> Оповещения, поднятые пользовательскими обнаружениями, доступны для оповещения и API инцидентов. Дополнительные сведения см. в [сайте Supported Microsoft 365 Defender API.](api-supported.md)
