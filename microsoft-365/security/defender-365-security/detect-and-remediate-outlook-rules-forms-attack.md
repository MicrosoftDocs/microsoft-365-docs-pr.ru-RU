...
**Сводка** Узнайте, как распознавать и устранять в Office 365 правила Outlook и настраиваемые впрыски формы.

## <a name="what-is-the-outlook-rules-and-custom-forms-injection-attack"></a>Что такое атака впрыскивания Правил Outlook и настраиваемой формы?

После того как злоумышленник получит доступ к организации, он попытается закрепиться, чтобы остаться в организации или вернуться после обнаружения. Это действие называется *созданием механизма сохранения.* Существует два способа, которыми злоумышленник может использовать Outlook для создания механизма сохранения:

- Используя правила Outlook.
- Вводя настраиваемые формы в Outlook.

Переустановка Outlook или даже предоставление пострадавшему лицу нового компьютера не помогут. При подключении новой установки Outlook к почтовому ящику все правила и формы синхронизируются из облака. Правила или формы обычно предназначены для удаленного запуска кода и установки вредоносных программ на локальной машине. Вредоносные программы крадут учетные данные или выполняют другие незаконные действия.

Хорошая новость: если клиенты Outlook будут исправлены до последней версии, вы не будете уязвимы перед угрозой, так как текущие клиентские по умолчанию Outlook блокируют оба механизма.
...
2. Злоумышленник впишется в почтовый ящик Exchange этого пользователя (Exchange Online или локальной Exchange).
...
## <a name="what-a-rules-and-custom-forms-injection-attack-might-look-like-office-365"></a>Как может выглядеть атака на введение правил и пользовательских форм в Office 365?
...
- **Индикаторы компромисса Правил:**
  ...
  - На локальном компьютере и посмотрите на новые процессы, которые возникают из PID Outlook.
...
1. Откройте клиент Outlook пользователей в качестве пользователя. Пользователю может потребоваться ваша помощь в проверке правил в почтовом ящике.

2. Обратитесь [к управлению сообщениями](https://support.microsoft.com/office/c24f5dea-9465-4df4-ad17-a50704d66c59) электронной почты, используя статью правил для процедур открытия интерфейса правил в Outlook.
...
- **MailboxRulesExport-*yyy-mm-dd*.csv**: Изучите правила (по одной строке) для условий действий, которые включают приложения или исполняемые:
  ...
  - **ActionCommand (столбец G).** Если в этом столбце перечислены приложения или любой файл с расширениями .exe или .zip или неизвестный вход, который ссылается на URL-адрес, правило, скорее всего, является вредоносным.

- **MailboxFormsExport-*yyy-mm-dd.csv.*** В общем, использование настраиваемой формы встречается редко. Если вы найдете какой-либо из них в этой книге, откройте почтовый ящик этого пользователя и изучите форму. Если организация намеренно не поместила ее туда, она, скорее всего, является вредоносной.

## <a name="how-to-stop-and-remediate-the-outlook-rules-and-forms-attack"></a>Остановка и исправление атаки Outlook Rules and Forms

Если вы найдете какие-либо доказательства любой из этих атак, исправление просто, просто удалите правило или форму из почтового ящика. Это можно сделать с клиентом Outlook или с помощью удаленной PowerShell для удаления правил.
...
- Используйте такой инструмент, как [Office 365 Secure Score для](https://securescore.office.com/) управления конфигурациями и поведением безопасности учетной записи.

### <a name="second-keep-your-outlook-clients-current"></a>Во-вторых: сохранение текущих клиентов Outlook

Полностью обновленные и исправленные версии Outlook 2013 и 2016 отключают действие правила и формы "Начните приложение" по умолчанию. Это гарантирует, что даже если злоумышленник нарушает учетную запись, действие правила и формы будет заблокировано. Вы можете установить последние обновления и исправления безопасности, следуя шагам в [обновлении Install Office.](https://support.microsoft.com/office/2ab296f3-7f03-43a2-8e50-46de917611c5)
...
- [Исправление безопасности Outlook 2016](https://support.microsoft.com/help/3191883)

- [Исправление безопасности Outlook 2013](https://support.microsoft.com/help/3191938)
...
- [Вредоносные правила Outlook](https://silentbreaksecurity.com/malicious-outlook-rules/) в SilentBreak Security Post о векторе правил предоставляют подробный обзор правил Outlook.

- [MAPI над HTTP и Mailrule Pwnage](https://sensepost.com/blog/2016/mapi-over-http-and-mailrule-pwnage/) в блоге Sensepost о Mailrule Pwnage обсуждает средство под названием Ruler, которое позволяет использовать почтовые ящики через правила Outlook.

- [Формы и оболочки Outlook в](https://sensepost.com/blog/2017/outlook-forms-and-shells/) блоге Sensepost о векторе угрозы форм.
