...
---
# <a name="attack-simulator-in-office-365-atp"></a>Симулятор атак в Office 365 ATP

Симулятор атак в Office 365 Advanced Threat Protection Plan 2 (ATP Plan 2) позволяет выполнять реалистичные, но имитируемые кампании фишинга и атаки паролей в Организации. Результаты кампаний можно использовать для определения и обучения уязвимых пользователей.

## <a name="what-do-you-need-to-know-before-you-begin"></a>Что нужно знать перед началом работы?

- Чтобы открыть центр соответствия требованиям & безопасности Office 365, перейдите на <https://protection.office.com/>страницу. Симулятор атак доступен в \> **симуляторе атак**с **управлением угрозами** .

  ![Управление угрозами — симулятор атак](../../media/ThreatMgmt-AttackSimulator.png)

- Дополнительные сведения о доступности симуляторов атак в различных подписках на Office 365 приведены в статье [Описание службы Advanced Threat Protection для office 365](https://docs.microsoft.com/office365/servicedescriptions/office-365-advanced-threat-protection-service-description).

- Необходимо быть участником группы ролей " **Управление организацией** " или " **администратор безопасности** ". Дополнительные сведения о группах ролей в центре безопасности & соответствия требованиям приведены в статье [разрешения в центре безопасности & Office 365](permissions-in-the-security-and-compliance-center.md).
...
- Для пользователей с почтовыми ящиками в Exchange Online можно выполнять только кампании фишинга или атаки на пароли.
...
Для получения дополнительных сведений о phishing-атаках и фишинговых фишингах, спеар, в разделе [Фишинг](https://docs.microsoft.com/windows/security/threat-protection/intelligence/phishing).

В симуляторе атак доступны два различных типа спеар фишинговых кампаний:

- **Спеар фишинга (учетные данные)**: атака пытается убедить получателей щелкнуть URL-адрес в сообщении. Если щелкнуть ссылку, пользователям будет предложено ввести учетные данные. В этом случае они берутся из одного из следующих расположений:
...
- **Спеар фишинга (вложение)**: атака пытается убедить получателей открыть DOCX-или PDF-вложение в сообщении. Вложение содержит то же содержимое, что и адрес фишинга по умолчанию, но первое предложение начинается с\<"отображаемое имя\>", которое отображается как последнее открытое сообщение электронной почты... ".
...
1. В центре безопасности & соответствия требованиям перейдите к \> **симулятору атак** **управления угрозами** .
...
- **Пароль принудительной силы (Словарная атака)**: при атаке *методом прямого*или *словаря* используется большой файл словаря паролей для учетной записи пользователя с тем, что один из них будет работать (многие пароли для одной учетной записи). Неправильные блокировки паролей помогают предотвратить атаки пароля методом прямого доступа.
