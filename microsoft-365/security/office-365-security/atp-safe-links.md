...
|Значение|Результат|
|---|---|
|`contoso.com` <p> или <p> `*contoso.com*`|Блокирует домен, дочерние домены и пути. Например,, `https://www.contoso.com` , `https://sub.contoso.com` и `https://contoso.com/abc` они заблокированы.|
...
|`*.contoso.com/*`|Разрешает доступ к домену, поддоменам и путям (например,,, `https://www.contoso.com` `https://www.contoso.com` `https://maps.contoso.com` или `https://www.contoso.com/a` ). <p> Эта запись по сути лучше `*contoso.com*` , так как она не допускает потенциально мошеннических сайтов, например, `https://www.falsecontoso.com` или `https://www.false.contoso.completelyfalse.com`|
