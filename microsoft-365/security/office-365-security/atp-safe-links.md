...
При нажатии кнопки " **вернуться** " на странице предупреждения будет закрыта страница (или может появиться пустая страница, которую пользователи могут закрыть). Тем не менее, если щелкнуть исходную ссылку еще раз, можно получить безопасные ссылки для повторного сканирования URL-адреса, поэтому страница предупреждения появится снова.
