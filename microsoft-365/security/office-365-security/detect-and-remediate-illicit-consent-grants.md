...
**Сводка**  Узнайте, как распознать и поправить атаку незаконных грантов на согласие в Microsoft 365.

## <a name="what-is-the-illicit-consent-grant-attack-in-microsoft-365"></a>Что такое атака незаконного предоставления согласия в Microsoft 365?
...
## <a name="what-does-an-illicit-consent-grant-attack-look-like-in-microsoft-365"></a>Как выглядит атака незаконного предоставления согласия в Microsoft 365?
...
1. Откройте портал **Microsoft 365 Defender и** <https://security.microsoft.com> выберите **аудит.** Или, чтобы перейти непосредственно на страницу **Аудит,** используйте <https://security.microsoft.com/auditlogsearch> .

2. На странице **Аудит** убедитесь, что вкладка **Поиска** выбрана, а затем настройте следующие параметры:
   - **Диапазон дат и времени**
   - **Действия.** Убедитесь, **что выбраны** результаты Show для всех действий.

   Когда вы закончите, нажмите кнопку **Поиск**.

3. Щелкните **столбец Activity** для сортировки результатов и поиска **согласия на приложение.**

4. Выберите запись из списка, чтобы увидеть сведения о действии. Проверьте, задает ли IsAdminContent true.
...
>
> Продолжительность сохранения записи аудита и поиска в журнале аудита зависит от Microsoft 365 подписки и, в частности, от типа лицензии, назначенной конкретному пользователю. Дополнительные сведения см. в [журнале Аудит.](../../compliance/search-the-audit-log-in-security-and-compliance.md)
>
...
- Приложения инвентаризации и их разрешения с помощью Azure Active Directory портала. Этот метод является тщательным, но вы можете проверить только одного пользователя в то время, которое может быть очень много времени, если у вас есть много пользователей, чтобы проверить.
...
Вы можете сделать это для пользователей с помощью портала Azure Active Directory или PowerShell или у вас есть индивидуальный доступ к приложениям.

### <a name="steps-for-using-the-azure-active-directory-portal"></a>Действия для использования Azure Active Directory Portal

Вы можете искать приложения, которым каждый пользователь предоставил разрешения с помощью портала Azure Active Directory <https://portal.azure.com> по ссылке .
...
Самый простой способ проверки атаки незаконного гранта на согласие — выполнить [Get-AzureADPSPermissions.ps1, ](https://gist.github.com/psignoret/41793f8c6211d2df5051d77ca3728c09)которая сбросит все гранты на согласие OAuth и приложения OAuth для всех пользователей в вашем аренде в один .csv файл.
...
3. Откройте сеанс PowerShell в качестве администратора и откройте папку, в которой сохранен сценарий.

4. Подключение каталога с помощью [Подключение AzureAD.](/powershell/module/azuread/connect-azuread)
...
1. В столбце ConsentType (столбец G) поиск значения "AllPrinciples". Разрешение AllPrincipals позволяет клиентской приложению получать доступ к содержимому каждого в аренде. Для Microsoft 365 приложениям необходимо это разрешение для правильной работы. Каждое приложение, не в microsoft с таким разрешением, должно быть тщательно рассмотрено.
...
После завершения доступа к приложениям для  инвентаризации просмотрите журнал аудита, чтобы определить полный объем нарушения. Поиск пострадавших пользователей, сроки доступа незаконного приложения к организации и разрешения, которые у приложения были. Вы можете искать журнал **аудита** на [Microsoft 365 Defender портале](../../compliance/search-the-audit-log-in-security-and-compliance.md).
...
- Вы можете отопросить разрешение приложения на портале Azure Active Directory по:
  1. Перейдите к пострадавшему пользователю в **лезвии Azure Active Directory пользователя.**
  2. Выбор **приложений**.
  3. Выберите незаконное приложение.
  4. Нажмите **Кнопку Удалить** в упражнении вниз.
...
- Основные объекты приложения и [службы в Azure Active Directory (Azure AD)](/azure/active-directory/develop/active-directory-application-objects) предоставляют обзор основных объектов приложения и служб, которые являются ключевыми для модели приложений.
