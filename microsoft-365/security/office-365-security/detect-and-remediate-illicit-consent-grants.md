...
- Инвентаризация приложений и их разрешений с помощью портала Azure Active Directory. Этот метод является тщательным, но вы можете проверить только одного пользователя за раз, что может быть очень трудоемким, если у вас много пользователей для проверки.
...
С помощью портала [Azure Active Directory](https://portal.azure.com/)можно найти приложения, которым любой пользователь предоставил разрешения.
...
Здесь вы будете показывать приложения, которые назначены пользователю, и разрешения, которые у них есть.
...
После завершения инвентаризации доступа к приложениям просмотрите журнал аудита, чтобы определить полную область нарушения.  Поиск затронутых пользователей, время, в которое у незаконного приложения был доступ к вашей организации, и разрешения, которые у приложения есть. Поиск по **журналу аудита** можно найти в Центре безопасности и [соответствия требованиям Microsoft 365.](../../compliance/search-the-audit-log-in-security-and-compliance.md)

> [!IMPORTANT]
> [Чтобы получить](../../compliance/enable-mailbox-auditing.md) эти [](../../compliance/turn-audit-log-search-on-or-off.md) сведения, необходимо включить аудит и аудит действий почтовых ящиков для администраторов и пользователей до атаки.
...
- Вы можете отоименовать предоставление согласия OAuth с помощью PowerShell, вы следуя шагам в [remove-AzureADOAuth2PermissionGrant.](https://docs.microsoft.com/powershell/module/azuread/Remove-AzureADOAuth2PermissionGrant)
...
- Вы можете отключить интегрированные приложения для своего тенантности. Это очень важный шаг, который отключает возможность предоставления пользователями согласия на уровне клиента. Это не позволит пользователям случайно предоставить доступ к вредоносному приложению. Это не рекомендуется, так как это серьезно снижает производительность работы пользователей со сторонними приложениями. Это можно сделать, выверив или выключив интегрированные [приложения.](../../admin/misc/user-consent.md)
...
- [Управление доступом к приложениям](https://docs.microsoft.com/azure/active-directory/active-directory-managing-access-to-apps) — это обзор возможностей, которые администраторы могут использовать для управления доступом пользователей к приложениям.
