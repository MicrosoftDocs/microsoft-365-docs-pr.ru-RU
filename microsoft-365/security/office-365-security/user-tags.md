...
Теги пользователей — это идентификаторы для определенных групп пользователей в [Microsoft Defender для Office 365.](office-365-atp.md) Существует два типа тегов пользователей:

- **Системные теги**: в [настоящее время учетные записи priority](../../admin/setup/priority-accounts.md) — единственный тип системного тега.
...
- Для учетных записей с [](https://docs.microsoft.com/exchange/monitoring/mail-flow-reports/mfr-email-issues-for-priority-accounts-report) приоритетом можно использовать отчет о проблемах электронной почты для отчета об учетных записях приоритетов в Центре администрирования Exchange (EAC).
...
- Откройте Центр безопасности и соответствия требованиям по ссылке <https://protection.office.com/>. Чтобы перейти непосредственно на **страницу "Теги пользователя",** откройте <https://protection.office.com/userTags> .
...
- Для выполнения процедур, описанных в этой статье, вам должны быть назначены разрешения в Центре безопасности и соответствия требованиям:
  - Для создания, изменения и удаления тегов пользователей необходимо быть  членом группы ролей "Управление организацией" или "Администратор **безопасности".**
...
- Вы также можете управлять учетной записью приоритета и отслеживать ее в Центре администрирования Microsoft 365. Инструкции см. в под [управлением и отслеживании учетных записей приоритетов.](../../admin/setup/priority-accounts.md)
...
4. На странице **"Назначение пользователей"** сделайте один из следующих действий:
   ...
   - Нажмите **кнопку "Добавить пользователей".** Во время вылета сделайте любое из следующих действий, чтобы добавить отдельных пользователей или группы:
     ...
     - Чтобы удалить существующие записи из списка под полем, нажмите кнопку **"Удалить** ![ значок ](../../media/scc-remove-icon.png) удаления".
...
3. В появились сведения только для чтения, просмотрите параметры.
...
**Примечание.** Встроенный тег учетной записи Priority удалить **нельзя.**
