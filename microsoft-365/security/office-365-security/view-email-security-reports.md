...
- **Просмотр данных по: контенту \> Malware**<sup>1</sup>: следующие сведения отображаются для защитника майкрософт для Office 365:
  ...
  - **Модуль защиты от вредоносных программ** : перехват вредоносных файлов в SharePoint Online, OneDrive и Teams с помощью защиты от вредоносных программ.
  - **Файл детонации** : детонации вредоносных файлов в SharePoint Online, OneDrive и Teams, используя безопасные вложения.
...
- **Разделите на: технология обнаружения** и **Просмотр данных по: \> Фишинг электронной почты** : отображаются следующие сведения:
  ...
  - **Созданная для ATP репутация URL-адресов**<sup>1</sup>: вредоносная репутация URL-адресов, созданная из защитника для Office 365 детонатионс в других защитниках для пользователей Office 365.
...
- **Разделите на: технология обнаружения** и **Просмотр данных по: \> вредоносные программы электронной почты** : отображаются следующие сведения:
  ...
  - **Созданная для ATP репутация файла**<sup>1</sup>: все вредоносные файлы, созданные ATP детонатионс.
  ...
  - **Файл детонации**<sup>1</sup>: файл детонации перехватывается безопасными вложениями.  
...
В **случае \> вредоносных программ для контента** можно изменить отчет **по дате начала** и **дате окончания** , а также значение **определения** .

Для **переопределения сообщений** можно изменить отчет с помощью следующих фильтров:

- **Дата начала** и **Дата окончания**
- **Причина переопределения**
- **Тег** : фильтр по тегу, который возвращает пользователей или группы, к которым применен определенный тег. Дополнительные сведения о тегах пользователя приведены в статье [теги пользователей](user-tags.md).
- **Домен**
