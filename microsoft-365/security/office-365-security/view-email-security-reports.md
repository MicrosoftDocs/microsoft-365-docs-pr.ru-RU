...
**Отчет о задержке почты** содержит сведения о доставке почты и задержках детонации в вашей организации. Дополнительные сведения см. в [отчете о задержке почты](view-reports-for-atp.md#mail-latency-report).
...
Отчет **о состоянии защиты от угроз** доступен как в EOP, так и в защитнике Майкрософт для Office 365; Тем не менее, отчеты содержат различные данные. Например, пользователи EOP могут просматривать сведения о вредоносных программах, обнаруженных в электронной почте, но не могут получить сведения о вредоносных файлах, обнаруженных службой [ATP для SharePoint, OneDrive или Microsoft Teams](atp-for-spo-odb-and-teams.md).

В отчете представлено количество сообщений электронной почты с вредоносным содержимым, такими как файлы или адреса веб-сайтов (URL-адреса), которые были заблокированы ядром защиты от вредоносных программ, [Автоматическая очистка (ZAP)](zero-hour-auto-purge.md)и защитник для Office 365, такие как [безопасные ссылки](atp-safe-links.md), [безопасные вложения](atp-safe-attachments.md)и [Защита от фишинга](set-up-anti-phishing-policies.md). Эти сведения можно использовать для определения тенденций или определения необходимости коррекции политик организации.

**Note**: важно понимать, что если сообщение отправлено пяти получателям, оно подсчитано как пять разных сообщений, а не одно сообщение.
...
- Защитник Майкрософт для Office 365: <https://protection.office.com/reportv2?id=TPSAggregateReportATP>
...
- **Просмотр данных по: контенту \> Malware**<sup>1</sup>: следующие сведения отображаются для защитника майкрософт для Office 365:

  - **Модуль защиты от вредоносных программ**: вредоносные файлы, обнаруженные в SharePoint, OneDrive и Microsoft Teams, [встроенными средствами обнаружения вирусов в Microsoft 365](virus-detection-in-spo.md).
  - **Файл детонации**: вредоносные файлы, обнаруженные [ATP для SharePoint, OneDrive и Microsoft Teams](atp-for-spo-odb-and-teams.md).
...
- **Просмотр данных по: переопределению сообщений**: отображаются следующие сведения о причине переопределения:

  - **Пропуск локального решения**
  - **Разрешить IP-адрес**
  - **Правило для почтового процесса**
  - **Разрешить отправку**
  - **Разрешить домен**
  - **ZAP не включен**
  - **Папка нежелательной почты не включена**
  - **Надежный отправитель пользователя**
  - **Надежный домен пользователя**
...
- **Разделите на: технология обнаружения** и **Просмотр данных по: \> Фишинг электронной почты**: отображаются следующие сведения:
  ...
  - **Созданная для ATP репутация URL-адресов**<sup>1</sup>: вредоносная репутация URL-адресов, созданная из защитника для Office 365 детонатионс в других клиентах Microsoft 365.
  - **Расширенный фильтр фишинга**: фишинговые сигналы на основе машинного обучения.
  - **Сбой защиты от подделки — DMARC**: ошибка проверки подлинности DMARC для сообщений.
  - **Защита от подделки-внутри-org**: отправитель пытается подменить домен получателя.
  - **Защита от спуфинга — внешний домен**: отправитель пытается подменить другой домен.
  - **Олицетворение торговой марки**: олицетворение хорошо известных торговых марок на основе отправителей.
  - **Олицетворение домена**<sup>1</sup>: олицетворение доменов, которые пользователь владеет или определяет.
  - **Репутация URL-адресов EOP**: репутация вредоносных URL-адресов.
  - **Общий фильтр фишинга**: фишинговые сигналы на основе правил аналитики.
  - **Другие**
  - **ФИШИНГ ZAP**<sup>2</sup>: автоматическая очистка фишинговых сообщений в течение нулевого часа.
  - **URL-адрес детонации**<sup>1</sup>
  - **Олицетворение пользователя**<sup>1</sup>: олицетворение пользователей, определенных администратором или изученным через интеллектуальную аналитику почтовых ящиков.
...
- **Разделите на: технология обнаружения** и **Просмотр данных по: \> вредоносные программы электронной почты**: отображаются следующие сведения:
  ...
  - **Репутация файла, созданного ATP**:<sup>1</sup>: вся вредоносная репутация файлов, созданная защитником для Office 365 детонатионс.
  - **Модуль защиты от вредоносных программ**<sup>1</sup>: обнаружение антивредоносных ядер.
  - **Блокировка типов файлов политики защиты от вредоносных программ**: это сообщения электронной почты, отфильтрованные из-за типа вредоносных файлов, определенных в сообщении.
  - **Файл детонации**<sup>1</sup>: обнаружение с помощью безопасных вложений.
...
При нажатии кнопки **фильтры** доступные фильтры зависят от диаграммы, которую Вы искали:

- Для **представления данных по: контентная \> вредоносная программа** можно изменять отчет по **дате начала** и **дате окончания**, а также значение **обнаружения** .

- Для **представления данных по: переопределению сообщений** можно изменить отчет с помощью следующих фильтров:

  - **Дата начала** и **Дата окончания**
  - **Причина переопределения**
  - **Тег**: Фильтрация результатов по пользователям или группам, к которым применен указанный тег пользователя (включая учетные записи приоритетов). Дополнительные сведения о тегах пользователя приведены в статье [теги пользователей](user-tags.md).
  - **Домен**
...
- **Просмотр данных по: контенту \> Вредоносные программы**:
  ...
  Если щелкнуть **фильтры** в этом представлении, можно изменить отчет по **дате начала** и **дате окончания**, а также **определить значение.**

- **Просмотр данных по: переопределению сообщений**:
...
**Отчет по защите от угроз для URL-адресов** доступен в защитнике Майкрософт для Office 365. Дополнительные сведения см. в разделе [URL-адрес защиты угроз](view-reports-for-atp.md#url-threat-protection-report).
