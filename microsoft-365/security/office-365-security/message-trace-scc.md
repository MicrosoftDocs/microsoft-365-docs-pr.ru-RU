...
Чтобы отсортировать результаты по значениям в этом столбце по возрастанию или убыванию, можно щелкнуть заглавные столбцы столбца.
...
После того как вы нажмете на строку, вы сможете  найти соответствующие записи для  сообщения, нажав кнопку "Найти связанное" или выбрав "Дополнительные параметры" "Дополнительные записи для этого ![ ](../../media/1ea52bbf-9d00-48ce-9362-307f7f6fb7fe.png) \> сообщения").
...
- **Recipient_status**: состояние доставки сообщения получателю. Если сообщение было отправлено нескольким получателям, в нем будут отображиться все получатели и соответствующее состояние для каждого из них в формате: \<*email address*\> ## \<*status*\> . Пример:
...
- **reference**: это поле содержит дополнительные сведения для определенных типов событий. Пример:
