...
- Для использования трассировки сообщений  необходимо быть  членом групп  ролей в области управления **организацией,** управления соответствием требованиям или службы поддержки в Exchange Online. Дополнительные сведения см. в статье [Разрешения в Exchange Online](/exchange/permissions-exo/permissions-exo).
  ...
  **Примечание.** Членство в соответствующей роли Azure Active Directory в центре администрирования Microsoft 365 предоставляет  пользователям необходимые разрешения и разрешения для других функций в Microsoft 365. Дополнительные сведения см. в статье [О ролях администраторов](../../admin/add-users/about-admin-roles.md).

- Максимальное количество сообщений, отображаемых в результатах трассировки сообщений, зависит от выбранного типа отчета (подробнее см. раздел [Выберите](#choose-report-type) тип отчета). Комлет [Get-HistoricalSearch](/powershell/module/exchange/get-historicalsearch) в Exchange Online PowerShell или автономный EOP PowerShell возвращает все сообщения в результатах.

## <a name="open-message-trace"></a>Трассировка открытого сообщения

Откройте центр & безопасности в центре соответствия требованиям, а затем перейдите <https://protection.office.com/> к трассировку **сообщения потока** \> **почты.**

Чтобы перейти непосредственно на **страницу трассировки сообщения,** откройте <https://protection.office.com/messagetrace> .
...
- **Получение статуса:** Сообщение было недавно получено Microsoft 365, но других данных о состоянии пока нет. Возвращайся через несколько минут.
...
> - Максимальный размер загружаемого отчета — 500 МБ. Если загружаемый отчет превышает 500 МБ, открыть его в Excel или Блокнот.
...
- **Дополнительные сведения.** В этом разделе содержатся следующие сведения:
  ...
  - **Размер сообщения**.
  - **С IP:** IP-адрес компьютера, отправив сообщение. Для исходящие сообщения, отправленные из Exchange Online, это значение пусто.
  - **IP:** IP-адрес или адреса, на которых служба попыталась доставить сообщение. Если в сообщении несколько получателей, они отображаются. Для входящие сообщения, отправленные в Exchange Online, это значение пусто.
...
- **Recipient_status:** состояние доставки сообщения получателю. Если сообщение было отправлено нескольким получателям, оно будет показывать всех получателей и соответствующий статус для каждого из них в формате: \<*email address*\> ## \<*status*\> . Пример.
...
- **connector_id:** имя соединитетеля источника или назначения. Дополнительные сведения о соединители в Exchange Online см. в [Office 365.](/Exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/use-connectors-to-configure-mail-flow)
...
- **source_context**: Дополнительные сведения, связанные с **исходным полем.** Пример.
  ...
  - `3489061114359050000`
- **источник:** Exchange Online, который отвечает за событие. Пример.
  - `AGENT`
  - `MAILBOXRULE`
  - `SMTP`
...
- **справка.** Это поле содержит дополнительные сведения для определенных типов событий. Пример.
  ...
  - **MAILBOXRULE.** Содержит **internal_message_id** входящих сообщений, из-за чего правило "Входящие" создает исходящие сообщения. Для других типов событий это поле обычно пусто.
...
- **message_info:** Дополнительные сведения о сообщении. Пример.
  - Дата начала сообщения в UTC и `DELIVER` `SEND` события. Дата начала — это время, когда сообщение впервые вступило в Exchange Online организации. Дата UTC представлена в формате дата-времени ISO 8601: , где = год, = месяц, = день, указывает начало компонента `yyyy-mm-ddThh:mm:ss.fffZ` `yyyy` `mm` `dd` `T` времени, = час, = минута, = секунда, = доли `hh` `mm` секунды, `ss` `fff` `Z` и означает `Zulu` , что это еще один способ обозначить UTC.
...
- **tenant_id**: значение GUID, представляю Exchange Online организации `39238e87-b5ab-4ef6-a559-af54c6b07b42` (например).
...
Поле **custom_data** для события используется различными агентами Exchange Online для входа в данные `AGENTINFO` обработки сообщений. Некоторые из наиболее интересных агентов описаны в следующих разделах.
...
|`Action=<ActionDefinition>`|Примененное действие. Список доступных действий см. в списке действий правила потока [почты в Exchange Online.](/exchange/security-and-compliance/mail-flow-rules/mail-flow-rule-actions)|
|`Mode=<Mode>`|Режим правила. Допустимые значения:<ul><li>**Применение.** Все действия по правилу будут применены.</li><li>**Test with Policy Советы:**: Любые действия наконечника политики будут отправлены, но другие действия по правоприменительных действий не будут действовать.</li><li>**Test without Policy Советы:** Действия будут перечислены в файле журнала, но отправители не будут уведомлены в любом случае, и действия по принужжительнию не будут действовать.</li></ul>|
|
