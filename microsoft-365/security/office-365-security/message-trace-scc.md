...
- **Recipient_status**: состояние доставки сообщения получателю. Если сообщение было отправлено нескольким получателям, отображаются все получатели и соответствующее состояние каждого из них в формате: \<*email address*\> ## \<*status*\> . Например:
