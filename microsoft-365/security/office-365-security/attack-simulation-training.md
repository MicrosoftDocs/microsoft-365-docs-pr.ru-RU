...
> [!NOTE]
> В любой момент во время создания имитации можно сохранить и закрыть, чтобы продолжить настройку имитации в дальнейшем.
...
- **Ссылка на вредоносные** программы будет запускать произвольный код из файла, хранящегося в известной службе общего доступа к файлам. Сообщение, отправленное пользователю, будет содержать ссылку на этот вредоносный файл. Открытие файла и помощь злоумышленнику в компрометации устройства целевого устройства.
...
- **Запущенные имитации** подсчитывают количество случаев использования этих полезной нагрузки в других имитациях.
- **Сложность,** доступная через **фильтры,** рассчитывается на основе количества индикаторов в полезной нагрузке, которые подсказывают, что целью атаки является атака. Чем больше индикаторов, тем сложнее.
...
Теперь, когда все настроено, вы можете запустить это моделирование немедленно или запланировать его на более позднее время. Вам также потребуется выбрать время окончания моделирования. Мы перестанет захватывать взаимодействие с этим моделированием после выбранного времени.

**Включить доставку часовой** пояс с учетом региона для доставки сообщений о имитации атак сотрудникам в рабочее время в зависимости от региона.
