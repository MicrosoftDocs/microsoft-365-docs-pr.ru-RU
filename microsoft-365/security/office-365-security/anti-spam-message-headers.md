...
|ARC|В протоколе ARC имеются следующие заголовки: <br/>• AAR. Записывает содержимое заголовка результатов проверки подлинности из DMARC. <br/>• AMS. Этот заголовок включает криптографические подписи сообщения. <br/>• AS. Включает криптографические подписи заголовков сообщений. Этот заголовок содержит тег проверки цепочки "cv=", включающий результат проверки цепочки в виде **none**, **pass** или **fail**.|
|CAT:|Категория политики защиты, применяемая для сообщения: <br/>MALW. Вредоносная программа <br/>HPHSH или HPHISH. Фишинг с высокой степенью уверенности <br/>PHSH. Фишинг <br/>HSPM. Нежелательное сообщение с высокой степенью уверенности <br/>SPOOF. Спуфинг <br/>SPM. Нежелательное сообщение <br/>BULK. Массовая рассылка <br/>DIMP. Подмена домена <br/>UIMP. Подмена пользователя <br/>GIMP. Аналитика почтовых ящиков<br/>Входящее сообщение может помечаться несколькими формами защиты и несколькими сканерами обнаружения. Политики имеют разные приоритеты, и применяется политика с наивысшим приоритетом. См. статью [Какая политика применяется, когда для электронной почты запускается несколько методов защиты и сканеров обнаружения](how-policies-and-protections-are-combined.md).|
...
|**Поле заголовка**|**Описание**|
|:-----|:-----|
|BCL|Уровень жалоб на массовую рассылку (BCL) сообщения помогает распознавать различные источники массовой рассылки. Он представлен в виде значения от 0 до 9. Информация о значениях и дополнительные сведения по BCL см. в статье [Уровни жалоб на массовые сообщения](bulk-complaint-level-values.md).|
