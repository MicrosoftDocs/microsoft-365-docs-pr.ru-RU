...
- Защита от нежелательной почты
  - Политики нежелательной почты (также известные как политики фильтрации содержимого)
  - Политики фильтрации исходящие нежелательной почты
  - Политики фильтрации подключений
- Политики для борьбы с вредоносными программами
- Соединители
- Правила потока почты (также известные как правила транспорта)

  > [!NOTE]
  > В настоящее время поддержка экспорта и импорта коллекции правил потока почты поддерживается только для планов подписки EOP Premium.
...
В автономных EOP PowerShell создайте каталог Export в расположении, которое легко найти и изменить на этот каталог. Например,
...
Следующий сценарий можно использовать для сбора всех пользователей почты, групп, параметров нежелательной почты, параметров антивредоносной программы, соединитеителей и правил потока почты в организации-источнике. Скопируйте и вставьте приведенный ниже текст в текстовый редактор (например, Блокнот), сохраните файл под именем Source_EOP_Settings.ps1 в только что созданном каталоге Export, а затем выполните следующую команду.
...
Теперь вы можете просмотреть и собрать сведения из Центра администрирования Microsoft 365 целевой организации, чтобы быстро проверить свои домены, когда придет время:

1. Во sign in to the Microsoft 365 admin center at <https://portal.office.com> .

2. Щелкните **Домены**.

   Если домены не отображут, нажмите кнопку **"Настройка** навигации", выберите "Настройка" и **"Сохранить".**
...
6. Добавьте проверочные записи типа TXT в записи DNS. Это позволит быстрее проверить домены в организации-источнике после их удаления из целевой организации. Дополнительные сведения о настройке DNS см. в записях создания DNS у любого поставщика услуг размещения [DNS для Microsoft 365.](../../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md)
...
Дополнительные сведения о настройке DNS см. в записях создания DNS у любого поставщика услуг размещения [DNS для Microsoft 365.](../../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md)
...
Следующий сценарий удаляет пользователей, группы и домены из клиента-источника с помощью Azure Active Directory PowerShell. Скопируйте и вставьте приведенный ниже текст в текстовый редактор (например, Блокнот), сохраните файл как C:\EOP\Export\Remove_Users_and_Groups.ps1 и выполните следующую команду.
...
1. Во sign in to the admin center at <https://portal.office.com> .
...
Для EOP лучше всего использовать Azure Active Directory для синхронизации локальной службы Active Directory с целевым клиентом. Дополнительные сведения о том, как это сделать, см. в подсети "Управление почтовыми пользователями с помощью синхронизации каталогов" в подсети "Управление [почтовыми пользователями в EOP".](manage-mail-users-in-eop.md) Вы также можете использовать следующий сценарий для воссоздания пользователей и групп из вашего клиента-источника. Примечание. Пароли пользователей не могут быть перемещены. Новые пароли пользователей создаются и сохраняются в файле с именем UsersAndGroups.ps1.
...
Если вы решили установить недопустимый адрес для записей MX, чтобы отправителю было необходимо в очереди отправлять почту во время перехода, необходимо вернуть для них правильное значение, указанное в Центре [администрирования.](https://admin.microsoft.com) Дополнительные сведения о настройке DNS см. в записях создания DNS у любого поставщика услуг размещения [DNS для Microsoft 365.](../../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md)
