...
> [!NOTE]
>
> - Настройки можно надежно переместить только из автономной (стандартной) организации EOP в другую стандартную организацию EOP или организацию, использующую клиентскую лицензию Exchange Enterprise CAL со службами (расширенную организацию EOP), а также из расширенной организации EOP в другую организацию аналогичного типа. Так как некоторые функции премиум-класса не поддерживаются в организациях стандарта EOP, переходы из организации EOP Premium в организацию стандарта EOP могут оказаться не успешными.
>
> - Эти инструкции предназначены для организаций EOP, выполняющих только фильтрацию. При перемещении данных из одной организации Exchange Online в другую следует придерживаться дополнительных рекомендаций. Эти инструкции не распространяются на организации Exchange Online.
...
- Защита от нежелательной почты
  - Политики по борьбе со спамом (также известные как политики фильтрации контента)
  - Политики фильтрации исходящие нежелательной почты
  - Политики фильтра подключения
- Политики по борьбе с вредоносными программами
- Соединители
- Правила потока почты (также известные как правила транспорта)

  > [!NOTE]
  > Поддержка cmdlet для экспорта и импорта коллекции правил потока почты в настоящее время поддерживается только для планов подписки на EOP Premium.

Самый простой способ собрать все параметры — использовать PowerShell. Чтобы подключиться к автономному EOP PowerShell, см. раздел [Подключение к PowerShell Exchange Online Protection](/powershell/exchange/connect-to-exchange-online-protection-powershell).
...
В автономных EOP PowerShell создайте каталог export в месте, которое легко найти и изменить в этом каталоге. Например:
...
Теперь вы можете просмотреть и собрать сведения из центра администрирования Microsoft 365 целевой организации, чтобы быстро проверить домены, когда придет время:

1. Во входе в центр администрирования Microsoft 365 на <https://portal.office.com> сайте .

2. Щелкните **Домены**.

   Если вы не видите домены, нажмите **кнопку Настройка** навигации, выберите **установку** и нажмите **кнопку Сохранить**.
...
6. Добавьте проверочные записи типа TXT в записи DNS. Это позволит быстрее проверять домены в организации-источнике после их удаления из целевой организации. Дополнительные сведения о настройке DNS см. в дополнительных сведениях о создании записей DNS в любом [поставщике DNS-хостинга для Microsoft 365.](../../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md)
...
Дополнительные сведения о настройке DNS см. в дополнительных сведениях о создании записей DNS в любом [поставщике DNS-хостинга для Microsoft 365.](../../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md)
...
Следующий сценарий удаляет пользователей, группы и домены из исходных клиентов с помощью Azure Active Directory PowerShell. Скопируйте и вставьте приведенный ниже текст в текстовый редактор (например, Блокнот), сохраните файл как C:\EOP\Export\Remove_Users_and_Groups.ps1 и выполните следующую команду.
...
1. Вход в центр администрирования по <https://portal.office.com> крайней .
...
Лучше всего использовать Azure Active Directory для синхронизации локального Active Directory с целевым клиентом. Дополнительные сведения о том, как это сделать, см. в "Использование синхронизации каталогов для управления пользователями почты" в "Управление пользователями почты [в EOP".](manage-mail-users-in-eop.md) Вы также можете использовать следующий скрипт для воссоздания пользователей и групп из исходных клиентов. Примечание. Пароли пользователей не могут быть перемещены. Новые пароли пользователей создаются и сохраняются в файле с именем UsersAndGroups.ps1.
...
Если вы решили настроить записи MX на недействительный адрес, чтобы вызвать очереди отправителей почты во время перехода, необходимо настроить их обратно к правильному значению, указанному в центре [администрирования.](https://admin.microsoft.com) Дополнительные сведения о настройке DNS см. в дополнительных сведениях о создании записей DNS в любом [поставщике DNS-хостинга для Microsoft 365.](../../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md)