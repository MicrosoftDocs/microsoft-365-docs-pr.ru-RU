...
---
# <a name="anti-spam-protection-faq-in-office-365"></a>Вопросы и ответы по защите от нежелательной почты в Office 365

В этом разделе приведены часто задаваемые вопросы и ответы о защите от нежелательной почты для пользователей Office 365 с почтовыми ящиками в Exchange Online или отдельном клиенте Exchange Online Protection (EOP) без почтовых ящиков Exchange Online.
...
О. **Для входящих сообщений**: большая часть спама удаляется с помощью фильтрации подключений, основанной на IP-адресе исходного сервера электронной почты. Политики защиты от нежелательной почты (также называемые политиками фильтрации нежелательной почты или фильтром содержимого) анализируют и классифицируют сообщения как спам, массовый или фишинг. По умолчанию сообщения, которые классифицируются как нежелательная почта или массовые, доставляются в папку нежелательной почты получателя, а сообщения, классифицированные как фишингы, отправляются в карантин. Вы можете изменить политику защиты от нежелательной почты по умолчанию (применяется ко всем получателям) или создать настраиваемые политики защиты от нежелательной почты с более жесткими настройками для определенных групп пользователей (например, можно отправить нежелательные сообщения на карантин руководителю). Дополнительные сведения см. [в статье Настройка политик защиты от нежелательной почты в Office 365](configure-your-spam-filter-policies.md) и [рекомендуемых параметрах политики защиты от нежелательной почты](recommended-settings-for-eop-and-office365-atp.md#eop-anti-spam-policy-settings).
 ...
 **Для исходящих сообщений**: сообщение направляется через [пул доставки с высоким уровнем риска](high-risk-delivery-pool-for-outbound-messages.md) или возвращается отправителю в отчете о недоставке (также называемом сообщением о недоставке или сообщении Bounce). Дополнительные сведения о защите от нежелательной почты см [в разделе элементы управления нежелательной почтой в Office 365](outbound-spam-controls.md).
...
О. Нежелательные сообщения и обычные сообщения могут отправляться корпорации Майкрософт для анализа несколькими способами. Для получения дополнительных сведений просмотрите [сообщения отчетов и файлы в корпорацию Майкрософт](report-junk-email-messages-to-microsoft.md).
...
О. Да. Несмотря на то, что мы рекомендуем указать для записи MX значение Office 365, мы понимаем, что в первую очередь вы намерены использовать почтовые сообщения в других случаях, кроме Office 365.
...
- **Исходящие**: Настройка маршрутизации промежуточных узлов из Office 365 в конечный сторонний поставщик.
