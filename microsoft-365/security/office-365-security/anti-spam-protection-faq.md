...
В этом разделе даны вопросы и ответы о защите от вредоносных программ для организаций Microsoft 365 с почтовыми ящиками в Exchange Online или автономных организациях Exchange Online Protection (EOP) без почтовых ящиков Exchange Online.
...
**Для входящие сообщений:** Большая часть спама удаляется с помощью фильтрации подключений, основанной на IP-адресе сервера электронной почты источника. Политики защиты от нежелательной почты (также известные как политики фильтрации нежелательной почты или политики фильтрации содержимого) проверяют и классифицируют сообщения как нежелательные, массовые или фишинговые. По умолчанию сообщения, классифицированные как нежелательные или массовые, доставляются в папку нежелательной почты получателя, а сообщения, классифицированные как фишинговые, находятся в карантине. Вы можете изменить политику нежелательной почты по умолчанию (применяется ко всем получателям) или создать настраиваемые политики нежелательной почты с более строгими настройками для определенных групп пользователей (например, можно карантин нежелательной почты, отсылаемой руководителям). Дополнительные сведения см. в подстройке ["Настройка](configure-your-spam-filter-policies.md) политик нежелательной почты" и ["Рекомендуемые](recommended-settings-for-eop-and-office365-atp.md#eop-anti-spam-policy-settings)параметры политики".

> [!IMPORTANT]
> В гибридных развертываниях, где EOP защищает почтовые ящики в локальной организации, необходимо настроить два правила потока почты Exchange (также известные как правила транспорта) в локальной организации Exchange, чтобы обнаруживать добавленные в сообщения загон фильтрации нежелательной почты EOP. Дополнительные сведения см. в статье [Настройка автономной службы EOP для доставки спама в папку нежелательной почты в гибридных средах](ensure-that-spam-is-routed-to-each-user-s-junk-email-folder.md).

 **Для исходящие сообщения:** Сообщение направляется через пул [](high-risk-delivery-pool-for-outbound-messages.md) доставки с высоким уровнем риска или возвращается отправителю в отчете о не доставке (также известном как отчет о возврате). Дополнительные сведения о защите от нежелательной почты для исходящие сообщения см. в [этой теме.](outbound-spam-controls.md)
...
После регистрации в службе и добавления домена фильтрация нежелательной почты автоматически включена. По умолчанию фильтрация нежелательной почты настроена для защиты без необходимости дополнительной настройки (за исключением ранее отмеченного исключения для автономных пользователей EOP в гибридных средах). Как администратор вы можете изменить параметры фильтрации нежелательной почты по умолчанию, чтобы наилучшим образом удовлетворить потребности вашей организации. Для большей детализации можно также создать политики и политики исходящие нежелательной почты, применяемые к указанным пользователям, группам или доменам в организации. Настраиваемые политики всегда имеют приоритет перед политикой по умолчанию, но вы можете изменить приоритеты (то есть порядок применения) своих настраиваемых политик.
...
Да. Дополнительные сведения о массовой рассылке электронной почты см. в сведениях о различиях между нежелательной и [массовой рассылкой электронной почты?](what-s-the-difference-between-junk-email-and-bulk-email.md)

## <a name="does-the-service-provide-url-filtering"></a>Обеспечивает ли служба фильтрацию URL-адресов?

Да, служба имеет фильтр URL-адресов, который проверяет наличие URL-адресов в сообщениях. Если будет обнаружен URL-адрес, связанный с заведомо нежелательным или вредоносным содержимым, сообщение будет помечено как спам.
...
Клиенты Exchange Online: [мониторинг, отчеты](https://docs.microsoft.com/exchange/monitoring/monitoring) и трассировка сообщений в Exchange Online

Пользователи автономных EOP: [отчеты и трассировка сообщений в Exchange Online Protection](reporting-and-message-trace-in-exchange-online-protection.md)

## <a name="someone-sent-me-a-message-and-i-cant-find-it-i-suspect-that-it-may-have-been-detected-as-spam-is-there-a-tool-that-i-can-use-to-find-out"></a>Кто-то отправил мне сообщение, и я не могу найти его. Я думаю, что, возможно, оно было распознано как нежелательная почта. Есть ли какое-нибудь средство, которое поможет найти это сообщение?

Да, средство трассировки сообщений позволяет отслеживать сообщения электронной почты по мере их прохода через службу, чтобы узнать, что с ними произошло. Дополнительные сведения об использовании средства трассировки сообщений для получения сведений о том, почему сообщение было помечено как нежелательное, см. в этой [теме.](https://docs.microsoft.com/exchange/monitoring/trace-an-email-message/message-trace-faq#was-a-message-marked-as-spam)
...
Если более половины почты, отправляемой пользователем через службу в течение определенного времени (например, в час), определяется службой EOP как спам, отправка сообщений пользователю будет заблокирована. В большинстве случаев, если исходящее сообщение определено как спам, оно отправляется через пул доставки высокого риска, что снижает вероятность добавления в список блокировки обычного пула исходящих IP-адресов.
...
Да. Хотя мы рекомендуем указать запись MX корпорации Майкрософт, мы понимаем, что существуют законные бизнес-причины для того, чтобы сначала перенаказать вашу электронную почту куда-то, кроме Майкрософт.

- **Inbound**: Change your MX records to point to the third-party provider, and then redirect the messages to EOP for additional processing. Дополнительные сведения см. в [расширенной фильтрации соединителях в Exchange Online.](https://docs.microsoft.com/exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/enhanced-filtering-for-connectors)

- **Исходящий:** настройте маршрутику через smart host из Microsoft 365 к сторонней службе назначения.
...
Да. Дополнительные сведения см. в под вопросе ["Защита конфиденциальности в Интернете"](https://support.microsoft.com/help/4091455)
...
- **Исходный домен электронной почты должен разрешаться в DNS.**
  ...
  Например, если отправитель user@fabrikam, домен fabrikam сопоберется с IP-адресом 192.0.43.10.

  Если отправляющий домен не имеет записи A и записи MX в DNS, служба будет маршрутизировать сообщение через свой пул доставки более высокого риска независимо от того, является ли содержимое сообщения нежелательной почтой. Дополнительные сведения о пуле доставки с более высоким уровнем риска см. в пуле доставки сообщений с высоким [уровнем риска.](high-risk-delivery-pool-for-outbound-messages.md)

- **Сервер исходящие почты должен иметь обратную запись DNS (PTR).**

  Например, если IP-адрес источника электронной почты — 192.0.43.10, обратная запись DNS будет `43-10.any.icann.org` .'
...
- **Форматирование исходящие сообщения о возврате.**

  При создании уведомлений о доставке (также известных как отчеты о доставке, отчеты о доставке или сообщения о возврате) отправителю следует следовать формату отскока, как указано в [документе RFC 3464.](https://www.ietf.org/rfc/rfc3464.txt)
