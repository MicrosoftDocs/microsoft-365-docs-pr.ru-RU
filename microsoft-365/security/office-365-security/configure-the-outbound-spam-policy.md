...
Если вы являетесь клиентом Office 365 с почтовыми ящиками в Exchange Online или отдельном клиенте Exchange Online Protection (EOP) без почтовых ящиков Exchange Online, исходящие сообщения электронной почты, отправляемые через EOP, автоматически проверяются на наличие нежелательной почты Отправка действий.
...
- Политика с именем Default имеет значение приоритета, которое **меньше** , чем вы можете изменить (политика всегда применяется последней). Все создаваемые настраиваемые политики всегда имеют более высокий приоритет, чем политика с именем Default.
...
- Вы открываете центр обеспечения безопасности & по <https://protection.office.com/>адресу. Чтобы перейти непосредственно на страницу **параметров защиты от нежелательной почты** , используйте <https://protection.office.com/antispam>.
...
- Перед выполнением этих процедур необходимо назначить разрешения. Для добавления, изменения и удаления политик исходящей нежелательной почты необходимо быть членом группы ролей " **Управление организацией** " или " **администратор безопасности** ". Для доступа только для чтения к политикам исходящей нежелательной почты необходимо быть участником группы ролей " **читатель безопасности** ". Дополнительные сведения о группах ролей в центре безопасности & соответствия требованиям приведены в статье [разрешения в центре безопасности & Office 365](permissions-in-the-security-and-compliance-center.md).
...
1. В центре безопасности & соответствия требованиям перейдите к разделу \> **Политика** \> **управления угрозами** **защиты от нежелательной почты**.
...
3. В открывшемся диалоговом окне **Политика фильтрации нежелательной почты** настройте указанные ниже параметры.
   ...
   - **Name**: введите уникальное описательное имя политики.
...
4. Необязательно Разверните раздел **уведомления** , чтобы настроить дополнительных пользователей, которые должны получать копии и уведомления о подозрительных исходящих сообщениях электронной почты:
   ...
   - **Отправлять копии подозрительных исходящих сообщений электронной почты определенным людям**: этот параметр добавляет указанных пользователей в качестве получателей скрытой копии в подозрительные исходящие сообщения. Чтобы включить этот параметр, выполните указанные ниже действия.
     ...
     в. Введите адрес электронной почты отправителя. Можно указать несколько адресов электронной почты, разделенных точкой с запятой (;) или по одному получателю в строке.

     г. Чтобы создать группу обеспечения доступности баз данных, щелкните ![Значок "Добавить"](../../media/c2dd8b3a-5a22-412c-a7fa-143f5b2b5612.png) для добавления получателей.

        Повторите эти действия столько раз, сколько необходимо.
     ...
     д. Когда закончите, нажмите кнопку **Сохранить**.
...
6. Потребоваться Разверните раздел **применено** , чтобы определить внутренних отправителей, к которым применяется политика.
    ...
    Проще всего нажать кнопку **Добавить условие** три раза, чтобы увидеть все доступные условия. Можно нажать кнопку ![удалить,](../../media/scc-remove-icon.png) чтобы удалить условия, которые не нужно настраивать.

    - **Домен отправителя**: указывает отправители из одного или нескольких настроенных обслуживаемых доменов в Office 365. Щелкните в поле **Добавить тег** , чтобы увидеть и выбрать домен. Еще раз щелкните поле **Добавить тег** , чтобы выбрать дополнительные домены, если доступно несколько доменов.

    - **Отправитель**: указывает одного или нескольких пользователей в Организации. Щелкните в разделе **Добавить тег** и начните ввод текста, чтобы отфильтровать список. Еще раз щелкните поле **Добавить тег** , чтобы выбрать дополнительных отправители.

    - **Отправитель является участником**: указывает одну или несколько групп в Организации. Щелкните в разделе **Добавить тег** и начните ввод текста, чтобы отфильтровать список. Еще раз щелкните поле **Добавить тег** , чтобы выбрать дополнительных отправители.

    - **Кроме случая**: чтобы добавить исключения для правила, нажмите три раза кнопку **Добавить условие** , чтобы просмотреть все доступные исключения. Параметры и поведение в точности аналогичны условиям.

7. Когда закончите, нажмите кнопку **Сохранить**.
...
Чтобы включить или отключить политику, задать порядок приоритетов политики или настроить уведомления о карантине конечных пользователей, ознакомьтесь со следующими разделами.
...
3. Обратите внимание, что в раскрытой политике отображается значение в столбце **включено** .

   Чтобы отключить политику, переместите переключатель влево. ![Отключить](../../media/scc-toggle-off.png)

   Чтобы включить политику, переместите переключатель вправо. ![Включить](../../media/963dfcd0-1765-4306-bcce-c3008c4406b9.png)
...
По умолчанию политикам исходящих сообщений по умолчанию назначен приоритет, основанный на порядке, в котором они были созданы (более новые политики имеют более высокий приоритет, чем старые политики). Чем ниже значение приоритета, тем выше приоритет политики (0 — самый высокий), а политики обрабатываются в порядке приоритета (более высокие политики обрабатываются до политик более низкого приоритета). У двух политик не может быть одинаковый приоритет.
...
Чтобы изменить приоритет политики, переместите политику вверх или вниз в списке (нельзя напрямую изменить номер **приоритета** в центре безопасности & соответствия требованиям).
...
3. Click ![Значок "стрелка вверх"](../../media/ITPro-EAC-UpArrowIcon.png) или ![Значок со стрелкой вниз](../../media/ITPro-EAC-DownArrowIcon.png) Перемещение настраиваемой политики нежелательной почты в списке приоритет.
...
3. В появившейся расширенной политике нажмите **удалить политику**.

4. В появившемся диалоговом окне с предупреждением нажмите кнопку **Да** .
...
- Правило применяется к членам группы с именем Contoso Executives Group.
