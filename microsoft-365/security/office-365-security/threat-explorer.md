...
- Исправления в фильтрах URL-адресов *(в* сравнении с доменом *URL* и url-адресом и *путем).* Обновления влияют на поиск сообщений, содержащих решение URL-адреса или щелчка. Мы включили поддержку поиска, не относящемся к протоколу, поэтому вы можете искать URL-адрес без `http` использования. По умолчанию поиск URL-адресов сопостает http, если не указано явно другое значение. Пример:
