...
Мы нацелены на улучшения качества платформы и данных для повышения точности и целостности данных для записей электронной почты. Этот набор обновлений включает консолидацию сведений о предварительной доставке и последующей доставке (пример действия, выполняемого по электронной почте в процессе ZAP) в одну запись вместе с дополнительными возможностями, такими как нежелательная почта вредоносности, угрозы уровня объекта (например, какой URL-адрес является вредоносным) и последними местами доставки. 
...
-   Событие: включает в себя события верхнего уровня, такие как первоначальная доставка, ручное исправление, ZAP, отправку и динамическую доставку.
...
Дополнительные действия состоят из примененных действий POST для доставки электронной почты и могут включать ZAP, ручное исправление (действие, выполняемое с помощью адми; n, например, обратимого удаления), динамической доставки и повторной обработки (сообщение электронной почты было ретроактивели определено как хорошее). 
...
>
> — Существуют новые поля и значения для технологий обнаружения и дополнительные действия (в частности, для сценариев ZAP). Оцените существующие сохраненные запросы и отслеживаемые запросы, чтобы убедиться, что они работают с новыми значениями. 
...
Набор улучшений фокуссед по URL-адресу и URL-адресу содержит следующие данные:
...
-   Исправления в фильтрах URL-адресов (URL-адрес и домен URL-адреса VS URL-адрес домена и путь): мы внесли обновления, чтобы искать сообщения, содержащие URL-адреса/щелкните вредоносности. В рамках этого мы включили поддержку поиска независимого протокола (то есть можно напрямую выполнить поиск по URL-адресу без HTTP). По умолчанию поиск URL-адресов сопоставляется с HTTP, если явно не указано иное. Например,
  ...
  а.    Поиск с `http://` префиксом "URL", "домен URL" и "домен URL-адрес и путь" в полях фильтра "URL", "домен URL" и "домен URL". Такое поведение является согласованным и должно отображать тот же результат.
  б.    Выполните поиск `https://` префикса в поле "URL-адрес". Если этот параметр не указан, `http://` используется префикс.
  в.     `/` в начале и в конце поля "URL-путь", "домен URL", "домен URL и путь" игнорируются. `/` в конце поля "URL" игнорируется. 
...
Обычно используется для сценариев исдельного сигнала ZAP, когда сообщение было определено как фишинг и удалено после доставки. Используется для подключения оповещения с соответствующими результатами в проводнике. Это один из Иокс для оповещения. 
