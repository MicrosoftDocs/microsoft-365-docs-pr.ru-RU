...
Кроме того, порядок приоритетов важен, если к одному лицу применяется несколько предустановленных политик безопасности и других политик.
...
### <a name="order-of-precedence-for-preset-security-policies-and-other-policies"></a>Порядок приоритетов для предустановленных политик безопасности и других политик
...
Другими словами, параметры политики защиты **"Строгая"**  переопределяют параметры стандартной политики защиты, которая переопределяет параметры из настраиваемой политики, которая переопределяет параметры из политики по умолчанию.
...
2. В **области "Стандартная защита"** или **"Строгая защита"** нажмите кнопку **"Изменить".**
...
Или, для массовой рассылки электронной почты, убедитесь, что значение BCL 6 или выше доставляет сообщение в папку нежелательной  почты для пользователей стандартной защиты, а значение BCL 4 или более высокое карантин сообщения для пользователей строгой защиты. [](bulk-complaint-level-values.md) 
