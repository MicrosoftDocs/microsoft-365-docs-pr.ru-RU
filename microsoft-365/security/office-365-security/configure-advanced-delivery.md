...
Мы хотим обеспечить безопасность вашей организации по [умолчанию,](secure-by-default.md)поэтому Exchange Online Protection (EOP) не разрешает безопасные списки или обход фильтрации для сообщений, которые привели к вредоносным программам или высоким уровнем уверенности в фишинге. Но мы признаем, что существуют определенные сценарии, которые требуют доставки неотобраченных сообщений. Например:
...
Для _предотвращения_ фильтрации этих сообщений в этих конкретных  сценариях используется усовершенствованая политика доставки в Microsoft 365. <sup>\*</sup> Усовершенствованая политика доставки гарантирует, что сообщения в этих сценариях не фильтруются:
...
- [Air и кластеризация в Defender для Office 365](office-365-air.md) игнорируют эти сообщения.
- В частности, для сторонних имитаций фишинга:
  - [Отправки администратора](admin-submission.md) создают автоматический ответ, в котором говорится, что сообщение является частью фишинговой кампании моделирования и не представляет реальной угрозы. Оповещения и AIR не будут запускаться.
  - [Безопасные ссылки в Defender для Office 365](safe-links.md) не блокируют и не взрывают указанные URL-адреса в этих сообщениях.
  - [Безопасные вложения в Defender для Office 365](safe-attachments.md) не взрывают вложения в этих сообщениях.
...
- [Обнаружение обозревателя угроз и обнаружения в режиме реального времени в плане Defender для Office 365 2](threat-explorer.md)
...
- Откройте Центр безопасности и соответствия требованиям на сайте <https://protection.office.com/>. Чтобы перейти непосредственно на **страницу advanced delivery,** откройте <https://protection.office.com/advanceddelivery> .
...
- **Сторонние фильтры.** Если запись MX домена не указывая на Office 365 (сообщения сначала перенаписыются где-то [еще),](secure-by-default.md) безопасность по умолчанию недоступна.
