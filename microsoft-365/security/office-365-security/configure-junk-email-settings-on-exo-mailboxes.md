...
> [!NOTE]
> Сообщения от отправителей, добавленных пользователями в собственные списки надежных отправителей, пропускают фильтрацию EOP (нежелательное значение —-1). Чтобы запретить пользователям добавлять записи в список надежных отправителей в Outlook, используйте групповую политику, как описано в разделе [сведения о параметрах нежелательной почты в Outlook](#about-junk-email-settings-in-outlook) далее в этой статье.
