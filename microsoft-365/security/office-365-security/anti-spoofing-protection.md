...
ms.locfileid: "43537525"
---
# <a name="anti-spoofing-protection-in-office-365"></a>Защита от спуфинга в Office 365

Если вы являетесь клиентом Office 365 с почтовыми ящиками в Exchange Online или автономным клиентом Exchange Online Protection (EOP) без почтовых ящиков Exchange Online, EOP включает функции, помогающие защитить вашу организацию от поддельных (поддельных) отправителей.
...
- **Аналитика подделки**: Просмотрите поддельные сообщения от отправителей во внутренних и внешних доменах и разрешите или заблокируйте этих отправителей. Дополнительные сведения см. в статье [Настройка аналитики подделки в Office 365](learn-about-spoof-intelligence.md).

- **Политики защиты от фишинга**: В EOP встроенная политика защиты от фишинга позволяет включать и отключать интеллектуальную информацию о поддельных сообщениях, включать или отключать идентификацию не прошедших проверку подлинности отправителей в Outlook и указывать действия для заблокированных поддельных отправителей (перейти в папку нежелательной почты или на карантин). Расширенные политики защиты от фишинга, доступные в Office 365 Advanced Threat Protection (ATP), также содержат параметры защиты от олицетворения (защищенные отправители и домены), параметры аналитики почтовых ящиков и настраиваемые расширенные пороги фишинга. Дополнительные сведения см. в статье [политика защиты от фишинга в Office 365](set-up-anti-phishing-policies.md).

- **Проверка подлинности электронной почты**: неотъемлемой частью любых усилий по борьбе со спуфингом является использование аутентификации электронной почты (также известной как проверка электронной почты) записями SPF, DKIM и DMARC в DNS. Вы можете настроить эти записи для своих доменов, чтобы почтовые системы назначения могли проверять достоверность сообщений, которые, как утверждают, были отправлены в ваших доменах. Для входящих сообщений Office 365 требует проверки подлинности электронной почты для доменов отправителей. Дополнительные сведения см. в статье [проверка подлинности электронной почты в Office 365](email-validation-and-authentication.md).

Технология Microsoft против спуфинга изначально была развернута только в организациях с Office 365 Office 365 Advanced Threat Protection (ATP). В октябре 2018 года в EOP была добавлена защита от спуфинга.
...
- **Спуфинг внутри организации**: также известен как спуфинг _самому себе_. Например,
  ...
  - CAT - это категория сообщения, обычно это SPM (спам), но иногда это может быть HSPM (высоконадежный спам) или PHISH (фишинг), в зависимости от того, какие другие типы шаблонов были обнаружены в сообщении.
...
- **Междоменная спуфинг**: домены отправителя и получателя отличаются и не имеют отношения друг к другу (также известные как внешние домены). Например,
  ...
  - Значение `reason=000` указывает, что сообщение не прошло явную аутентификацию электронной почты. `reason=001` означает, что сообщение не прошло неявную аутентификацию электронной почты.

  - SFTY - уровень безопасности сообщения. 9 указывает на фишинг, .22 указывает на междоменную спуфинг.

Дополнительные сведения о значениях категории и составной проверки подлинности (compauth), связанных со спуфингом, см. в разделе [Заголовки сообщений защиты от нежелательной почты в Office 365](anti-spam-message-headers.md).

Дополнительные сведения о DMARC см. в разделе [Использование DMARC для проверки электронной почты в Office 365](use-dmarc-to-validate-email.md).
...
Организация Office 365 ATP может использовать Threat Explorer в Центре безопасности и соответствия требованиям для просмотра информации о попытках фишинга. Дополнительные сведения см. в разделе [Исследование угроз Office 365 и ответ на них](office-365-ti.md)
...
Например, Габриэла Лауреано (glaureano@contoso.com) заинтересована в наблюдении за птицами, присоединяется к списку рассылки birdwatchers@fabrikam.com и отправляет в список следующее сообщение:

> **От:** "Габриела Лауреано" \<glaureano@contoso.com\> <br/> **Кому:** Бирдватчер \<списка обсуждений, birdwatchers@fabrikam.com\> <br/> 
...
> **От:** "Габриела Лауреано" \<glaureano@contoso.com\> <br/> **Кому:** Бирдватчер \<списка обсуждений, birdwatchers@fabrikam.com\> <br/> **Тема:** [BIRDWATCHERS] Прекрасный вид голубой сойки на вершине горы Рейнир на этой неделе <br/><br/> Кто-нибудь хочет посмотреть птицу на этой неделе на горе Рейнир? <br/><br/> Это сообщение было отправлено в список обсуждения для любителей птиц Birdwatchers. Вы можете отменить подписку в любое время.
...
- Вашей организации не принадлежит список рассылки:
  ...
  - Создайте заявку в службу поддержки Office 365, чтобы создать переопределение для списка рассылки, чтобы оно считалось законным. Для получения дополнительной информации см. [Поддержка контактов для бизнес-продуктов - Справка администратора](../../admin/contact-support-for-business-products.md).
...
Если вы являетесь администратором, который в настоящее время отправляет сообщения в Office 365, вам необходимо убедиться, что ваша электронная почта правильно аутентифицирована. В противном случае он может быть помечен как спам или фишинг. Дополнительные сведения см. в разделе [Решения для законных отправителей, отправляющих электронную почту без проверки подлинности](email-validation-and-authentication.md#solutions-for-legitimate-senders-who-are-sending-unauthenticated-email).
