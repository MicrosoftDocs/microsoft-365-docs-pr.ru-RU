...
description: Узнайте, как настроить протокол DMARC (Domain-based Message Authentication, Reporting, and Conformance), чтобы проверить сообщения, отправленные из вашей организации.
ms.openlocfilehash: c6c9d394f1c3087cdc03f3433a9e2b4ec07033bb
ms.sourcegitcommit: 2614f8b81b332f8dab461f4f64f3adaa6703e0d6
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/21/2020
ms.locfileid: "43631101"
---
...
Вам не нужно абсолютно ничего делать, чтобы настроить протокол DMARC в отношении почты, которую вы получаете в Microsoft 365. Мы позаботились об этом за вас. Если хотите узнать, что происходит с почтой, которая не прошла наши проверки DMARC, ознакомьтесь c разделом [Как Microsoft 365 обрабатывает входящую почту, не прошедшую проверки DMARC](use-dmarc-to-validate-email.md#inbounddmarcfail).
...
- [Шаг 2. Настройка SPF для вашего домена](use-dmarc-to-validate-email.md#ConfigSPF)

- [Шаг 3. Настройка DKIM для вашего личного домена](use-dmarc-to-validate-email.md#ConfigDKIM)

- [Шаг 4. Создание записи DMARC TXT для вашего домена](use-dmarc-to-validate-email.md#CreateDMARCRecord)
...
Теперь, когда у вас есть список всех допустимых отправителей, можно приступать к выполнению действий, приведенных в статье [Настройка SPF для предотвращения спуфинга](set-up-spf-in-office-365-to-help-prevent-spoofing.md).
...
Чтобы узнать подробнее о параметрах, которые следует использовать, ознакомьтесь с понятиями, изложенными в разделе [Рекомендации по реализации протокола DMARC в Microsoft 365](use-dmarc-to-validate-email.md#DMARCbestpractices).
...
Создав запись, необходимо обновить ее у регистратора доменных имен. Инструкции по добавлению записи DMARC TXT в свои записи DNS для Microsoft 365 см. в статье [Создание записей DNS для Microsoft 365 при самостоятельном управлении записями DNS](https://support.office.com/article/b0f3fdca-8a80-4e8e-9ef3-61e8a2a9ab23).
...
Если для отправляющего сервера задана политика отклонения DMARC `p=reject`, EOP будет отмечать сообщения как спуфинг, а не отклонять их. Другими словами, для исходящей почты служба Microsoft 365 рассматривает политики `p=reject` и `p=quarantine` как одинаковые. Администраторы могут настроить действия для сообщений, классифицированных как спуфинг в [политике защиты от фишинга](set-up-anti-phishing-policies.md).
  
Такая настройка Microsoft 365 обусловлена тем, что некоторая допустимая электронная почта может не пройти проверки DMARC. Например, сообщение может не пройти проверки DMARC, если оно отправлено в список рассылки, который в последствии пересылает его всем получателям, указанным в списке. Если эти сообщения будут отклонены в Microsoft 365, получатели могут не получить допустимые сообщения, причем получить их будет невозможно. Вместо этого такие сообщения будут по-прежнему не проходить проверки DMARC, однако они будут отмечены как спам, а не отклонены. При необходимости пользователи могут воспользоваться приведенными ниже способами, чтобы получить такие сообщения.
...
В настоящее время Microsoft 365 использует ARC для подтверждения результатов, если корпорация Майкрософт является подтверждающим центром ARC, но в будущем планируется добавить поддержку сторонних подтверждающих центров. 
...
- [Заголовки сообщений защиты от спама](anti-spam-message-headers.md) включают поля синтаксиса и заголовка, используемые Microsoft 365 для проверок DMARC. 
