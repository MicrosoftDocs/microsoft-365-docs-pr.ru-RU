...
Если для отправляющего сервера задана политика отклонения DMARC `p=reject`, EOP будет отмечать сообщения как спуфинг, а не отклонять их. Другими словами, для исходящей почты служба Office 365 рассматривает политики `p=reject` и `p=quarantine` как одинаковые. Администраторы могут настроить действия для сообщений, классифицированных как спуфинг в [политике защиты от фишинга](set-up-anti-phishing-policies.md).
  
Такая настройка Office 365 обусловлена тем, что некоторая допустимая электронная почта может не пройти проверку DMARC. Например, это возможно, если сообщение отправлено в список рассылки, который пересылает его всем получателям, указанным в списке. Если эти сообщения будут отклонены в Office 365, получатели могут не получить допустимые сообщения, причем получить их будет невозможно. Вместо этого такие сообщения по-прежнему не будут проходить проверку DMARC, но будут помечаться как спуфинг, а не отклоняться. Пользователи смогут по-прежнему получить эти сообщения в папку "Входящие" следующим образом:
...
Дополнительные сведения см. в статье [Создание списков надежных отправителей в Office 365](create-safe-sender-lists-in-office-365).

## <a name="how-office-365-utilizes-authenticated-received-chain-arc"></a>Как Office 365 использует Authenticated Received Chain (ARC)
<a name="ARC"> </a>

Все размещенные почтовые ящики в Office 365 теперь получают преимущества ARC с улучшенной надежностью доставки сообщений и защитой от спуфинга. ARC сохраняет результаты проверки подлинности писем от всех посредников (переходов), когда письмо направляется с исходного сервера в почтовый ящик получателя. До ARC изменения, вносимые посредниками в маршрут письма, например правила пересылки или автоматические подписи, могли приводить к сбоям DMARC к моменту поступления письма в почтовый ящик получателя. При использовании ARC криптографическая сохранность результатов проверки подлинности позволяет Office 365 подтверждать подлинность отправителя сообщения электронной почты. 

В настоящее время Office 365 использует ARC для подтверждения результатов, если корпорация Майкрософт является подтверждающим центром ARC, но в будущем планируется добавить поддержку сторонних подтверждающих центров. 
