...
>[!IMPORTANT]
> В настоящее время таблица находится в бета-версии и предлагается на краткосрочной основе, чтобы вы могли использовать события для регистрации `AADSignInEventsBeta` в Azure Active Directory (AAD). Со временем все сведения о схеме для входов будут перемещаться в `IdentityLogonEvents` таблицу.<br><br>
...
| `LogonType`                       | string        | Тип сеанса, в частности интерактивного, удаленного интерактивного (RDP), сети, пакета и службы                                                                              |
...
| `AccountDisplayName`              | string        | Имя пользователя учетной записи, отображаемой в адресной книге. Как правило, сочетание заданного или имени, средней инициалки и фамилии или фамилии.                             |
