...
После подготовки пилотной среды необходимо протестировать средства управления инцидентами защитника Microsoft 365 и автоматическое исследование и устранение проблем. Мы поможем вам имитировать сложную атаку, которая использует расширенные методы для скрытия определения. Атака перечисляет открытые сеансы SMB на контроллерах домена и получает последние IP-адреса устройств пользователей. В эту категорию атак обычно нет файлов, отброшенных на устройстве жертвы — они происходят только в памяти. Они «выходят с земли», используя существующие системные средства и средства администрирования и вводят свой код в системные процессы для скрытия их выполнения, что позволяет им обнаруживать избежать и хранить их на устройстве.
...
Во время имитации атака внедряет шеллкоде в инноцент процесс. Сценарий требует использования notepad.exe. Мы выбрали этот процесс для имитации, но злоумышленники, скорее всего, будут работать с длительным системным процессом, например svchost.exe. После этого шеллкоде будет обращаться к серверу командного и-управления злоумышленника, чтобы получить инструкции по выполнению этой процедуры. Сценарий предпринимает попытку выполнения запросов реконнаиссанце к контроллеру домена (DC). Реконнаиссанце позволяет злоумышленнику получить сведения о последних учетных данных пользователя. После получения этих сведений злоумышленники могут позднее переместиться в сети, чтобы получить доступ к определенной конфиденциальной учетной записи.
...
1.  Убедитесь, что у клиента [включен Защитник Microsoft 365](https://docs.microsoft.com/microsoft-365/security/mtp/mtp-enable#starting-the-service).
...
2.  Проверьте конфигурацию контроллера домена:
    ...
    - Проверьте контроллер домена для [защитника Майкрософт на предмет удостоверения](https://docs.microsoft.com/azure/security-center/security-center-wdatp) и включите [Удаленное управление](https://docs.microsoft.com/windows-server/administration/server-manager/configure-remote-management-in-server-manager).    
    - Убедитесь, что [защитник Майкрософт для удостоверений и интеграция Microsoft Cloud App Security](https://docs.microsoft.com/cloud-app-security/aatp-integration) включены.
...
3.  Проверьте конфигурацию тестового устройства.
    ...
    1.  Устройство работает под управлением Windows 10 версии 1903 или более поздней версии.
    
    1.  Тестовое устройство присоединяется к тестовому домену.
    
    1.  [Включите антивирусную программу "Защитник Windows"](https://docs.microsoft.com/windows/security/threat-protection/windows-defender-antivirus/configure-windows-defender-antivirus-features). Если у вас возникают проблемы с включением антивирусной программы "Защитник Windows", обратитесь к [разделу Устранение неполадок](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/troubleshoot-onboarding#ensure-that-windows-defender-antivirus-is-not-disabled-by-a-policy).
    
    1.  Убедитесь, что тестовое устройство подключено [к защитнику Майкрософт для конечной точки](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/configure-endpoints).
...
При завершении этого сценария в консоли PowerShell появится сообщение об ошибке.
...
Защитник Microsoft 365 взаимосвязан со всеми связанными оповещениями и обобщением различных продуктов в одну сущность инцидента. Таким образом, защитник Microsoft 365 отображает более широкое описание атак, позволяя аналитикам SOC уяснить и реагировать на сложные угрозы.
...
2.  Выберите новый элемент, щелкнув окружность слева от имени инцидента. На боковой панели отображаются дополнительные сведения об инциденте, в том числе все связанные оповещения. Каждый инцидент имеет уникальное имя, которое описывает атрибуты оповещений, которые он содержит.
    ...
    Фильтрация оповещений, отображаемых на панели мониторинга, может осуществляться на основе ресурсов службы: защитник Майкрософт для удостоверения, Microsoft Cloud App Security, защитник Майкрософт для конечной точки, Microsoft 365 Defender и защитник Майкрософт для Office 365.  

3.  Чтобы получить дополнительные сведения об инциденте, выберите **страницу открыть инцидент** .

    На странице **инцидент** можно просмотреть все оповещения и сведения, связанные с инцидентом. Эти сведения включают сущности и активы, участвующие в оповещении, источник обнаружения оповещений (Microsoft Defender для удостоверения, ЕДР) и причину, по которой они были связаны друг с другом. Просмотр списка оповещений об инциденте показывает ход атаки. В этом представлении можно просматривать и изучать отдельные оповещения.
...
>[!NOTE]
>Мы рассмотрим только некоторые из оповещений, создаваемых во время имитации атаки. В зависимости от версии Windows и продуктов защитника Microsoft 365, выполняемых на тестовом устройстве, вы можете увидеть больше оповещений, которые отображаются в незначительном порядке.

![Снимок экрана с созданными оповещениями](../../media/mtp/fig6.png) 


**Предупреждение: наблюдалось внедрение подозрительных процессов (Источник: защитник Майкрософт для конечной точки ЕДР)**
...
Чтобы разрешить аналитикам SOC перехватывать эти расширенные атаки, глубокие датчики памяти в защитнике Майкрософт для конечной точки обеспечивают облачную службу с непредусмотренным отображением различных способов внедрения кода в нескольких процессах. На следующем рисунке показано, как защитник для конечной точки обнаруживается и оповещается при попытке ввести код для <i>notepad.exe</i>.

![Снимок экрана с предупреждением о внедрении потенциально вредоносного кода](../../media/mtp/fig7.png) 


**Предупреждение: неожиданное поведение, которое выполняет процесс, без аргументов командной строки (Источник: защитник Майкрософт для конечной точки ЕДР)**

Защитник Майкрософт для обнаружения конечных точек часто нацелен на наиболее распространенный атрибут приема атаки. Этот метод гарантирует долговечность и порождает отрезку, с помощью которой злоумышленники могут переключаться на более новые тактики.

Мы используем крупномасштабные учебные алгоритмы для установки нормального поведения распространенных процессов в Организации и во всем мире и следит за тем, когда эти процессы показывают аномальные варианты поведения. Эти нетипичные действия часто указывают на то, что дополнительный код появился и работает в надежном для других процессах.
...
Выберите IP-адрес в дереве процессов оповещений, чтобы просмотреть страницу сведений об IP-адресе.
...
**Предупреждение: реконнаиссанце пользователя и IP-адреса (SMB) (Источник: защитник Майкрософт для удостоверения)**
...
### <a name="review-the-device-timeline-microsoft-defender-for-endpoint"></a>Просмотр временной шкалы устройства [защитник Майкрософт для конечной точки]
После изучения различных оповещений в этом инциденте вернитесь к странице инцидент, который вы ранее проучили. Перейдите на вкладку **устройства** на странице инцидент, чтобы просмотреть устройства, участвующие в этом инциденте, как указано в защитнике Майкрософт для конечной точки и защитника Майкрософт для удостоверения.

Выберите имя устройства, на котором была выполнена атака, чтобы открыть страницу сущностей для этого конкретного устройства. На этой странице вы можете видеть оповещения, которые были активированы и связанные с ними события.

Перейдите на вкладку **временная** шкала, чтобы открыть временную шкалу для устройств и просмотреть все события и поведения, которые отслеживаются на устройстве в хронологическом порядке, с обработкой оповещений.
...
Выберите имя пользователя, чтобы открыть страницу профиля пользователя, на котором можно выполнить дальнейшие расследования. [Узнайте больше о том, как изучить рискованных пользователей](https://docs.microsoft.com/cloud-app-security/tutorial-ueba#identify).
<br>
...
Вернитесь к инциденту на портале Центра безопасности Microsoft 365. На вкладке " **расследования** " на странице **инцидент** отображаются автоматические расследования, инициированные защитником Майкрософт для удостоверения и защитника Майкрософт для конечной точки. На снимке экрана ниже показано только автоматическое исследование, инициированное защитником для конечной точки. По умолчанию защитник для конечной точки автоматически исправлять артефакты, обнаруженные в очереди, требующие исправления.

![Снимок экрана с автоматизированным расследованиям, связанным с инцидентом](../../media/mtp/fig14.png)

Выберите оповещение, которое инициировало исследование, чтобы открыть страницу " **сведения об расследовании** ". Вы увидите следующие сведения:
...
Во время автоматического исследования защитник Майкрософт для конечной точки определил notepad.exe процесс, который был добавлен в качестве одного из артефактов, требующих исправления. Защитник для конечной точки автоматически приостановил подозрительный внедренный процесс в рамках автоматизированного исправления. 
...
Выберите **Управление инцидентом**. Задайте для параметра состояние **разрешение инцидента** и выберите соответствующую классификацию.

После устранения инцидента он закрывает все связанные оповещения в центре безопасности Microsoft 365 и связанных с ним порталах.
...
Для этого сценария необходимо указать один внутренний почтовый ящик и устройство. Кроме того, вам потребуется внешняя учетная запись электронной почты для отправки тестового сообщения.

1.  Убедитесь, что у клиента [включен Защитник Microsoft 365](https://docs.microsoft.com/microsoft-365/security/mtp/mtp-enable#starting-the-service).
2.  Определите целевой почтовый ящик, который будет использоваться для получения электронной почты.
    а.  Этот почтовый ящик должен отслеживаться защитником Майкрософт для Office 365 b.  Устройству из требования 3 требуется доступ к этому почтовому ящику
3.  Настройка тестового устройства: a.  Убедитесь, что вы используете Windows 10 версии 1903 или более поздней версии.
    ...
    г.  [Встроенный в защитник Майкрософт для конечной точки](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/configure-endpoints).
...
3.  Создание запроса, который начинает сбор событий электронной почты.
    ...
    1.  В области запрос нажмите кнопку Создать.
    
    1.  Дважды щелкните таблицу Емаилевентс в схеме.

        ```
        EmailEvents 
        ```                                        

    1.  Изменение временного кадра на последние 24 часа. При условии, что сообщение, отправленное при выполнении эмуляции выше, было изменено в течение 24 часов, в противном случае изменяется временной кадр.
    ...
    1.  Выполните запрос.  В зависимости от среды для пилотного проекта может быть много результатов.  
   ...
   В этом примере сообщение электронной почты было отправлено с помощью учетной записи Yahoo. Щелкните **+** значок рядом с **Yahoo.com** в разделе сендерфромдомаин, а затем нажмите кнопку **Применить** , чтобы добавить выбранный домен в запрос.  Используйте домен или учетную запись электронной почты, которая использовалась для отправки тестового сообщения, на этапе 1 запуска имитации для фильтрации результатов.  Выполните запрос еще раз, чтобы получить меньший набор результатов, чтобы убедиться, что вы видите сообщение от имитации.
    ...
    1.  Щелкните строки, полученные в запросе, чтобы проверить запись.
...
6.  Затем используйте значение **SHA256** из таблицы **емаилаттачментинфо** , чтобы найти **девицефиливентс** (действия с файлами, которые произошли в конечной точке) для этого хеша.  Обычно это поле является хэшем SHA256 для вложения.
    ...
    В полученную таблицу теперь включены сведения о конечной точке (защитник Майкрософт для конечной точки), такие как имя устройства, какое действие было выполнено (в данном случае отфильтровано, включая только события Филекреатед), а также место хранения файла. Также будет включено имя учетной записи, связанное с процессом.
...
Теперь вы создали запрос, который определит все входящие сообщения электронной почты, в которых пользователь открыл или сохранил вложение. Вы также можете уточнить этот запрос, чтобы отфильтровать конкретные домены отправителей, размеры файлов, типы файлов и т. д.
...
После создания запроса, определяющего информацию о том, что вы хотели бы **получать оповещения** о происходящих в будущем случаях, можно создать пользовательское определение из запроса. 
...
5.  Выберите область для правила оповещения. Так как в этом запросе используются устройства, группы устройств важны в этом пользовательском определении в соответствии с контекстом "защитник Майкрософт для конечной точки". При создании настраиваемого обнаружения, которое не включает устройства в качестве затронутых сущностей, область действия не применяется.  
...
Чтобы узнать больше о расширенном поиске, в следующих веб-трансляциях вы узнаете о возможностях расширенного поиска в защитнике Microsoft 365, чтобы создать запросы с перекрестными элементами, выполнить сведение к объектам и создать настраиваемые обнаружения и действия по исправлению.
...
|:-----|:-----|:-----|:-----|:-----|
| Эпизод 1: базовые понятия KQL | Мы расскажем об основных возможностях расширенного обнаружения в защитнике Microsoft 365. Сведения о доступных дополнительных данных по поиску и базовому синтаксису KQL и операторам. | [ MP4](https://aka.ms/MTP15JUL20_MP4) | [YouTube](https://youtu.be/0D9TkGjeJwM) | [Эпизод 1: файл CSL в Git](https://github.com/microsoft/Microsoft-threat-protection-Hunting-Queries/blob/master/Webcasts/TrackingTheAdversary/Episode%201%20-%20KQL%20Fundamentals.csl) |
