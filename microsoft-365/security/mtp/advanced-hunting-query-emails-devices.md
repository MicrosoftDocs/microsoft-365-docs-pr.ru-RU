...
---
# <a name="hunt-for-threats-across-devices-emails-apps-and-identities"></a>Поиск угроз на разных устройствах, электронных письмах, приложениях и удостоверениях
...
Схема [расширенных поисков](advanced-hunting-schema-tables.md) предоставляет обширную информацию об устройстве в различных таблицах. Например, [таблица DeviceInfo предоставляет](advanced-hunting-deviceinfo-table.md) всесторонние сведения об устройстве на основе регулярно сводные данные о событиях. Этот запрос использует таблицу для проверки того, вошел ли потенциально скомпрометированный пользователь () на какие-либо устройства, а затем перечисляет оповещения, срабатывав на `DeviceInfo` `<account-name>` этих устройствах.
...
[Автоматическая очистка (ZAP)](../office-365-security/zero-hour-auto-purge.md) автоматически очищает вредоносные сообщения после их отправки. В случае сбой zap вредоносный код может в конечном итоге запуститься на устройстве и оставить учетные записи скомпрометенными. Этот запрос проверяет действия, которые выполнили получатели сообщений электронной почты, которые не были успешно рассмотрены zaP.
