...
- Проверьте параметры RBAC для защитника Майкрософт в [центре безопасности защитника Microsoft](https://securitycenter.windows.com/) в разделе **Параметры** > **разрешений** > **.** Выберите соответствующую роль, чтобы назначить разрешение " **Управление параметрами безопасности** ".
...
Простые запросы, например те, которые не используют оператор `project` or `summarize` для настройки или агрегирования результатов, обычно возвращают эти общие столбцы.

Существует несколько способов, позволяющих обеспечить более сложные запросы возвращать эти столбцы. Например, если вы предпочитаете объединить и подсчитать подсчета сущностями в столбце `DeviceId`, например, вы по `Timestamp` -прежнему можете возвратить его из самого последнего события, использующего каждое уникальное `DeviceId`значение.

Ниже приведен пример запроса, который подсчитывает количество уникальных машин`DeviceId`() с обнаружением вирусов и использует этот счетчик для поиска компьютеров с более чем пятью обнаружениями. Чтобы получить последнюю версию `Timestamp`, он использует `summarize` оператор с `arg_max` функцией.
...
- **МИТРЕ ATT&а методы** — одна или несколько способов атаки, определяемых правилом, описанных в разделе [МИТРЕ ATT&а Framework](https://attack.mitre.org/)
...
Определите столбцы в результатах запроса, где ожидается Поиск основной затронутой или затронутой сущности. Например, запрос может возвращать адреса отправителя`SenderFromAddress` ( `SenderMailFromAddress`или) и получателей`RecipientEmailAddress`(). Определение того, какие из этих столбцов представляют основную затронутую сущность, помогает службе собирать соответствующие оповещения, сопоставлять инциденты и целевые действия отклика.
...
### <a name="3-specify-actions-on-files-or-machines"></a>3. Указание действий для файлов или компьютеров.
Настраиваемое правило обнаружения может автоматически выполнять действия с файлами или машинами, возвращенными запросом.

#### <a name="actions-on-machines"></a>Действия на компьютерах
Эти действия применяются к машинам в `DeviceId` столбце результатов запроса:
- **Изоляция компьютера** — использует защитник Microsoft Defender для применения полной сетевой изоляции, что предотвращает подключение компьютера к любому приложению или службе. [Дополнительные сведения о изоляции компьютеров ATP для защитника Майкрософт](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/respond-machine-alerts#isolate-machines-from-the-network)
- **Сбор пакета для расследования** — сбор сведений о компьютере в ZIP-файле. [Дополнительные сведения о пакете исследования Microsoft Defender ATP](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/respond-machine-alerts#collect-investigation-package-from-machines)
- **Запустить проверку антивирусной** программы — выполняет полную проверку антивирусной программы "Защитник Windows" на компьютере
- **Initiate расследования** — инициирует [Автоматическое исследование](mtp-autoir.md) на компьютере

#### <a name="actions-on-files"></a>Действия с файлами
Если выбран этот параметр, действие **файла на карантине** применяется к файлам `SHA1`в `InitiatingProcessSHA1`результатах `SHA256`запроса, `InitiatingProcessSHA256` , или в столбце. Это действие удаляет файл из текущего расположения и помещает копию в карантин.
...
### <a name="4-set-the-rule-scope"></a>4. Задайте область применения правила.
...
### <a name="5-review-and-turn-on-the-rule"></a>5. Проверьте и включите правило.
...
- **Включение**выключения — включение или отключение правила для его запуска**Turn off**  / 
...
- [Обзор пользовательских обнаружений](custom-detections-overview.md)
