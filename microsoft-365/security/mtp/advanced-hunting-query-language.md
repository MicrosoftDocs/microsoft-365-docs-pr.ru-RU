...
Первый элемент, по конвейеру, — это фильтр времени, областью действия в течение предыдущих семи дней. Ограничение диапазона времени помогает гарантировать, что запросы будут работать хорошо, возвращать управляемые результаты и не дополним время.
...
| `string` | Строка символа в UTF-8, заключенная в одиночные кавычка ( ) или `'` двойные кавычка ( `"` ). [Дополнительные информацию о строках](https://docs.microsoft.com/azure/data-explorer/kusto/query/scalar-data-types/string) |
...
- Поместите курсор в любую часть запроса, чтобы выбрать этот запрос перед его запуском. При этом будет запускаться только выбранный запрос. Чтобы выполнить другой запрос, переместийте курсор соответствующим образом и выберите запрос **"Выполнить".**
...
>[!NOTE]
>Кроме примеров базовых запросов, вы можете получить доступ к [общим запросам](advanced-hunting-shared-queries.md) для определенных сценариев охоты на угрозы. Изучите общие запросы в левой части страницы или в репозитории [запросов GitHub.](https://aka.ms/hunting-queries)
