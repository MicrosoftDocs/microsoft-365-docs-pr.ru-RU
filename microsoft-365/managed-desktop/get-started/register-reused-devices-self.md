...
> [!IMPORTANT]
> Все устройства, для которых вы хотите получить эту информацию, должны работать под управлением Windows 10 версии 1703 или более поздней. Кроме того, вам потребуется устройство, которое является клиентом Configuration Manager, подключенным к сайту Configuration Manager (текущей ветви). Кроме того, необходимо настроить роль системы сайта точки формирования отчетов в среде с включенными службами SQL Server Reporting Services. 
