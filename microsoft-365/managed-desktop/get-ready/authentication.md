...
- Убедитесь, что подключение Azure Active Directory (AAD) настроено и использует локальный сервер Active Directory, работающий под управлением Windows Server 2008 R2 или более поздней версии.
- Убедитесь, что для подключения AAD установлена поддерживаемая версия и настроена синхронизация этих трех атрибутов с Azure AD: 
    - DNS-имя домена локальной службы Active Directory (где находятся конечные пользователи)
    - NetBIOS из локальной службы Active Directory Йор (где находятся конечные пользователи)
...
С помощью Windows Hello для бизнеса устройства, управляемые Майкрософт, также предоставляют пользователям быстрые возможности для работы с паролями. Чтобы убедиться, что Windows Hello для бизнеса будет работать без предоставления имени участника-пользователя и паролей, перейдите на страницу [Настройка устройств, подключенных к Azure AD, для локальной системы единого входа с помощью Windows Hello для бизнеса](https://docs.microsoft.com/windows/security/identity-protection/hello-for-business/hello-hybrid-aadj-sso-base) , чтобы проверить требования, а затем следуйте указаниям Здесь представлены действия.
