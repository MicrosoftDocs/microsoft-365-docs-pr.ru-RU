...
---
# <a name="choose-the-domain-to-use-when-creating-microsoft-365-groups"></a>Выбор домена для создания групп Microsoft 365

В некоторых организациях для разных подразделений используются отдельные домены электронной почты. Вы можете указать домен, который будет использоваться при создании пользователями групп Microsoft 365.
...
Прежде чем запускать командлеты PowerShell, скачайте и установите модуль, который позволит вам общаться с вашей организацией. Проверьте подключение [к Exchange Online с помощью удаленной powerShell.](https://go.microsoft.com/fwlink/p/?LinkId=785881)
...
Предположим, что в вашей организации также настроены поддомены. Вы также хотите, чтобы группы создавались в этих доменах:
...
> [!NOTE]
> При стираемом EAP они оцениваются в порядке приоритета. Значение 1 означает наивысший приоритет. После совпадения EAP не оценивается ни один дополнительный EAP, а адреса, которые помещаются в группе, будут соответствовать EAP. > если ни один EAP не соответствует указанным условиям, группа будет подготовка в принятом домене организации по умолчанию. Дополнительные сведения о добавлении принятого домена можно найти в под управлением принятых доменов [в Exchange Online.](https://go.microsoft.com/fwlink/p/?LinkId=785428)
  
### <a name="scenario-1"></a>Сценарий 1

В следующем примере показано, как подготовка всех групп Microsoft 365 в организации в groups.contoso.com домене.
...
Предположим, вы хотите контролировать, в каких под доменах создаются группы Microsoft 365. Вам нужны указанные ниже группы и пользователи.
  
- Группы, созданные учащимися (пользователями, для которых в отделе установлено students.groups.contoso.com домене).   Используйте эту команду:
    
  ```
  New-EmailAddressPolicy -Name StudentsGroups -IncludeUnifiedGroupRecipients -EnabledEmailAddressTemplates "SMTP:@students.groups.contoso.com","smtp:@groups.contoso.com" -ManagedByFilter {Department -eq 'Students'} -Priority 1
  ```

- Группы, созданные преподавателями (пользователями, для которых в отделе установлено **faculty.contoso.com)** в faculty.groups.contoso.com домене.  Используйте эту команду:
    
  ```
  New-EmailAddressPolicy -Name FacultyGroups -IncludeUnifiedGroupRecipients -EnabledEmailAddressTemplates "SMTP:@faculty.groups.contoso.com","smtp:@groups.contoso.com" -ManagedByFilter {Department -eq 'Faculty' -or EmailAddresses -like "*faculty.contoso.com*"} -Priority 2
  ```

- Группы, созданные любыми другими, создаются в groups.contoso.com домене. Используйте эту команду:
...
- Группа пользователей определяется с помощью стандартных запросов (свойств пользователей), которые уже доступны. Ознакомьтесь [с фильтруемыми свойствами параметра -RecipientFilter](https://docs.microsoft.com/powershell/exchange/recipientfilter-properties) для поддерживаемых фильтруемых свойств. 
